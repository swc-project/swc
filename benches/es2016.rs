...
import { Subscriber } from '../../Subscriber';import \
     { TeardownLogic, PartialObserver } from '../../types';
...
export interface AjaxRequest {
  ...
  progressSubscriber?: PartialObserver<ProgressEvent>;responseType?: \
     string;
}
...
/**
 * We need this JSDoc comment for affecting ESDoc.* @extends {Ignored}\
  ...
  constructor(urlOrRequest: string | AjaxRequest) {
    ...
    const request: AjaxRequest \
     ...
     withCredentials: false,
      headers: {},
      method: 'GET',
      responseType: 'json',timeout: \
     0,
    };

    if (typeof urlOrRequest === 'string') {
      request.url = urlOrRequest;} else {
      \
     for (const prop in urlOrRequest) {
        if (urlOrRequest.hasOwnProperty(prop)) {
          (request \
     as any)[prop] = (urlOrRequest as any)[prop];
        }
      }
    }
 ...
 */
export class \
     AjaxSubscriber<T> extends Subscriber<Event> {
  // @ts-ignore: Property has no initializer and \
     is not definitely assigned
  private xhr: XMLHttpRequest;
  private done: boolean = false;

  constructor(destination: \
     Subscriber<T>, public request: AjaxRequest) {
    super(destination);
    ...
    if (!request.crossDomain && !this.getHeader(headers, \
     'X-Requested-With')) {
    ...
    let contentTypeHeader = this.getHeader(headers, 'Content-Type');if \
     (!contentTypeHeader && typeof request.body !== 'undefined' && !isFormData(request.body)) {
      \
    ...
    request.body = this.serializeBody(request.body, this.getHeader(request.headers, \
     'Content-Type'));
  ...
  next(e: Event): void {
    this.done = true;
    const destination \
     = this.destination as Subscriber<any>;
    let result: AjaxResponse;
    try {
      result = new \
     AjaxResponse(e, this.xhr, this.request);
    } catch (err) {
  ...
  private send(): void {
    const {
      request,
      request: \
     { user, method, url, async, password, headers, body },
    } = this;
    try {
      const xhr = \
     (this.xhr = request.createXHR!());

      // set up the events before open XHR
      // https://developer.mozilla.org/en/docs/Web/API/XMLHttpRequest/Using_XMLHttpRequest// \
     You need to add the event listeners before calling open() on the request.
      // Otherwise the \
     progress events will not fire.
     ...
     url!, async!);
      ...
      // timeout, responseType and withCredentials can be set once the XHR \
     is open
      if (async) {
     ...
     as any;
      ...
      // finally send the requestif (body) \
     {
        xhr.send(body);
      } else {
        xhr.send();
  ...
  private setupEvents(xhr: XMLHttpRequest, request: AjaxRequest) \
    ...
    xhr.ontimeout = (e: ProgressEvent) \
     => {
      ...
      let error;
      try {
        error = new AjaxTimeoutError(xhr, \
     request); // TODO: Make betterer.
      } catch (err) {
        error = err;
      }
      this.error(error);\
...
export class AjaxResponse {
  ...
  constructor(public originalEvent: Event, public xhr: XMLHttpRequest, public request: \
     AjaxRequest) {
