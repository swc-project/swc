import {D,F,y,b as s}from'../index.f66dda46.js';var isServerSide = typeof document === 'undefined';
  ...
  return {
    ...
    _removeFromQueue: function _removeFromQueue(type, payload) {
      if (type === TITLE || type === TEMPLATE) {
        ...
        if (index === 0) document.title = applyTitleTemplate(titleQueue[0] || '', titleTemplateQueue[0]);
      } else {
    ...
    _change: function _change(type, prevPayload, payload) {
      if (type === TITLE || type === TEMPLATE) {
        ...
        if (queue.indexOf(payload) === 0) {
          document.title = applyTitleTemplate(queue[queue.indexOf(payload)], titleTemplateQueue[0]);
        }
      } else {
        changeOrCreateMetaTag(metaQueue[metaQueue.indexOf(prevPayload)] = payload);
      }
    },
    _reset:  undefined,
    toStatic: function toStatic() {
      var title = applyTitleTemplate(titleQueue[titleQueue.length - 1], titleTemplateQueue[titleTemplateQueue.length - 1]);
      var visited = new Set();
...
var useLink = function useLink(options) {
  ...
  }, [options.href, options.media, options.as, options.rel, options.crossorigin, options.type, options.hreflang]);
  y(function () {
    hasMounted.current = true;
    var preExistingElements = document.querySelectorAll("link[rel=\"" + options.rel + "\"]");
    ...
    if (node.current) {
      originalOptions.current = Object.keys(options).reduce(function (acc, key) {
...
var useMeta = function useMeta(options) {
  ...
  y(function () {
    if (hasMounted.current) {
      dispatcher._change(META, metaObject.current, metaObject.current = {
...
var useTitle = function useTitle(title, template) {
  ...
  y(function () {
    if (hasMounted.current) {
      dispatcher._change(template ? TEMPLATE : TITLE, prevTitle.current, prevTitle.current = title);
    }
  }, [title, template]);
  y(function () {
    hasMounted.current = true;

    dispatcher._addToQueue(template ? TEMPLATE : TITLE, prevTitle.current = title);
...
var toStatic = defaultDispatcher.toStatic;export{useTitleTemplate as a,useTitle as b,useMeta as c,useLink as d,toStatic as t,useLang as u};