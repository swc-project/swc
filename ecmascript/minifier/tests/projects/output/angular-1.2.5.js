!function(window, document, undefined) {
    function minErr(module) {
        return function() {
            var code = arguments[0], template = arguments[1], templateArgs = arguments, message, i;
            for(message = '[' + (module ? module + ':' : '') + code + '] ' + template.replace(/\{\d+\}/g, function(match) {
                var index = +match.slice(1, -1), arg;
                if (index + 2 < templateArgs.length) {
                    if (arg = templateArgs[index + 2], 'function' == typeof arg) return arg.toString().replace(/ ?\{[\s\S]*$/, '');
                    if ('undefined' == typeof arg) return 'undefined';
                    if ('string' !== typeof arg) return toJson(arg);
                    return arg;
                }
                return match;
            }), message = message + '\nhttp://errors.angularjs.org/1.2.5/' + (module ? module + '/' : '') + code, i = 2; i < arguments.length; i++)message = message + (2 == i ? '?' : '&') + 'p' + (i - 2) + '=' + encodeURIComponent(function(obj) {
    ...
    var lowercase = function(string) {
        ...
        }, msie, jqLite, jQuery, slice = [].slice, push = [].push, toString = Object.prototype.toString, ngMinErr = minErr('ng'), angular = window.angular || (window.angular = {
    ...
    function extend(dst) {
        ...
        return forEach(arguments, function(obj) {
            dst !== obj && forEach(obj, function(value, key) {
                dst[key] = value;
            });
    ...
    var trim = function() {
        if (!String.prototype.trim) return function(value) {
    ...
    if (9 > msie) nodeName_ = function(element) {
        return element = element.nodeName ? element : element[0], element.scopeName && element.scopeName != 'HTML' ? uppercase(element.scopeName + ':' + element.nodeName) : element.nodeName;
    };
    else nodeName_ = function(element) {
    ...
    function copy(source, destination) {
        ...
        if (!destination) {
            ...
            if (destination === source) throw ngMinErr('cpi', "Can't copy! Source and destination are identical.");
            if (isArray(source)) {
            ...
            if ('object' == t1) {
                if (isArray(o1)) {
                    ...
                    if (o1.length == o2.length) {
                        for(key = 0; key < o1.length; key++)if (!equals(o1[key], o2[key])) return !1;
                        return !0;
                    }
                } else if (isDate(o1)) return isDate(o2) && o1.getTime() == o2.getTime();
                else if (isRegExp(o1) && isRegExp(o2)) return o1.toString() == o2.toString();
                else {
                    ...
                    for(key in o2)if ((keySet.hasOwnProperty(key) || key.charAt(0) === '$' || o2[key] !== void 0) && !isFunction(o2[key])) return !1;
                    return !0;
                }
            }
        }
    ...
    function startingTag(element) {
        ...
        try {
            return element[0].nodeType === 3 ? lowercase(elemHtml) : elemHtml.match(/^(<[^>]+>)/)[1].replace(/^<([\w\-]+)/, function(match, nodeName) {
    ...
    function parseKeyValue(keyValue) {
        ...
        return forEach((keyValue || "").split('&'), function(keyValue) {
            if (keyValue) {
                if (key_value = keyValue.split('='), key = tryDecodeURIComponent(key_value[0]), isDefined(key)) {
    ...
    function angularInit(element, bootstrap) {
        ...
        forEach(names, function(name) {
            ...
            if (!appElement) {
                var className = ' ' + element1.className + ' ', match = /\sng[:\-]app(:\s*([\w\d_]+);?)?\s/.exec(className);
                if (match) appElement = element1, module = (match[2] || '').replace(/\s+/g, ',');
                else forEach(element1.attributes, function(attr) {
                    !appElement && names[attr.name] && (appElement = element1, module = attr.value);
                });
            }
    ...
    function bootstrap(element, modules) {
        var doBootstrap = function() {
                ...
                return injector.invoke(['$rootScope', '$rootElement', '$compile', '$injector', '$animate', function(scope, element1, compile, injector1, animate) {
    ...
    function snake_case(name, separator) {
        return separator ||= '_', name.replace(SNAKE_CASE_REGEXP, function(letter, pos) {
    ...
    function getter(obj, path, bindFnToScope) {
        if (!path) return obj;
        for(var keys = path.split('.'), key, lastInstance = obj, len = keys.length, i = 0; len > i; i++)key = keys[i], obj && (obj = (lastInstance = obj)[key]);
    ...
    function setupModuleLoader(window) {
        var $injectorMinErr = minErr('$injector'), ngMinErr1 = minErr('ng');
        ...
        return angular1.$$minErr = angular1.$$minErr || minErr, ensure(angular1, 'module', function() {
            ...
            return function(name, requires, configFn) {
                return (function(name1, context) {
                    if ('hasOwnProperty' === name) throw ngMinErr1('badname', 'hasOwnProperty is not a valid {0} name', 'module');
                    ...
                    function invokeLater(provider, method, insertMethod) {
                        return function() {
                            return invokeQueue[insertMethod || 'push']([provider,
                                method,
    ...
    var jqCache = JQLite.cache = {
        }, jqName = JQLite.expando = 'ng-' + new Date().getTime(), jqId = 1, addEventListenerFn = window.document.addEventListener ? function(element, type, fn) {
    ...
    var SPECIAL_CHARS_REGEXP = /([\:\-\_]+(.))/g, MOZ_HACK_REGEXP = /^moz([A-Z])/, jqLiteMinErr = minErr('jqLite');
    ...
    function jqLitePatchJQueryRemove(name, dispatchThis, filterElems, getterIfNoArguments) {
        ...
        function removePatch(param) {
            var list = filterElems && param ? [this.filter(param)] : [this], fireEvent = dispatchThis, set, setIndex, element, childIndex, children;
            if (!getterIfNoArguments || null != param) for(; list.length;)for(set = list.shift(), setIndex = 0, set.length; set.length > setIndex; setIndex++){
                if (element = jqLite(set[setIndex]), fireEvent) element.triggerHandler('$destroy');
                else fireEvent = !fireEvent;
                for(childIndex = 0, (children = element.children()).length; (children = element.children()).length > childIndex; childIndex++)list.push(jQuery(children[childIndex]));
            }
    ...
    function JQLite(element) {
        ...
        if (isString(element)) {
            ...
            div.innerHTML = '<div>&#160;</div>' + element, div.removeChild(div.firstChild), jqLiteAddNodes(this, div.childNodes), jqLite(document.createDocumentFragment()).append(this);
    ...
    function jqLiteOff(element, type, fn, unsupported) {
        ...
        if (!handle) return;
        if (isUndefined(type)) forEach(events, function(eventHandler, type) {
            removeEventListenerFn(element, type, eventHandler), delete events[type];
        });
        else forEach(type.split(' '), function(type) {
            if (isUndefined(fn)) removeEventListenerFn(element, type, events[type]), delete events[type];
    ...
    function jqLiteRemoveData(element, name) {
        ...
        if (expandoStore) {
            ...
            }, '$destroy'), jqLiteOff(element)), delete jqCache[expandoId], element[jqName] = void 0;
        }
    }
    function jqLiteExpandoStore(element, key, value) {
        var expandoId = element[jqName], expandoStore = jqCache[expandoId || -1];
        if (isDefined(value)) !expandoStore && (element[jqName] = expandoId = jqNextId(), expandoStore = jqCache[expandoId] = {
    ...
    function jqLiteInheritedData(element, name, value) {
        element = jqLite(element), element[0].nodeType == 9 && (element = element.find('html'));
        for(var names = isArray(name) ? name : [name]; element.length;){
            for(var i = 0, ii = names.length; ii > i; i++)if ((value = element.data(names[i])) !== void 0) return value;
    ...
    var JQLitePrototype = JQLite.prototype = {
            ready: function(fn) {
                ...
                else this.on('DOMContentLoaded', trigger), JQLite(window).on('load', trigger);
            },
            toString: function() {
                var value = [];
    ...
    forEach({
        ...
        css: function(element, name, value) {
            if (name = camelCase(name), isDefined(value)) element.style[name] = value;
                ...
                var val;
                return 8 >= msie && (val = element.currentStyle && element.currentStyle[name], '' === val && (val = 'auto')), val ||= element.style[name], 8 >= msie && (val = '' === val ? void 0 : val), val;
            }
        },
        attr: function(element, name, value) {
            ...
            if (BOOLEAN_ATTR[lowercasedName]) {
                if (isDefined(value)) {
                    if (!!value) element[name] = !0, element.setAttribute(name, lowercasedName);
                    else element[name] = !1, element.removeAttribute(lowercasedName);
                } else return element[name] || (element.attributes.getNamedItem(name) || noop).specified ? lowercasedName : void 0;
        ...
        text: function() {
            ...
            if (9 > msie) NODE_TYPE_TEXT_PROPERTY[1] = 'innerText', NODE_TYPE_TEXT_PROPERTY[3] = 'nodeValue';
            else NODE_TYPE_TEXT_PROPERTY[1] = NODE_TYPE_TEXT_PROPERTY[3] = 'textContent';
        ...
        JQLite.prototype[name] = function(arg1, arg2) {
            var i, key;
            if (fn !== jqLiteEmpty && (fn.length == 2 && fn !== jqLiteHasClass && fn !== jqLiteController ? arg1 : arg2) === void 0) {
                if (isObject(arg1)) {
                    for(i = 0; i < this.length; i++)if (fn === jqLiteData) fn(this[i], arg1);
                    else for(key in arg1)fn(this[i], key, arg1[key]);
                    return this;
                } else {
                    for(var value = fn.$dv, jj = (void 0) === value ? Math.min(this.length, 1) : this.length, j = 0; jj > j; j++){
    ...
    function createEventHandler(element, events) {
        var eventHandler = function(event, type) {
            ...
            }), !event.target && (event.target = event.srcElement || document), isUndefined(event.defaultPrevented) && (event.preventDefault = function() {
                event.defaultPrevented = !0, event.preventDefault.call(event);
            }, event.defaultPrevented = !1), event.isDefaultPrevented = function() {
                return event.defaultPrevented || event.returnValue === !1;
            }, forEach(events[type || event.type], function(fn) {
                fn.call(element, event);
            }), 8 >= msie) event.preventDefault = null, event.stopPropagation = null, event.isDefaultPrevented = null;
            else delete event.preventDefault, delete event.stopPropagation, delete event.isDefaultPrevented;
        };
    ...
    forEach({
        ...
        on: function onFn(element, type, fn, unsupported) {
            ...
            }), !handle && jqLiteExpandoStore(element, 'handle', handle = createEventHandler(element, events)), forEach(type.split(' '), function(type) {
        ...
        append: function(element, node) {
            forEach(new JQLite(node), function(child) {
                (element.nodeType === 1 || element.nodeType === 11) && element.appendChild(child);
            });
        },
        ...
        after: function(element, newElement) {
            ...
            forEach(new JQLite(newElement), function(node) {
                element.parentNode.insertBefore(node, index.nextSibling), index = node;
            });
        },
        ...
        next: function(element) {
            ...
            for(var elm = element.nextSibling; null != elm && elm.nodeType !== 1;)elm = elm.nextSibling;
            return elm;
        },
        find: function(element, selector) {
            if (element.getElementsByTagName) return element.getElementsByTagName(selector);
            return [];
        },
        clone: jqLiteClone,
        triggerHandler: function(element, eventName, eventData) {
            ...
            return eventData ||= [], void forEach(eventFns, function(fn) {
                fn.apply(element, [{
                    preventDefault: noop,
        ...
        JQLite.prototype[name] = function(arg1, arg2, arg3) {
            for(var value, i = 0; i < this.length; i++)if (isUndefined(value)) value = fn(this[i], arg1, arg2, arg3), isDefined(value) && (value = jqLite(value));
    ...
    var FN_ARGS = /^function\s*[^\(]*\(\s*([^\)]*)\)/m, FN_ARG_SPLIT = /,/, FN_ARG = /^\s*(_?)(\S+?)\1\s*$/, STRIP_COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg, $injectorMinErr = minErr('$injector');
    function annotate(fn) {
        var $inject, fnText, argDecl, last;
        if ('function' == typeof fn) !($inject = fn.$inject) && ($inject = [], fn.length && (fnText = fn.toString().replace(STRIP_COMMENTS, ''), argDecl = fnText.match(FN_ARGS), forEach(argDecl[1].split(FN_ARG_SPLIT), function(arg) {
            ...
            return forEach(modulesToLoad, function(module) {
                ...
                try {
                    if (isString(module)) for(moduleFn = angularModule(module), runBlocks = runBlocks.concat(loadModules(moduleFn.requires)).concat(moduleFn._runBlocks), invokeQueue = moduleFn._invokeQueue, i = 0, ii = invokeQueue.length; ii > i; i++){
                        var invokeArgs = invokeQueue[i], provider1 = providerInjector.get(invokeArgs[0]);
                        provider1[invokeArgs[1]].apply(provider1, invokeArgs[2]);
                    }
                    else if (isFunction(module)) runBlocks.push(providerInjector.invoke(module));
                    else if (isArray(module)) runBlocks.push(providerInjector.invoke(module));
        ...
        function createInternalInjector(cache, factory1) {
            function getService(serviceName) {
                if (cache.hasOwnProperty(serviceName)) {
                    ...
                    return path.unshift(serviceName), cache[serviceName] = INSTANTIATING, cache[serviceName] = factory1(serviceName);
                } finally{
                    path.shift();
                }
            }
            function invoke(fn, self, locals) {
                var args = [], $inject = annotate(fn), i, key;
        ...
        this.disableAutoScrolling = function() {
            ...
            function getFirstAnchor(list) {
                ...
                return forEach(list, function(element) {
                    result || lowercase(element.nodeName) !== 'a' || (result = element);
                }), result;
            }
            function scroll() {
                var hash = $location.hash(), elm;
                if (!hash) $window.scrollTo(0, 0);
                else if (elm = document1.getElementById(hash)) elm.scrollIntoView();
                else if (elm = getFirstAnchor(document1.getElementsByName(hash))) elm.scrollIntoView();
    ...
    function Browser(window1, document, $log, $sniffer) {
        var self = this, rawDocument = document[0], location = window1.location, history = window1.history, setTimeout = window1.setTimeout, clearTimeout = window1.clearTimeout, pendingDeferIds = {
        };
        self.isMock = !1;
        ...
        self.notifyWhenNoOutstandingRequests = function(callback) {
            if (forEach(pollFns, function(pollFn) {
                pollFn();
            }), 0 === outstandingRequestCount) callback();
        ...
        self.url = function(url, replace) {
            if (location !== window1.location && (location = window1.location), url) {
                if (lastBrowserUrl == url) return;
                if (lastBrowserUrl = url, $sniffer.history) {
                    if (replace) history.replaceState(null, '', url);
                    else history.pushState(null, '', url), baseElement.attr('href', baseElement.attr('href'));
        ...
        function fireUrlChange() {
            ...
            lastBrowserUrl = self.url(), forEach(urlChangeListeners, function(listener) {
                listener(self.url());
            });
        }
        self.onUrlChange = function(callback) {
            if (!urlChangeInit) {
                if ($sniffer.history && jqLite(window1).on('popstate', fireUrlChange), $sniffer.hashchange) jqLite(window1).on('hashchange', fireUrlChange);
        ...
        var lastCookies = {
            }, lastCookieString = '', cookiePath = self.baseHref();
        self.cookies = function(name, value) {
            var cookieLength, cookieArray, cookie, i, index;
            if (name) {
                if ((void 0) === value) rawDocument.cookie = escape(name) + "=;path=" + cookiePath + ";expires=Thu, 01 Jan 1970 00:00:00 GMT";
                else isString(value) && (cookieLength = (rawDocument.cookie = escape(name) + '=' + escape(value) + ';path=' + cookiePath).length + 1, cookieLength > 4096 && $log.warn("Cookie '" + name + "' possibly not set or overflowed because it was too large (" + cookieLength + " > 4096 bytes)!"));
            } else {
                ...
                }, i = 0; i < cookieArray.length; i++)cookie = cookieArray[i], index = cookie.indexOf('='), index > 0 && (name = unescape(cookie.substring(0, index)), lastCookies[name] === void 0 && (lastCookies[name] = unescape(cookie.substring(index + 1))));
                return lastCookies;
            }
    ...
    function $CacheFactoryProvider() {
        this.$get = function() {
            ...
            function cacheFactory(cacheId, options) {
                ...
                var size = 0, stats = extend({
                    }, options, {
                        id: cacheId
                    }), data = {
                    }, capacity = options && options.capacity || Number.MAX_VALUE, lruHash = {
                    }, freshEnd = null, staleEnd = null;
                return caches[cacheId] = {
                    ...
                    remove: function(key) {
                        var lruEntry = lruHash[key];
                        if (!lruEntry) return;
                        freshEnd == lruEntry && (freshEnd = lruEntry.p), lruEntry == staleEnd && (staleEnd = lruEntry.n), link(lruEntry.n, lruEntry.p), delete lruHash[key], delete data[key], size--;
                    },
                    removeAll: function() {
                        data = {
                        }, size = 0, lruHash = {
    ...
    function $CompileProvider($provide, $$sanitizeUriProvider) {
        ...
        this.directive = function registerDirective(name, directiveFactory) {
            if (assertNotHasOwnProperty(name, 'directive'), isString(name)) assertArg(directiveFactory, 'directiveFactory'), !hasDirectives.hasOwnProperty(name) && (hasDirectives[name] = [], $provide.factory(name + 'Directive', ['$injector', '$exceptionHandler', function($injector, $exceptionHandler) {
                var directives = [];
                return forEach(hasDirectives[name], function(directiveFactory, index) {
                    try {
                        ...
                        else !directive.compile && directive.link && (directive.compile = valueFn(directive.link));
                        directive.priority = directive.priority || 0, directive.index = index, directive.name = directive.name || name, directive.require = directive.require || directive.controller && directive.name, directive.restrict = directive.restrict || 'A', directives.push(directive);
                    } catch (e) {
                        $exceptionHandler(e);
                    }
                }), directives;
            ...
            if (isDefined(regexp)) return $$sanitizeUriProvider.aHrefSanitizationWhitelist(regexp), this;
            return $$sanitizeUriProvider.aHrefSanitizationWhitelist();
        }, this.imgSrcSanitizationWhitelist = function(regexp) {
            if (isDefined(regexp)) return $$sanitizeUriProvider.imgSrcSanitizationWhitelist(regexp), this;
            ...
            Attributes.prototype = {
                ...
                $set: function(key, value, writeAttr, attrName) {
                    var booleanKey = getBooleanAttrName(this.$$element[0], key), nodeName;
                    if (booleanKey && (this.$$element.prop(key, value), attrName = booleanKey), this[key] = value, attrName) this.$attr[key] = attrName;
                    else attrName = this.$attr[key], !attrName && (this.$attr[key] = attrName = snake_case(key, '-'));
                    if (nodeName = nodeName_(this.$$element), ('A' === nodeName && 'href' === key || 'IMG' === nodeName && 'src' === key) && (this[key] = value = $$sanitizeUri(value, 'src' === key)), !1 !== writeAttr) {
            ...
            function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
                !($compileNodes instanceof jqLite) && ($compileNodes = jqLite($compileNodes)), forEach($compileNodes, function(node, index) {
                    node.nodeType == 3 && node.nodeValue.match(/\S+/) && ($compileNodes[index] = node = jqLite(node).wrap('<span></span>').parent()[0]);
                });
                ...
                return function(scope, cloneConnectFn, transcludeControllers) {
                    ...
                    for(var i = 0, ii = $linkNode.length; ii > i; i++){
                        var node = $linkNode[i];
                        (node.nodeType == 1 || node.nodeType == 9) && $linkNode.eq(i).data('$scope', scope);
                    }
            ...
            function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
                for(var linkFns = [], nodeLinkFn, childLinkFn, directives, attrs, linkFnFound, i = 0; i < nodeList.length; i++)attrs = new Attributes(), directives = collectDirectives(nodeList[i], [], attrs, 0 === i ? maxPriority : void 0, ignoreDirective), nodeLinkFn = directives.length ? applyDirectivesToNode(directives, nodeList[i], attrs, transcludeFn, $rootElement, null, [], [], null) : null, childLinkFn = nodeLinkFn && nodeLinkFn.terminal || !nodeList[i].childNodes || !nodeList[i].childNodes.length ? null : compileNodes(nodeList[i].childNodes, nodeLinkFn ? nodeLinkFn.transclude : transcludeFn), linkFns.push(nodeLinkFn), linkFns.push(childLinkFn), linkFnFound = linkFnFound || nodeLinkFn || childLinkFn;
                return linkFnFound ? compositeLinkFn : null;
                function compositeLinkFn(scope, nodeList, $rootElement, boundTranscludeFn) {
                    var nodeLinkFn, childLinkFn, node, $node, childScope, i, ii, n, stableNodeList = [];
                    for(i = 0, ii = nodeList.length; ii > i; i++)stableNodeList.push(nodeList[i]);
                    for(i = 0, n = 0, ii = linkFns.length; ii > i; n++)if (node = stableNodeList[n], nodeLinkFn = linkFns[i++], childLinkFn = linkFns[i++], $node = jqLite(node), nodeLinkFn) {
                        if (nodeLinkFn.scope) childScope = scope.$new(), $node.data('$scope', childScope), safeAddClass($node, 'ng-scope');
                        else childScope = scope;
                        if (nodeLinkFn.transclude, nodeLinkFn.transclude || !boundTranscludeFn && transcludeFn) nodeLinkFn(childLinkFn, childScope, node, $rootElement, createBoundTranscludeFn(scope, nodeLinkFn.transclude || transcludeFn));
                        else nodeLinkFn(childLinkFn, childScope, node, $rootElement, boundTranscludeFn);
                    } else childLinkFn && childLinkFn(scope, node.childNodes, void 0, boundTranscludeFn);
                }
            }
            ...
            function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
                ...
                case 8:
                    try {
                        match = /^\s*directive\:\s*([\d\w\-_]+)\s+(.*)$/.exec(node.nodeValue), match && (nName = directiveNormalize(match[1]), addDirective(directives, nName, 'M', maxPriority, ignoreDirective) && (attrs[nName] = trim(match[2])));
                    } catch (e) {
                    }
                    break;
                }
            ...
            function groupScan(node, attrStart, attrEnd) {
                ...
                if (attrStart && node.hasAttribute && node.hasAttribute(attrStart)) do {
                    ...
                    node.nodeType == 1 && (node.hasAttribute(attrStart) && depth++, node.hasAttribute(attrEnd) && depth--), nodes.push(node), node = node.nextSibling;
                }while (depth > 0);
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                for(var terminalPriority = -Number.MAX_VALUE, newScopeDirective, controllerDirectives = previousCompileContext.controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasElementTranscludeDirective = !1, $compileNode = templateAttrs.$$element = jqLite(compileNode), directive, directiveName, $template, replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, linkFn, directiveValue, i = 0, ii = directives.length; ii > i; i++){
                    ...
                    if ((directiveValue = directive.scope) && (newScopeDirective ||= directive, !directive.templateUrl && (assertNoDuplicate('new/isolated scope', newIsolateScopeDirective, directive, $compileNode), isObject(directiveValue) && (newIsolateScopeDirective = directive))), directiveName = directive.name, !directive.templateUrl && directive.controller && (directiveValue = directive.controller, controllerDirectives ||= {
                    }, assertNoDuplicate("'" + directiveName + "' controller", controllerDirectives[directiveName], directive, $compileNode), controllerDirectives[directiveName] = directive), directiveValue = directive.transclude) {
                    ...
                    if (directive.template) {
                        if (assertNoDuplicate('template', templateDirective, directive, $compileNode), templateDirective = directive, directiveValue = isFunction(directive.template) ? directive.template($compileNode, templateAttrs) : directive.template, directiveValue = denormalizeTemplate(directiveValue), directive.replace) {
                            if (replaceDirective = directive, $template = jqLite('<div>' + trim(directiveValue) + '</div>').contents(), compileNode = $template[0], $template.length != 1 || compileNode.nodeType !== 1) throw $compileMinErr('tplrt', "Template for directive '{0}' must have exactly one root element. {1}", directiveName, '');
                    ...
                    if (directive.templateUrl) assertNoDuplicate('template', templateDirective, directive, $compileNode), templateDirective = directive, directive.replace && (replaceDirective = directive), nodeLinkFn = compileTemplateUrl(directives.splice(i, directives.length - i), $compileNode, templateAttrs, jqCollection, childTranscludeFn, preLinkFns, postLinkFns, {
                        ...
                        if (linkFn = directive.compile($compileNode, templateAttrs, childTranscludeFn), isFunction(linkFn)) addLinkFns(null, linkFn, attrStart, attrEnd);
                        else linkFn && addLinkFns(linkFn.pre, linkFn.post, attrStart, attrEnd);
                    } catch (e) {
                ...
                function addLinkFns(pre, post, attrStart, attrEnd) {
                    pre && (attrStart && (pre = groupElementsLinkFnWrapper(pre, attrStart, attrEnd)), pre.require = directive.require, (directive === newIsolateScopeDirective || directive.$$isolateScope) && (pre = cloneAndAnnotateFn(pre, {
                        isolateScope: !0
                    })), preLinkFns.push(pre)), post && (attrStart && (post = groupElementsLinkFnWrapper(post, attrStart, attrEnd)), post.require = directive.require, (directive === newIsolateScopeDirective || directive.$$isolateScope) && (post = cloneAndAnnotateFn(post, {
                        isolateScope: !0
                ...
                function getControllers(require, $element, elementControllers) {
                    ...
                    if (isString(require)) {
                        for(; (value = require.charAt(0)) == '^' || '?' == value;)require = require.substr(1), '^' == value, optional ||= '?' == value;
                        if (value = null, elementControllers && (value = elementControllers[require]), value ||= $element.inheritedData('$' + require + 'Controller'), !value && !optional) throw $compileMinErr('ctreq', "Controller '{0}', required by directive '{1}', can't be found!", require, directiveName);
                        return value;
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    var attrs, i, linkFn, controller, isolateScope, elementControllers = {
                        }, transcludeFn1;
                    if (compileNode === linkNode) attrs = templateAttrs;
                    else attrs = shallowCopy(templateAttrs, new Attributes(jqLite(linkNode), templateAttrs.$attr));
                    if (attrs.$$element, newIsolateScopeDirective) {
                        var LOCAL_REGEXP = /^\s*([@=&])(\??)\s*(\w*)\s*$/, $linkNode = jqLite(linkNode);
                        if (isolateScope = scope.$new(!0), templateDirective && templateDirective === newIsolateScopeDirective.$$originalDirective) $linkNode.data('$isolateScope', isolateScope);
                        ...
                        safeAddClass($linkNode, 'ng-isolate-scope'), forEach(newIsolateScopeDirective.scope, function(definition, scopeName) {
                            ...
                            case '=':
                                ...
                                if (parentGet = $parse(attrs[attrName]), parentGet.literal) compare = equals;
                                else compare = function(a, b) {
                                    return a === b;
                                };
                                parentSet = parentGet.assign || function() {
                                    ...
                                    if (!compare(parentValue, isolateScope[scopeName])) {
                                        if (!compare(parentValue, lastValue)) isolateScope[scopeName] = parentValue;
                                        else parentSet(scope, parentValue = isolateScope[scopeName]);
                                    }
                            ...
                            case '&':
                                ...
                                break;
                            default: throw $compileMinErr('iscp', "Invalid isolate scope definition for directive '{0}'. Definition: {... {1}: '{2}' ...}", newIsolateScopeDirective.name, scopeName, definition);
                            }
                        });
                    }
                    for(transcludeFn1 = boundTranscludeFn && controllersBoundTransclude, controllerDirectives && forEach(controllerDirectives, function(directive1) {
                        var locals = {
                                $scope: directive1 === newIsolateScopeDirective || directive1.$$isolateScope ? isolateScope : scope,
                                $element: attrs.$$element,
                                $attrs: attrs,
                                $transclude: transcludeFn1
                            }, controllerInstance;
                        controller = directive1.controller, '@' == controller && (controller = attrs[directive1.name]), controllerInstance = $controller(controller, locals), elementControllers[directive1.name] = controllerInstance, !hasElementTranscludeDirective && attrs.$$element.data('$' + directive1.name + 'Controller', controllerInstance), directive1.controllerAs && (locals.$scope[directive1.controllerAs] = controllerInstance);
                    }), i = 0; i < preLinkFns.length; i++)try {
                        linkFn = preLinkFns[i], linkFn(linkFn.isolateScope ? isolateScope : scope, attrs.$$element, attrs, linkFn.require && getControllers(linkFn.require, attrs.$$element, elementControllers), transcludeFn1);
                    } catch (e) {
                        $exceptionHandler(e, startingTag(attrs.$$element));
                    }
                    var scopeToChild = scope;
                    for(newIsolateScopeDirective && (newIsolateScopeDirective.template || newIsolateScopeDirective.templateUrl === null) && (scopeToChild = isolateScope), childLinkFn && childLinkFn(scopeToChild, linkNode.childNodes, void 0, boundTranscludeFn), i = postLinkFns.length - 1; i >= 0; i--)try {
                        linkFn = postLinkFns[i], linkFn(linkFn.isolateScope ? isolateScope : scope, attrs.$$element, attrs, linkFn.require && getControllers(linkFn.require, attrs.$$element, elementControllers), transcludeFn1);
                    } catch (e) {
                        $exceptionHandler(e, startingTag(attrs.$$element));
                    }
                    function controllersBoundTransclude(scope1, cloneAttachFn) {
                        var transcludeControllers;
                        return arguments.length < 2 && (cloneAttachFn = scope1, scope1 = void 0), hasElementTranscludeDirective && (transcludeControllers = elementControllers), boundTranscludeFn(scope1, cloneAttachFn, transcludeControllers);
                    }
                }
            }
            ...
            function addDirective(tDirectives, name, location, maxPriority, ignoreDirective, startAttrName, endAttrName) {
                ...
                var match = null;
                if (hasDirectives.hasOwnProperty(name)) for(var directive, directives = $injector.get(name + 'Directive'), i = 0, ii = directives.length; ii > i; i++)try {
                    directive = directives[i], ((void 0) === maxPriority || maxPriority > directive.priority) && directive.restrict.indexOf(location) != -1 && (startAttrName && (directive = inherit(directive, {
                        $$start: startAttrName,
                        $$end: endAttrName
            ...
            function mergeTemplateAttributes(dst, src) {
                ...
                forEach(dst, function(value, key) {
                    ...
                    if ('class' == key) safeAddClass($element, value), dst.class = (dst.class ? dst.class + ' ' : '') + value;
                    else if ('style' == key) $element.attr('style', $element.attr('style') + ';' + value), dst.style = (dst.style ? dst.style + ';' : '') + value;
                    else key.charAt(0) != '$' && !dst.hasOwnProperty(key) && (dst[key] = value, dstAttr[key] = srcAttr[key]);
                });
            }
            function compileTemplateUrl(directives, $compileNode, tAttrs, $rootElement, childTranscludeFn, preLinkFns, postLinkFns, previousCompileContext) {
                var linkQueue = [], afterTemplateNodeLinkFn, afterTemplateChildLinkFn, beforeTemplateCompileNode = $compileNode[0], origAsyncDirective = directives.shift(), derivedSyncDirective = extend({
                    }, origAsyncDirective, {
                        templateUrl: null,
                        transclude: null,
                        replace: null,
                ...
                return $compileNode.empty(), $http.get($sce.getTrustedResourceUrl(templateUrl), {
                    ...
                    if (content = denormalizeTemplate(content), origAsyncDirective.replace) {
                        if ($template = jqLite('<div>' + trim(content) + '</div>').contents(), compileNode = $template[0], $template.length != 1 || compileNode.nodeType !== 1) throw $compileMinErr('tplrt', "Template for directive '{0}' must have exactly one root element. {1}", origAsyncDirective.name, templateUrl);
                        tempTemplateAttrs = {
                            $attr: {
                            }
                    ...
                    for(directives.unshift(derivedSyncDirective), afterTemplateNodeLinkFn = applyDirectivesToNode(directives, compileNode, tAttrs, childTranscludeFn, $compileNode, origAsyncDirective, preLinkFns, postLinkFns, previousCompileContext), forEach($rootElement, function(node, i) {
                        compileNode == node && ($rootElement[i] = $compileNode[0]);
                    }), afterTemplateChildLinkFn = compileNodes($compileNode[0].childNodes, childTranscludeFn); linkQueue.length;){
                        var scope = linkQueue.shift(), beforeTemplateLinkNode = linkQueue.shift(), linkRootElement = linkQueue.shift(), boundTranscludeFn = linkQueue.shift(), linkNode = $compileNode[0];
                        if (beforeTemplateCompileNode !== beforeTemplateLinkNode && (linkNode = jqLiteClone(compileNode), replaceWith(linkRootElement, jqLite(beforeTemplateLinkNode), linkNode)), afterTemplateNodeLinkFn.transclude) childBoundTranscludeFn = createBoundTranscludeFn(scope, afterTemplateNodeLinkFn.transclude);
                    ...
                    if (linkQueue) linkQueue.push(scope), linkQueue.push(node), linkQueue.push(rootElement), linkQueue.push(boundTranscludeFn);
                    else afterTemplateNodeLinkFn(afterTemplateChildLinkFn, scope, node, rootElement, boundTranscludeFn);
                };
            }
            ...
            function addAttrInterpolateDirective(node, directives, value, name) {
                ...
                if ("multiple" === name && nodeName_(node) === "SELECT") throw $compileMinErr("selmulti", "Binding to the 'multiple' attribute is not supported. Element: {0}", startingTag(node));
                directives.push({
                    priority: 100,
                    compile: function() {
                        return {
                            pre: function(scope, element, attr) {
                                ...
                                if (/^(on[a-z]+|formaction)$/.test(name)) throw $compileMinErr('nodomevents', "Interpolations for HTML DOM event attributes are disallowed.  Please use the ng- versions (such as ng-click instead of onclick) instead.");
                                if (interpolateFn = $interpolate(attr[name], !0, getTrustedContext(node, name)), !interpolateFn) return;
                                attr[name] = interpolateFn(scope), ($$observers[name] || ($$observers[name] = [])).$$inter = !0, (attr.$$observers && attr.$$observers[name].$$scope || scope).$watch(interpolateFn, function(newValue, oldValue) {
                                    if ('class' === name && newValue != oldValue) attr.$updateClass(newValue, oldValue);
            ...
            function replaceWith($rootElement, elementsToRemove, newNode) {
                var firstElementToRemove = elementsToRemove[0], removeCount = elementsToRemove.length, parent = firstElementToRemove.parentNode, i;
                if ($rootElement) for(i = 0; i < $rootElement.length; i++)if ($rootElement[i] == firstElementToRemove) {
                    $rootElement[i++] = newNode;
                    for(var j = i, j2 = j + removeCount - 1, jj = $rootElement.length; jj > j; j++, j2++)if (jj > j2) $rootElement[j] = $rootElement[j2];
    ...
    function $ControllerProvider() {
        ...
        this.register = function(name, constructor) {
            if (assertNotHasOwnProperty(name, 'controller'), isObject(name)) extend(controllers, name);
    ...
    function parseHeaders(headers) {
        ...
        return forEach(headers.split('\n'), function(line) {
            if (i = line.indexOf(':'), key = lowercase(trim(line.substr(0, i))), val = trim(line.substr(i + 1)), key) {
    ...
    function headersGetter(headers) {
        ...
        return function(name) {
            if (!headersObj && (headersObj = parseHeaders(headers)), name) return headersObj[lowercase(name)] || null;
            return headersObj;
        };
    }
    ...
    function $HttpProvider() {
        var PROTECTION_PREFIX = /^\)\]\}',?\n/, CONTENT_TYPE_APPLICATION_JSON = {
                ...
                transformResponse: [function(data) {
                    return isString(data) && (data = data.replace(PROTECTION_PREFIX, ''), /^\s*(\[|\{[^\{])/.test(data) && /[\}\]]\s*$/.test(data) && (data = fromJson(data))), data;
                }],
        ...
        this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector', function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
            ...
            function $http(requestConfig) {
                ...
                var chain = [function(config1) {
                        headers = config1.headers;
                        var reqData = transformData(config1.data, headersGetter(headers), config1.transformRequest);
                        return isUndefined(config1.data) && forEach(headers, function(value, header) {
                            lowercase(header) === 'content-type' && delete headers[header];
                        }), isUndefined(config1.withCredentials) && !isUndefined(defaults.withCredentials) && (config1.withCredentials = defaults.withCredentials), sendReq(config1, reqData, headers).then(transformResponse, transformResponse);
                    }, void 0], promise = $q.when(config);
                ...
                function mergeHeaders(config1) {
                    var defHeaders = defaults.headers, reqHeaders = extend({
                        }, config1.headers), defHeaderName, lowercaseDefHeaderName, reqHeaderName;
                    defHeaders = extend({
                    }, defHeaders.common, defHeaders[lowercase(config1.method)]), execHeaders(defHeaders), execHeaders(reqHeaders);
                    ...
                    function execHeaders(headers1) {
                        ...
                        forEach(headers1, function(headerFn, header) {
                            if (isFunction(headerFn)) {
                                if (headerContent = headerFn(), null != headerContent) headers1[header] = headerContent;
            ...
            return $http.pendingRequests = [], createShortMethods('get', 'delete', 'head', 'jsonp'), createShortMethodsWithData('post', 'put'), $http.defaults = defaults, $http;
            ...
            function sendReq(config, reqData, reqHeaders) {
                ...
                if ($http.pendingRequests.push(config), promise.then(removePendingReq, removePendingReq), (config.cache || defaults.cache) && config.cache !== !1 && config.method == 'GET' && (cache = isObject(config.cache) ? config.cache : isObject(defaults.cache) ? defaults.cache : defaultCache), cache) {
                    if (cachedResp = cache.get(url), isDefined(cachedResp)) {
                        ...
                        if (isArray(cachedResp)) resolvePromise(cachedResp[1], cachedResp[0], copy(cachedResp[2]));
                ...
                function done(status, response, headersString) {
                    if (cache) {
                        if (isSuccess(status)) cache.put(url, [status,
                            response, parseHeaders(headersString)]);
            ...
            function buildUrl(url, params) {
                ...
                return forEachSorted(params, function(value, key) {
                    ...
                    !isArray(value) && (value = [value]), forEach(value, function(v) {
    ...
    function createHttpBackend($browser, XHR1, $browserDefer, callbacks, rawDocument) {
        return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
            var status;
            if ($browser.$$incOutstandingRequestCount(), url ||= $browser.url(), lowercase(method) == 'jsonp') {
                var callbackId = '_' + (callbacks.counter++).toString(36);
                callbacks[callbackId] = function(data) {
                    callbacks[callbackId].data = data;
                };
                var jsonpDone = jsonpReq(url.replace('JSON_CALLBACK', 'angular.callbacks.' + callbackId), function() {
                    if (callbacks[callbackId].data) completeRequest(callback, 200, callbacks[callbackId].data);
            ...
            function completeRequest(callback, status1, response, headersString) {
                timeoutId && $browserDefer.cancel(timeoutId), jsonpDone = xhr = null, status1 = 'file' == urlResolve(url).protocol && 0 === status1 ? response ? 200 : 404 : status1, status1 = 1223 == status1 ? 204 : status1, callback(status1, response, headersString), $browser.$$completeOutstandingRequest(noop);
            }
        };
        function jsonpReq(url, done) {
            ...
            if (script.type = 'text/javascript', script.src = url, msie && 8 >= msie) script.onreadystatechange = function() {
                /loaded|complete/.test(script.readyState) && doneWrapper();
            };
            else script.onload = script.onerror = function() {
                doneWrapper();
            };
    ...
    function $InterpolateProvider() {
        ...
        this.startSymbol = function(value) {
            ...
            function $interpolate(text, mustHaveExpression, trustedContext) {
                for(var startIndex, endIndex, index = 0, parts = [], length = text.length, hasInterpolation = !1, fn, exp, concat1 = []; length > index;)if ((startIndex = text.indexOf(startSymbol, index)) != -1 && (endIndex = text.indexOf(endSymbol, startIndex + startSymbolLength)) != -1) index != startIndex && parts.push(text.substring(index, startIndex)), parts.push(fn = $parse(exp = text.substring(startIndex + startSymbolLength, endIndex))), fn.exp = exp, index = endIndex + endSymbolLength, hasInterpolation = !0;
                else index != length && parts.push(text.substring(index)), index = length;
                ...
                if (!mustHaveExpression || hasInterpolation) return concat1.length = length, fn = function(context) {
                    try {
                        for(var i = 0, ii = length, part; ii > i; i++){
                            if ('function' == typeof (part = parts[i])) {
                                if (part = part(context), trustedContext) part = $sce.getTrusted(trustedContext, part);
    ...
    function $IntervalProvider() {
        this.$get = ['$rootScope', '$window', '$q', function($rootScope, $window, $q) {
            ...
            return interval.cancel = function(promise) {
                if (promise && promise.$$intervalId in intervals) return intervals[promise.$$intervalId].reject('canceled'), clearInterval(promise.$$intervalId), delete intervals[promise.$$intervalId], !0;
                return !1;
            }, interval;
        }];
    }
    ...
    function parseAbsoluteUrl(absoluteUrl, locationObj, appBase) {
        ...
        locationObj.$$protocol = parsedUrl.protocol, locationObj.$$host = parsedUrl.hostname, locationObj.$$port = int(parsedUrl.port) || ({
            'http': 80,
            'https': 443,
            'ftp': 21
    ...
    function LocationHtml5Url(appBase, basePrefix) {
        ...
        parseAbsoluteUrl(appBase, this, appBase), this.$$parse = function(url) {
            ...
            this.$$url = encodePath(this.$$path) + (search ? '?' + search : '') + (this.$$hash ? '#' + encodeUriSegment(this.$$hash) : ''), this.$$absUrl = appBaseNoFile + this.$$url.substr(1);
        }, this.$$rewrite = function(url) {
            var appUrl, prevAppUrl;
            if ((appUrl = beginsWith(appBase, url)) !== void 0) {
                if (prevAppUrl = appUrl, (appUrl = beginsWith(basePrefix, appUrl)) !== void 0) return appBaseNoFile + (beginsWith('/', appUrl) || appUrl);
                return appBase + prevAppUrl;
            } else if ((appUrl = beginsWith(appBaseNoFile, url)) !== void 0) return appBaseNoFile + appUrl;
    ...
    function LocationHashbangUrl(appBase, hashPrefix) {
        ...
        parseAbsoluteUrl(appBase, this, appBase), this.$$parse = function(url) {
            ...
            function removeWindowsDriveName(path, url, base) {
                var windowsFilePathExp = /^\/?.*?:(\/.*)/, firstPathSegmentMatch;
                if (url.indexOf(base) === 0 && (url = url.replace(base, '')), windowsFilePathExp.exec(url)) return path;
                return firstPathSegmentMatch = windowsFilePathExp.exec(path), firstPathSegmentMatch ? firstPathSegmentMatch[1] : path;
            }
        }, this.$$compose = function() {
            this.$$url = encodePath(this.$$path) + (search ? '?' + search : '') + (this.$$hash ? '#' + encodeUriSegment(this.$$hash) : ''), this.$$absUrl = appBase + (this.$$url ? hashPrefix + this.$$url : '');
        }, this.$$rewrite = function(url) {
    ...
    function LocationHashbangInHtml5Url(appBase, hashPrefix) {
        ...
        this.$$rewrite = function(url) {
            ...
            if (appUrl = beginsWith(appBaseNoFile, url)) return appBase + hashPrefix + appUrl;
    ...
    LocationHashbangInHtml5Url.prototype = LocationHashbangUrl.prototype = LocationHtml5Url.prototype = {
        ...
        search: function(search, paramValue) {
            ...
            case 1:
                ...
                break;
            default: if (isUndefined(paramValue) || null === paramValue) delete this.$$search[search];
    ...
    function $LocationProvider() {
        ...
        this.hashPrefix = function(prefix) {
            ...
            if (html5Mode) appBase = serverBase(initialUrl) + (baseHref || '/'), LocationMode = $sniffer.history ? LocationHtml5Url : LocationHashbangInHtml5Url;
            ...
            $location = new LocationMode(appBase, '#' + hashPrefix), $location.$$parse($location.$$rewrite(initialUrl)), $rootElement.on('click', function(event) {
                ...
                for(var elm = jqLite(event.target); lowercase(elm[0].nodeName) !== 'a';)if (elm[0] === $rootElement[0] || !(elm = elm.parent())[0]) return;
            ...
            return $rootScope.$watch(function() {
                ...
                return (!changeCounter || oldUrl != $location.absUrl()) && (changeCounter++, $rootScope.$evalAsync(function() {
                    if ($rootScope.$broadcast('$locationChangeStart', $location.absUrl(), oldUrl).defaultPrevented) $location.$$parse(oldUrl);
                    else $browser.url($location.absUrl(), currentReplace), afterLocationChange(oldUrl);
        ...
        this.debugEnabled = function(flag) {
            ...
            function formatError(arg) {
                if (arg instanceof Error) {
                    if (arg.stack) arg = arg.message && arg.stack.indexOf(arg.message) === -1 ? 'Error: ' + arg.message + '\n' + arg.stack : arg.stack;
                    else arg.sourceURL && (arg = arg.message + '\n' + arg.sourceURL + ':' + arg.line);
                }
                return arg;
            }
            function consoleLog(type) {
                ...
                if (logFn.apply) return function() {
                    var args = [];
    ...
    Lexer.prototype = {
        ...
        lex: function(text) {
            ...
            for(var token, json = []; this.index < this.text.length;){
                if (this.ch = this.text.charAt(this.index), this.is('"\'')) this.readString(this.ch);
                    ...
                    if (fn3) this.tokens.push({
                        index: this.index,
                        text: ch3,
                        fn: fn3
                    }), this.index += 3;
                    else if (fn2) this.tokens.push({
                        index: this.index,
                        text: ch2,
                        fn: fn2
                    }), this.index += 2;
                    else if (fn) this.tokens.push({
                        index: this.index,
                        text: this.ch,
                        fn: fn,
        ...
        isIdent: function(ch) {
            return 'a' <= ch && 'z' >= ch || 'A' <= ch && 'Z' >= ch || '_' === ch || '$' === ch;
        },
        ...
        throwError: function(error, start, end) {
            this.index;
            var colStr = isDefined(start) ? 's ' + start + '-' + this.index + ' [' + this.text.substring(start, this.index) + ']' : ' ' + this.index;
        ...
        readNumber: function() {
            for(var number = '', start = this.index; this.index < this.text.length;){
                ...
                if ('.' == ch || this.isNumber(ch)) number += ch;
                    ...
                    else if (this.isExpOperator(ch) && peekCh && this.isNumber(peekCh) && number.charAt(number.length - 1) == 'e') number += ch;
                    else if (this.isExpOperator(ch) && (!peekCh || !this.isNumber(peekCh)) && number.charAt(number.length - 1) == 'e') this.throwError('Invalid exponent');
                    else break;
                }
                this.index++;
            }
        ...
        readIdent: function() {
            for(var parser = this, ident = '', start = this.index, peekIndex, methodName, ch; this.index < this.text.length;){
                if (ch = this.text.charAt(this.index), '.' === ch || this.isIdent(ch) || this.isNumber(ch)) '.' === ch && this.index, ident += ch;
                else break;
                this.index++;
            }
            if (this.index) for(peekIndex = this.index; peekIndex < this.text.length;){
                if (ch = this.text.charAt(peekIndex), '(' === ch) {
                    methodName = ident.substr(this.index - start + 1), ident = ident.substr(0, this.index - start), this.index = peekIndex;
                    break;
                }
        ...
        readString: function(quote) {
            ...
            for(var string = '', rawString = quote, escape = !1; this.index < this.text.length;){
                ...
                else if (ch === quote) return this.index++, this.tokens.push({
                    index: start,
                    text: rawString,
                    string: string,
                    json: !0,
    ...
    Parser.ZERO = function() {
        ...
        parse: function(text, json) {
            ...
            return this.tokens.length !== 0 && this.throwError('is an unexpected token', this.tokens[0]), value.literal = !!value.literal, value.constant = !!value.constant, value;
        },
        primary: function() {
            var primary;
            ...
            for(var next, context; next = this.expect('(', '[', '.');)if (next.text === '(') primary = this.functionCall(primary, context), context = null;
            else if (next.text === '[') context = primary, primary = this.objectIndex(primary);
        ...
        peek: function(e1, e2, e3, e4) {
            if (this.tokens.length > 0) {
                ...
                if (e1 === t || e2 === t || e3 === t || e4 === t || (e1 || e2 || !e3) && !e4) return token;
            }
            return !1;
        },
        expect: function(e1, e2, e3, e4) {
            var token = this.peek(e1, e2, e3, e4);
            if (token) return this.json && !token.json && this.throwError('is not valid json', token), this.tokens.shift(), token;
            return !1;
        },
        consume: function(e1) {
        ...
        statements: function() {
            for(var statements = [];;)if (this.tokens.length > 0 && !this.peek('}', ')', ';', ']') && statements.push(this.filterChain()), !this.expect(';')) return statements.length === 1 ? statements[0] : function(self, locals) {
        ...
        filterChain: function() {
            for(var left = this.expression(), token;;)if (token = this.expect('|')) left = this.binaryFn(left, token.fn, this.filter());
            else return left;
        },
        filter: function() {
            for(var token = this.expect(), argsFn = [];;)if (token = this.expect(':')) argsFn.push(this.expression());
            else {
        ...
        assignment: function() {
            var left = this.ternary(), right, token;
            if (token = this.expect('=')) return !left.assign && this.throwError('implies assignment but [' + this.text.substring(0, token.index) + '] can not be assigned to', token), right = this.ternary(), function(scope, locals) {
                return left.assign(scope, right(scope, locals), locals);
            };
            return left;
        },
        ternary: function() {
            var left = this.logicalOR(), middle, token;
            if (token = this.expect('?')) {
                if (middle = this.ternary(), token = this.expect(':')) return this.ternaryFn(left, middle, this.ternary());
                this.throwError('expected :', token);
            } else return left;
        },
        logicalOR: function() {
            for(var left = this.logicalAND(), token;;)if (token = this.expect('||')) left = this.binaryFn(left, token.fn, this.logicalAND());
            else return left;
        },
        logicalAND: function() {
            var left = this.equality(), token;
            return (token = this.expect('&&')) && (left = this.binaryFn(left, token.fn, this.logicalAND())), left;
        },
        equality: function() {
            var left = this.relational(), token;
            return (token = this.expect('==', '!=', '===', '!==')) && (left = this.binaryFn(left, token.fn, this.equality())), left;
        },
        relational: function() {
            var left = this.additive(), token;
            return (token = this.expect('<', '>', '<=', '>=')) && (left = this.binaryFn(left, token.fn, this.relational())), left;
        },
        additive: function() {
            for(var left = this.multiplicative(), token; token = this.expect('+', '-');)left = this.binaryFn(left, token.fn, this.multiplicative());
            return left;
        },
        multiplicative: function() {
            for(var left = this.unary(), token; token = this.expect('*', '/', '%');)left = this.binaryFn(left, token.fn, this.unary());
            return left;
        },
        unary: function() {
            var token;
            if (this.expect('+')) return this.primary();
            if (token = this.expect('-')) return this.binaryFn(Parser.ZERO, token.fn, this.unary());
            if (token = this.expect('!')) return this.unaryFn(token.fn, this.unary());
            return this.primary();
        },
        ...
        objectIndex: function(obj) {
            ...
            return this.consume(']'), extend(function(self, locals) {
                ...
                if (!o) return void 0;
                return v = ensureSafeObject(o[i], parser.text), v && v.then && parser.options.unwrapPromises && (p = v, !('$$v' in v) && (p.$$v = void 0, p.then(function(val) {
                    p.$$v = val;
                })), v = v.$$v), v;
            }, {
    ...
    function setter(obj, path, setValue, fullExp, options) {
        ...
        for(var element = path.split('.'), key, i = 0; element.length > 1; i++){
            ...
            }, obj[key] = propertyObj), obj = propertyObj, obj.then && options.unwrapPromises && (promiseWarning(fullExp), !("$$v" in obj) && obj.then(function(val) {
                obj.$$v = val;
            }), obj.$$v === void 0 && (obj.$$v = {
            }), obj = obj.$$v);
        }
    ...
    function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
        return ensureSafeMemberName(key0, fullExp), ensureSafeMemberName(key1, fullExp), ensureSafeMemberName(key2, fullExp), ensureSafeMemberName(key3, fullExp), ensureSafeMemberName(key4, fullExp), !options.unwrapPromises ? function(scope, locals) {
            ...
            if (pathVal = pathVal[key0], key1 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key1], key2 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key2], key3 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key3], key4 && null !== pathVal && (void 0) === pathVal) return pathVal;
            return pathVal = pathVal[key4];
        } : function(scope, locals) {
            var pathVal = locals && locals.hasOwnProperty(key0) ? locals : scope, promise;
            if (null == pathVal) return pathVal;
            if (pathVal = pathVal[key0], pathVal && pathVal.then && (promiseWarning(fullExp), !("$$v" in pathVal) && (promise = pathVal, promise.$$v = void 0, promise.then(function(val) {
                promise.$$v = val;
            })), pathVal = pathVal.$$v), key1 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key1], pathVal && pathVal.then && (promiseWarning(fullExp), !("$$v" in pathVal) && (promise = pathVal, promise.$$v = void 0, promise.then(function(val) {
                promise.$$v = val;
            })), pathVal = pathVal.$$v), key2 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key2], pathVal && pathVal.then && (promiseWarning(fullExp), !("$$v" in pathVal) && (promise = pathVal, promise.$$v = void 0, promise.then(function(val) {
                promise.$$v = val;
            })), pathVal = pathVal.$$v), key3 && null !== pathVal && (void 0) === pathVal) return pathVal;
            if (pathVal = pathVal[key3], pathVal && pathVal.then && (promiseWarning(fullExp), !("$$v" in pathVal) && (promise = pathVal, promise.$$v = void 0, promise.then(function(val) {
                promise.$$v = val;
            })), pathVal = pathVal.$$v), key4 && null !== pathVal && (void 0) === pathVal) return pathVal;
            return pathVal = pathVal[key4], pathVal && pathVal.then && (promiseWarning(fullExp), !("$$v" in pathVal) && (promise = pathVal, promise.$$v = void 0, promise.then(function(val) {
                promise.$$v = val;
    ...
    function getterFn(path, options, fullExp) {
        ...
        if (options.csp) {
            if (6 > pathKeysLength) fn = cspSafeGetterFn(pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4], fullExp, options);
            else fn = function(scope, locals) {
                var i = 0, val;
    ...
    function $ParseProvider() {
        ...
        this.unwrapPromises = function(value) {
            if (isDefined(value)) return $parseOptions.unwrapPromises = !!value, this;
            return $parseOptions.unwrapPromises;
        }, this.logPromiseWarnings = function(value) {
            if (isDefined(value)) return $parseOptions.logPromiseWarnings = value, this;
            return $parseOptions.logPromiseWarnings;
        }, this.$get = ['$filter', '$sniffer', '$log', function($filter, $sniffer, $log) {
            return $parseOptions.csp = $sniffer.csp, promiseWarning = function(fullExp) {
                if (!$parseOptions.logPromiseWarnings || promiseWarningCache.hasOwnProperty(fullExp)) return;
                promiseWarningCache[fullExp] = !0, $log.warn('[$parse] Promise found in the expression `' + fullExp + "`. Automatic unwrapping of promises in Angular expressions is deprecated.");
            }, function(exp) {
    ...
    function qFactory(nextTick, exceptionHandler) {
                ...
                return deferred = {
                    ...
                    promise: {
                        then: function(callback, errback, progressback) {
                            var result = defer(), wrappedCallback = function(value1) {
                                    try {
                                        result.resolve((isFunction(callback) ? callback : defaultCallback)(value1));
                                    } catch (e) {
                            ...
                            if (pending) pending.push([wrappedCallback,
                                wrappedErrback,
                                wrappedProgressback]);
                            else value.then(wrappedCallback, wrappedErrback, wrappedProgressback);
                        ...
                        "finally": function(callback) {
                            ...
                            function handleCallback(value1, isResolved) {
                                ...
                                if (callbackOutput && isFunction(callbackOutput.then)) return callbackOutput.then(function() {
                            ...
                            return this.then(function(value1) {
        ...
        return {
            ...
            when: function(value, callback, errback, progressback) {
                var result = defer(), done, wrappedErrback = function(reason) {
                        try {
                ...
                return nextTick(function() {
                    ref(value).then(function(value1) {
                        if (done) return;
                        done = !0, result.resolve(ref(value1).then(function(value1) {
                            try {
                                return (isFunction(callback) ? callback : defaultCallback)(value1);
                            } catch (e) {
                                return exceptionHandler(e), reject(e);
                            }
    ...
    function $RootScopeProvider() {
        ...
        this.digestTtl = function(value) {
            ...
            function Scope() {
                this.$id = nextUid(), this.$$phase = this.$parent = this.$$watchers = this.$$nextSibling = this.$$prevSibling = this.$$childHead = this.$$childTail = null, this.this = this.$root = this, this.$$destroyed = !1, this.$$asyncQueue = [], this.$$postDigestQueue = [], this.$$listeners = {
            ...
            Scope.prototype = {
                ...
                $new: function(isolate) {
                    ...
                    if (isolate) child = new Scope(), child.$root = this.$root, child.$$asyncQueue = this.$$asyncQueue, child.$$postDigestQueue = this.$$postDigestQueue;
                    else ChildScope = function() {
                    }, ChildScope.prototype = this, child = new ChildScope(), child.$id = nextUid();
                    if (child.this = child, child.$$listeners = {
                    }, child.$parent = this, child.$$watchers = child.$$nextSibling = child.$$childHead = child.$$childTail = null, child.$$prevSibling = this.$$childTail, this.$$childHead) this.$$childTail.$$nextSibling = child, this.$$childTail = child;
                ...
                $watch: function(watchExp, listener, objectEquality) {
                    ...
                    return lastDirtyWatch = null, !isFunction(listener) && (watcher.fn = function(newVal, oldVal, scope) {
                        compileToFn(listener || noop, 'listener')(scope);
                    }), 'string' == typeof watchExp && get.constant && (watcher.fn = function(newVal, oldVal, scope) {
                        watcher.fn.call(this, newVal, oldVal, scope), arrayRemove(array, watcher);
                    }), !array && (array = scope.$$watchers = []), array.unshift(watcher), function() {
                        arrayRemove(array, watcher);
                    };
                },
                $watchCollection: function(obj, listener) {
                    var self = this, oldValue, newValue, changeDetected = 0, objGetter = $parse(obj), internalArray = [], internalObject = {
                        }, oldLength = 0;
                    return this.$watch(function() {
                        newValue = objGetter(self);
                        var newLength, key;
                        if (!isObject(newValue)) newValue !== oldValue && (oldValue = newValue, changeDetected++);
                        else if (isArrayLike(newValue)) {
                            internalArray !== oldValue && (oldValue = internalArray, oldLength = oldValue.length = 0, changeDetected++), newLength = newValue.length, newLength !== oldLength && (changeDetected++, oldValue.length = oldLength = newLength);
                            for(var i = 0; newLength > i; i++)oldValue[i] !== newValue[i] && (changeDetected++, oldValue[i] = newValue[i]);
                        } else {
                            internalObject !== oldValue && (oldValue = internalObject = {
                            }, oldLength = 0, changeDetected++), newLength = 0;
                            for(key in newValue)if (newValue.hasOwnProperty(key)) {
                                if (newLength++, oldValue.hasOwnProperty(key)) oldValue[key] !== newValue[key] && (changeDetected++, oldValue[key] = newValue[key]);
                                else oldLength++, oldValue[key] = newValue[key], changeDetected++;
                            }
                ...
                $digest: function() {
                    var watch, value, last, asyncQueue = this.$$asyncQueue, postDigestQueue = this.$$postDigestQueue, length, dirty, ttl = TTL, next, current, target = this, watchLog = [], logIdx, logMsg, asyncTask;
                    ...
                    do {
                        ...
                        traverseScopesLoop: do {
                            if (current.$$watchers) for(length = current.$$watchers.length; length--;)try {
                                if (watch = current.$$watchers[length]) {
                                    if ((value = watch.get(current)) !== (last = watch.last) && !(watch.eq ? equals(value, last) : 'number' == typeof value && 'number' == typeof last && isNaN(value) && isNaN(last))) dirty = !0, lastDirtyWatch = watch, watch.last = watch.eq ? copy(value) : value, watch.fn(value, initWatchVal === last ? value : last, current), 5 > ttl && (logIdx = 4 - ttl, !watchLog[logIdx] && (watchLog[logIdx] = []), logMsg = isFunction(watch.exp) ? 'fn: ' + (watch.exp.name || watch.exp.toString()) : watch.exp, logMsg += '; newVal: ' + toJson(value) + '; oldVal: ' + toJson(last), watchLog[logIdx].push(logMsg));
                ...
                $destroy: function() {
                    ...
                    if (this.$broadcast('$destroy'), this.$$destroyed = !0, this === $rootScope) return;
                ...
                $emit: function(name, args) {
                    var namedListeners, scope = this, stopPropagation = !1, event = {
                            name: name,
                            targetScope: scope,
                    ...
                    do {
                        for(namedListeners = scope.$$listeners[name] || [], event.currentScope = scope, i = 0, length = namedListeners.length; length > i; i++){
                            if (!namedListeners[i]) {
    ...
    function $$SanitizeUriProvider() {
        ...
        this.aHrefSanitizationWhitelist = function(regexp) {
            ...
            return function(uri, isImage) {
                if (!msie || msie >= 8) {
                    if (urlResolve(uri).href, urlResolve(uri).href !== '' && !urlResolve(uri).href.match(isImage ? imgSrcSanitizationWhitelist : aHrefSanitizationWhitelist)) return 'unsafe:' + urlResolve(uri).href;
                }
                return uri;
            };
        };
    ...
    function adjustMatcher(matcher) {
        ...
        else throw $sceMinErr('imatcher', 'Matchers may only be "self", string patterns or RegExp objects');
    }
    ...
    function $SceDelegateProvider() {
        ...
        this.resourceUrlWhitelist = function(value) {
            ...
            function isResourceUrlAllowedByPolicy(url) {
                var parsedUrl = urlResolve(url.toString()), i, n, allowed = !1;
            ...
            return byType[SCE_CONTEXTS.HTML] = generateHolderType(trustedValueHolderBase), byType[SCE_CONTEXTS.CSS] = generateHolderType(trustedValueHolderBase), byType[SCE_CONTEXTS.URL] = generateHolderType(trustedValueHolderBase), byType[SCE_CONTEXTS.JS] = generateHolderType(trustedValueHolderBase), byType[SCE_CONTEXTS.RESOURCE_URL] = generateHolderType(byType[SCE_CONTEXTS.URL]), {
                ...
                getTrusted: function(type, maybeTrusted) {
                    ...
                    } else if (type === SCE_CONTEXTS.HTML) return htmlSanitizer(maybeTrusted);
                    throw $sceMinErr('unsafe', 'Attempting to use an unsafe value in a safe context.');
                },
                valueOf: function(maybeTrusted) {
                    if (maybeTrusted instanceof trustedValueHolderBase) return maybeTrusted.$$unwrapTrustedValue();
                    return maybeTrusted;
                }
            };
        }];
    ...
    function $SnifferProvider() {
        this.$get = ['$window', '$document', function($window, $document) {
            var eventSupport = {
                ...
                }).userAgent), document1 = $document[0] || {
                }, documentMode = document1.documentMode, vendorPrefix, bodyStyle = document1.body && document1.body.style, transitions = !1, animations = !1, match;
            if (bodyStyle) {
                for(var prop in bodyStyle)if (match = /^(Moz|webkit|O|ms)(?=[A-Z])/.exec(prop)) {
            ...
            return {
                ...
                hasEvent: function(event) {
                    ...
                    return isUndefined(eventSupport[event]) && (eventSupport[event] = 'on' + event in document1.createElement('div')), eventSupport[event];
                },
                csp: csp(),
                vendorPrefix: vendorPrefix,
    ...
    var urlParsingNode = document.createElement("a"), originUrl = urlResolve(window.location.href, !0);
    function urlResolve(url, base) {
        return msie && (urlParsingNode.setAttribute("href", urlParsingNode.href), urlParsingNode.href), urlParsingNode.setAttribute('href', urlParsingNode.href), {
    ...
    function filterFilter() {
        return function(array, expression, comparator) {
            ...
            if (predicates.check = function(value) {
                for(var j = 0; j < predicates.length; j++)if (!predicates[j](value)) return !1;
                return !0;
            }, 'function' !== comparatorType) {
                if ('boolean' === comparatorType && comparator) comparator = function(obj, text) {
                    return angular.equals(obj, text);
                };
                else comparator = function(obj, text) {
            ...
            var search = function(obj, text) {
                ...
                case "boolean":
                case "number":
                case "string": return comparator(obj, text);
                case "object":
                    switch(typeof text){
                    case "object": return comparator(obj, text);
                    default:
            ...
            case "object":
                for(var key in expression)if ('$' == key) !function() {
    ...
    function currencyFilter($locale) {
        ...
        return function(amount, currencySymbol) {
            return isUndefined(formats.CURRENCY_SYM) && formats.CURRENCY_SYM, formatNumber(amount, formats.PATTERNS[1], formats.GROUP_SEP, formats.DECIMAL_SEP, 2).replace(/\u00A4/g, formats.CURRENCY_SYM);
        };
    }
    ...
    function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
        ...
        } else fractionSize > 0 && number > -1 && 1 > number && (formatedText = number.toFixed(fractionSize));
    ...
    function padNumber(num, digits, trim1) {
        for(0 > num && (num = -num), num = '' + num; num.length < digits;)num = '0' + num;
        return trim1 && (num = num.substr(num.length - digits)), '-' + num;
    }
    function dateGetter(name, size, offset, trim1) {
        return offset ||= 0, function(date) {
            var value = date['get' + name]();
            return (offset > 0 || value > -offset) && (value += offset), 0 === value && -12 == offset && (value = 12), padNumber(value, size, trim1);
        };
    }
    ...
    function dateFilter($locale) {
        ...
        function jsonStringToDate(string) {
            ...
            if (match = string.match(R_ISO8601_STR)) {
                ...
                var h = int(match[4] || 0) - tzHour, m = int(match[5] || 0) - tzMin, s = int(match[6] || 0), ms = Math.round(parseFloat('0.' + (match[7] || 0)) * 1000);
        ...
        return function(date, format) {
            ...
            if (format ||= 'mediumDate', format = $locale.DATETIME_FORMATS[format] || format, isString(date)) {
                if (/^\-?\d+$/.test(date)) date = int(date);
                else date = jsonStringToDate(date);
            }
            if (isNumber(date) && (date = new Date(date)), !isDate(date)) return date;
            for(; format;)if (match = /((?:[^yMdHhmsaZE']+)|(?:'(?:[^']|'')*')|(?:E+|y+|M+|d+|H+|h+|m+|s+|a|Z))(.*)/.exec(format)) parts = concat(parts, match, 1), format = parts.pop();
            ...
            return forEach(parts, function(value) {
                fn = ({
                    ...
                    a: function(date1, formats) {
                        return date1.getHours() < 12 ? formats.AMPMS[0] : formats.AMPMS[1];
                    },
                    Z: function(date1) {
                        var zone = -1 * date1.getTimezoneOffset(), paddedZone = zone >= 0 ? "+" : "";
                        return paddedZone += padNumber(Math[zone > 0 ? 'floor' : 'ceil'](zone / 60), 2) + padNumber(Math.abs(zone % 60), 2), paddedZone;
                    }
                })[value], text += fn ? fn(date, $locale.DATETIME_FORMATS) : value.replace(/(^'|'$)/g, '').replace(/''/g, "'");
            }), text;
        };
    }
    function jsonFilter() {
    ...
    var htmlAnchorDirective = valueFn({
            ...
            compile: function(element, attr) {
                if (8 >= msie && (attr.href || attr.name || attr.$set('href', ''), element.append(document.createComment('IE fix'))), !attr.href && !attr.name) return function(scope, element1) {
            ...
            return {
                ...
                link: function(scope, element, attr) {
                    attr.$observe(normalized, function(value) {
                        if (!value) return;
                        attr.$set(attrName, value), msie && element.prop(attrName, attr[attrName]);
                    });
                }
            };
        };
    ...
    function FormController(element, attrs) {
        ...
        form.$name = attrs.name || attrs.ngForm, form.$dirty = !1, form.$pristine = !0, form.$valid = !0, form.$invalid = !1, parentForm.$addControl(form), element.addClass(PRISTINE_CLASS), toggleValidCss(!0);
    ...
    var formDirectiveFactory = function(isNgForm) {
        ...
        }, formDirective = formDirectiveFactory(), ngFormDirective = formDirectiveFactory(!0), inputType = {
            'text': textInputType,
    ...
    function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        if ($sniffer.hasEvent('input')) element.on('input', listener);
            ...
            element.on('keydown', function(event) {
                ...
                if (91 === key || 15 < key && 19 > key || 37 <= key && 40 >= key) return;
                deferListener();
        ...
        var pattern = attr.ngPattern, patternValidator, match, validate = function(regexp, value) {
            if (ctrl.$isEmpty(value) || regexp.test(value)) return ctrl.$setValidity('pattern', !0), value;
            return ctrl.$setValidity('pattern', !1), void 0;
        };
        if (pattern) {
            if (match = pattern.match(/^\/(.*)\/([gim]*)$/)) pattern = new RegExp(match[1], match[2]), patternValidator = function(value) {
                return validate(pattern, value);
            };
            else patternValidator = function(value) {
        ...
        if (attr.ngMinlength) {
            var minlength = int(attr.ngMinlength), minLengthValidator = function(value) {
                if (!ctrl.$isEmpty(value) && value.length < minlength) return ctrl.$setValidity('minlength', !1), void 0;
        ...
        if (attr.ngMaxlength) {
            var maxlength = int(attr.ngMaxlength), maxLengthValidator = function(value) {
                if (!ctrl.$isEmpty(value) && value.length > maxlength) return ctrl.$setValidity('maxlength', !1), void 0;
    ...
    function numberInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        if (textInputType(scope, element, attr, ctrl, $sniffer, $browser), ctrl.$parsers.push(function(value) {
            ...
            if (empty || /^\s*(\-|\+)?(\d+|(\d*(\.\d*)))\s*$/.test(value)) return ctrl.$setValidity('number', !0), '' === value ? null : empty ? value : parseFloat(value);
            ...
            var minValidator = function(value) {
                ...
                if (!ctrl.$isEmpty(value) && min > value) return ctrl.$setValidity('min', !1), void 0;
        ...
        if (attr.max) {
            var maxValidator = function(value) {
                ...
                if (!ctrl.$isEmpty(value) && value > max) return ctrl.$setValidity('max', !1), void 0;
    ...
    function urlInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        var urlValidator = function(value) {
            if (ctrl.$isEmpty(value) || /^(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?$/.test(value)) return ctrl.$setValidity('url', !0), value;
    ...
    function emailInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        var emailValidator = function(value) {
            if (ctrl.$isEmpty(value) || /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,6}$/.test(value)) return ctrl.$setValidity('email', !0), value;
    ...
    var inputDirective = ['$browser', '$sniffer', function($browser, $sniffer) {
        ...
        }], VALID_CLASS = 'ng-valid', INVALID_CLASS = 'ng-invalid', PRISTINE_CLASS = 'ng-pristine', DIRTY_CLASS = 'ng-dirty', ngChangeDirective = valueFn({
            require: 'ngModel',
            link: function(scope, element, attr, ctrl) {
                ...
                link: function(scope, element, attr) {
                    ...
                    function ngClassWatchAction(newVal) {
                        if (!0 === selector || scope.$index % 2 === selector) {
                            ...
                            else !equals(newVal, oldVal) && attr.$updateClass(newClasses, flattenClasses(oldVal));
                        }
                        oldVal = copy(newVal);
                    }
    ...
    (function() {
        if (jQuery = window.jQuery) jqLite = jQuery, extend(jQuery.fn, {
            scope: JQLitePrototype.scope,
            isolateScope: JQLitePrototype.isolateScope,
            controller: JQLitePrototype.controller,
            injector: JQLitePrototype.injector,
            inheritedData: JQLitePrototype.inheritedData
        }), jqLitePatchJQueryRemove('remove', !0, !0, !1), jqLitePatchJQueryRemove('empty', !1, !1, !1), jqLitePatchJQueryRemove('html', !1, !1, !0);
        else jqLite = JQLite;
        ...
        angularModule('ng', ['ngLocale'], ['$provide', function($provide) {
            $provide.provider({
                ...
                form: formDirective,
                script: ['$templateCache', function($templateCache) {
                    return {
                        restrict: 'E',
                        terminal: !0,
                        compile: function(element, attr) {
                ...
                select: ['$compile', '$parse', function($compile, $parse) {
                    var NG_OPTIONS_REGEXP = /^\s*(.*?)(?:\s+as\s+(.*?))?(?:\s+group\s+by\s+(.*))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+(.*?)(?:\s+track\s+by\s+(.*?))?$/;
                    return {
                        restrict: 'E',
                        require: ['select', '?ngModel'],
                        controller: ['$element', '$scope', '$attrs', function($element, $scope, $attrs) {
                            var self = this, optionsMap = {
                                }, ngModelCtrl = {
                                    $setViewValue: noop
                        ...
                        link: function(scope, element, attr, ctrls) {
                            if (!ctrls[1]) return;
                            for(var selectCtrl = ctrls[0], ngModelCtrl = ctrls[1], multiple = attr.multiple, optionsExp = attr.ngOptions, nullOption = !1, emptyOption, optionTemplate = jqLite(document.createElement('option')), unknownOption = optionTemplate.clone(), i = 0, children = element.children(), ii = children.length; ii > i; i++)if (children[i].value === '') {
                            ...
                            function setupAsSingle(scope1, selectElement, ngModelCtrl1, selectCtrl1) {
                                ngModelCtrl1.$render = function() {
                                    var viewValue = ngModelCtrl1.$viewValue;
                                    if (selectCtrl1.hasOption(viewValue)) unknownOption.parent() && unknownOption.remove(), selectElement.val(viewValue), '' === viewValue && emptyOption.prop('selected', !0);
                                    else if (isUndefined(viewValue) && emptyOption) selectElement.val('');
                                    else selectCtrl1.renderUnknownOption(viewValue);
                                }, selectElement.on('change', function() {
                                    scope1.$apply(function() {
                                        unknownOption.parent() && unknownOption.remove(), ngModelCtrl1.$setViewValue(selectElement.val());
                                    });
                                });
                            }
                            function setupAsMultiple(scope1, selectElement, ctrl) {
                                var lastView;
                                ctrl.$render = function() {
                                    forEach(selectElement.find('option'), function(option) {
                                        option.selected = isDefined(new HashMap(ctrl.$viewValue).get(option.value));
                                    });
                            ...
                            function setupAsOptions(scope1, selectElement, ctrl) {
                                ...
                                var displayFn = $parse(match[2] || match[1]), valueName = match[4] || match[6], keyName = match[5], groupByFn = $parse(match[3] || ''), valueFn1 = $parse(match[2] ? match[1] : valueName), valuesFn = $parse(match[7]), track = match[8], trackFn = track ? $parse(match[8]) : null, optionGroupsCache = [[{
                                    element: selectElement,
                                    label: ''
                                }]];
                                nullOption && ($compile(nullOption)(scope1), nullOption.removeClass('ng-scope'), nullOption.remove()), selectElement.empty(), selectElement.on('change', function() {
                                    scope1.$apply(function() {
                                        var optionGroup, collection = valuesFn(scope1) || [], locals = {
                                            }, key, value, index, groupIndex, trackIndex;
                                        if (multiple) for(value = [], groupIndex = 0, optionGroupsCache.length; groupIndex < optionGroupsCache.length; groupIndex++)for(optionGroup = optionGroupsCache[groupIndex], index = 1, optionGroup.length; index < optionGroup.length; index++)if (optionGroup[index].element[0].selected) {
                                            if (key = optionGroup[index].element.val(), keyName && (locals[keyName] = key), trackFn) for(trackIndex = 0; trackIndex < collection.length; trackIndex++)if (locals[valueName] = collection[trackIndex], trackFn(scope1, locals) == key) break;
                                            else locals[valueName] = collection[key];
                                            value.push(valueFn1(scope1, locals));
                                        }
                                        else if (key = selectElement.val(), '?' == key) value = void 0;
                                        else if ('' === key) value = null;
                                        else if (trackFn) for(trackIndex = 0; trackIndex < collection.length; trackIndex++)if (locals[valueName] = collection[trackIndex], trackFn(scope1, locals) == key) {
                                            value = valueFn1(scope1, locals);
                                            break;
                                        }
                                        else locals[valueName] = collection[key], keyName && (locals[keyName] = key), value = valueFn1(scope1, locals);
                                        ctrl.$setViewValue(value);
                                    });
                                }), ctrl.$render = render, scope1.$watch(render);
                                function render() {
                                    var optionGroups = {
                                            '': []
                                        }, optionGroupNames = [''], optionGroupName, optionGroup, option, existingParent, existingOptions, existingOption, modelValue = ctrl.$modelValue, values = valuesFn(scope1) || [], keys = keyName ? sortedKeys(values) : values, key, length, groupIndex, index, locals = {
                                        }, selected, selectedSet = !1, lastElement, element, label;
                                    if (multiple) {
                                        if (trackFn && isArray(modelValue)) {
                                            selectedSet = new HashMap([]);
                                            for(var trackIndex = 0; trackIndex < modelValue.length; trackIndex++)locals[valueName] = modelValue[trackIndex], selectedSet.put(trackFn(scope1, locals), modelValue[trackIndex]);
                                    ...
                                    for(index = 0; length = keys.length, length > index; index++){
                                        ...
                                        if (locals[valueName] = values[key], optionGroupName = groupByFn(scope1, locals) || '', !(optionGroup = optionGroups[optionGroupName]) && (optionGroup = optionGroups[optionGroupName] = [], optionGroupNames.push(optionGroupName)), multiple) selected = isDefined(selectedSet.remove(trackFn ? trackFn(scope1, locals) : valueFn1(scope1, locals)));
                                        else {
                                            if (trackFn) {
                                                var modelCast = {
                                                };
                                                modelCast[valueName] = modelValue, selected = trackFn(scope1, modelCast) === trackFn(scope1, locals);
                                            } else selected = modelValue === valueFn1(scope1, locals);
                                            selectedSet ||= selected;
                                        }
                                        label = displayFn(scope1, locals), label = isDefined(label) ? label : '', optionGroup.push({
                                            id: trackFn ? trackFn(scope1, locals) : keyName ? keys[index] : index,
                                            label: label,
                                            selected: selected
                                        });
                                    }
                                    if (!multiple) {
                                        if (nullOption || null === modelValue) optionGroups[''].unshift({
                                            id: '',
                                            label: '',
                                            selected: !selectedSet
                                        });
                                        else !selectedSet && optionGroups[''].unshift({
                                            id: '?',
                                            label: '',
                                            selected: !0
                                        });
                                    }
                                    for(groupIndex = 0, optionGroupNames.length; groupIndex < optionGroupNames.length; groupIndex++){
                                        if (optionGroupName = optionGroupNames[groupIndex], optionGroup = optionGroups[optionGroupName], optionGroupsCache.length <= groupIndex) existingParent = {
                                            element: jqLite(document.createElement('optgroup')).clone().attr('label', optionGroupName),
                                            label: optionGroup.label
                                        }, existingOptions = [existingParent], optionGroupsCache.push(existingOptions), selectElement.append(existingParent.element);
                                        else existingOptions = optionGroupsCache[groupIndex], existingParent = existingOptions[0], existingParent.label != optionGroupName && existingParent.element.attr('label', existingParent.label = optionGroupName);
                                        for(lastElement = null, index = 0, length = optionGroup.length; length > index; index++)if (option = optionGroup[index], existingOption = existingOptions[index + 1]) lastElement = existingOption.element, existingOption.label !== option.label && lastElement.text(existingOption.label = option.label), existingOption.id !== option.id && lastElement.val(existingOption.id = option.id), lastElement[0].selected !== option.selected && lastElement.prop('selected', existingOption.selected = option.selected);
                                        else {
                                            if (option.id === '' && nullOption) element = nullOption;
                                            ...
                                            }), lastElement) lastElement.after(element);
                ...
                option: ['$interpolate', function($interpolate) {
                    return {
                        restrict: 'E',
                        priority: 100,
                        compile: function(element, attr) {
                            ...
                            return function(scope, element, attr1) {
                                ...
                                if (selectCtrl && selectCtrl.databound) element.prop('selected', !1);
                                else selectCtrl = {
                                    addOption: noop,
                                    removeOption: noop
                                };
                                if (interpolateFn) scope.$watch(interpolateFn, function(newVal, oldVal) {
                                    attr1.$set('value', newVal), newVal !== oldVal && selectCtrl.removeOption(oldVal), selectCtrl.addOption(newVal);
                                });
                ...
                ngBindHtml: ['$sce', '$parse', function($sce, $parse) {
                    return function(scope, element, attr) {
                ...
                ngBindTemplate: ['$interpolate', function($interpolate) {
                    return function(scope, element, attr) {
                ...
                ngHide: ['$animate', function($animate) {
                    return function(scope, element, attr) {
                    ...
                    return {
                        ...
                        link: function($scope, $element, $attr, ctrl, $transclude) {
                            ...
                            $scope.$watch($attr.ngIf, function(value) {
                                if (toBoolean(value)) !childScope && (childScope = $scope.$new(), $transclude(childScope, function(clone) {
                                    clone[clone.length++] = document.createComment(' end ngIf: ' + $attr.ngIf + ' '), block = {
                                        clone: clone
                                    }, $animate.enter(clone, $element.parent(), $element);
                                }));
                                else childScope && (childScope.$destroy(), childScope = null), block && ($animate.leave(getBlockElements(block.clone)), block = null);
                            });
                        }
                    };
                }],
                ngInclude: ['$http', '$templateCache', '$anchorScroll', '$animate', '$sce', function($http, $templateCache, $anchorScroll, $animate, $sce) {
                    return {
                        restrict: 'ECA',
                        priority: 400,
                        terminal: !0,
                        ...
                        compile: function(element, attr) {
                            ...
                            return function(scope, $element, $attr, ctrl, $transclude) {
                                var changeCounter = 0, currentScope, currentElement, cleanupLastIncludeContent = function() {
                                ...
                                scope.$watch($sce.parseAsResourceUrl(srcExp), function(src) {
                                    ...
                                    if (src) $http.get(src, {
                                        ...
                                        if (changeCounter !== thisChangeId) return;
                                        var newScope = scope.$new();
                                        ctrl.template = response;
                                        var clone = $transclude(newScope, function(clone) {
                                            cleanupLastIncludeContent(), $animate.enter(clone, null, $element, function() {
                                                isDefined(autoScrollExp) && (!autoScrollExp || scope.$eval(autoScrollExp)) && $anchorScroll();
                                            });
                                        });
                                        currentScope = newScope, currentElement = clone, currentScope.$emit('$includeContentLoaded'), scope.$eval(onloadExp);
                                    }).error(function() {
                                        changeCounter === thisChangeId && cleanupLastIncludeContent();
                                    }), scope.$emit('$includeContentRequested');
                                    else cleanupLastIncludeContent(), ctrl.template = null;
                                });
                            };
                        }
                    };
                ...
                ngPluralize: ['$locale', '$interpolate', function($locale, $interpolate) {
                    var BRACE = /{}/g;
                    return {
                        restrict: 'EA',
                        link: function(scope, element, attr) {
                            var numberExp = attr.count, whenExp = attr.$attr.when && element.attr(attr.$attr.when), offset = attr.offset || 0, whens = scope.$eval(whenExp) || {
                                }, whensExpFns = {
                                }, startSymbol = $interpolate.startSymbol(), endSymbol = $interpolate.endSymbol();
                            forEach(attr, function(expression, attributeName) {
                                /^when(Minus)?(.+)$/.test(attributeName) && (whens[lowercase(attributeName.replace('when', '').replace('Minus', '-'))] = element.attr(attr.$attr[attributeName]));
                            }), forEach(whens, function(expression, key) {
                                whensExpFns[key] = $interpolate(expression.replace(BRACE, startSymbol + numberExp + '-' + offset + endSymbol));
                            }), scope.$watch(function() {
                                var value = parseFloat(scope.$eval(numberExp));
                                if (!isNaN(value)) return !(value in whens) && (value = $locale.pluralCat(value - offset)), whensExpFns[value](scope, element, !0);
                                return '';
                            }, function(newVal) {
                                element.text(newVal);
                            });
                ...
                ngRepeat: ['$parse', '$animate', function($parse, $animate) {
                    ...
                    return {
                        ...
                        $$tlb: !0,
                        link: function($scope, $element, $attr, ctrl, $transclude) {
                            var expression = $attr.ngRepeat, match = expression.match(/^\s*(.+)\s+in\s+(.*?)\s*(\s+track\s+by\s+(.+)\s*)?$/), trackByExp, trackByExpGetter, trackByIdExpFn, trackByIdArrayFn, trackByIdObjFn, lhs, rhs, valueIdentifier, keyIdentifier, hashFnLocals = {
                                $id: hashKey
                            };
                            ...
                            else trackByIdArrayFn = function(key, value) {
                            ...
                            $scope.$watchCollection(rhs, function(collection) {
                                var index, length, previousNode = $element[0], nextNode, nextBlockMap = {
                                    }, childScope, key, value, trackById, trackByIdFn, collectionKeys, block, nextBlockOrder = [], elementsToRemove;
                                ...
                                for(index = 0, length = collectionKeys.length; length > index; index++){
                                    ...
                                    childScope[valueIdentifier] = value, keyIdentifier && (childScope[keyIdentifier] = key), childScope.$index = index, childScope.$first = 0 === index, childScope.$last = index === collectionKeys.length - 1, childScope.$middle = !(childScope.$first || childScope.$last), childScope.$odd = !(childScope.$even = (1 & index) == 0), !block.scope && $transclude(childScope, function(clone) {
                ...
                ngShow: ['$animate', function($animate) {
                    return function(scope, element, attr) {
                ...
                ngSwitch: ['$animate', function($animate) {
                    return {
                        restrict: 'EA',
                        require: 'ngSwitch',
                ...
                ngModel: function() {
                    return {
                        ...
                        controller: ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', function($scope, $exceptionHandler, $attr, $element, $parse) {
                            ...
                            this.$setValidity = function(validationErrorKey, isValid) {
                                ...
                                if (isValid) $error[validationErrorKey] && invalidCount--, !invalidCount && (toggleValidCss(!0), this.$valid = !0, this.$invalid = !1);
                                else toggleValidCss(!1), this.$invalid = !0, this.$valid = !1, invalidCount++;
                ...
                ngValue: function() {
                    return {
                        ...
                        compile: function(tpl, tplAttr) {
                            if (/^(true|false|\d+)$/.test(tplAttr.ngValue)) return function(scope, elm, attr) {
                ...
                ngInclude: ['$compile', function($compile) {
                    return {
                        restrict: 'ECA',
                        priority: -400,
                        require: 'ngInclude',
                ...
                $animate: ['$provide', function($provide1) {
                    this.$$selectors = {
                    }, this.register = function(name, factory) {
                        var key = name + '-animation';
                        if (name && name.charAt(0) != '.') throw $animateMinErr('notcsel', "Expecting class selector starting with '.' got '{0}'.", name);
                        this.$$selectors[name.substr(1)] = key, $provide1.factory(key, factory);
                    }, this.$get = ['$timeout', function($timeout) {
                        return {
                            enter: function(element, parent, after, done) {
                                if (after) after.after(element);
                                else (!parent || !parent[0]) && (parent = after.parent()), parent.append(element);
