!function() {
    ...
    associate: function(keys) {
        for(var obj = {
        }, length = Math.min(this.length, keys.length), i = 0; i < length; i++)obj[keys[i]] = this[i];
        return obj;
    },
    link: function(object) {
        for(var result = {
        }, i = 0, l = this.length; i < l; i++)for(var key in object)if (object[key](this[i])) {
...
!function() {
    ...
    var XMLHTTP, MSXML2, MSXML, document = this.document, window = document.window = this, ua = navigator.userAgent.toLowerCase(), platform = navigator.platform.toLowerCase(), UA = ua.match(/(opera|ie|firefox|chrome|version)[\s\/:]([\w\d\.]+)?.*?(safari|version[\s\/:]([\w\d\.]+)|$)/) || [null, "unknown", 0], mode = "ie" == UA[1] && document.documentMode, Browser = this.Browser = {
...
Event1.Keys = {
    ...
    local.search = function(context, expression, append, first) {
        ...
        }, hasOthers = !!(append && append.length), contextIsDocument = 9 == context.nodeType;
        ...
        search: for(i = 0; currentExpression = expressions[i]; i++)for(j = 0; currentBit = currentExpression[j]; j++){
            ...
            }, lastBit ? (this.uniques = uniques, this.found = found) : (this.uniques = {
            }, this.found = []), 0 === j) {
...
var IFrame = new Type("IFrame", function() {
    ...
    delete props.onload, props.id = props.name = [props.id, props.name, iframe ? iframe.id || iframe.name : "IFrame_" + String.uniqueID()].pick();
    var onLoad = function() {
    ...
    if (nodes && nodes.length) for(var node, uniques = {
    }, i = 0; node = nodes[i++];){
...
Elements.prototype = {
    ...
    Element1.implement({
        ...
        clone: function(contents, keepid) {
            contents = !1 !== contents;
            var i, clone = this.cloneNode(contents), ce = [clone], te = [this];
    ...
    var proto = Element1.prototype, addEvent = proto.addEvent, removeEvent = proto.removeEvent, relay = function(old, method) {
        ...
        removeEvent: function(type, match, fn, _uid) {
            var __uid, s, storage = this.retrieve("$delegates", {
            }), stored = storage[type];
    ...
    Color: {
        parse: function(value) {
            return value.match(/^#[0-9a-f]{3,6}$/i) ? value.hexToRgb(!0) : !!(value = value.match(/(\d+),\s*(\d+),\s*(\d+)/)) && [value[1], value[2], value[3]];
        },
        compute: function(from, to, delta) {
    ...
    fade: function(how) {
        var method, toggle, fade = this.get("tween"), args = ["opacity"].append(arguments);
