!function() {
    ...
    enumerables && (enumerables = ['hasOwnProperty', 'valueOf', 'isPrototypeOf', 'propertyIsEnumerable', 'toLocaleString', 'toString', 'constructor']), Function1.prototype.overloadSetter = function(usePlural) {
        var self = this;
        return function(a, b) {
        ...
        return function(a) {
            var args, result;
            if ('string' != typeof a) args = a;
            else if (arguments.length > 1) args = arguments;
            else usePlural && (args = [a]);
            if (args) {
                result = {
                };
    ...
    Function1.from = function(item) {
        ...
        if (null == item) return [];
        return Type.isEnumerable(item) && 'string' != typeof item ? typeOf(item) == 'array' ? item : slice.call(item) : [item];
    ...
    var Type = this.Type = function(name, object) {
        ...
        return object.extend(this), object.$constructor = Type, object.prototype.$constructor = object, object;
    };
    Type.isEnumerable = function(item) {
        return null != item && 'number' == typeof item.length && Object.prototype.toString.call(item) != '[object Function]';
    };
    var hooks = {
            ...
            (null == previous || !previous.$protected) && (this.prototype[name] = method), this[name] == null && typeOf(method) == 'function' && extend.call(this, name, function(item) {
    ...
    var force = function(name, object, methods) {
        ...
        return isType && (object.forEachMethod = function(fn) {
            if (!prototype.propertyIsEnumerable(methods[0])) for(var i = 0, l = methods.length; l > i; i++)fn.call(prototype, prototype[methods[i]], methods[i]);
            for(var key in prototype)fn.call(prototype, prototype[key], key);
        }), force;
    };
    force('String', String, ['charAt', 'charCodeAt', 'concat', 'indexOf', 'lastIndexOf', 'match', 'quote', 'replace', 'search', 'slice', 'split', 'substr', 'substring', 'trim', 'toLowerCase', 'toUpperCase'])('Array', Array, ['pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift', 'concat', 'join', 'slice', 'indexOf', 'lastIndexOf', 'filter', 'forEach', 'every', 'map', 'some', 'reduce', 'reduceRight'])('Number', Number, ['toExponential', 'toFixed', 'toLocaleString', 'toPrecision'])('Function', Function1, ['apply', 'call', 'bind'])('RegExp', RegExp, ['exec', 'test'])('Object', Object, ['create', 'defineProperty', 'defineProperties', 'keys', 'getPrototypeOf', 'getOwnPropertyDescriptor', 'getOwnPropertyNames', 'preventExtensions', 'isExtensible', 'seal', 'isSealed', 'freeze', 'isFrozen'])('Date', Date, ['now']), Object.extend = extend.overloadSetter(), Date.extend('now', function() {
        return +new Date;
        ...
        for(var key in object)Object.prototype.hasOwnProperty.call(object, key) && fn.call(bind, object[key], key, object);
    ...
    Native.type = Type.type, Native.implement = function(objects, methods) {
        ...
        return instanceOf(item, Array) || Array.type(item);
    ...
    every: function(fn, bind) {
        for(var i = 0, l = this.length >>> 0; l > i; i++)if (i in this && !fn.call(bind, this[i], i, this)) return !1;
        return !0;
    },
    filter: function(fn, bind) {
        for(var results = [], value, i = 0, l = this.length >>> 0; l > i; i++)i in this && (value = this[i], fn.call(bind, value, i, this) && results.push(value));
        return results;
    },
    indexOf: function(item, from) {
        for(var length = this.length >>> 0, i = 0 > from ? Math.max(0, length + from) : from || 0; length > i; i++)if (this[i] === item) return i;
        return -1;
    },
    ...
    some: function(fn, bind) {
        for(var i = 0, l = this.length >>> 0; l > i; i++)if (i in this && fn.call(bind, this[i], i, this)) return !0;
        return !1;
    },
    clean: function() {
    ...
    associate: function(keys) {
        for(var obj = {
            }, length = Math.min(this.length, keys.length), i = 0; length > i; i++)obj[keys[i]] = this[i];
        return obj;
    },
    link: function(object) {
        for(var result = {
            }, i = 0, l = this.length; l > i; i++)for(var key in object)if (object[key](this[i])) {
    ...
    flatten: function() {
        for(var array = [], i = 0, l = this.length; l > i; i++){
            ...
            array = array.concat('array' == type || 'collection' == type || 'arguments' == type || instanceOf(this[i], Array) ? Array.flatten(this[i]) : this[i]);
        }
        return array;
    },
    pick: function() {
        for(var i = 0, l = this.length; l > i; i++)if (this[i] != null) return this[i];
        return null;
    },
...
String.implement({
    ...
    pass: function(args, bind) {
        return null != args && (args = Array.from(args)), function() {
            return this.apply(bind, args || arguments);
        };
    },
    ...
    create: function(options) {
        return options ||= {
            ...
            args = null != args ? Array.from(args) : Array.slice(arguments, options.event ? 1 : 0), options.event && (args = [event || window.event].extend(args));
            var returns = function() {
                return this.apply(options.bind || null, args);
            };
    ...
    bindWithEvent: function(bind, args) {
        return null != args && (args = Array.from(args)), function(event) {
            return this.apply(bind, null == args ? arguments : [event].concat(args));
        };
    },
...
!function() {
    ...
    var document = this.document, window = document.window = this, ua = navigator.userAgent.toLowerCase(), platform = navigator.platform.toLowerCase(), UA = ua.match(/(opera|ie|firefox|chrome|version)[\s\/:]([\w\d\.]+)?.*?(safari|version[\s\/:]([\w\d\.]+)|$)/) || [null, 'unknown', 0], mode = UA[1] == 'ie' && document.documentMode, Browser = this.Browser = {
    ...
    Browser[Browser.name] = !0, Browser[Browser.name + parseInt(Browser.version, 10)] = !0, Browser.Platform[Browser.Platform.name] = !0, Browser.Request = (function() {
    ...
    if (Browser.Plugins.Flash = {
        ...
        if (!0 === exec) Browser.exec(scripts);
        else typeOf(exec) == 'function' && exec(scripts, text);
        return text;
    }), Browser.extend({
    ...
    try {
        ...
        Array.from = function(item) {
            if ('string' != typeof item && Type.isEnumerable(item) && typeOf(item) != 'array') {
        ...
        ['pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift', 'concat', 'join', 'slice'].each(function(name) {
            Array[name] = function(item) {
                return prototype[name].apply(Array.from(item), prototype.slice.call(arguments, 1));
            };
        });
    }
    ...
    if (Browser.firefox) {
        if (Browser.Engine.gecko = !0, Browser.version >= 3) setEngine('gecko', 19);
        else setEngine('gecko', 18);
    }
    if (Browser.safari || Browser.chrome) switch(Browser.Engine.webkit = !0, Browser.version){
    case 2:
    ...
    if (Browser.opera) {
        if (Browser.Engine.presto = !0, Browser.version >= 9.6) setEngine('presto', 960);
        else if (Browser.version >= 9.5) setEngine('presto', 950);
        else setEngine('presto', 925);
    }
    if (Browser.name == 'unknown') switch((ua.match(/(?:webkit|khtml|gecko)/) || [])[0]){
    case 'webkit':
    case 'khtml':
    ...
    var _keys = {
            ...
            if (!win && (win = window), event ||= win.event, event.$extended) return event;
            ...
            for(var type = this.type = event.type, target = event.target || event.srcElement; target && target.nodeType == 3;)target = target.parentNode;
            if (this.target = document.id(target), type.indexOf('key') == 0) {
                var code = this.code = event.which || event.keyCode;
                if (this.key = _keys[code] || Object.keyOf(Event1.Keys, code), 'keydown' == type) {
                    if (code > 111 && 124 > code) this.key = 'f' + (code - 111);
                    else code > 95 && 106 > code && (this.key = code - 96);
                }
                this.key == null && (this.key = String.fromCharCode(code).toLowerCase());
            } else if ('click' == type || 'dblclick' == type || 'contextmenu' == type || 'DOMMouseScroll' == type || type.indexOf('mouse') == 0) {
                var doc = win.document;
                if (doc = !doc.compatMode || doc.compatMode == 'CSS1Compat' ? doc.html : doc.body, this.page = {
                    x: event.pageX != null ? event.pageX : event.clientX + doc.scrollLeft,
                    y: event.pageY != null ? event.pageY : event.clientY + doc.scrollTop
                }, this.client = {
                    x: event.pageX != null ? event.pageX - win.pageXOffset : event.clientX,
                    y: event.pageY != null ? event.pageY - win.pageYOffset : event.clientY
                }, ('DOMMouseScroll' == type || 'mousewheel' == type) && (this.wheel = event.wheelDelta ? event.wheelDelta / 120 : -(event.detail || 0) / 3), this.rightClick = event.which == 3 || event.button == 2, 'mouseover' == type || 'mouseout' == type) {
                    for(var related = event.relatedTarget || event[('mouseover' == type ? 'from' : 'to') + 'Element']; related && related.nodeType == 3;)related = related.parentNode;
...
Event1.Keys = {
    ...
    var Class = this.Class = new Type('Class', function(params) {
            ...
            var newClass = (function() {
                ...
                return this.$caller = null, this.$caller = this.caller = null, this.initialize ? this.initialize.apply(this, arguments) : this;
            ...
            var name = this.$caller.$name, parent1 = this.$caller.$owner.parent, previous = parent1 ? parent1.prototype[name] : null;
            ...
            var wrapper = (function() {
                ...
                return this.caller = current, this.$caller = wrapper, this.$caller = current, this.caller = caller, method.apply(this, arguments);
            }).extend({
                $owner: self,
                $origin: method,
                $name: key
            ...
            if (Class.Mutators.hasOwnProperty(key)) {
                if (value = Class.Mutators[key].call(this, value), null == value) return this;
            }
            ...
            return klass.$prototyping = !0, delete klass.$prototyping, new klass;
        };
    Class.implement('implement', implement.overloadSetter()), Class.Mutators = {
        Extends: function(parent1) {
            this.parent = parent1, this.prototype = getInstance(parent1);
        },
    ...
    this.Events = new Class({
        ...
        addEvent: function(type, fn, internal) {
            if (type = removeOn(type), $empty == fn) return this;
            return this.$events[type] = (this.$events[type] || []).include(fn), internal && (fn.internal = !0), this;
        },
        ...
        fireEvent: function(type, args, delay) {
            ...
            return args = Array.from(args), events.each(function(fn) {
        ...
        setOptions: function() {
            ...
            if (this.addEvent) for(var option in options){
                ...
                this.addEvent(option, options[option]), delete options[option];
            }
            return this;
        }
    });
    ...
    var parsed, separatorIndex, combinatorIndex, reversed, cache = {
        ...
        }, reUnescape = /\\/g, parse = function(expression, isReversed) {
    ...
    function parser(rawMatch, separator, combinator, combinatorChildren, tagName, id, className, attributeKey, attributeOperator, attributeQuote, attributeValue, pseudoMarker, pseudoClass, pseudoQuote, pseudoClassQuotedValue, pseudoClassValue) {
        if (separator || -1 === separatorIndex) {
            if (parsed.expressions[++separatorIndex] = [], combinatorIndex = -1, separator) return '';
        }
        if (combinator || combinatorChildren || -1 === combinatorIndex) {
            combinator ||= ' ';
            var currentSeparator = parsed.expressions[separatorIndex];
            reversed && currentSeparator[combinatorIndex] && (currentSeparator[combinatorIndex].reverseCombinator = reverseCombinator(' ')), currentSeparator[++combinatorIndex] = {
                combinator: ' ',
                tag: '*'
            };
        }
        ...
        else if (pseudoClass) pseudoClassValue ||= pseudoClassQuotedValue, pseudoClassValue = pseudoClassValue ? pseudoClassValue.replace(reUnescape, '') : null, !currentParsed.pseudos && (currentParsed.pseudos = []), currentParsed.pseudos.push({
    ...
    local.isNativeCode = function(fn) {
        ...
        return document.xmlVersion || document.xml || Object.prototype.toString.call(document) == '[object XMLDocument]' || document.nodeType == 9 && document.documentElement.nodeName != 'HTML';
        ...
        }, features.root = root, features.isXMLDocument = this.isXML(document), features.brokenStarGEBTN = features.starSelectsClosedQSA = features.idGetsName = features.brokenMixedCaseQSA = features.brokenGEBCN = features.brokenCheckedQSA = features.brokenEmptyAttributeQSA = features.isHTMLDocument = features.nativeMatchesSelector = !1;
        ...
        if (features.isHTMLDocument) {
            ...
            if (testNode.getElementsByClassName) {
                try {
                    testNode.innerHTML = '<a class="f"></a><a class="b"></a>', testNode.getElementsByClassName('b').length, testNode.firstChild.className = 'b', cachedGetElementsByClassName = testNode.getElementsByClassName('b').length != 2;
                } catch (e) {
                }
                try {
                    testNode.innerHTML = '<a class="a"></a><a class="f b a"></a>', brokenSecondClassNameGEBCN = testNode.getElementsByClassName('a').length != 2;
                } catch (e) {
                }
            ...
            if (testNode.querySelectorAll) {
                ...
                try {
                    testNode.innerHTML = '<select><option selected="selected">a</option></select>', features.brokenCheckedQSA = testNode.querySelectorAll(':checked').length == 0;
                } catch (e) {
                }
                try {
                    testNode.innerHTML = '<a class=""></a>', features.brokenEmptyAttributeQSA = testNode.querySelectorAll('[class*=""]').length != 0;
                } catch (e) {
                }
            }
            try {
                testNode.innerHTML = '<form action="s"><input id="action"/></form>', brokenFormAttributeGetter = testNode.firstChild.getAttribute('action') != 's';
            } catch (e) {
            }
        ...
        features.contains = nativeRootContains && nativeDocumentContains ? function(context, node) {
            ...
            if (node) do if (context === node) return !0;
    ...
    var reSimpleSelector = /^([#.]?)((?:[\w-]+|\*))$/, qsaFailExpCache = {
    };
    local.search = function(context, expression, append, first) {
        ...
        var parsed, i, uniques = this.uniques = {
            }, hasOthers = !(!append || !append.length), contextIsDocument = context.nodeType == 9;
        ...
        if ('string' == typeof expression) {
            ...
            simpleSelectors: if (simpleSelector) {
                ...
                if (!symbol) {
                    ...
                    if (context.getElementsByClassName && !this.brokenGEBCN) {
                        ...
                        for(i = 0; node = nodes[i++];)(!hasOthers || !uniques[this.getUID(node)]) && found.push(node);
                    } else for(nodes = context.getElementsByTagName('*'), i = 0; node = nodes[i++];){
                        if (className = node.className, !className || !new RegExp('(^|\\s)' + Slick.escapeRegExp(name) + '(\\s|$)').test(className)) continue;
            ...
            querySelector: if (context.querySelectorAll) {
                if (!this.isHTMLDocument || qsaFailExpCache[expression] || this.brokenMixedCaseQSA || this.brokenCheckedQSA && expression.indexOf(':checked') > -1 || this.brokenEmptyAttributeQSA && /\[.+[*$^]=(?:""|'')?\]/.test(expression) || !contextIsDocument && expression.indexOf(',') > -1 || Slick.disableQSA) break querySelector;
        ...
        }, this.push = !hasOthers && (first || parsed.length == 1 && parsed.expressions[0].length == 1) ? this.pushArray : this.pushUID, null == found && (found = []);
        var j, m, n, combinator, tag, id, classList, classes, attributes, pseudos, currentItems, currentExpression, currentBit, lastBit;
        search: for(i = 0; currentExpression = parsed.expressions[i]; i++)for(j = 0; currentBit = currentExpression[j]; j++){
            ...
            }, lastBit) this.uniques = uniques, this.found = found;
            else this.uniques = {
            }, this.found = [];
            if (0 === j) {
                if (this[combinator](context, tag, currentBit.id, currentBit.classes, currentBit.attributes, currentBit.pseudos, currentBit.classList), first && lastBit && found.length) break search;
            } else if (first && lastBit) for(m = 0, n = currentItems.length; n > m; m++)if (this[combinator](currentItems[m], tag, currentBit.id, currentBit.classes, currentBit.attributes, currentBit.pseudos, currentBit.classList), found.length) break search;
            else for(m = 0, n = currentItems.length; n > m; m++)this[combinator](currentItems[m], tag, currentBit.id, currentBit.classes, currentBit.attributes, currentBit.pseudos, currentBit.classList);
        ...
        return results.sort(this.documentSorter), results;
    }, local.cacheNTH = {
    }, local.matchNTH = /^([+-]?\d*)?([a-z]+)?([+-]\d+)?$/, local.parseNTHArgument = function(argument) {
        ...
        return function(node, argument) {
            ...
            argument ||= 'n';
            var parsed = this.cacheNTH.n || this.parseNTHArgument('n');
            if (!parsed) return !1;
        ...
        if (!parsed) return !0;
        var simpleExpCounter = 0, i;
        for(i = 0; currentExpression = parsed.expressions[i]; i++)if (currentExpression.length == 1) {
        ...
        var item;
        for(i = 0; item = this.search(this.document, parsed)[i++];)if (item === node) return !0;
        return !1;
        ...
        var i, part, cls;
        if (classes) for(i = classes.length; i--;)if (cls = this.getAttribute(node, 'class'), !cls || !classes[i].regexp.test(cls)) return !1;
        if (attributes) for(i = attributes.length; i--;)if (part = attributes[i], part.operator ? !part.test(this.getAttribute(node, part.key)) : !this.hasAttribute(node, part.key)) return !1;
        if (pseudos) for(i = pseudos.length; i--;)if (part = pseudos[i], !this.matchPseudo(node, part.key, part.value)) return !1;
        return !0;
    };
    var combinators = {
        ' ': function(node, tag, id, classes, attributes, pseudos, classList) {
            ...
            if (this.isHTMLDocument) {
                getById: if (id) {
                    ...
                    return this.push(item, tag, null, classes, attributes, pseudos), void 0;
                }
            ...
            getByTag: {
                if (children = node.getElementsByTagName(tag), !children || !children.length) break getByTag;
                for(!this.brokenStarGEBTN && (tag = null), i = 0; item = children[i++];)this.push(item, null, id, classes, attributes, pseudos);
            }
        },
        '>': function(node, tag, id, classes, attributes, pseudos) {
            if (node = node.firstChild) do node.nodeType == 1 && this.push(node, tag, id, classes, attributes, pseudos);
            while (node = node.nextSibling);
        },
        '+': function(node, tag, id, classes, attributes, pseudos) {
            for(; node = node.nextSibling;)if (node.nodeType == 1) {
        ...
        '!+': function(node, tag, id, classes, attributes, pseudos) {
            for(; node = node.previousSibling;)if (node.nodeType == 1) {
                this.push(node, tag, id, classes, attributes, pseudos);
                break;
            }
        },
    ...
    var pseudos = {
        ...
        'first-child': function(node) {
            for(; node = node.previousSibling;)if (node.nodeType == 1) return !1;
            return !0;
        },
        ...
        'only-child': function(node) {
            for(var prev = node; prev = prev.previousSibling;)if (prev.nodeType == 1) return !1;
            for(var next = node; next = next.nextSibling;)if (next.nodeType == 1) return !1;
            return !0;
        },
    ...
    Slick.version = '1.1.7', Slick.search = function(context, expression, append) {
        ...
        if (!selector || node === selector) return !0;
        return local.setDocument(node), local.matchNode(node, selector);
...
var Element1 = function(tag, props) {
    ...
    if (!props && (props = {
        ...
        tag = parsed.tag == '*' ? 'div' : parsed.tag, parsed.id && props.id == null && (props.id = parsed.id);
        var attributes = parsed.attributes;
        if (attributes) for(var attr, i = 0, l = attributes.length; l > i; i++){
            if (attr = attributes[i], props[attr.key] != null) continue;
            if (attr.value != null && attr.operator == '=') props[attr.key] = attr.value;
            else attr.value || attr.operator || (props[attr.key] = !0);
        }
        parsed.classList && props.class == null && (props.class = parsed.classList.join(' '));
    }
    return document.newElement(tag, props);
};
Browser.Element && (Element1.prototype = Browser.Element.prototype, Element1.prototype._fireEvent = function(type, event) {
    return Element1.prototype.fireEvent.call(this, type, event);
...
var IFrame = new Type('IFrame', function() {
        ...
        params.iframe && (iframe = document.id(params.iframe)), delete props.onload, props.id = props.name = [props.id, props.name, iframe ? iframe.id || iframe.name : 'IFrame_' + String.uniqueID()].pick(), iframe = new Element1(iframe || 'iframe', props);
        var onLoad = function() {
            (props.onload || function() {
            }).call(iframe.contentWindow);
        };
        if (window.frames[props.id]) onLoad();
        else iframe.addListener('load', onLoad);
        return iframe;
    }), Elements = this.Elements = function(nodes) {
        if (nodes && nodes.length) for(var uniques = {
...
Elements.prototype = {
    ...
    splice.call(object, 1, 1), object[1] == 1 && Elements.implement('splice', (function() {
    ...
    Document.implement({
        newElement: function(tag, props) {
            return props && props.checked != null && (props.defaultChecked = props.checked), createElementAcceptsHTML && props && (tag = '<' + tag, props.name && (tag += ' name="' + escapeQuotes(props.name) + '"'), props.type && (tag += ' type="' + escapeQuotes(props.type) + '"'), tag += '>', delete props.name, delete props.type), this.id(this.createElement(tag)).set(props);
        }
    });
})(), (function() {
    ...
    }), window.$ == null && Window.implement('$', function(el, nc) {
    ...
    !document.contains && Document.implement(contains), !document.createElement('div').contains && Element1.implement(contains), Element1.implement('hasChild', function(element) {
        ...
        Slick.search = function(context, expression, append) {
            return addSlickPseudos(), Slick.search.call(this, context, expression, append);
        }, Slick.find = function(context, expression) {
            return addSlickPseudos(), Slick.find.call(this, context, expression);
        }, Slick.match = function(node, selector) {
            return addSlickPseudos(), Slick.match.call(this, node, selector);
        };
    ...
    }), window.$$ == null && Window.implement('$$', function(selector) {
        var elements = new Elements;
        if (arguments.length == 1 && 'string' == typeof selector) return Slick.search(this.document, selector, elements);
    ...
    }), properties.html = 'innerHTML', properties.text = document.createElement('div').textContent == null ? 'innerText' : 'textContent', Object.forEach(properties, function(real, key) {
    ...
    var bools = ['compact', 'nowrap', 'ismap', 'declare', 'noshade', 'checked', 'disabled', 'readOnly', 'multiple', 'selected', 'noresize', 'defer', 'defaultChecked', 'autofocus', 'controls', 'autoplay', 'loop'], booleans = {
    };
    ...
    el.type != 'button' && (propertySetters.type = function(node, value) {
        node.setAttribute('type', value);
    }), el = null;
    var input = document.createElement('input');
    input.value = 't', input.type = 'submit', input.value != 't' && (propertySetters.type = function(node, type) {
        node.type = type, node.value = node.value;
    }), input = null;
    var pollutesGetAttribute = function(div) {
        return div.random = 'attribute', div.getAttribute('random') == 'attribute';
    ...
    Element1.implement({
        setProperty: function(name, value) {
            ...
            if (setter) setter(this, value);
                ...
                if (null == value) this.removeAttribute(name), pollutesGetAttribute && delete attributeWhiteList[name];
                else this.setAttribute(name, '' + value), pollutesGetAttribute && (attributeWhiteList[name] = !0);
            }
            return this;
        },
        ...
        toQueryString: function() {
            ...
            return this.getElements('input, select, textarea').each(function(el1) {
                var type = el1.type;
                if (!el1.name || el1.disabled || 'submit' == type || 'reset' == type || 'file' == type || 'image' == type) return;
                var value = el1.get('tag') == 'select' ? el1.getSelected().map(function(opt) {
                    return document.id(opt).get('value');
                }) : ('radio' == type || 'checkbox' == type) && !el1.checked ? null : el1.get('value');
                Array.from(value).each(function(val) {
                    'undefined' != typeof val && queryString.push(encodeURIComponent(el1.name) + '=' + encodeURIComponent(val));
                });
            }), queryString.join('&');
        }
    });
    ...
    Element1.implement({
        ...
        clone: function(contents, keepid) {
            ...
            for(contents && (ce.append(Array.from(clone.getElementsByTagName('*'))), te.append(Array.from(this.getElementsByTagName('*')))), i = ce.length; i--;){
                ...
                if (!keepid && node.removeAttribute('id'), node.clearAttributes) {
                    if (node.clearAttributes(), node.mergeAttributes(element), node.removeAttribute('uniqueNumber'), node.options) for(var no = node.options, j = no.length; j--;)no[j].selected = element.options[j].selected;
                }
                var prop = {
                    input: 'checked',
                    option: 'selected',
            ...
            if (Browser.ie) {
                ...
                for(i = co.length; i--;)co[i].outerHTML = this.getElementsByTagName('object')[i].outerHTML;
            }
            return document.id(clone);
        }
    }), [Element1,
        Window,
        Document].invoke('implement', {
        addListener: function(type, fn) {
            if ('unload' == type) {
                var old = fn;
                fn = function() {
                    this.removeListener('unload', fn), old();
                };
        ...
        store: function(property, value) {
            var storage1 = get(Slick.uidOf(this));
        ...
        set: function(html) {
            if (null == html) html = '';
            else typeOf(html) == 'array' && (html = html.join(''));
            this.innerHTML = html;
        },
        erase: function() {
            this.innerHTML = '';
    ...
    tr = null, supportsTableInnerHTML && supportsTRInnerHTML && !supportsHTML5Elements && (Element1.Properties.html.set = (function(set) {
        var translations = {
            table: [1, '<table>', '</table>'],
        ...
        return translations.thead = translations.tfoot = translations.tbody, function(html) {
            ...
            if (wrap || supportsHTML5Elements || (wrap = [0, '', '']), !wrap) return Element1.Properties.html.set.call(this, html);
    ...
    null.innerHTML = '<select><option>s</option></select>', null.firstChild.value != 's' && (Element1.Properties.value = {
        set: function(value) {
    ...
    var hasOpacity = html.style.opacity != null, hasFilter = html.style.filter != null, reAlpha = /alpha\(opacity=([\d.]+)\)/i, setVisibility = function(element, opacity) {
            ...
            if ((!element.currentStyle || !element.currentStyle.hasLayout) && (style.zoom = 1), null == opacity || 1 == opacity) opacity = '';
            else opacity = 'alpha(opacity=' + (100 * opacity).limit(0, 100).round() + ')';
            ...
            return null == opacity && (opacity = element.style.visibility == 'hidden' ? 0 : 1), opacity;
    ...
    Element1.implement({
        getComputedStyle: function(property) {
            ...
            return computed ? computed.getPropertyValue(floatName == property ? 'float' : property.hyphenate()) : null;
        },
        setStyle: function(property, value) {
            if ('opacity' == property) return null != value && (value = parseFloat(value)), setOpacity(this, value), this;
            if (property = ('float' == property ? floatName : property).camelCase(), typeOf(value) != 'string') {
    ...
    if (Element1.Properties.events = {
        ...
        removeEvents: function(events) {
            ...
            } else attached[events] && (attached[events].keys.each(function(fn) {
        ...
        fireEvent: function(type, args, delay) {
            var events = this.retrieve('events');
            if (!events || !events[type]) return this;
            return args = Array.from(args), events[type].keys.each(function(fn) {
                if (delay) fn.delay(delay, this, args);
                else fn.apply(this, args);
            }, this), this;
        },
        cloneEvents: function(from, type) {
            ...
            if (!events) return this;
            if (!type) for(var eventType in events)this.cloneEvents(from, eventType);
            else events[type] && events[type].keys.each(function(fn) {
    ...
    !window.addEventListener && (Element1.NativeEvents.propertychange = 2, Element1.Events.change = {
        base: function() {
            var type = this.type;
            return this.get('tag') == 'input' && ('radio' == type || 'checkbox' == type) ? 'propertychange' : 'change';
        },
    ...
    var proto = Element1.prototype, addEvent = proto.addEvent, removeEvent = proto.removeEvent, relay = function(old, method) {
            ...
            addEvent: function(type, match, fn) {
                ...
                if (stored) for(var _uid in stored)if (stored[_uid].fn == fn && stored[_uid].match == match) return this;
            ...
            removeEvent: function(type, match, fn, _uid) {
                ...
                var __uid, s;
                if (fn) for(__uid in stored)if (s = stored[__uid], s.match == match && s.fn == fn) return delegation.removeEvent.call(this, type, match, fn, __uid);
                else for(__uid in stored)s = stored[__uid], s.match == match && delegation.removeEvent.call(this, type, match, s.fn, __uid);
                return this;
            }
        };
    [Element1,
    ...
    Element1.implement({
        ...
        getOffsetParent: brokenOffsetParent ? function() {
            ...
            for(var isOffsetCheck = styleString(element, 'position') == 'static' ? isOffsetStatic : isOffset; element = element.parentNode;)if (isOffsetCheck(element)) return element;
            return null;
        } : function() {
            var element = this;
            if (isBody(element) || styleString(element, 'position') == 'fixed') return null;
        ...
        getOffsets: function() {
            ...
            for(; element && !isBody(element);){
                if (position.x += element.offsetLeft, position.y += element.offsetTop, Browser.firefox) {
                    ...
                    var parent = element.parentNode;
                    parent && styleString(parent, 'overflow') != 'visible' && (position.x += leftBorder(parent), position.y += topBorder(parent));
    ...
    var Fx = this.Fx = new Class({
        ...
        check: function() {
            ...
            case 'cancel': return this.cancel(), !0;
            case 'chain': return this.chain(this.caller.pass(arguments, this)), !1;
            }
            return !1;
        },
        ...
        stop: function() {
            if (this.isRunning()) {
                if (this.time = null, pullInstance.call(this, this.options.fps), this.frames == this.frame) this.fireEvent('complete', this.subject), !this.callChain() && this.fireEvent('chainComplete', this.subject);
    ...
    var instances = {
            ...
            list.push(this), !timers[fps] && (timers[fps] = (function() {
                for(var now = Date.now(), i = this.length; i--;){
                    var instance = this[i];
                    instance && instance.step(now);
                }
            }).periodical(Math.round(1000 / fps), list));
    ...
    prepare: function(element, property, values) {
        ...
        if (null == to) {
            ...
            if (unit && from.slice(-unit.length) != unit && parseFloat(from) != 0) {
                ...
                if (!/px$/.test(value)) {
                    if (value = element.style[('pixel-' + property).camelCase()], null == value) {
    ...
    parse: function(value) {
        return value = Function.from(value)(), value = 'string' == typeof value ? value.split(' ') : Array.from(value), value.map(function(val) {
            val = String(val);
            var found = !1;
    ...
    search: function(selector) {
        ...
        return Array.each(document.styleSheets, function(sheet, j) {
            ...
            Array.each(rules, function(rule, i) {
                ...
                if (!selectorText || !new RegExp('^' + selector.escapeRegExp() + '$').test(selectorText)) return;
                Object.each(Element1.Styles, function(value, style) {
                    if (!rule.style[style] || Element1.ShortStyles[style]) return;
                    value = String(rule.style[style]), to[style] = /^rgb/.test(value) ? value.rgbToHex() : value;
                });
            });
    ...
    Color: {
        parse: function(value) {
            if (value.match(/^#[0-9a-f]{3,6}$/i)) return value.hexToRgb(!0);
            return (value = value.match(/(\d+),\s*(\d+),\s*(\d+)/)) ? [value[1], value[2], value[3]] : !1;
        },
        compute: function(from, to, delta) {
    ...
    set: function(property, now) {
        return arguments.length == 1 && (now = property, property = this.property || this.options.property), this.render(this.element, property, now, this.options.unit), this;
    },
    ...
    fade: function(how) {
        ...
        if ('set' == method || 0 != to) this.setStyle('visibility', 0 == to ? 'hidden' : 'visible');
    ...
    return Object.append(easeIn, {
        ...
        easeInOut: function(pos) {
            return (0.5 >= pos ? transition(2 * pos, params) : 2 - transition(2 * (1 - pos), params)) / 2;
        }
    });
}, Fx.Transitions = {
    ...
    var empty = function() {
            ...
            onStateChange: function() {
                ...
                if (this.running = !1, this.status = 0, Function.attempt((function() {
                ...
                }, this.options.isSuccess.call(this, this.status)) this.success(this.response.text, this.response.xml);
                else this.failure();
            },
            isSuccess: function() {
            ...
            send: function(options) {
                ...
                case 'element':
                    ...
                    break;
                case 'object':
                case 'hash': data = Object.toQueryString(data);
                }
                ...
                var xhr = this.xhr;
                if (progressSupport && (xhr.onloadstart = this.loadstart.bind(this), xhr.onprogress = this.progress.bind(this)), xhr.open(method.toUpperCase(), url, this.options.async, this.options.user, this.options.password), this.options.user && 'withCredentials' in xhr && (xhr.withCredentials = !0), xhr.onreadystatechange = this.onStateChange.bind(this), Object.each(this.headers, function(value, key) {
                    try {
                        xhr.setRequestHeader(key, value);
                    } catch (e) {
                        this.fireEvent('exception', [key,
                            value]);
                    }
                }, this), this.fireEvent('request'), xhr.send(data), !this.options.async) this.onStateChange();
                else this.options.timeout && (this.timer = this.timeout.delay(this.options.timeout, this));
                return this;
            },
            cancel: function() {
...
var Cookie = new Class({
    ...
    write: function(value) {
        if (this.options.encode && (value = encodeURIComponent(value)), this.options.domain && (value += '; domain=' + this.options.domain), this.options.path && (value += '; path=' + this.options.path), this.options.duration) {
            var date = new Date();
            date.setTime(date.getTime() + this.options.duration * 86400000), value += '; expires=' + date.toGMTString();
        }
...
Cookie.write = function(key, value, options) {
    ...
    var ready, loaded, checks = [], shouldPoll, timer, testElement = document.createElement('div'), domready = function() {
            if (clearTimeout(timer), ready) return;
            Browser.loaded = ready = !0, document.removeListener('DOMContentLoaded', domready).removeListener('readystatechange', check), document.fireEvent('domready'), window.fireEvent('domready');
        }, check = function() {
    ...
    if (testElement.doScroll && !doScrollWorks() && (checks.push(doScrollWorks), shouldPoll = !0), document.readyState && checks.push(function() {
        var state = document.readyState;
        return 'loaded' == state || 'complete' == state;
    }), 'onreadystatechange' in document) document.addListener('readystatechange', check);
    else shouldPoll = !0;
    shouldPoll && poll(), Element1.Events.domready = {
        onAdd: function(fn) {
            ready && fn.call(this);
        }
    ...
    var Swiff = this.Swiff = new Class({
        ...
        initialize: function(path, options) {
            ...
            for(var callBack in callBacks)Swiff.CallBacks[this.instance][callBack] = (function(option) {
                return function() {
                    return option.apply(this.object, arguments);
                };
            })(callBacks[callBack]), vars[callBack] = 'Swiff.CallBacks.' + this.instance + '.' + callBack;
            if (params.flashVars = Object.toQueryString(vars), Browser.ie) properties.classid = 'clsid:D27CDB6E-AE6D-11cf-96B8-444553540000', params.movie = path;
