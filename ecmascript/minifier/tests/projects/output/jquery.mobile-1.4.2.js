!function(root, doc, factory) {
    ...
    var $, rcapitals, doc, bool, docElem, refNode, fakeBody, div, $, support, $, self, $win, dummyFnToInitNavigate, $, undefined, path, $base, $, undefined, $, path, initialHref, $, heldCall, curr, diff, handler, lastCall, $, baseElement, base, $, $, $, $, undefined, rInitialLetter, iconposClass, $, $, $, meta, initialContent, disabledZoom, enabledZoom, disabledInitially, $, $, undefined, rDividerListItem, origDefaultFilterCallback;
    jQuery.mobile = {
    ...
    }, $.extend($.support, support), $.extend($.mobile.support, support), (function($, undefined) {
        $.extend($.support, {
        ...
        var nokiaLTE7_3, w, ua, platform, wkmatch, wkversion, ffmatch, ffversion, operammobilematch, omversion, link, fauxBase, base, supports, element, documentElement, getComputedStyle, ua, fakeBody = $("<body>").prependTo("html"), fbCSS = fakeBody[0].style, vendors = [
            "Webkit",
            "Moz",
            "O"
        ...
        }), fakeBody.remove(), nokiaLTE7_3 = (ua = window.navigator.userAgent).indexOf("Nokia") > -1 && (ua.indexOf("Symbian/3") > -1 || ua.indexOf("Series60/5") > -1) && ua.indexOf("AppleWebKit") > -1 && ua.match(/(BrowserNG|NokiaBrowser)\/7\.[0-3]/), $.mobile.gradeA = function() {
    ...
    })(jQuery), $win = ($ = jQuery).mobile.window, dummyFnToInitNavigate = function() {
        ...
        popstate: function(event) {
            ...
            beforeNavigate.originalEvent = event, $win.trigger(beforeNavigate), beforeNavigate.isDefaultPrevented() || (event.historyState && $.extend(state, event.historyState), newEvent.originalEvent = event, setTimeout(function() {
        ...
        addSearchParams: function(url, params) {
            var u = path.parseUrl(url), p = "object" == typeof params ? $.param(params) : params, s = u.search || "?";
        ...
        isPermittedCrossDomainRequest: function(docUrl, reqUrl) {
            return $.mobile.allowCrossDomainPages && ("file:" === docUrl.protocol || "content:" === docUrl.protocol) && -1 !== reqUrl.search(/^https?:/);
        }
        ...
        direct: function(opts) {
            ...
            newActiveIndex !== undefined && (this.activeIndex = newActiveIndex, this.previousIndex = a), newActiveIndex < a ? (opts.present || opts.back || $.noop)(this.getActive(), "back") : newActiveIndex > a ? (opts.present || opts.forward || $.noop)(this.getActive(), "forward") : newActiveIndex === undefined && opts.missing && opts.missing(this.getActive());
        }
    }), path = ($ = jQuery).mobile.path, initialHref = location.href, $.mobile.Navigator = function(history) {
        this.history = history, this.ignoreInitialHashChange = !0, $.mobile.window.bind({
        ...
        squash: function(url, data) {
            ...
            return href = path.squash(url), state = $.extend({
                hash: hash,
                url: href
        ...
        hash: function(url, href) {
            ...
            return parsed = path.parseUrl(url), (loc = path.parseLocation()).pathname + loc.search === parsed.pathname + parsed.search ? parsed.hash ? parsed.hash : parsed.pathname + parsed.search : path.isPath(url) ? (resolved = path.parseUrl(href)).pathname + resolved.search + (path.isPreservableHash(resolved.hash) ? resolved.hash.replace("#", "") : "") : url;
        },
        go: function(url, data, noEvents) {
            var state, href, hash, popstateEvent, isPopStateEvent = $.event.special.navigate.isPushStateEnabled();
            href = path.squash(url), hash = this.hash(url, href), noEvents && hash !== path.stripHash(path.parseLocation().hash) && (this.preventNextHashChange = noEvents), this.preventHashAssignPopState = !0, window.location.hash = hash, this.preventHashAssignPopState = !1, state = $.extend({
                url: href,
                hash: hash,
                title: document.title
            }, data), isPopStateEvent && ((popstateEvent = new $.Event("popstate")).originalEvent = {
                type: "popstate",
                state: null
            }, this.squash(url, state), noEvents || (this.ignorePopState = !0, $.mobile.window.trigger(popstateEvent))), this.history.add(state.url, state);
        },
        popstate: function(event) {
            var hash, state;
            if ($.event.special.navigate.isPushStateEnabled()) return this.preventHashAssignPopState ? (this.preventHashAssignPopState = !1, void event.stopImmediatePropagation()) : this.ignorePopState ? void (this.ignorePopState = !1) : !event.originalEvent.state && 1 === this.history.stack.length && this.ignoreInitialHashChange && (this.ignoreInitialHashChange = !1, location.href === initialHref) ? void event.preventDefault() : (hash = path.parseLocation().hash, !event.originalEvent.state && hash) ? (state = this.squash(hash), this.history.add(state.url, state), void (event.historyState = state)) : void this.history.direct({
        ...
        (diff = (curr = new Date().getTime()) - lastCall) >= 250 ? (lastCall = curr, $(this).trigger("throttledresize")) : (heldCall && clearTimeout(heldCall), heldCall = setTimeout(handler, 250 - diff));
        ...
        element: (baseElement = ($ = jQuery)("head").children("base")).length ? baseElement : $("<base>", {
            href: $.mobile.path.documentBase.hrefNoHash
        }).prependTo($("head")),
        linkSelector: "[src], link[href], a[rel='external'], :jqmData(ajax='false'), a[target]",
        set: function(href) {
            !!$.mobile.dynamicBaseEnabled && $.support.dynamicBaseTag && base.element.attr("href", $.mobile.path.makeUrlAbsolute(href, $.mobile.path.documentBase));
        },
        rewrite: function(href, page) {
            var newPath = $.mobile.path.get(href);
            page.find(base.linkSelector).each(function(i, link) {
                var thisAttr = $(link).is("[href]") ? "href" : $(link).is("[src]") ? "src" : "action", thisUrl = $(link).attr(thisAttr);
                thisUrl = thisUrl.replace(location.protocol + "//" + location.host + location.pathname, ""), /^(\w+:|#|\/)/.test(thisUrl) || $(link).attr(thisAttr, newPath + thisUrl);
            });
        },
        reset: function() {
            base.element.attr("href", $.mobile.path.documentBase.hrefNoSearch);
        }
    }, $.mobile.base = base, (function($, undefined) {
        $.mobile.widgets = {
        };
    ...
    }, $.mobile.defaultTransitionHandler = $.mobile.transitionHandlers.sequential, $.mobile.transitionFallbacks = {
    }, $.mobile._maybeDegradeTransition = function(transition) {
        return transition && !$.support.cssTransform3d && $.mobile.transitionFallbacks[transition] && (transition = $.mobile.transitionFallbacks[transition]), transition;
    }, $.mobile.getMaxScrollForTransition = $.mobile.getMaxScrollForTransition || function() {
        return 3 * $.mobile.getScreenHeight();
    }, jQuery.mobile.transitionFallbacks.flip = "fade", jQuery.mobile.transitionFallbacks.flow = "fade", jQuery.mobile.transitionFallbacks.pop = "fade", ($ = jQuery).mobile.transitionHandlers.slide = $.mobile.transitionHandlers.simultaneous, $.mobile.transitionFallbacks.slide = "fade", jQuery.mobile.transitionFallbacks.slidedown = "fade", jQuery.mobile.transitionFallbacks.slidefade = "fade", jQuery.mobile.transitionFallbacks.slideup = "fade", jQuery.mobile.transitionFallbacks.turn = "fade", ($ = jQuery).mobile.degradeInputs = {
        color: !1,
        date: !1,
        datetime: !1,
        "datetime-local": !1,
        ...
        url: !1,
        week: !1
    }, $.mobile.page.prototype.options.degradeInputs = $.mobile.degradeInputs, $.mobile.degradeInputsWithin = function(target) {
        (target = $(target)).find("input").not($.mobile.page.prototype.keepNativeSelector()).each(function() {
            var html, hasType, findstr, repstr, element = $(this), type = this.getAttribute("type"), optType = $.mobile.degradeInputs[type] || "text";
            $.mobile.degradeInputs[type] && (findstr = (hasType = (html = $("<div>").html(element.clone()).html()).indexOf(" type=") > -1) ? /\s+type=["']?\w+['"]?/ : /\/?>/, repstr = " type=\"" + optType + "\" data-" + $.mobile.ns + "type=\"" + type + "\"" + (hasType ? "" : ">"), element.replaceWith(html.replace(findstr, repstr)));
        });
    }, (function($, window, undefined) {
    ...
    })(jQuery, this), $ = jQuery, rInitialLetter = /([A-Z])/g, iconposClass = function(iconpos) {
        ...
        _create: function() {
            ...
            this._ui = ui, this._renderedOptions = this._getOptions(this.options), this.options.enhanced ? (ui.heading = $(".ui-collapsible-heading", this.element[0]), ui.content = ui.heading.next(), ui.anchor = $("a", ui.heading[0]).first(), ui.status = ui.anchor.children(".ui-collapsible-heading-status")) : this._enhance(elem, ui), this._on(ui.heading, {
                tap: function() {
        ...
        return this.each(function() {
            var iterator, letter, $this = $(this), o = $.extend({
                grid: null
    ...
    })(jQuery), disabledZoom = (initialContent = (meta = ($ = jQuery)("meta[name=viewport]")).attr("content")) + ",maximum-scale=1, user-scalable=no", enabledZoom = initialContent + ",maximum-scale=10, user-scalable=yes", disabledInitially = /(user-scalable[\s]*=[\s]*no)|(maximum-scale[\s]*=[\s]*1)[$,\s]/.test(initialContent), $.mobile.zoom = $.extend({
    }, {
        enabled: !disabledInitially,
        locked: !1,
        disable: function(lock) {
            disabledInitially || $.mobile.zoom.locked || (meta.attr("content", disabledZoom), $.mobile.zoom.enabled = !1, $.mobile.zoom.locked = lock || !1);
        },
        enable: function(unlock) {
            disabledInitially || $.mobile.zoom.locked && !0 !== unlock || (meta.attr("content", enabledZoom), $.mobile.zoom.enabled = !0, $.mobile.zoom.locked = !1);
        },
        restore: function() {
            disabledInitially || (meta.attr("content", initialContent), $.mobile.zoom.enabled = !0);
        }
    }), (function($, undefined) {
    ...
    })(jQuery), rDividerListItem = /(^|\s)ui-li-divider(\s|$)/, origDefaultFilterCallback = ($ = jQuery).mobile.filterable.prototype.options.filterCallback, $.mobile.filterable.prototype.options.filterCallback = function(index, searchValue) {
        ...
        _create: function() {
            ...
            for(this._super(), $.extend(this, {
                _widget: null
            }), idx = recognizedWidgets.length - 1; idx > -1; idx--)if (widgetName = recognizedWidgets[idx], $.mobile[widgetName]) {
        ...
        _setInput: function(selector) {
            ...
            selector || !this._isSearchInternal() && (updatePlaceholder = !1, selector = $("<input data-" + $.mobile.ns + "type='search' placeholder='" + opts.filterPlaceholder + "'></input>").jqmData("ui-filterable-" + this.uuid + "-internal", !0), $("<form class='ui-filterable'></form>").append(selector).submit(function(evt) {
                evt.preventDefault(), selector.blur();
            }).insertBefore(this.element), $.mobile.textinput && (null != this.options.filterTheme && (textinputOpts.theme = opts.filterTheme), selector.textinput(textinputOpts))), this._super(selector), this._isSearchInternal() && updatePlaceholder && this._search.attr("placeholder", this.options.filterPlaceholder);
        },
        _setOptions: function(options) {
            var ret = this._super(options);
            return undefined !== options.filterPlaceholder && this._isSearchInternal() && this._search.attr("placeholder", options.filterPlaceholder), undefined !== options.filterTheme && this._search && $.mobile.textinput && this._search.textinput("option", "theme", options.filterTheme), ret;
        },
        _destroy: function() {
        ...
        _syncTextInputOptions: function(options) {
            ...
            if (this._isSearchInternal() && $.mobile.textinput) {
                for(idx in $.mobile.textinput.prototype.options)undefined !== options[idx] && ("theme" === idx && null != this.options.filterTheme ? textinputOptions[idx] = this.options.filterTheme : textinputOptions[idx] = options[idx]);
