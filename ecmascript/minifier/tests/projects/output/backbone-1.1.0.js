(function() {
    var root = this, previousBackbone = root.Backbone, array = [];
    array.push;
    ...
    var Events = Backbone.Events = {
            ...
            once: function(name, callback, context) {
                ...
                var once = _.once(function() {
                    this.off(name, once), callback.apply(this, arguments);
                });
            ...
            off: function(name, callback, context) {
                ...
                for(names = name ? [name] : _.keys(this._events), i = 0, names.length; i < names.length; i++)if (name = names[i], events = this._events[name]) {
                    if (this._events[name] = retain = [], callback || context) for(j = 0, events.length; j < events.length; j++)ev = events[j], (callback && callback !== ev.callback && callback !== ev.callback._callback || context && context !== ev.context) && retain.push(ev);
            ...
            stopListening: function(obj, name, callback) {
                ...
                !callback && 'object' == typeof name && (callback = this), obj && ((listeningTo = {
                })[obj._listenId] = obj);
    ...
    var Model = Backbone.Model = function(attributes, options) {
        ...
        options || (options = {
        }), this.cid = _.uniqueId('c'), this.attributes = {
        }, options.collection && (this.collection = options.collection), options.parse && (attrs = this.parse(attrs, options) || {
        }), attrs = _.defaults({
        }, attrs, _.result(this, 'defaults')), this.set(attrs, options), this.changed = {
    ...
    _.extend(Model.prototype, Events, {
        ...
        set: function(key, val, options) {
            ...
            changes = [], this._changing, this._changing = !0, !this._changing && (this._previousAttributes = _.clone(this.attributes), this.changed = {
            }), this.attributes, this._previousAttributes, this.idAttribute in attrs && (this.id = attrs[this.idAttribute]);
            for(attr in attrs){
                if (val = attrs[attr], !_.isEqual(this.attributes[attr], val) && changes.push(attr), !_.isEqual(this._previousAttributes[attr], val)) this.changed[attr] = val;
                else delete this.changed[attr];
                options.unset ? delete this.attributes[attr] : this.attributes[attr] = val;
            }
            if (!options.silent) {
                changes.length && (this._pending = !0);
                for(var i = 0, l = changes.length; l > i; i++)this.trigger('change:' + changes[i], this, this.attributes[changes[i]], options);
            }
            if (this._changing) return this;
            if (!options.silent) for(; this._pending;)this._pending = !1, this.trigger('change', this, options);
        ...
        changedAttributes: function(diff) {
            ...
            for(var attr in diff){
                if (_.isEqual((this._changing ? this._previousAttributes : this.attributes)[attr], val = diff[attr])) continue;
                (changed || (changed = {
                }))[attr] = val;
            }
            return changed;
        ...
        save: function(key, val, options) {
            ...
            attrs && options.wait && (this.attributes = _.extend({
            }, attributes, attrs)), options.parse === void 0 && (options.parse = !0);
            var model = this, success = options.success;
            return options.success = function(resp) {
                model.attributes = attributes;
                var serverAttrs = model.parse(resp, options);
                if (options.wait && (serverAttrs = _.extend(attrs || {
                }, serverAttrs)), _.isObject(serverAttrs) && !model.set(serverAttrs, options)) return !1;
                success && success(model, resp, options), model.trigger('sync', model, resp, options);
    ...
    var Collection = Backbone.Collection = function(models, options) {
        options || (options = {
        }), options.model && (this.model = options.model), options.comparator !== void 0 && (this.comparator = options.comparator), this._reset(), this.initialize.apply(this, arguments), models && this.reset(models, _.extend({
            silent: !0
        }, options));
    };
    _.extend(Collection.prototype, Events, {
        ...
        set: function(models, options) {
            ...
            var i, l, id, model, attrs, existing, sort, at = options.at, sortable = this.comparator && null == at && options.sort !== !1, sortAttr = _.isString(this.comparator) ? this.comparator : null, toAdd = [], toRemove = [], modelMap = {
                }, add = options.add, merge = options.merge, remove = options.remove, order = !sortable && add && remove ? [] : !1;
            for(i = 0, l = models.length; l > i; i++){
                ...
                if (existing = this.get(id)) remove && (modelMap[existing.cid] = !0), merge && (attrs = attrs === model ? model.attributes : attrs, options.parse && (attrs = existing.parse(attrs, options)), existing.set(attrs, options), sortable && !sort && existing.hasChanged(sortAttr) && (sort = !0)), models[i] = existing;
                else if (add) {
                    if (model = models[i] = this._prepareModel(attrs, options), !model) continue;
                    toAdd.push(model), model.on('all', this._onModelEvent, this), this._byId[model.cid] = model, model.id != null && (this._byId[model.id] = model);
                }
            ...
            if (toAdd.length || order && order.length) {
                if (sortable && (sort = !0), this.length += toAdd.length, null != at) for(i = 0, l = toAdd.length; l > i; i++)this.models.splice(at + i, 0, toAdd[i]);
                else {
        ...
        sort: function(options) {
            ...
            if (options || (options = {
            }), _.isString(this.comparator) || this.comparator.length === 1) this.models = this.sortBy(this.comparator, this);
        ...
        create: function(model, options) {
            ...
            return options.success = function(model, resp, options) {
                options.wait && this.add(model, options), success && success(model, resp, options);
        ...
        _prepareModel: function(attrs, options) {
            ...
            return this.trigger('invalid', this, model.validationError, options), !1;
        },
        ...
        _onModelEvent: function(event, model, collection, options) {
            ...
            'destroy' === event && this.remove(model, options), model && event === 'change:' + model.idAttribute && (delete this._byId[model.previous(model.idAttribute)], model.id != null && (this._byId[model.id] = model)), this.trigger.apply(this, arguments);
        }
    ...
    _.extend(View.prototype, Events, {
        ...
        delegateEvents: function(events) {
            ...
            for(var key in events){
                ...
                if (!_.isFunction(method) && (method = this[events[key]]), !method) continue;
                var match = key.match(delegateEventSplitter), eventName = match[1], selector = match[2];
                if (method = _.bind(method, this), eventName += '.delegateEvents' + this.cid, '' === selector) this.$el.on(eventName, method);
        ...
        params.type === 'GET' || options.emulateJSON || (params.processData = !1), params.type === 'PATCH' && noXhrPatch && (params.xhr = function() {
    ...
    _.extend(Router.prototype, Events, {
        ...
        _bindRoutes: function() {
            ...
            for(var route, routes = _.keys(this.routes); (route = routes.pop()) != null;)this.route(route, this.routes[route]);
        },
    ...
    var History1 = Backbone.History = function() {
        ...
        }, routeStripper = /^[#\/]|\s+$/g, rootStripper = /^\/+|\/+$/g, trailingSlash = /\/$/, pathStripper = /[?#].*$/;
    History1.started = !1, _.extend(History1.prototype, Events, {
        ...
        getFragment: function(fragment, forcePushState) {
            if (null == fragment) {
                if (this._hasPushState || !this._wantsHashChange || forcePushState) {
                    ...
                    !fragment.indexOf(root1) && (fragment = fragment.slice(root1.length));
        ...
        start: function(options) {
            ...
            var fragment = this.getFragment(), docMode = document.documentMode, oldIE = /msie [\w.]+/.exec(navigator.userAgent.toLowerCase()) && (!docMode || 7 >= docMode);
            if (this.root = ('/' + this.root + '/').replace(rootStripper, '/'), oldIE && this._wantsHashChange && (this.iframe = Backbone.$('<iframe src="javascript:0" tabindex="-1" />').hide().appendTo('body')[0].contentWindow, this.navigate(fragment)), this._hasPushState) Backbone.$(window).on('popstate', this.checkUrl);
        ...
        navigate: function(fragment, options) {
            ...
            if (fragment = fragment.replace(pathStripper, ''), this.fragment === fragment) return;
            ...
            else if (this._wantsHashChange) this._updateHash(this.location, fragment, options.replace), this.iframe && fragment !== this.getFragment(this.getHash(this.iframe)) && (!options.replace && this.iframe.document.open().close(), this._updateHash(this.iframe.location, fragment, options.replace));
