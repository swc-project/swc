...
impl<'a, 'b, P: swc_ecma_visit::Fold> PassBuilder<'a, 'b, P> {
        ...
        P: 'cmt,
    {
        ...
        let compat_pass = if let Some(env) = self.env {
            ...
            Either::Right(chain!(
                import_assertions(),
                Optional::new(compat::es2021::es2021(), self.target <= JscTarget::Es2020),
                Optional::new(compat::es2020::es2020(), self.target <= JscTarget::Es2019),
                Optional::new(typescript::strip(), syntax.typescript()),
                Optional::new(compat::es2018(), self.target <= JscTarget::Es2017),
                Optional::new(compat::es2017(), self.target <= JscTarget::Es2016),
                Optional::new(compat::es2016(), self.target <= JscTarget::Es2015),
                Optional::new(
                    compat::es2015(
                        self.global_mark,
                        comments.clone(),
