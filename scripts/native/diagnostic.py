...
# Isolated CI experiments; these changes are never published as artifacts.
import os
experiment = os.environ["EXPERIMENT"]
if experiment != "current":
    p = Path("crates/swc_native_addon/src/integrity.rs")
    s = p.read_text()
    assert "return 1024 * 1024;" in s
    p.write_text(s.replace("return 1024 * 1024;", "return (_raw_len as usize).min(32 * 1024 * 1024);"))
if experiment == "bulk-no-asm":
    p = Path("crates/swc_native_addon/Cargo.toml")
    s = p.read_text()
    p.write_text(s.replace('zstd = { version = "=0.13.3", default-features = false }', 'zstd = { version = "=0.13.3", default-features = false, features = ["no_asm"] }'))
