...
const [product, rawPath, carrierPath, filename] = process.argv.slice(2);
...
try {
 ...
 const smoke=(entry,addon,cache)=>{
  const result=spawnSync(process.execPath,[resolve('scripts/native/smoke.cjs'),product,entry,addon,'x86_64-apple-darwin'],{encoding:'utf8',env:{...process.env,SWC_NATIVE_BINDING_CACHE:cache}});
 ...
 const result={target:'x86_64-apple-darwin',...measureLoads({raw,carrier,copyRaw:i=>variant('cold-raw-'+i,rawPath),smoke,cacheRoot})};
