...
use swc_ecma_utils::{
    is_valid_prop_ident, private_ident, quote_ident, quote_str, ExprFactory, FunctionFactory,
};
...
impl VisitMut for Umd {
    ...
    fn visit_mut_module(&mut self, module: &mut Module) {
        ...
        // "use strict";
        if self.config.config.strict_mode && !has_use_strict(module_items) {
...
impl Umd {
    fn handle_import_export(
        ...
        link.into_iter().for_each(
            |(src, LinkItem(src_span, link_specifier_set, mut link_flag))| {
                ...
                if self.config.config.no_interop || is_swc_detault_helper {
    ...
    fn adapter(&mut self, exported_name: Ident, is_export_assign: bool) -> (FnExpr, Vec<Param>) {
        ...
        let exports = quote_ident!(DUMMY_SP.apply_mark(self.unresolved_mark), "exports");
        ...
        let module_exports = module.clone().make_member(exports);
        ...
        if !is_export_assign && !self.dep_list.is_empty() {
