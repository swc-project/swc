...
//!
//! It also exports standalone formatting functions (format_loc,
//! format_primitive, etc.) that require no state.
...
pub fn format_primitive(prim: &crate::PrimitiveValue) -> String {
    match prim {
        ...
        crate::PrimitiveValue::String(s) => {
            // Format like JS JSON.stringify: escape control chars and quotes but NOT
            // non-ASCII unicode
...
///
/// Both `DebugPrinter` structs delegate to this for formatting shared
/// constructs like Places, Identifiers, Scopes, Types, InstructionValues, etc.
...
impl<'a> PrintFormatter<'a> {
    ...
    /// Write a line without adding indentation (used when copying pre-formatted
    /// output)
    ...
    /// Format an InstructionValue. The `inner_func_formatter` callback is
    /// invoked for FunctionExpression/ObjectMethod to format the inner
    /// HirFunction. If None, a placeholder is printed instead.
