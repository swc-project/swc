...
//! Environment configuration, ported from EnvironmentConfigSchema in
//! Environment.ts.
//!
//! Contains feature flags and custom hook definitions that control compiler
//! behavior.
...
use crate::{
    type_config::{TypeConfig, ValueKind},
    Effect,
};
...
pub struct EnvironmentConfig {
    ...
    /// If true, emit code to reset the memo cache on source file changes
    /// (HMR/fast refresh). If null (None), HMR detection is conditionally
    /// enabled based on NODE_ENV/__DEV__.
    #[serde(default)]
    ...
    /// Hook guard configuration. When set, wraps hook calls with dispatcher
    /// guard calls.
    #[serde(default)]
    pub enable_emit_hook_guards: Option<ExternalFunctionConfig>,

    /// Instrumentation configuration. When set, emits calls to instrument
    /// functions.
    #[serde(default)]
