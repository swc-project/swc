...
use crate::{
    object_shape::*,
    type_config::{
        AliasingEffectConfig, AliasingSignatureConfig, ApplyArgConfig, ApplyArgHoleKind,
        BuiltInTypeRef, TypeConfig, TypeReferenceConfig, ValueKind, ValueReason,
    },
    Effect, Type,
};
...
/// - **Overlay mode** (`base=Some`): holds a `&'static HashMap` base plus a
///   small extras HashMap. Lookups check extras first, then base. Inserts go
///   into extras. Cloning only copies the extras map (the base pointer is
///   shared).
                ...
                .map(|props| {
                    props
                        ...
                        .map(|(key, value)| {
                            ...
                            // Validate hook-name vs hook-type consistency (matching TS
                            // installTypeConfig)
                            if let Some(errs) = errors {
                                ...
                                if expect_hook != is_hook {
                                    errs.push(format!(
                                        "Expected type for object property '{}' from module '{}' \
...
fn build_object_shape(shapes: &mut ShapeRegistry) {
    // BuiltInObject: has toString() returning Primitive (matches TS BuiltInObjectId
    // shape)
    ...
    // BuiltInMixedReadonly: has explicit method types + wildcard returning
    // MixedReadonly (matches TS BuiltInMixedReadonlyId shape)
...
fn build_ref_shapes(shapes: &mut ShapeRegistry) {
    ...
    // BuiltInRefValue: { *: Object { shapeId: BuiltInRefValue } }
    // (self-referencing)
    add_object(
        shapes,
...
fn build_state_shapes(shapes: &mut ShapeRegistry) {
    ...
    // BuiltInUseTransition: [0] = Primitive (isPending), [1] = startTransition
    // function
    add_object(
        shapes,
...
/// Build the default globals registry. This corresponds to TS
/// `DEFAULT_GLOBALS`.
///
...
/// Build the React API types (REACT_APIS from TS). Returns the list of (name,
/// type) pairs so they can be reused as properties of the React namespace
/// object (matching TS behavior where the SAME type objects are used in both
/// DEFAULT_GLOBALS and the React namespace).
fn build_react_apis(
    shapes: &mut ShapeRegistry,
...
/// Build typed globals and return them as a list for use as globalThis/global
/// properties.
fn build_typed_globals(
    ...
    // where the same type objects are used as both standalone globals and React.*
    // properties)
