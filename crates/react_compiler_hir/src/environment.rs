...
use crate::{
    ...
    globals::{self, Global, GlobalRegistry},
    object_shape::{
        add_hook, default_mutating_hook, default_nonmutating_hook, FunctionSignature, HookKind,
        HookSignatureBuilder, ShapeRegistry, BUILT_IN_MIXED_READONLY_ID,
    },
    *,
};
...
/// Output mode for the compiler, mirrored from the entrypoint's
/// CompilerOutputMode. Stored on Environment so pipeline passes can access it.
...
impl Environment {
    ...
    ///
    /// The child shares the same config, globals, and shapes, and receives
    /// copies of all arenas (identifiers, types, scopes, functions) so that
    /// references from the outlined HIR remain valid. Block/scope counters
    /// start past the cloned data to avoid ID conflicts.
    ...
    /// Take errors added after position `since_count`, leaving earlier errors
    /// in place. Used to detect new errors added by a specific pass.
    ...
    /// Resolve a non-local binding to its type. Ported from TS
    /// `getGlobalDeclaration`.
    ///
    /// The `loc` parameter is used for error diagnostics when validating module
    /// type configurations. Pass `None` if no source location is available.
                        ...
                        if expect_hook != is_hook {
                            self.record_error(
                                ...
                                .with_description(format!(
                                    "Expected type for `import ... from '{}'` {} based on the \
                                     module name",
                                    module,
                                    if expect_hook {
                                        "to be a hook"
    ...
    /// Get the fallthrough (wildcard `*`) property type for computed property
    /// access. Ported from TS `getFallthroughPropertyType`.
    ...
    pub fn generate_globally_unique_identifier_name(&mut self, name: Option<&str>) -> String {
        ...
        // 3. CamelCase: replace '-' sequences + optional following char with uppercase
        //    of that char
        ...
        // CamelCase conversion: replace sequences of '-' followed by optional char with
        // uppercase
    ...
    /// Record an outlined function (extracted during outlineFunctions or
    /// outlineJSX). Corresponds to TS `env.outlineFunction(fn, type)`.
    ...
    /// Check whether the function type for an identifier has a noAlias
    /// signature. Looks up the identifier's type and checks its function
    /// signature.
    ...
    /// Looks up the identifier's type and delegates to
    /// `get_hook_kind_for_type`.
...
/// Returns true if the name follows React naming conventions (component or
/// hook). Components start with an uppercase letter; hooks match `use[A-Z0-9]`.
