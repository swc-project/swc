...
use crate::{
    environment::Environment, ArrayElement, ArrayPatternElement, BasicBlock, BlockId, HirFunction,
...
/// Yields lvalues from
/// DeclareLocal/StoreLocal/DeclareContext/StoreContext/Destructure/
/// PostfixUpdate/PrefixUpdate. Equivalent to TS `eachInstructionValueLValue`.
...
/// Like `each_instruction_operand` but takes `functions` directly instead of
/// `env`. Useful when borrow splitting prevents passing the full `Environment`.
...
/// Like `each_instruction_value_operand` but takes `functions` directly instead
/// of `env`. Useful when borrow splitting prevents passing the full
/// `Environment`.
...
/// Equivalent to TS `mapTerminalSuccessors` — but mutates in place instead of
/// returning a new terminal.
...
/// Yields ALL block IDs referenced by a terminal (successors + fallthroughs +
/// internal blocks). Unlike `each_terminal_successor` which yields only
/// standard control flow successors, this function yields every block ID that
/// `map_terminal_successors` would visit.
...
impl ScopeBlockTraversal {
    ...
    /// Returns true if the given scope is currently 'active', i.e. if the scope
    /// start block but not the scope fallthrough has been recorded.
...
/// Convenience wrapper around `each_instruction_value_operand` that maps to
/// ids.
...
//       | InstructionValue::ObjectMethod { lowered_func, .. } = &mut
// instr.value {       let func = &mut env.functions[lowered_func.func.0 as
// usize];       for ctx in func.context.iter_mut() { ... }
//   }
//

/// In-place mutation of all operand places in an InstructionValue.
/// Does NOT handle FunctionExpression/ObjectMethod context — callers handle
/// those separately.
...
/// In-place mutation of an InstructionValue's lvalues (DeclareLocal,
/// StoreLocal, DeclareContext, StoreContext, Destructure, PostfixUpdate,
/// PrefixUpdate). Does NOT include the instruction's top-level lvalue — use
/// `for_each_instruction_lvalue_mut` for that.
...
/// Matches the same variants as TS `mapInstructionLValues` (skips
