...
use crate::{visitors::each_terminal_successor, BlockId, HirFunction, Terminal};
...
/// predecessors of nodes that post-dominate `target_id` from which execution
/// may not reach `target_id`. Intuitively, these are the earliest blocks from
...
/// Walks up the post-dominator tree to collect all blocks that post-dominate
/// `target_id`.
...
/// on this chain (meaning every path through the function must pass through
/// it).
