...
impl<'a> Traverse<'a, TransformState<'a>> for TypeScriptNamespace<'a, '_> {
    ...
    fn enter_program(&mut self, program: &mut Program<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        // Inserting the `let` declaration multiple times will reallocate the whole
        // statements vec every time a namespace declaration is encountered.
...
impl<'a> TypeScriptNamespace<'a, '_> {
    fn handle_nested(
        ...
        // If it's a namespace, we need additional checks to determine if it can return
        // early.
        if flags.is_namespace_module() {
            ...
            // ```
            //
            // When both `NameSpaceModule` and `ValueModule` are present, we need to check
            // the current declaration flags. If the current declaration is
            // `NameSpaceModule`, we can return early because it's a type-only
            // namespace and doesn't emit any JS code, otherwise we need to
        ...
        match body {
            ...
            TSModuleDeclarationBody::TSModuleDeclaration(declaration) => {
                ...
                let export_named_decl = ctx
                    .ast
                    .plain_export_named_declaration_declaration(SPAN, declaration);
    ...
    fn create_variable_declaration(
        ...
        let declarations = {
            ...
            let decl = ctx
                .ast
                .variable_declarator(SPAN, kind, pattern, None, false);
            ctx.ast.vec1(decl)
        };
        ctx.ast
            .declaration_variable(SPAN, kind, declarations, false)
    }
    ...
    fn transform_namespace(
        ...
        let arguments = {
            ...
            let mut logical_right = {
                ...
                let assign_expr =
                    ctx.ast
                        .expression_assignment(SPAN, op, assign_left, assign_right);
            ...
            if let Some(parent_binding) = parent_binding {
                ...
                let assign_right = {
                    ...
                    ctx.ast
                        .expression_logical(SPAN, logical_left.into(), op, logical_right)
                };
                let op = AssignmentOperator::Assign;
                logical_right = ctx
                    .ast
            ...
            let expr =
                ctx.ast
                    .expression_logical(SPAN, logical_left, LogicalOperator::Or, logical_right);
    ...
    fn handle_variable_declaration(
        ...
        ctx.ast.vec_from_array([
            ...
            ctx.ast
                .statement_expression(SPAN, ctx.ast.expression_sequence(SPAN, assignments)),
        ])
    }
    ...
    fn is_redeclaration_namespace(id: &BindingIdentifier<'a>, ctx: &TraverseCtx<'a>) -> bool {
        ...
        // Find first value declaration because only value declaration will emit JS
        // code.
        redeclarations
            .iter()
            .find(|rd| rd.flags.is_value())
