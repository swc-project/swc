...
impl TypeScript {
    pub fn new(options: &TypeScriptOptions, ctx: &TransformCtx) -> Self {
        Self {
            ...
            is_class_properties_plugin_enabled: ctx.is_class_properties_plugin_enabled,
            set_public_class_fields: ctx.assumptions.set_public_class_fields,
        }
    }
}

impl VisitMutHook<TraverseCtx<'_>> for TypeScript {
    fn enter_program(&mut self, program: &mut Program, ctx: &mut TraverseCtx) {
    ...
    fn enter_arrow_expr(&mut self, expr: &mut ArrowExpr, ctx: &mut TraverseCtx) {
        self.annotations.enter_arrow_function_expression(expr, ctx);
    }

    fn enter_var_declarator(&mut self, decl: &mut VarDeclarator, ctx: &mut TraverseCtx) {
        self.annotations.enter_variable_declarator(decl, ctx);
    }

    fn enter_pat(&mut self, pat: &mut Pat, ctx: &mut TraverseCtx) {
        self.annotations.enter_binding_pattern(pat, ctx);
    }

    fn enter_call_expr(&mut self, expr: &mut CallExpr, ctx: &mut TraverseCtx) {
        self.annotations.enter_call_expression(expr, ctx);
    }

    fn enter_opt_chain_expr(&mut self, expr: &mut OptChainExpr, ctx: &mut TraverseCtx) {
        self.annotations
    ...
    fn enter_param(&mut self, param: &mut Param, ctx: &mut TraverseCtx) {
    ...
    fn enter_new_expr(&mut self, expr: &mut NewExpr, ctx: &mut TraverseCtx) {
    ...
    fn enter_if_stmt(&mut self, stmt: &mut IfStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_if_statement(stmt, ctx);
    }

    fn enter_while_stmt(&mut self, stmt: &mut WhileStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_while_statement(stmt, ctx);
    }

    fn enter_do_while_stmt(&mut self, stmt: &mut DoWhileStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_do_while_statement(stmt, ctx);
    }

    fn enter_for_stmt(&mut self, stmt: &mut ForStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_for_statement(stmt, ctx);
    }

    fn enter_for_in_stmt(&mut self, stmt: &mut ForInStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_for_in_statement(stmt, ctx);
    }

    fn enter_for_of_stmt(&mut self, stmt: &mut ForOfStmt, ctx: &mut TraverseCtx) {
        self.annotations.enter_for_of_statement(stmt, ctx);
    }

    fn enter_tagged_tpl(&mut self, expr: &mut TaggedTpl, ctx: &mut TraverseCtx) {
        self.annotations.enter_tagged_template_expression(expr, ctx);
    }

    fn enter_jsx_element(&mut self, elem: &mut JSXElement, ctx: &mut TraverseCtx) {
        self.annotations.enter_jsx_element(elem, ctx);
    }

    fn enter_jsx_fragment(&mut self, elem: &mut JSXFragment, ctx: &mut TraverseCtx) {
        self.annotations.enter_jsx_fragment(elem, ctx);
    }

    fn enter_ts_export_assignment(&mut self, node: &mut TsExportAssignment, ctx: &mut TraverseCtx) {
    ...
    fn enter_import_decl(&mut self, node: &mut ImportDecl, ctx: &mut TraverseCtx) {
        if let Some(rewrite_extensions) = &mut self.rewrite_extensions {
            rewrite_extensions.enter_import_decl(node, ctx);
        }
    }

    fn enter_export_all(&mut self, node: &mut ExportAll, ctx: &mut TraverseCtx) {
        if let Some(rewrite_extensions) = &mut self.rewrite_extensions {
            rewrite_extensions.enter_export_all(node, ctx);
        }
    }

    fn enter_named_export(&mut self, node: &mut NamedExport, ctx: &mut TraverseCtx) {
        if let Some(rewrite_extensions) = &mut self.rewrite_extensions {
