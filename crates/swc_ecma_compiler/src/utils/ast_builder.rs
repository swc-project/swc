...
pub fn wrap_statements_in_arrow_function_iife<'a>(
    ...
    let params = ctx
        .ast
        .alloc_formal_parameters(SPAN, kind, ctx.ast.vec(), NONE);
    ...
    ctx.ast
        .expression_call(span, arrow, NONE, ctx.ast.vec(), false)
}
...
pub fn create_super_call<'a>(
    ...
    ctx.ast.expression_call(
        ...
        ctx.ast.vec1(
            ctx.ast
                .argument_spread_element(SPAN, args_binding.create_read_expression(ctx)),
        ),
        false,
    )
}
...
pub fn create_class_constructor<'a, 'c>(
    ...
    // class. `constructor(..._args) { super(..._args); /* prop initialization
    // */ }` TODO(improve-on-babel): We can use `arguments` instead of creating
    // `_args`.
    ...
    let stmts = if has_super_class {
        ...
        ctx.ast.vec_from_iter(
            iter::once(
                ctx.ast
                    .statement_expression(SPAN, create_super_call(&args_binding, ctx)),
            )
            .chain(stmts_iter),
        )
    } else {
    ...
    ClassElement::MethodDefinition(
        ctx.ast.alloc_method_definition(
            ...
            PropertyKey::StaticIdentifier(
                ctx.ast
                    .alloc_identifier_name(SPAN, Atom::from("constructor")),
            ),
