...
impl<'a> CompilerImpl<'a> {
            ...
            match &mut *base {
                OptChainBase::Member(m) => {
                    ...
                    chain.push(if optional {
                        Gathering::OptMember(m.take(), self.memoize_optional_chain(&next, false))
                    } else {
                ...
                OptChainBase::Call(c) => {
                    ...
                    chain.push(if optional {
                        Gathering::OptCall(
                            c.take().into(),
                            self.memoize_optional_chain(&next, true),
                        )
                    } else {
                    ...
                    match &mut current {
                        Expr::Member(m) => {
                            ...
                            let this = ctx.unwrap_or_else(|| {
                                let this = self.memoize_optional_chain(&m.obj, true);
    ...
    fn should_memo_optional_chain(&self, expr: &Expr, is_call: bool) -> bool {
    ...
    fn memoize_optional_chain(&mut self, expr: &Expr, is_call: bool) -> Memo {
        if self.should_memo_optional_chain(expr, is_call) {
            let memo = alias_ident_for(expr, "_this");
            self.es2020_optional_chaining_vars.push(VarDeclarator {
                span: DUMMY_SP,
    ...
    pub(crate) fn visit_mut_stmt_like_for_optional_chaining<T>(&mut self, stmts: &mut Vec<T>)
    where
        T: Send + Sync + StmtLike + VisitMutWith<Self>,
        Vec<T>: VisitMutWith<Self>,
    {
        let uninit = self.es2020_optional_chaining_vars.take();
        for stmt in stmts.iter_mut() {
            stmt.visit_mut_with(self);
        }

        if !self.es2020_optional_chaining_vars.is_empty() {
            prepend_stmt(
                stmts,
                T::from(
                    VarDecl {
                        ...
                        decls: mem::take(&mut self.es2020_optional_chaining_vars),
                        ..Default::default()
                    }
                    .into(),
                ),
        ...
        self.es2020_optional_chaining_vars = uninit;
    }

    /// Transform optional chaining expressions
    pub(crate) fn transform_optional_chaining(&mut self, e: &mut Expr) -> bool {
        match e {
            // foo?.bar -> foo == null ? void 0 : foo.bar
            Expr::OptChain(v) => {
                let data = self.gather_optional_chain(v.take(), Vec::new());
                *e = self.construct_optional_chain(data, false);
                true
            }
            ...
            Expr::Unary(UnaryExpr {
                ...
                match &mut **arg {
                    ...
                    Expr::OptChain(v) => {
                        let data = self.gather_optional_chain(v.take(), Vec::new());
                        *e = self.construct_optional_chain(data, true);
                        true
                    }
                    _ => false,
                }
    ...
    /// Handle optional chaining in assignment patterns
    ...
    pub(crate) fn visit_mut_pat_for_optional_chaining(&mut self, n: &mut Pat) {
