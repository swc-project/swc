...
impl<'a> ClassPropertiesSuperConverter<'a, '_, '_> {
    ...
    fn transform_static_member_expression_impl(
        ...
        let property = ctx
            .ast
            .expression_string_literal(property.span, property.name, None);
    ...
    //
    // `#[inline]` so can bail out fast without a function call if `callee` is not a member
    // expression with `super` as member expression object (fairly rare).
    ...
    fn transform_call_expression_for_super_computed_member_expr(
        ...
        let callee = &mut call_expr.callee;
        let Expression::ComputedMemberExpression(member) = callee else {
            unreachable!()
        };
    ...
    /// Transform assignment expression where the left-hand side is a member
    /// expression with `super`.
    ///
    /// * `super.prop = value` -> `_superPropSet(_Class, "prop", value, _Class,
    ///   1)`
    /// * `super.prop += value` -> `_superPropSet(_Class, "prop",
    ///   _superPropGet(_Class, "prop", _Class) + value, _Class, 1)`
    /// * `super.prop &&= value` -> `_superPropGet(_Class, "prop", _Class) &&
    ...
    ///   _superPropSet(_Class, prop, value, _Class, 1)`
    //
    // `#[inline]` so can bail out fast without a function call if `left` is not a
    // member expression with `super` as member expression object (fairly rare).
    // Actual transform is broken out into separate functions.
    ...
    /// Transform assignment expression where the left-hand side is a static
    ...
    fn transform_assignment_expression_for_super_static_member_expr(
        ...
        let property =
            ctx.ast
                .expression_string_literal(member.property.span, member.property.name, None);
        *expr =
            self.transform_super_assignment_expression_impl(span, operator, property, value, ctx);
    }

    /// Transform assignment expression where the left-hand side is a computed
    /// member expression with `super` as member expr object.
    ///
    /// * `super[prop] = value` -> `_superPropSet(_Class, prop, value, _Class,
    ///   1)`
    /// * `super[prop] += value` -> `_superPropSet(_Class, prop,
    ...
    fn transform_assignment_expression_for_super_computed_member_expr(
        ...
        let AssignmentTarget::ComputedMemberExpression(member) = left else {
            unreachable!()
        };
    ...
    /// Transform assignment expression where the left-hand side is a member
    /// expression with `super` as member expr object.
    ///
    /// * `=` -> `_superPropSet(_Class, <prop>, <value>, _Class, 1)`
    /// * `+=` -> `_superPropSet(_Class, <prop>, _superPropGet(_Class, <prop>,
    ///   _Class) + <value>, 1)`
    ...
    fn transform_super_assignment_expression_impl(
        ...
        if operator == AssignmentOperator::Assign {
            ...
            if let Some(operator) = operator.to_binary_operator() {
                // `super[prop] += value`
                // -> `_superPropSet(_Class, prop, _superPropGet(_Class, prop, _Class) + value,
                // _Class, 1)`
                ...
                // `super[prop] &&= value`
                // -> `_superPropGet(_Class, prop, _Class) && _superPropSet(_Class, prop, value,
                // _Class, 1)`
                ...
                // _Class, 1)`
                ctx.ast
                    .expression_logical(span, get_call, operator, set_call)
            } else {
    ...
    /// Transform update expression where the argument is a member expression
    /// with `super`.
    ///
    /// * `++super.prop` or `super.prop--` See
    ///   [`Self::transform_update_expression_for_super_static_member_expr`]
    ///
    /// * `++super[prop]` or `super[prop]--` See
    ///   [`Self::transform_update_expression_for_super_computed_member_expr`]
    //
    // `#[inline]` so can bail out fast without a function call if `argument` is not
    // a member expression with `super` as member expression object (fairly
    // rare). Actual transform is broken out into separate functions.
    pub(super) fn transform_update_expression_for_super_assignment_target(
        &mut self,
        expr: &mut Expression<'a>,
        ctx: &mut TraverseCtx<'a>,
    ) {
        let Expression::UpdateExpression(update_expr) = expr else {
            unreachable!()
        };
    ...
    /// Transform update expression (`++` or `--`) where argument is a static
    /// member expression with `super`.
    ///
    /// * `++super.prop` ->
    /// ```js
    ...
    /// Transform update expression (`++` or `--`) where argument is a computed
    /// member expression with `super`.
    ///
    /// * `++super[prop]` ->
    /// ```js
    ...
    /// Transform update expression (`++` or `--`) where argument is a member
    /// expression with `super`.
    ///
    /// * `++super[prop]` ->
    /// ```js
    ...
    fn transform_super_update_expression_impl(
        ...
        let temp_binding = self
            ...
            .ctx
            .var_declarations
            .create_uid_var(temp_var_name_base, ctx);
        ...
        if prefix {
            ...
            // `(_super$prop = _superPropGet(_Class, prop, _Class), _super$prop2 =
            // _super$prop++, _super$prop)`
            ...
            ctx.ast.expression_sequence(
                span,
                ctx.ast
                    .vec_from_array([set_call, temp_binding2.create_read_expression(ctx)]),
            )
        }
    }
    ...
    fn create_super_prop_get(
        ...
        let arguments = if is_callee {
            ...
            let two = ctx
                .ast
                .expression_numeric_literal(SPAN, 2.0, None, NumberBase::Decimal);
            ctx.ast
                .vec_from_array([class, property, receiver, Argument::from(two)])
        } else {
            // `(_Class, prop, _Class)`
            ctx.ast.vec_from_array([class, property, receiver])
        };

        // `_superPropGet(_Class, prop, _Class)` or `_superPropGet(_Class, prop, _Class,
        // 2)`
    ...
    /// * [`ClassPropertiesSuperConverterMode::Static`] (_Class, _Class)
    ///
    /// * [`ClassPropertiesSuperConverterMode::PrivateMethod`]
    ///   (_Class.prototype, this)
    ///
    /// * [`ClassPropertiesSuperConverterMode::StaticPrivateMethod`] (_Class,
    ///   this)
    fn get_class_binding_arguments(
        ...
        let temp_binding = self
            ...
            .current_class_mut()
            .bindings
            .get_or_init_static_binding(ctx);
        ...
        let receiver = match self.mode {
            ...
            ClassPropertiesSuperConverterMode::PrivateMethod => {
                // TODO(improve-on-babel): `superPropGet` and `superPropSet` helper function has
                // a flag to use `class.prototype` rather than `class`. We
                // should consider using that flag here. <https://github.com/babel/babel/blob/1fbdb64a7fcc3488797e312506dbacff746d4e41/packages/babel-helpers/src/helpers/superPropGet.ts>
