...
impl<'a> ClassProperties<'a, '_> {
    ...
    pub(super) fn substitute_temp_var_for_method_computed_key(
        ...
        // This check also results in exit for non-computed keys e.g. `class C { 'x'()
        // {} 123() {} }`.
        if !self.ctx.key_needs_temp_var(key, ctx) {
            return;
        }

        // TODO(improve-on-babel): It's unnecessary to create temp vars for method keys
        // unless:
        // 1. Properties also have computed keys.
        // 2. Some of those properties' computed keys have side effects and require temp
        //    vars.
        // 3. At least one property satisfying the above is after this method, or class
        //    contains a static block which is being transformed (static blocks are
        //    always evaluated after computed keys, regardless of order)
    ...
    /// * If instance prop, replaces existing key with assignment (it'll be
    ///   moved to before class later).
    /// * Returns `_x`.
    ...
    ///
    /// We do this process in 2 passes so that the computed key is still present
    /// within the class during traversal of the class body, so any other
    /// transforms can run on it. Now that we're exiting the class, we can
    /// move the assignment `_foo = foo()` out of the class to where it
    /// needs to be.
    pub(super) fn extract_instance_prop_computed_key(
        ...
        if !matches!(&prop.key, PropertyKey::AssignmentExpression(_)) {
            // This field is going to be removed, but if the key is a computed key and may
            // have side effects, we need to extract the key and place it before
            // the class to preserve the original behavior.
        ...
        {
            let PropertyKey::AssignmentExpression(assign_expr) = &prop.key else {
                unreachable!()
            };
