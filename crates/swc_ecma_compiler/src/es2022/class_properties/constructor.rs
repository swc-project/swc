...
//!
//! When a class has instance properties / instance private properties, we need
//! to either:
...
//!
//! Oxc's output uses Babel's helpers (`_defineProperty`,
//! `_classPrivateFieldInitSpec` etc).
//!
...
//! 3. constructor only contains a single `super()` call at top level of the
//!    function.
//!
//! Where a class with superclass has an existing constructor containing 1 or
//! more `super()` calls nested within the constructor, we do more like ESBuild
//! does. We insert a single arrow function `_super` at top of the function and
//! replace all `super()` calls with `_super()`.
//!
//! Input:
//! ```js
//! class C extends S {
...
//!
//! Babel handles this case correctly for standard properties, but Babel's
//! approach is problematic for us because Babel outputs the property
//! initializers twice if there are 2 x `super()` calls. We would need to use
//! `CloneIn` and then duplicate all the `ReferenceId`s etc.
//!
//! Instead, we create a `_super` function containing property initializers
//! *outside* the class and convert `super()` calls to `_super(super())`.
//!
//! Input:
//! ```js
//! class C extends S {
...
use super::{utils::exprs_into_stmts, ClassProperties};
use crate::{
    context::TraverseCtx,
    utils::ast_builder::{
...
pub(super) struct InstanceInitScopes {
    ...
    /// Scope of class constructor, if initializers will be inserted into
    /// constructor, (either directly, or in `_super` function within
    /// constructor) and constructor's scope contains any bindings.
    /// This is used for renaming symbols if any shadow symbols referenced by
    /// instance prop initializers.
...
impl<'a> ClassProperties<'a, '_> {
    ...
    ///
    /// Returns:
    /// * `InstanceInitScopes` detailing the `ScopeId`s required for
    ///   transforming instance property initializers.
    /// * `InstanceInitsInsertLocation` detailing where instance property
    ///   initializers should be inserted.
    ///
    /// * `super()` first appears as a top level statement in constructor body
    ///   (common case):
    ///   * Do not alter constructor.
    ///   * No `_super` function is required.
    ///   * Returns `InstanceInitsInsertLocation::Statements`, specifying
    ///     statement index where inits should be inserted.
    ...
    ///   * `_super` function will need to be inserted at top of class
    ///     constructor.
    ...
    ///
    /// If a `_super` function is required, binding for `_super` is recorded in
    /// the returned `InstanceInitsInsertLocation`, and `ScopeId` for
    /// `_super` function is returned as `insert_in_scope_id` in returned
    /// `InstanceInitScopes`.
    ///
    /// This function does not create the `_super` function or insert it. That
    /// happens later.
    pub(super) fn replace_super_in_constructor(
        constructor: &mut Function<'a>,
        ctx: &mut TraverseCtx<'a>,
    ) -> (InstanceInitScopes, InstanceInitsInsertLocation<'a>) {
        // Find any `super()`s in constructor params and replace with
        // `_super.call(super())`
        ...
        if let Some((super_func_scope_id, insert_location)) = replacer.replace(constructor) {
            ...
            // Property initializers will be inserted in a `_super` function *outside*
            // class.
        ...
        // Only include `constructor_scope_id` in return value if constructor's scope
        // has some bindings. If it doesn't, no need to check for shadowed
        // symbols in instance prop initializers, because no bindings to clash
        // with.
    ...
    // TODO: Handle private props in constructor params `class C { #x; constructor(x
    // = this.#x) {} }`.
    ...
    pub(super) fn insert_constructor(
        ...
        let mut stmts = ctx
            .ast
            .vec_with_capacity(inits.len() + usize::from(has_super_class));

        // Add `super(..._args);` statement and `..._args` param if class has a super
        // class. `constructor(..._args) { super(..._args); /* prop
        // initialization */ }`
        let mut params_rest = None;
        if has_super_class {
            let args_binding = ctx.generate_uid(
                "args",
                constructor_scope_id,
    ...
    /// Insert instance property initializers into constructor body at
    /// `insertion_index`.
    ...
    pub(super) fn create_super_function_inside_constructor(
        ...
        //
        // TODO(improve-on-babel): When not in loose mode, inits are
        // `_defineProperty(this, propName, value)`. `_defineProperty` returns
        // `this`, so last statement could be `return _defineProperty(this, propName,
        // value)`, rather than an additional `return this` statement.
        // Actually this wouldn't work at present, as `_classPrivateFieldInitSpec(this,
        // _prop, value)` does not return `this`. We could alter it so it does
        // when we have our own helper package.
        let args_binding = ctx.generate_uid(
            "args",
            super_func_scope_id,
            SymbolFlags::FunctionScopedVariable,
        ...
        let body_exprs = ctx.ast.expression_sequence(
            ...
            ctx.ast.vec_from_iter(
                iter::once(super_call)
                    .chain(inits)
                    .chain(iter::once(this_expr)),
            ),
        );
    ...
    pub(super) fn create_super_function_outside_constructor(
        ...
        // TODO: This should be parent scope if insert `_super` function as expression
        // before class expression.
        ...
        let return_stmt = ctx
            .ast
            .statement_return(SPAN, Some(ctx.ast.expression_this(SPAN)));
        // `<inits>; return this;`
        let body_stmts = ctx
            .ast
            .vec_from_iter(exprs_into_stmts(inits, ctx).chain([return_stmt]));
        ...
        // TODO: Need to add `_super` function to class as a static method, and then
        // remove it again in exit phase - so other transforms run on it in
        // between. TODO: Need to transform `super` and references to class name
        // in initializers. TODO: If static block transform is not enabled, it's
        // possible to construct the class within the static block `class C {
        // static { new C() } }` and that'd run before `_super` is defined. So
        // it needs to go before the class, not after, in that case.
        let init = if self.current_class().is_declaration {
            ...
            // TODO: This isn't right. Should not be adding to `insert_after_exprs` in entry
            // phase.
        ...
        self.ctx
            .var_declarations
            .insert_let(super_binding, init, ctx);
    }

    /// Rename any symbols in constructor which clash with symbols used in
    /// initializers
    fn rename_clashing_symbols(
        ...
        for (&symbol_id, name) in clashing_symbols.iter_mut() {
            ...
            *name = new_name;
            ...
            ctx.scoping_mut()
                .rename_symbol(symbol_id, constructor_scope_id, new_name.as_str());
        }
...
impl<'a> VisitMut<'a> for ConstructorParamsSuperReplacer<'a, '_> {
    ...
    fn visit_expression(&mut self, expr: &mut Expression<'a>) {
        ...
        {
            // Walk `CallExpression`'s arguments here rather than falling through to
            // `walk_expression` below to avoid infinite loop as `super()` gets
    ...
    fn visit_property_definition(&mut self, prop: &mut PropertyDefinition<'a>) {
        // `super()` in computed key of property or method refers to super binding of
        // parent class. So visit computed `key`, but not `value`.
        // ```js
    ...
    fn visit_accessor_property(&mut self, prop: &mut AccessorProperty<'a>) {
        // Visit computed `key` but not `value`, for same reasons as
        // `visit_property_definition` above. TODO: Are decorators in scope?
        self.visit_decorators(&mut prop.decorators);
        if prop.computed {
            self.visit_property_key(&mut prop.key);
        }
...
struct ConstructorBodySuperReplacer<'a, 'ctx> {
    ...
    constructor_scope_id: ScopeId,
    /// Binding for `_super` function.
    /// Initially `None`. Binding is created if `super()` is found in position
    /// other than top-level, that requires a `_super` function.
    super_binding: Option<BoundIdentifier<'a>>,
    ctx: &'ctx mut TraverseCtx<'a>,
}
...
impl<'a, 'ctx> ConstructorBodySuperReplacer<'a, 'ctx> {
    ...
    /// If `super()` found first as a top level statement (`constructor() { let
    /// x; super(); }`), does not alter constructor, and returns
    /// `InstanceInitsInsertLocation::ExistingConstructor` and constructor's
    /// `ScopeId`.
    ///
    /// Otherwise, replaces any `super()` calls with `_super()` and returns
    /// `InstanceInitsInsertLocation::SuperFnInsideConstructor`, and `ScopeId`
    /// for `_super` function.
    fn replace(
        mut self,
        constructor: &mut Function<'a>,
        ...
        'outer: loop {
            ...
            if self.super_binding.is_none() {
                ...
                // This is weird, but legal code. It would be a runtime error if the class is
                // constructed (unless the constructor returns early).
                // In reasonable code, we should never get here.
                // Handle this weird case of no `super()` by inserting initializers in a
                // `_super` function which is never called. That is pointless,
                // but not inserting the initializers anywhere would leave
                // `Semantic` in an inconsistent state. What we get is
                // completely legal output and correct `Semantic`, just longer than it
                // could be. But this should very rarely happen in practice, and minifier will
                // delete the `_super` function as dead code.
                // So set `super_binding` and exit the loop, so it's treated as if `super()` was
                // found in a nested position.
