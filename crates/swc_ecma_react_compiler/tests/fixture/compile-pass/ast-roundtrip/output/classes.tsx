...
declare function readSetting<TValue extends string = string>(name: string): TValue;
...
@decorate
export class Derived<TName extends string = "derived", TMeta extends Record<string, unknown> = {
    source: string;
}> extends Base<TName> {
    ...
    #secret = 1;
    static #shared = 0;
    constructor(public readonly displayName: TName, private count = 0, metadata = {
        source: displayName
    } as TMeta){
        super(displayName);
        this.value = count;
    ...
    async *values<TYield extends number = number>(): AsyncGenerator<TYield, void, unknown> {
