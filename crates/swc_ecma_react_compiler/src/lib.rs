...
//! Vendored SWC adapter for the Rust React Compiler from
//! `facebook/react#36173` at commit `72adadf3097cd60e1f320d609e53f19eaf3e81cc`.
...
thread_local! {
    ...
    static LAST_COMMENTS: RefCell<Option<swc_common::comments::SingleThreadedComments>> = const { RefCell::new(None) };

    /// Thread-local storage for blank line positions.
    /// Contains information about where to insert blank lines during emit.
    static BLANK_LINE_POSITIONS: RefCell<Vec<BlankLinePosition>> = const { RefCell::new(Vec::new()) };
}
...
pub struct TransformResult {
    ...
    /// Comments extracted from the compiled AST (for use with
    /// `emit_with_comments`).
...
/// Source syntax accepted by the high-level source APIs.
...
/// Parser configuration for the high-level source APIs.
...
/// High-level emitted output used by the node binding and package wrapper.
pub struct SourceTransformOutput {
    pub code: String,
...
fn parse_module_from_source(
    ...
    let syntax = match parser.syntax {
        SourceSyntax::EcmaScript => swc_ecma_parser::Syntax::Es(swc_ecma_parser::EsSyntax {
            jsx: parser.jsx,
            decorators: parser.decorators,
            ..Default::default()
        }),
        SourceSyntax::TypeScript => {
    ...
    .map_err(|_| String::from("Syntax Error"))?;
...
/// Parses source text with an explicit parser configuration, then runs the
/// React-compiler prefilter.
pub fn is_required_source_with_parser(
    source_text: &str,
    parser: SourceParser,
) -> Result<bool, String> {
    let module = parse_module_from_source(source_text, parser)?;
    Ok(prefilter::has_react_like_functions(&module))
}
...
pub fn transform(
    ...
    let source_type = if source_text
        .lines()
        .next()
        .is_some_and(|line| line.contains("@script"))
    {
    ...
    if let Some(ref mut swc_mod) = swc_module {
        ...
        if has_source_items {
            ...
            let synthetic_span =
                swc_common::Span::new(swc_common::BytePos(1), swc_common::BytePos(1));
...
pub fn transform_source(source_text: &str, options: PluginOptions) -> TransformResult {
    match try_transform_source_with_parser(source_text, options, SourceParser::default()) {
        Ok(result) => result,
...
/// Parses source text with an explicit parser configuration, then transforms.
pub fn try_transform_source_with_parser(
    source_text: &str,
    options: PluginOptions,
    parser: SourceParser,
) -> Result<TransformResult, String> {
    let module = parse_module_from_source(source_text, parser)?;
    Ok(transform(&module, source_text, options))
}

/// Parses source text, transforms it, and emits code.
pub fn try_transform_source_to_code_with_parser(
    source_text: &str,
    options: PluginOptions,
    parser: SourceParser,
) -> Result<SourceTransformOutput, String> {
    let result = try_transform_source_with_parser(source_text, options, parser)?;
    let code = match result.module {
        Some(ref module) => emit(module),
        None => normalize_source(source_text),
    };

    Ok(SourceTransformOutput {
        code,
        diagnostics: result
            .diagnostics
            .into_iter()
...
fn reposition_comment_blank_lines(code: &str) -> String {
    ...
    while i < lines.len() {
        // Look for pattern: blank line followed by comment block followed by
        // declaration
                ...
                if comment_end < lines.len() && comment_end > comment_start {
                    ...
                    if is_declaration {
                        ...
                        if prev_non_empty {
                            ...
                            for line in lines.iter().take(comment_end).skip(comment_start) {
                                result.push(*line);
                            }
...
/// Returns Some(expanded) if the line contains an expandable object, None
/// otherwise.
...
/// Split array content into individual elements, respecting nested
/// braces/brackets.
...
pub fn lint_source(source_text: &str, options: PluginOptions) -> LintResult {
    match try_lint_source_with_parser(source_text, options, SourceParser::default()) {
        Ok(result) => result,
        Err(_) => LintResult {
            diagnostics: vec![],
        },
...
/// Parses source text with an explicit parser configuration, then lints.
pub fn try_lint_source_with_parser(
    source_text: &str,
    options: PluginOptions,
    parser: SourceParser,
) -> Result<LintResult, String> {
    let module = parse_module_from_source(source_text, parser)?;
    Ok(lint(&module, source_text, options))
}
