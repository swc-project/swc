...
//!
//! This is the inverse of `convert_ast.rs`. It takes a
//! `react_compiler_ast::File` (which represents the compiler's Babel-compatible
//! output) and produces SWC AST nodes suitable for code generation via
//! `swc_codegen`.
...
/// Convert a `react_compiler_ast::File` into an SWC `Module` and extracted
/// comments.
...
/// Convert a `react_compiler_ast::File` into an SWC `Module` and extracted
/// comments. When `source_text` is provided, type declarations can be extracted
/// from the original source for perfect fidelity.
...
impl ReverseCtx {
    ...
    /// Convert a BaseNode's start/end to an SWC Span without extracting
    /// comments. Use this for sub-nodes where comments should not be
    /// duplicated.
    ...
    /// Extract comments from a BaseNode and register them with the SWC comments
    /// store.
    ...
    /// Escape non-ASCII characters and special characters (like tab) in a
    /// string value to \uXXXX or \xXX sequences, matching Babel's codegen
    /// output. Returns the raw string representation wrapped in double
    /// quotes.
    ...
    /// Convert an expression that may be used inside a chain (optional
    /// chaining).
    ///
