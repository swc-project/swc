...
impl ReverseCtx {
    ...
    /// (with 0-based `index` offsets), not the 1-based `start`/`end` offsets
    /// the forward conversion emits. Recover the span from `loc` so codegen
    /// can still emit source mappings for compiled function bodies.
    fn span_from_loc(base: &BaseNode) -> Span {
        let Some(loc) = &base.loc else {
            return DUMMY_SP;
        };
        match (loc.start.index, loc.end.index) {
            (Some(start), Some(end)) => Span::new(BytePos(start + 1), BytePos(end + 1)),
            (Some(start), None) => Span::new(BytePos(start + 1), BytePos(start + 1)),
            _ => DUMMY_SP,
        }
    }
