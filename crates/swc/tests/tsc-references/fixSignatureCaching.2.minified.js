...
(function(undefined) {
    ...
    return impl.FALLBACK_PHONE = "UnknownPhone", impl.FALLBACK_TABLET = "UnknownTablet", impl.FALLBACK_MOBILE = "UnknownMobile", isArray = "isArray" in Array ? Array.isArray : function(value) {
        ...
        convertPropsToRegExp(mobileDetectRules.oss), convertPropsToRegExp(mobileDetectRules.phones), convertPropsToRegExp(mobileDetectRules.tablets), convertPropsToRegExp(mobileDetectRules.uas), convertPropsToRegExp(mobileDetectRules.utils), // copy some patterns to oss0 which are tested first (see issue#15)
     ...
     */ impl.prepareVersionNo = function(version) {
        ...
        if (undefined === cache.mobile) {
            if (// first check for stronger tablet rules, then phone (see issue#5)
    ...
    impl.mobileGrade = function(t) {
        ...
        return(// Apple iOS 3.2-5.1 - Tested on the original iPad (4.3 / 5.0), iPad 2 (4.3), iPad 3 (5.1), original iPhone (3.1), iPhone 3 (3.2), 3GS (4.3), 4 (4.3 / 5.0), and 4S (5.1)
        ...
        t.version("Opera") >= 10 && !$isMobile ? "A" : t.os("iOS") && 4.3 > t.version("iPad") || t.os("iOS") && 3.1 > t.version("iPhone") || t.os("iOS") && 3.1 > t.version("iPod") || // Blackberry 5.0: Tested on the Storm 2 9550, Bold 9770
        t.is("Blackberry") && t.version("BlackBerry") >= 5 && 6 > t.version("BlackBerry") || //Opera Mini (5.0-6.5) - Tested on iOS 3.2/4.3 and Android 2.3
        t.version("Opera Mini") >= 5.0 && 6.5 >= t.version("Opera Mini") && (t.version("Android") >= 2.3 || t.is("iOS")) || // Nokia Symbian^3 - Tested on Nokia N8 (Symbian^3), C7 (Symbian^3), also works on N97 (Symbian^1)
        t.match("NokiaN8|NokiaC7|N97.*Series60|Symbian/3") || // @todo: report this (tested on Nokia N71)
        t.version("Opera Mobi") >= 11 && t.is("SymbianOS") ? "B" : (// Blackberry 4.x - Tested on the Curve 8330
        5.0 > t.version("BlackBerry") || // Windows Mobile - Tested on the HTC Leo (WinMo 5.2)
        t.match("MSIEMobile|Windows CE.*Mobile") || t.version("Windows Mobile"), "C"));
