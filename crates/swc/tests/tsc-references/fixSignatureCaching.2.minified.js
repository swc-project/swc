...
!function(define1, undefined) {
    define1(function() {
        ...
        var equalIC = function(a, b) {
            ...
            for(i = 0; i < len; ++i)if (valueLC === array[i].toLowerCase()) return !0;
            return !1;
        ...
        return impl.FALLBACK_PHONE = "UnknownPhone", impl.FALLBACK_TABLET = "UnknownTablet", impl.FALLBACK_MOBILE = "UnknownMobile", isArray = "isArray" in Array ? Array.isArray : function(value) {
            ...
            for(key in mobileDetectRules.props)if (hasOwnProp.call(mobileDetectRules.props, key)) {
                isArray(values = mobileDetectRules.props[key]) || (values = [
                    values
                ]);
                len = values.length;
                for(i = 0; i < len; ++i){
                    (verPos = (value = values[i]).indexOf("[VER]")) >= 0 && (value = value.substring(0, verPos) + "([\\w._\\+]+)" + value.substring(verPos + 5));
                    values[i] = RegExp(value, "i");
                }
            ...
            if (hasOwnProp.call(props, propertyName)) {
                len = (patterns = props[propertyName]).length;
                for(i = 0; i < len; ++i)if (null !== (match = patterns[i].exec(userAgent))) return match[1];
            }
            return null;
            ...
            var numbers;
            1 === (numbers = version.split(/[a-z._ \/\-]/i)).length && (version = numbers[0]);
            if (numbers.length > 1) {
                version = numbers[0] + ".";
                numbers.shift();
            ...
            if (undefined === cache.mobile) {
                ...
                if (impl.isMobileFallback(userAgent)) {
                    if (undefined === (phoneSized = MobileDetect.isPhoneSized(maxPhoneWidth))) {
