...
!function(TypeScript1) {
    var preFindMemberScope = function(ast, parent, walker) {
        ...
        if (hasFlag(ast.flags, memScope.matchFlag) && (memScope.pos < 0 || memScope.pos == ast.limChar)) {
            memScope.ast = ast;
            null == ast.type && memScope.pos >= 0 && memScope.flow.inScopeTypeCheck(ast, memScope.scope);
        ...
        ast.nodeType == NodeType.Script && context.pos > limChar && (limChar = context.pos);
        if (minChar <= context.pos && limChar >= context.pos) {
            switch(ast.nodeType){
                ...
                case NodeType.FuncDecl:
                    if (context.skipNextFuncDeclForClass) context.skipNextFuncDeclForClass = !1;
                    else {
                        context.scopeGetter = function() {
    ...
    var EnclosingScopeContext = function() {
        ...
        function EnclosingScopeContext(logger, script, text, pos, isMemberCompletion) {
            ...
            this.skipNextFuncDeclForClass = !1;
