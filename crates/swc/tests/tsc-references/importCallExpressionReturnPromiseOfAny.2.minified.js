...
Promise.resolve(`${directory}\\${moduleFile}`).then((p)=>_interopRequireWildcard(require(p)));
...
Promise.resolve(getSpecifier()).then((p)=>_interopRequireWildcard(require(p)));
Promise.resolve(whatToLoad ? getSpecifier() : "defaulPath").then((p)=>_interopRequireWildcard(require(p)));
