...
!function(TypeScript1) {
    var pushAssignScope = function(scope, context, type, classType, fnc) {
        ...
        if (mod) {
            ...
            (aggScope = new SymbolAggregateScope(mod.symbol)).addParentScope(memberScope);
            aggScope.addParentScope(context.scopeChain.scope);
            pushAssignScope(aggScope, context, null, null, null);
            mod.containedScope = aggScope;
            mod.symbol && context.typeFlow.addLocalsFromScope(mod.containedScope, mod.symbol, moduleDecl.vars, mod.members.privateMembers, !0);
        }
        ...
        if (classType) {
            ...
            (aggScope = new SymbolAggregateScope(classType.symbol)).addParentScope(memberScope);
            aggScope.addParentScope(context.scopeChain.scope);
            ...
            (aggScope = new SymbolAggregateScope(instanceType.symbol)).addParentScope(context.scopeChain.scope);
        ...
        (aggScope = new SymbolAggregateScope(interfaceType.symbol)).addParentScope(memberScope);
        aggScope.addParentScope(context.scopeChain.scope);
        pushAssignScope(aggScope, context, null, null, null);
        ...
        if (null == funcDecl.type || hasFlag(funcDecl.type.symbol.flags, SymbolFlags.TypeSetDuringScopeAssignment)) {
            ...
            if (isStatic) {
                null == outerFnc.type.members && container.getType().memberScope && (outerFnc.type.members = container.type.memberScope.valueMembers);
                funcScope = context.scopeChain.fnc.type.memberScope;
                outerFnc.innerStaticFuncs[outerFnc.innerStaticFuncs.length] = funcDecl;
            } else funcScope = !funcDecl.isConstructor && container && container.declAST && container.declAST.nodeType == NodeType.FuncDecl && container.declAST.isConstructor && !funcDecl.isMethod() ? context.scopeChain.thisType.constructorScope : context.scopeChain.scope;
            nameText && "__missing" != nameText && !funcDecl.isAccessor() && (fgSym = funcScope.findLocal(nameText, !1, !1));
            context.typeFlow.checker.createFunctionSignature(funcDecl, container, funcScope, fgSym, null == fgSym);
            (!funcDecl.accessorSymbol && funcDecl.fncFlags & FncFlags.ClassMethod && container && (!fgSym || fgSym.declAST.nodeType != NodeType.FuncDecl) && funcDecl.isAccessor() || fgSym && fgSym.isAccessor()) && (funcDecl.accessorSymbol = context.typeFlow.checker.createAccessorSymbol(funcDecl, fgSym, container.getType(), funcDecl.isMethod() && isStatic, !0, funcScope, container));
            funcDecl.type.symbol.flags |= SymbolFlags.TypeSetDuringScopeAssignment;
        }
        funcDecl.name && funcDecl.type && (funcDecl.name.sym = funcDecl.type.symbol);
        ...
        if (!funcDecl.isOverload) {
            ...
            funcDecl.isConstructor && context.scopeChain.thisType && (context.scopeChain.thisType.constructorScope = locals);
            ...
            if (!funcDecl.isSpecialFn()) {
                ...
                if ((funcDecl.fncFlags & FncFlags.Signature) == FncFlags.None && funcDecl.vars) {
                    context.typeFlow.addLocalsFromScope(locals, fgSym, funcDecl.vars, funcTable, !1);
                    context.typeFlow.addLocalsFromScope(statics, fgSym, funcDecl.statics, funcStaticTable, !1);
                }
                if (signature.parameters) for(var len = signature.parameters.length, i = 0; i < len; i++){
        ...
        if (ast) {
            if (ast.nodeType == NodeType.ModuleDeclaration) {
                ...
                context.modDeclChain.length >= 1 && (context.typeFlow.checker.currentModDecl = context.modDeclChain[context.modDeclChain.length - 1]);
            } else ast.nodeType == NodeType.ClassDeclaration ? popAssignScope(context) : ast.nodeType == NodeType.InterfaceDeclaration ? popAssignScope(context) : ast.nodeType == NodeType.With ? popAssignScope(context) : ast.nodeType == NodeType.FuncDecl ? (!ast.isConstructor || hasFlag(ast.fncFlags, FncFlags.ClassMethod)) && !ast.isOverload && popAssignScope(context) : ast.nodeType == NodeType.Catch ? ast.param && popAssignScope(context) : go = !1;
        }
