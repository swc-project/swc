    ...
    class Indenter {
        ...
        CanIndentComment(token, node) {
            switch(node.AuthorNode.Details.Kind){
                ...
                case AuthorParseNodeKind.apnkFncDecl:
                    var result = !0;
                    return ParseNodeExtensions.FindChildrenWithEdge(node, AuthorParseNodeEdge.apneArgument).foreach((argumentNode)=>{
        ...
        FillInheritedIndentation(tree) {
            ...
            if (null != indentNode) {
                ...
                this.smartIndent || null === tree.StartNodePreviousSibling || null == indentNode.Parent || ParseNodeExtensions.GetChildren(indentNode.Parent).foreach((sibling)=>{
