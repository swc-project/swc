//// [spellingUncheckedJS.js]
import _class_call_check from "@swc/helpers/src/_class_call_check.mjs";
import _get from "@swc/helpers/src/_get.mjs";
import _get_prototype_of from "@swc/helpers/src/_get_prototype_of.mjs";
import _inherits from "@swc/helpers/src/_inherits.mjs";
import _create_super from "@swc/helpers/src/_create_super.mjs";
export var inModule = 1;
function f() {
    locale.toFixed(), localf.toExponential();
}
inmodule.toFixed();
var Classe = function() {
    "use strict";
    function Classe() {
        _class_call_check(this, Classe), this.non = "oui";
    }
    return Classe.prototype.methode = function() {
        return this.none;
    }, Classe;
}(), Derivee = function(Classe) {
    "use strict";
    _inherits(Derivee, Classe);
    var _super = _create_super(Derivee);
    function Derivee() {
        return _class_call_check(this, Derivee), _super.apply(this, arguments);
    }
    return Derivee.prototype.methode = function() {
        return _get(_get_prototype_of(Derivee.prototype), "none", this);
    }, Derivee;
}(Classe), object = {
    spaaace: 3
};
object.spaaaace, object.spaace = 12, object.fresh = 12, other.puuuce, new Date().getGMTDate();
var atoc = setIntegral(function() {
    return console.log("ok");
}, 500);
AudioBuffin, Jimmy, Jon;
//// [other.js]
var Jimmy = 1, John = 2;
Jon;
var other = {
    puuce: 4
};
