...
!function(TypeScript) {
    var globalAstWalkerFactory, ChildrenWalkers, getAstWalkerFactory = function() {
        ...
        function AstWalkOptions() {
            ...
            this.goChildren = !0;
            this.goNextSibling = !0;
            this.reverseSiblings = !1;
        }
    ...
    var AstWalker = function() {
        ...
        return AstWalker.prototype.walk = function(ast, parent) {
            ...
            void 0 === preAst && (preAst = ast);
            if (this.options.goChildren) {
                var svGoSib = this.options.goNextSibling;
                this.options.goNextSibling = !0;
        ...
        return _proto.walk = function(ast, pre, post, options, state) {
            ...
            for(var e in NodeType._map)if (void 0 === this.childrenWalkers[e]) throw Error("initWalkers function is not up to date with enum content!");
        }, AstWalkerFactory;
    }();
    ...
    !function(ChildrenWalkers) {
        ...
        ChildrenWalkers.walkUnaryExpressionChildren = function(preAst, parent, walker) {
            preAst.castTerm && (preAst.castTerm = walker.walk(preAst.castTerm, preAst));
            preAst.operand && (preAst.operand = walker.walk(preAst.operand, preAst));
        };
        ChildrenWalkers.walkBinaryExpressionChildren = function(preAst, parent, walker) {
            if (walker.options.reverseSiblings) {
                preAst.operand2 && (preAst.operand2 = walker.walk(preAst.operand2, preAst));
                preAst.operand1 && walker.options.goNextSibling && (preAst.operand1 = walker.walk(preAst.operand1, preAst));
            } else {
                preAst.operand1 && (preAst.operand1 = walker.walk(preAst.operand1, preAst));
                preAst.operand2 && walker.options.goNextSibling && (preAst.operand2 = walker.walk(preAst.operand2, preAst));
            }
        };
        ...
        ChildrenWalkers.walkCallExpressionChildren = function(preAst, parent, walker) {
            walker.options.reverseSiblings || (preAst.target = walker.walk(preAst.target, preAst));
            preAst.arguments && walker.options.goNextSibling && (preAst.arguments = walker.walk(preAst.arguments, preAst));
            walker.options.reverseSiblings && walker.options.goNextSibling && (preAst.target = walker.walk(preAst.target, preAst));
        };
        ChildrenWalkers.walkTrinaryExpressionChildren = function(preAst, parent, walker) {
            preAst.operand1 && (preAst.operand1 = walker.walk(preAst.operand1, preAst));
            preAst.operand2 && walker.options.goNextSibling && (preAst.operand2 = walker.walk(preAst.operand2, preAst));
            preAst.operand3 && walker.options.goNextSibling && (preAst.operand3 = walker.walk(preAst.operand3, preAst));
        };
        ChildrenWalkers.walkFuncDeclChildren = function(preAst, parent, walker) {
            preAst.name && (preAst.name = walker.walk(preAst.name, preAst));
            preAst.arguments && preAst.arguments.members.length > 0 && walker.options.goNextSibling && (preAst.arguments = walker.walk(preAst.arguments, preAst));
            preAst.returnTypeAnnotation && walker.options.goNextSibling && (preAst.returnTypeAnnotation = walker.walk(preAst.returnTypeAnnotation, preAst));
            preAst.bod && preAst.bod.members.length > 0 && walker.options.goNextSibling && (preAst.bod = walker.walk(preAst.bod, preAst));
        };
        ChildrenWalkers.walkBoundDeclChildren = function(preAst, parent, walker) {
            preAst.id && (preAst.id = walker.walk(preAst.id, preAst));
            preAst.init && (preAst.init = walker.walk(preAst.init, preAst));
            preAst.typeExpr && walker.options.goNextSibling && (preAst.typeExpr = walker.walk(preAst.typeExpr, preAst));
        };
        ...
        ChildrenWalkers.walkForStatementChildren = function(preAst, parent, walker) {
            preAst.init && (preAst.init = walker.walk(preAst.init, preAst));
            preAst.cond && walker.options.goNextSibling && (preAst.cond = walker.walk(preAst.cond, preAst));
            preAst.incr && walker.options.goNextSibling && (preAst.incr = walker.walk(preAst.incr, preAst));
            preAst.body && walker.options.goNextSibling && (preAst.body = walker.walk(preAst.body, preAst));
        };
        ChildrenWalkers.walkForInStatementChildren = function(preAst, parent, walker) {
            preAst.lval = walker.walk(preAst.lval, preAst);
            walker.options.goNextSibling && (preAst.obj = walker.walk(preAst.obj, preAst));
            preAst.body && walker.options.goNextSibling && (preAst.body = walker.walk(preAst.body, preAst));
        };
        ChildrenWalkers.walkIfStatementChildren = function(preAst, parent, walker) {
            preAst.cond = walker.walk(preAst.cond, preAst);
            preAst.thenBod && walker.options.goNextSibling && (preAst.thenBod = walker.walk(preAst.thenBod, preAst));
            preAst.elseBod && walker.options.goNextSibling && (preAst.elseBod = walker.walk(preAst.elseBod, preAst));
        };
        ...
        ChildrenWalkers.walkCaseStatementChildren = function(preAst, parent, walker) {
            preAst.expr && (preAst.expr = walker.walk(preAst.expr, preAst));
            preAst.body && walker.options.goNextSibling && (preAst.body = walker.walk(preAst.body, preAst));
        };
        ChildrenWalkers.walkSwitchStatementChildren = function(preAst, parent, walker) {
            preAst.val && (preAst.val = walker.walk(preAst.val, preAst));
            preAst.caseList && walker.options.goNextSibling && (preAst.caseList = walker.walk(preAst.caseList, preAst));
        };
        ...
        ChildrenWalkers.walkTryCatchChildren = function(preAst, parent, walker) {
            preAst.tryNode && (preAst.tryNode = walker.walk(preAst.tryNode, preAst));
            preAst.catchNode && walker.options.goNextSibling && (preAst.catchNode = walker.walk(preAst.catchNode, preAst));
        };
        ChildrenWalkers.walkTryFinallyChildren = function(preAst, parent, walker) {
            preAst.tryNode && (preAst.tryNode = walker.walk(preAst.tryNode, preAst));
            preAst.finallyNode && walker.options.goNextSibling && (preAst.finallyNode = walker.walk(preAst.finallyNode, preAst));
        };
        ...
        ChildrenWalkers.walkCatchChildren = function(preAst, parent, walker) {
            preAst.param && (preAst.param = walker.walk(preAst.param, preAst));
            preAst.body && walker.options.goNextSibling && (preAst.body = walker.walk(preAst.body, preAst));
        };
        function walkRecordChildren(preAst, parent, walker) {
            preAst.name = walker.walk(preAst.name, preAst);
            walker.options.goNextSibling && preAst.members && (preAst.members = walker.walk(preAst.members, preAst));
        }
        ...
        ChildrenWalkers.walkClassDeclChildren = function(preAst, parent, walker) {
            walkRecordChildren(preAst, parent, walker);
            walker.options.goNextSibling && preAst.extendsList && (preAst.extendsList = walker.walk(preAst.extendsList, preAst));
            walker.options.goNextSibling && preAst.implementsList && (preAst.implementsList = walker.walk(preAst.implementsList, preAst));
        };
        ...
        ChildrenWalkers.walkImportDeclChildren = function(preAst, parent, walker) {
            preAst.id && (preAst.id = walker.walk(preAst.id, preAst));
            preAst.alias && (preAst.alias = walker.walk(preAst.alias, preAst));
        };
        ...
        ChildrenWalkers.walkLabeledStatementChildren = function(preAst, parent, walker) {
            ...
            walker.options.goNextSibling && (preAst.stmt = walker.walk(preAst.stmt, preAst));
        };
