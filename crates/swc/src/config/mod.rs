        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for IsModuleVisitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("boolean or a string \"unknown\"")
            }
...
/// Configuration related to source maps generation.
...
impl fmt::Display for SourceMapsConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(
            match *self {
                SourceMapsConfig::Bool(false) => "false",
                SourceMapsConfig::Bool(true) => "true",
                SourceMapsConfig::Inline => "inline",
                SourceMapsConfig::Linked => "linked",
            },
            f,
        )
    }
...
impl FromStr for SourceMapsConfig {
    type Err = String;

    fn from_str(input: &str) -> Result<SourceMapsConfig, Self::Err> {
        match input {
            "false" => Ok(SourceMapsConfig::Bool(false)),
            "true" => Ok(SourceMapsConfig::Bool(true)),
            "inline" => Ok(SourceMapsConfig::Inline),
            "linked" => Ok(SourceMapsConfig::Linked),
            _ => Err(format!(
                "invalid source maps value: got \"{}\" but expected either one of {}",
                input.to_owned(),
                vec![
                    SourceMapsConfig::Bool(false).to_string(),
                    SourceMapsConfig::Bool(true).to_string(),
                    SourceMapsConfig::Inline.to_string(),
                    SourceMapsConfig::Linked.to_string(),
                ]
                .join(", "),
            )),
        }
...
impl<'de> Deserialize<'de> for SourceMapsConfig {
    ...
    {
        struct SourceMapsConfigVisitor;

        impl<'de> Visitor<'de> for SourceMapsConfigVisitor {
            type Value = SourceMapsConfig;

            fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
                formatter.write_str("boolean or a string \"inline\", \"linked\"")
            }
        ...
        deserializer.deserialize_any(SourceMapsConfigVisitor)
    }
}
...
impl fmt::Display for InputSourceMap {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(
            match *self {
                InputSourceMap::Bool(false) => "false",
                InputSourceMap::Bool(true) => "true",
                InputSourceMap::Inline => "inline",
            },
            f,
        )
    }
...
impl<'de> Deserialize<'de> for InputSourceMap {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        struct InputSourceMapVisitor;

        impl<'de> Visitor<'de> for InputSourceMapVisitor {
            type Value = InputSourceMap;

            fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
                formatter.write_str("boolean or a string \"inline\"")
            }
            ...
            {
                match s {
                    "inline" => Ok(InputSourceMap::Inline),
                    _ => Err(serde::de::Error::invalid_value(Unexpected::Str(s), &self)),
                }
            }
        }

        deserializer.deserialize_any(InputSourceMapVisitor)
    }
}
...
impl Options {
        ...
        P: 'a + swc_ecma_visit::Fold,
    {
        ...
        Ok(BuiltInput {
            ...
            target: es_version,
            is_module,
            source_map_path: self.source_map_path.clone(),
...
pub struct Config {
    ...
    /// - true: Generates a source map file, but without to append a
    ///   sourceMappingURL comment to the end of the output.
    /// - inline: Generates a source map file and appends a sourceMappingURL
    ///   comment to the end of the output, containing the base64 encoded source
    ///   map.
    /// - linked: Generates a source map file and appends a sourceMappingURL
    ///   comment to the end of the output, containing an URL to the generated
    ///   source map file.
    #[serde(default)]
    pub source_maps: Option<SourceMapsConfig>,
