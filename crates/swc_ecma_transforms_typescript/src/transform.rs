...
impl VisitMut for TypeScript {
    ...
    fn visit_mut_module(&mut self, n: &mut Module) {
        ...
        for item in n.body.drain(..) {
            ...
            // This matches the behavior of StripType on the main branch
...
impl VisitMut for TypeStripper<'_> {
    ...
    fn visit_mut_stmts(&mut self, n: &mut Vec<Stmt>) {
        ...
        for stmt in n.drain(..) {
            // Skip empty statements with dummy spans - these are generated by the transform
            // when removing TypeScript constructs. User-written empty statements have real
            // spans.
            if matches!(&stmt, Stmt::Empty(e) if e.span.is_dummy()) {
                continue;
            }
