...
impl VisitMutHook<TypeScriptCtx> for StripTypeHook {
    ...
    fn exit_assign_pat_prop(&mut self, _: &mut AssignPatProp, _: &mut TypeScriptCtx) {
    ...
    fn exit_auto_accessor(&mut self, n: &mut AutoAccessor, _: &mut TypeScriptCtx) {
        ...
        n.definite = false;
        n.is_override = false;
        // Note: is_abstract is NOT set to false here because exit_class_members
        // needs it for filtering It will be set to false in
        // exit_class_members after filtering
    }
    ...
    fn exit_key_value_pat_prop(&mut self, _: &mut KeyValuePatProp, _: &mut TypeScriptCtx) {
        // Children already visited
    }

    fn exit_method_prop(&mut self, _: &mut MethodProp, _: &mut TypeScriptCtx) {
    ...
    fn exit_param(&mut self, _: &mut Param, _: &mut TypeScriptCtx) {
    ...
    fn exit_ts_module_block(&mut self, _: &mut TsModuleBlock, ctx: &mut TypeScriptCtx) {
        ctx.strip_type.in_namespace = false;
        // Note: children are already visited by VisitMutWithHook
    }

    fn exit_ts_param_prop(&mut self, _: &mut TsParamProp, _: &mut TypeScriptCtx) {
