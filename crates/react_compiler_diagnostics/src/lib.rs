...
pub struct Position {
    pub line: u32,
    pub column: u32,
    /// Byte offset in the source file. Preserved for logger event
    /// serialization.
...
pub enum CompilerDiagnosticDetail {
    Error {
        ...
        /// points to an identifier node. Preserved for logger event
        /// serialization to match Babel's SourceLocation.identifierName
        /// field.
        #[serde(skip)]
...
impl CompilerDiagnostic {
    ...
    pub fn primary_location(&self) -> Option<&SourceLocation> {
        self.details.iter().find_map(|d| match d {
            CompilerDiagnosticDetail::Error { loc, .. } => loc.as_ref(), /* identifier_name */
            // covered by ..
            _ => None,
        })
    }
