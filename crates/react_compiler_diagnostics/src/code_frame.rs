...
/// Represents a marker line entry: either mark the whole line (true) or a
/// [column, length] range.
#[derive(Clone, Debug)]
enum MarkerEntry {
    WholeLine,
    ...
    if line_diff > 0 {
        ...
        for i in 0..=line_diff {
            ...
            if start_column == 0 {
                ...
                // source[lineNumber - 1] gives us the source line (0-indexed array, 1-indexed
                // line numbers) But we don't have access to source lines here,
                // so we pass the length through. Actually, Babel accesses
                // source[lineNumber - 1].length. We need to thread source lines.
                // For now, this is handled in code_frame_columns where we have access to source
                // lines. We use a placeholder that will be filled in later.
                marker_lines.push((line_number, MarkerEntry::Range(start_column, 0)));
            // 0 = placeholder
            } else if i == line_diff {
                marker_lines.push((line_number, MarkerEntry::Range(0, end_column)));
            } else {
                marker_lines.push((line_number, MarkerEntry::Range(0, 0))); // 0
                                                                            // =
                                                                            // placeholder
                                                                            // for
                                                                            // full
...
/// Produce a code frame matching @babel/code-frame's codeFrameColumns() in
/// non-highlighted mode.
///
/// Columns are 0-based (matching the Rust/AST convention). They are converted
/// to 1-based internally to match Babel's convention (the JS caller already
/// does column + 1).
            ...
            MarkerEntry::Range(col, len) => {
                if line_diff > 0 {
                    ...
                    if i == 0 && *len == 0 {
                        ...
                        // which evaluates to source[startLine] (0-indexed array, 1-indexed line
                        // number). This means all middle lines use the
                        // length of source[startLine], which is the line at
                        // 0-indexed position startLine in the source array.
            ...
            let marker_line_str = match marker_entry {
                MarkerEntry::Range(col, len) => {
                    ...
                    let safe_end = if byte_end < line.len() && !line.is_char_boundary(byte_end) {
                        let mut safe_end = byte_end;
                        while safe_end > 0 && !line.is_char_boundary(safe_end) {
                            safe_end -= 1;
                        }
                        safe_end
                    } else {
...
pub fn format_compiler_error(err: &CompilerError, source: &str, filename: Option<&str>) -> String {
    ...
    let header = format!("Found {count} error{plural}:\n\n");
