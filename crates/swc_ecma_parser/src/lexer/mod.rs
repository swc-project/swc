...
pub use token::Token;
pub(crate) use token::{NextTokenAndSpan, TokenAndSpan, TokenFlags, TokenValue};
...
impl<'a> Lexer<'a> {
    pub(crate) fn new_with_comments(
        syntax: Syntax,
        target: EsVersion,
        input: StringInput<'a>,
    ) -> Self {
        let mut lexer = Self::new(syntax, target, input, None);
        lexer.comments_buffer = Some(CommentsBuffer::new());
        lexer
    }
