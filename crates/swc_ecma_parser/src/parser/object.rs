...
impl<I: Tokens> ParseObject<Expr> for Parser<I> {
    ...
    fn parse_object_prop(&mut self) -> PResult<Self::Prop> {
            ...
            && !(is!(self, '}') && matches!(key, PropName::Ident(..)))
        {
            ...
            return Ok(PropOrSpread::Prop(Prop::KeyValue(
                KeyValueProp {
                    key,
                    value: Expr::Invalid(Invalid {
        ...
        if (self.input.syntax().typescript() && is!(self, '<')) || is!(self, '(') {
            return self
                ...
                .map(|function| Prop::Method(MethodProp { key, function }.into()))
        ...
        if is_one_of!(self, '=', ',', '}') {
            ...
            if eat!(self, '=') {
                ...
                return Ok(PropOrSpread::Prop(Prop::Assign(
                    AssignProp {
                        span,
                        key: ident.into(),
                        value,
    ...
    fn parse_object_prop(&mut self) -> PResult<Self::Prop> {
        ...
        if eat!(self, ':') {
            ...
            return Ok(ObjectPatProp::KeyValue(
                KeyValuePatProp { key, value }.into(),
            ));
        }
        let key = match key {
            PropName::Ident(ident) => ident,
