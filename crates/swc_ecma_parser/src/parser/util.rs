use super::*;
use crate::token::{IdentKind, Keyword, WordKind};

impl Context {
    pub(crate) fn is_reserved(self, word: WordKind) -> bool {
        match word {
            WordKind::Keyword(Keyword::Let) => self.strict,
            WordKind::Keyword(Keyword::Await) => {
                self.in_async || self.in_static_block || self.strict
            }
            ...
            WordKind::Ident(IdentKind::Known(known_ident!("enum"))) => true,
