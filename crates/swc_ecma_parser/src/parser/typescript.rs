...
impl<I: Tokens> Parser<I> {
        ...
        F: FnMut(&mut Self) -> PResult<(BytePos, T)>,
    {
        ...
        loop {
            ...
            if kind == ParsingContext::EnumMembers {
                ...
                let cur = cur.clone().to_string(self.input());
                self.emit_err(
                    self.input().cur_span(),
    ...
    pub(super) fn ts_next_token_can_follow_modifier<'a>(&mut self) -> bool {
        ...
        let cur = self.input().cur();
        !self.input().had_line_break_before_cur() && cur.is_lbracket()
            || cur.is_lbrace()
            || cur.is_star()
            || cur.is_dotdotdot()
            || cur.is_hash()
        ...
        F: FnMut(&mut Self) -> PResult<Box<TsType>>,
    {
        ...
        let ty = self.parse_constituent_type()?;
        ...
        if self.input().is(operator) {
            ...
            while self.input_mut().eat(operator) {
                ...
                types.push(self.parse_constituent_type()?);
            }
    ...
    pub fn parse_ts_modifier(
        ...
        if let Some(pos) = pos {
            ...
            if self.try_parse_ts_bool(|p| Ok(Some(self.ts_next_token_can_follow_modifier())))? {
    ...
    pub fn parse_ts_type_args(&mut self) -> PResult<Box<TsTypeParamInstantiation>> {
        ...
        let params = self.in_type(|p| {
            ...
            // tokens.
            self.ts_in_no_context(|p| {
                if self.input().is(Token::LSHIFT) {
                    self.input_mut().cut_lshift();
                } else {
                    expect!(self, Token::LESS);
                }
                self.parse_ts_delimited_list(ParsingContext::TypeParametersOrArguments, |p| {
                    trace_cur!(self, parse_ts_type_args__arg);
        ...
        self.expect_without_advance(Token::GREATER)?;
    ...
    pub fn parse_ts_type_ref(&mut self) -> PResult<TsTypeRef> {
        ...
        let type_params = if !self.input().had_line_break_before_cur()
            && (self.input().is(Token::LESS) || self.input().is(Token::LSHIFT))
        {
    ...
    pub fn parse_ts_type_ann(
        ...
        self.in_type(|p| {
            ...
            trace_cur!(self, parse_ts_type_ann__after_colon);
    ...
    fn eat_then_parse_ts_type(&mut self, token_to_eat: Token) -> PResult<Option<Box<TsType>>> {
        ...
        self.in_type(|p| {
            if !self.input_mut().eat(token_to_eat) {
                return Ok(None);
            }

            self.parse_ts_type(p).map(Some)
        })
    }
    ...
    fn expect_then_parse_ts_type(
        ...
        self.in_type(|p| {
            if !self.input_mut().eat(token) {
                let got = format!("{:?}", self.input().cur());
                syntax_error!(
                    p,
                    self.input().cur_span(),
    ...
    fn parse_ts_type_param(
        ...
        let default = self.eat_then_parse_ts_type(Token::EQUAL)?;
    ...
    pub fn parse_ts_type_or_type_predicate_ann(
        ...
        self.in_type(|p| {
            let return_token_start = self.input().cur_pos();
            if !self.input_mut().eat(return_token) {
            ...
            let type_pred_start = self.input().cur_pos();
            let has_type_pred_asserts = self.input().cur().is_asserts() && {
                let ctx = self.ctx();
            ...
            if has_type_pred_asserts {
                self.assert_and_bump(Token::ASSERTS);
            }

            let has_type_pred_is = self.is_ident_ref()
            ...
            let node = Box::new(TsType::TsTypePredicate(TsTypePredicate {
                span: self.span(type_pred_start),
            ...
            Ok(Box::new(TsTypeAnn {
                span: self.span(return_token_start),
                type_ann: node,
            }))
        })
    }
    ...
    pub(super) fn try_parse_ts_type_args(&mut self) -> Option<Box<TsTypeParamInstantiation>> {
        ...
        self.try_parse_ts(|p| {
            ...
            {
                Ok(None)
            } else if self.input().had_line_break_before_cur()
                || self.input().cur().is_bin_op()
                || !p.is_start_of_expr()
            {
                Ok(Some(type_args))
            } else {
    ...
    fn parse_ts_enum_member(&mut self) -> PResult<TsEnumMember> {
        ...
        let id = if cur.is_str() {
            ...
            TsEnumMemberId::Ident(Ident::new_no_ctxt(atom!(""), self.span(start)))
        } else if cur.is_error() {
            let err = self.input_mut().expect_error_token_and_bump();
            return Err(err);
        } else {
        ...
        let init = if self.input_mut().eat(Token::EQUAL) {
            Some(self.parse_assignment_expr()?)
        } else if self.input().cur().is_comma() || self.input().cur().is_rbrace() {
            None
        } else {
            let start = self.cur_pos();
            self.bump();
    ...
    fn parse_ts_heritage_clause_element(&mut self) -> PResult<TsExprWithTypeArgs> {
        ...
        match *expr {
            ...
            _ => {
                let type_args = if self.input().is(Token::LESS) {
    ...
    fn skip_ts_parameter_start(&mut self) -> PResult<bool> {
        ...
        if cur.is_void() {
            ...
            self.bump();
            Ok(true)
        } else if (cur.is_lbrace() || cur.is_lbracket())
    ...
    fn is_ts_unambiguously_start_of_fn_type(&mut self) -> PResult<bool> {
        ...
        if self.skip_ts_parameter_start()? {
            ...
            if self.input_mut().eat(Token::RPAREN) && self.input().cur().is_arrow() {
                // ( xxx ) =>
                return Ok(true);
            }
        }
    ...
    fn is_ts_external_module_ref(&mut self) -> bool {
        debug_assert!(self.input().syntax().typescript());
        self.input().is(Token::REQUIRE) && peek!(self).is_some_and(|t| t.is_lparen())
    }
    ...
    pub fn parse_ts_mapped_type(&mut self) -> PResult<TsMappedType> {
        ...
        if cur.is_plus() || cur.is_minus() {
            ...
            self.bump(); // +, -
            expect!(self, Token::QUESTION);
        } else if self.input_mut().eat(Token::QUESTION) {
    ...
    pub fn parse_ts_type_alias_decl(&mut self, start: BytePos) -> PResult<Box<TsTypeAliasDecl>> {
        ...
        let type_ann = self.expect_then_parse_ts_type(Token::EQUAL, "=")?;
        self.expect_general_semi()?;
    ...
    fn parse_ts_infer_type(&mut self) -> PResult<TsInferType> {
        ...
        let constraint = self.try_parse_ts(|p| {
            expect!(self, Token::EXTENDS);
            let constraint = self.parse_ts_non_conditional_type();
            if self.ctx().contains(Context::DisallowConditionalTypes)
                || !self.input().is(Token::QUESTION)
            {
                constraint.map(Some)
            } else {
                Ok(None)
    ...
    fn parse_ts_non_conditional_type(&mut self) -> PResult<Box<TsType>> {
        ...
        if (self.input().is(Token::ABSTRACT) && peek!(self).is_some_and(|cur| cur.is_new()))
    ...
    pub fn parse_ts_type(&mut self) -> PResult<Box<TsType>> {
        ...
        self.do_outside_of_context(Context::DisallowConditionalTypes, |p| {
            ...
            if self.input().had_line_break_before_cur() || !self.input_mut().eat(Token::EXTENDS) {
                return Ok(ty);
            }

            let check_type = ty;
            let extends_type = self.do_inside_of_context(
    ...
    fn parse_ts_property_or_method_signature(
        ...
        let optional = self.input_mut().eat(Token::QUESTION);
    ...
    fn parse_ts_type_member(&mut self) -> PResult<TsTypeElement> {
        ...
        if let Some(v) = self.try_parse_ts(|p| {
            ...
            let is_get = if self.input_mut().eat(Token::GET) {
                true
            } else {
    ...
    pub fn parse_ts_interface_decl(&mut self, start: BytePos) -> PResult<Box<TsInterfaceDecl>> {
        ...
        if self.input().is(Token::EXTENDS) {
            ...
            while !self.input().cur().is_eof() && !self.input().is(Token::LBRACE) {
                self.bump();
            }
        }
    ...
    fn parse_ts_type_query(&mut self) -> PResult<TsTypeQuery> {
        ...
        let type_args = if !self.input().had_line_break_before_cur() && self.input().is(Token::LESS)
        {
            let ret = self.do_outside_of_context(
                Context::ShouldNotLexLtOrGtAsType,
                Self::parse_ts_type_args,
    ...
    fn parse_ts_non_array_type(&mut self) -> PResult<Box<TsType>> {
        ...
        if cur.is_known_ident()
            ...
            || cur.is_break()
        {
            if self.input().is(Token::ASSERTS) && peek!(self).is_some_and(|peek| peek.is_this()) {
                self.bump();
            ...
            let peeked_is_dot = peek!(self).is_some_and(|cur| cur.is_dot());
    ...
    pub fn try_parse_ts_declare(
        ...
        self.do_inside_of_context(Context::InDeclare, |p| {
            ...
            if self.input().is(Token::CONST) && peek!(p).is_some_and(|peek| peek.is_enum()) {
    ...
    pub fn try_parse_ts_generic_async_arrow_fn(
        ...
        self.do_inside_of_context(Context::InAsync, |p| {
            self.do_outside_of_context(Context::InGenerator, |p| {
