...
use super::Parser;
use crate::{
    arena::parser::{class_and_fn::IsSimpleParameterList, pat::PatType, util::ExprExt},
...
impl<'a, I: Tokens> Parser<'a, I> {
    pub fn parse_expr(&mut self) -> PResult<Expr<'a>> {
        ...
        if is!(self, ',') {
            let mut exprs = Vec::new_in(self.alloc);
            exprs.push(expr);
            while eat!(self, ',') {
                exprs.push(self.parse_assignment_expr()?);
            }

            return Ok(Expr::Seq(self.ast(SeqExpr {
    ...
    fn parse_assignment_expr_base(&mut self) -> PResult<Expr<'a>> {
            ...
            && (peeked_is!(self, IdentName) || peeked_is!(self, JSXName))
        {
            ...
            let res = self.with_ctx(ctx).try_parse_ts(|p| {
                ...
                match &mut arrow {
                    Expr::Arrow(arrow) => {
                        arrow.span = Span::new(type_parameters.span.lo, arrow.span.hi);
                        arrow.type_params = Some(type_parameters);
                    }
                    _ => unexpected!(p, "("),
                }
                Ok(Some(arrow))
    ...
    fn finish_assignment_expr(&mut self, start: BytePos, cond: Expr<'a>) -> PResult<Expr<'a>> {
        trace_cur!(self, finish_assignment_expr);

        match self.input.cur() {
            Some(&Token::AssignOp(op)) => {
                let left = if op == AssignOp::Assign {
                    match AssignTarget::try_from_pat(
                    ...
                    // TODO
                    match AssignTarget::try_from_expr(cond, self.alloc) {
                        Ok(v) => v,
                        Err(v) => {
                ...
                Ok(Expr::Assign(self.ast(AssignExpr {
                    span: span!(self, start),
                    op,
                    // TODO:
                    left,
    ...
    fn parse_cond_expr(&mut self) -> PResult<Expr<'a>> {
        ...
        if eat!(self, '?') {
            ...
            Ok(Expr::Cond(self.ast(CondExpr {
                span,
                test,
                cons,
                alt,
    ...
    #[cfg_attr(feature = "tracing-spans", tracing::instrument(skip_all))]
    pub(crate) fn parse_primary_expr(&mut self) -> PResult<Expr<'a>> {
        ...
        if let Some(tok) = self.input.cur() {
            match tok {
                tok!("this") => {
                    self.input.bump();
                    return Ok(Expr::This(self.ast(ThisExpr {
                        span: span!(self, start),
                    })));
                }
                ...
                tok!("async") => {
                    ...
                    if can_be_arrow && self.input.syntax().typescript() && peeked_is!(self, '<') {
                        ...
                        if let Some(res) = self.try_parse_ts(|p| {
                            ...
                            return Ok(Expr::Arrow(self.ast(res)));
                        }
                    }
                ...
                | Token::Str { .. } => {
                    ...
                    return Ok(Expr::Lit(self.ast(lit)));
                }
                    ...
                    if let Some(Token::Regex(..)) = self.input.cur() {
                        ...
                        match bump!(self) {
                            Token::Regex(exp, flags) => {
                                ...
                                return Ok(Expr::Lit(self.ast(Lit::Regex(self.ast(Regex {
                                    span,
                                    exp,
                                    flags,
                                })))));
                ...
                tok!('`') => {
                    ...
                    let tpl = self.with_ctx(ctx).parse_tpl(false)?;
                    return Ok(Expr::Tpl(self.ast(tpl)));
                }
            ...
            || is!(self, IdentRef)
        {
                ...
                && is!(self, BindingIdent)
            {
                ...
                if ctx.expr_ctx.for_loop_init && is!(self, "of") && !peeked_is!(self, "=>") {
                    ...
                    return Ok(Expr::Ident(self.ast(id)));
                }
                ...
                if self.input.syntax().typescript() && ident.sym == "as" && !is!(self, "=>") {
                    ...
                    return Ok(Expr::TsAs(self.ast(TsAsExpr {
                        span: span!(self, start),
                        expr: Expr::Ident(self.ast(id)),
                        type_ann,
                    })));
                }

                // async a => body
                let arg = Pat::Ident(self.ast(ident));
                ...
                return Ok(Expr::Arrow(self.ast(ArrowExpr {
                    span: span!(self, start),
                    body,
                    params,
                    is_async: true,
                ...
                let mut params = Vec::new_in(self.alloc);
                params.push(Pat::Ident(self.ast(id.into())));
                let body =
        ...
        if eat!(self, '#') {
            ...
            return Ok(Expr::PrivateName(self.ast(PrivateName {
                span: span!(self, start),
                name: id.sym,
            })));
        }
    ...
    fn parse_array_lit(&mut self) -> PResult<Expr<'a>> {
        ...
        let span = span!(self, start);
        Ok(Expr::Array(self.ast(ArrayLit { span, elems })))
    }

    #[allow(dead_code)]
    fn parse_member_expr(&mut self) -> PResult<Expr<'a>> {
    ...
    fn parse_member_expr_or_new_expr_inner(&mut self, is_new_expr: bool) -> PResult<Expr<'a>> {
        ...
        if eat!(self, "new") {
            if eat!(self, '.') {
                if eat!(self, "target") {
                    let span = span!(self, start);
                    let expr = Expr::MetaProp(self.ast(MetaPropExpr {
                        span,
            ...
            if is_new_expr {
                match &callee {
                    Expr::OptChain(opt_chain) if opt_chain.optional => {
                        syntax_error!(
                            self,
                            opt_chain.span,
                    ...
                    Expr::Member(member_expr) => {
                        if let Expr::OptChain(opt_chain) = &member_expr.obj {
                            if opt_chain.optional {
                                syntax_error!(
                                    self,
                                    opt_chain.span,
                                    SyntaxError::OptChainCannotFollowConstructorCall
            ...
            if !is_new_expr || is!(self, '(') {
                ...
                let new_expr = Callee::Expr(Expr::New(self.ast(NewExpr {
                    span: span!(self, start),
                    callee,
                    args,
                    type_args,
            ...
            return Ok(Expr::New(self.ast(NewExpr {
                span: span!(self, start),
                callee,
                args: None,
                type_args,
        ...
        if eat!(self, "super") {
            let base = Callee::Super(self.ast(Super {
                span: span!(self, start),
            }));
        ...
        let obj = if let Some(type_args) = type_args {
            ...
            Expr::TsInstantiation(self.ast(TsInstantiation {
                expr: obj,
                type_args,
                span: span!(self, start),
            }))
    ...
    pub(crate) fn parse_args(
        ...
        self.with_ctx(ctx).parse_with(|p| {
            ...
            let mut first = true;
            let mut expr_or_spreads = Vec::with_capacity_in(2, p.alloc);
    ...
    /// ...AssignmentExpression[+In, ?Yield, ?Await]
    pub(crate) fn parse_expr_or_spread(&mut self) -> PResult<ExprOrSpread<'a>> {
    ...
    fn parse_paren_expr_or_arrow_fn(
        ...
        if self.syntax().typescript() && self.ctx().in_cond_expr && is!(self, ':') {
            ...
            if let Some(expr) = self.try_parse_ts(|p| {
                ...
                let params: Vec<Pat> = Vec::from_iter_in(
                    p.parse_paren_items_as_params(items_ref.clone_in(p.alloc), trailing_comma)?,
                    p.alloc,
                );
                ...
                Ok(Some(Expr::Arrow(p.ast(ArrowExpr {
        ...
        if has_pattern || return_type.is_some() || is!(self, "=>") {
            ...
            let params: Vec<Pat> = Vec::from_iter_in(
                self.parse_paren_items_as_params(paren_items, trailing_comma)?,
                self.alloc,
            );
            ...
            if let BlockStmtOrExpr::BlockStmt(..) = &arrow_expr.body {
                if let Some(&Token::BinOp(..)) = self.input.cur() {
                    ...
                    let errorred_expr =
                        self.parse_bin_op_recursively(Expr::Arrow(self.ast(arrow_expr)), 0)?;
            ...
            return Ok(Expr::Arrow(self.ast(arrow_expr)));
        } else {
        ...
        let mut expr_or_spreads = Vec::new_in(self.alloc);
        for item in paren_items {
            expr_or_spreads.push(match item {
                AssignTargetOrSpread::ExprOrSpread(e) => Ok(e.into_inner()),
                _ => syntax_error!(self, item.span(), SyntaxError::InvalidExpr),
            }?)
        }
        if let Some(async_span) = async_span {
            // It's a call expression
            return Ok(Expr::Call(self.ast(CallExpr {
                span: span!(self, async_span.lo()),
                callee: Callee::Expr(Expr::Ident(
                    self.ast(Ident::new_no_ctxt("async".into(), async_span)),
                )),
        ...
        if expr_or_spreads.len() == 1 {
            let expr_or_spread = expr_or_spreads.into_iter().next().unwrap();
            ...
            Ok(Expr::Paren(self.ast(ParenExpr {
                span: span!(self, expr_start),
                expr: expr_or_spread.expr,
            })))
        } else {
            debug_assert!(expr_or_spreads.len() >= 2);

            let mut exprs = Vec::with_capacity_in(expr_or_spreads.len(), self.alloc);
            for expr in expr_or_spreads {
                if expr.spread.is_some() {
                    syntax_error!(self, expr.expr.span(), SyntaxError::SpreadInParenExpr);
                } else {
            ...
            let seq_expr = Expr::Seq(self.ast(SeqExpr {
                span: Span::new(
    ...
    fn parse_tpl_elements(
        &mut self,
        is_tagged_tpl: bool,
    ) -> PResult<(Vec<'a, Expr<'a>>, Vec<'a, TplElement>)> {
        ...
        let mut quasis = Vec::new_in(self.alloc);
    ...
    pub(crate) fn parse_tpl(&mut self, is_tagged_tpl: bool) -> PResult<Tpl<'a>> {
    ...
    fn parse_subscript(
        ...
        if self.input.syntax().typescript() {
            if !self.input.had_line_break_before_cur() && is!(self, '!') {
                ...
                return Ok((
                    Expr::TsNonNull(self.ast(TsNonNullExpr {
                        span: span!(self, start),
                        expr,
                    })),
                    true,
                ...
                let result = self.with_ctx(ctx).try_parse_ts(|p| {
                        ...
                        })?
                    {
                        ...
                        if let Some(async_arrow_fn) = async_arrow_fn {
                            return Ok(Some((Expr::Arrow(p.ast(async_arrow_fn)), true)));
                        }
                    }
                    ...
                    if !no_call && is!(p, '(') {
                        ...
                        if let Callee::Expr(Expr::OptChain(_)) = &obj {
                            return Ok(Some((
                        ...
                        p.parse_tagged_tpl(
                            match mut_obj_opt {
                                Some(Callee::Expr(obj)) => obj.take(p.alloc),
                                _ => unreachable!(),
                            },
                            Some(type_args),
                        )
                        .map(|expr| (Expr::TaggedTpl(p.ast(expr)), true))
                        .map(Some)
                    } else if is_one_of!(p, '=', "as", "satisfies") {
                        Ok(Some((
                            Expr::TsInstantiation(p.ast(TsInstantiation {
                                span: span!(p, start),
                ...
                (match obj {
                    ...
                    Callee::Super(obj) => {
                            ...
                            && !self.input.syntax().allow_super_outside_method()
                        {
                            ...
                            Expr::SuperProp(self.ast(SuperPropExpr {
                                span,
                                obj: obj.into_inner(),
                                prop: SuperProp::Computed(self.ast(prop)),
                            }))
                        }
                    }
                    Callee::Expr(obj) => {
                        ...
                        let expr = MemberExpr {
                            span,
                            obj,
                            prop: MemberProp::Computed(self.ast(prop)),
                        };
        ...
        if eat!(self, '.') {
            let prop = self.parse_maybe_private_name().map(|e| match e {
                ...
                Either::Right(i) => MemberProp::Ident(self.ast(i)),
            })?;
                    ...
                    callee @ Callee::Import(_) => match prop {
                        MemberProp::Ident(ident) => {
                            ...
                            match ident.sym.as_str() {
                                "meta" => Expr::MetaProp(self.ast(MetaPropExpr {
                                    span,
        ...
        match obj {
            Callee::Expr(expr) => {
                ...
                if is!(self, '`') {
                    ...
                    return Ok((Expr::TaggedTpl(self.ast(tpl)), true));
                }
    ...
    pub(crate) fn parse_lhs_expr(&mut self) -> PResult<Expr<'a>> {
        ...
        if self.input.syntax().jsx() {
            match *cur!(self, true) {
                Token::JSXText { .. } => {
                    return self
                        .parse_jsx_text()
                        .map(|jsx_text| Expr::Lit(self.ast(Lit::JSXText(self.ast(jsx_text)))))
                }
                Token::JSXTagStart => {
                    return self.parse_jsx_element().map(|e| match e {
                        Either::Left(l) => Expr::JSXFragment(self.ast(l)),
                        Either::Right(r) => Expr::JSXElement(self.ast(r)),
                    });
                }
                _ => {}
            }
        ...
        if eat!(self, "super") {
            let obj = Callee::Super(self.ast(Super {
                span: span!(self, start),
            }));
        ...
        match callee {
            Expr::New(mut new_expr) if new_expr.args.is_none() => {
    ...
    fn parse_args_or_pats_inner(
        ...
        while !eof!(self) && !is!(self, ')') {
            ...
            let optional = if self.input.syntax().typescript() {
                if is!(self, '?') {
                        ...
                        || peeked_is!(self, '=')
                    {
                        ...
                        arg = ExprOrSpread {
                            spread: None,
                            expr: Expr::Cond(self.ast(CondExpr {
            ...
            if optional || (self.input.syntax().typescript() && is!(self, ':')) {
                ...
                items.push(AssignTargetOrSpread::ExprOrSpread(self.ast(arg)));
            }
            ...
            if eat!(self, "=>") && {
                ...
                let params: Vec<Pat> = Vec::from_iter_in(
                    self.parse_paren_items_as_params(items.clone_in(self.alloc), None)?
                        .into_iter(),
                    self.alloc,
                );
                ...
                items.push(AssignTargetOrSpread::ExprOrSpread(self.ast(ExprOrSpread {
                    expr: Expr::Arrow(self.ast(ArrowExpr {
                        span,
                        body,
                        is_async,
                        is_generator: false,
...
pub(crate) enum AssignTargetOrSpread<'a> {
    // #[tag("ExprOrSpread")]
    ExprOrSpread(Box<'a, ExprOrSpread<'a>>),
    // #[tag("*")]
    Pat(Pat<'a>),
}
...
impl<'a, I: Tokens> Parser<'a, I> {
        ...
        if is!(self, ';')
            || (!is!(self, '*')
                ...
                && !cur!(self, false)
                    ...
                    .unwrap_or(true))
        {
            Ok(Expr::Yield(self.ast(YieldExpr {
                span: span!(self, start),
                arg: None,
                delegate: false,
            })))
    ...
    fn at_possible_async(&mut self, expr: &Expr<'a>) -> PResult<bool> {
    ...
    pub(crate) fn parse_lit(&mut self) -> PResult<Lit<'a>> {
        ...
        let v = match cur!(self, true) {
            ...
            Word(Word::True) | Word(Word::False) => {
                ...
                Lit::Bool(self.ast(Bool { span, value }))
            }
            Token::Str { .. } => match bump!(self) {
                Token::Str { value, raw } => Lit::Str(self.ast(Str {
                    span: span!(self, start),
                    value,
                    raw: Some(raw),
                })),
            ...
            Token::Num { .. } => match bump!(self) {
                Token::Num { value, raw } => Lit::Num(self.ast(Number {
                    span: span!(self, start),
                    value,
                    raw: Some(raw),
                })),
            ...
            Token::BigInt { .. } => match bump!(self) {
                Token::BigInt { value, raw } => Lit::BigInt(self.ast(BigInt {
                    span: span!(self, start),
    ...
    pub(crate) fn parse_dynamic_import_or_import_meta(
        &mut self,
        start: BytePos,
        no_call: bool,
    ) -> PResult<Expr<'a>> {
            ...
            match &*ident.sym {
                "meta" => {
                    ...
                    self.parse_subscripts(
                        Callee::Expr(Expr::MetaProp(self.ast(expr))),
                        no_call,
                        false,
                    )
                }
    ...
    fn parse_dynamic_import_call(
        ...
        let import = Callee::Import(self.ast(Import {
            span: span!(self, start),
            phase,
        }));
    ...
    pub(crate) fn check_assign_target(&mut self, expr: &Expr<'a>, deny_call: bool) {
    ...
    pub(crate) fn is_start_of_expr(&mut self) -> PResult<bool> {
...
fn unwrap_ts_non_null<'e, 'a>(mut expr: &'e Expr<'a>) -> &'e Expr<'a> {
