...
use super::{IsDirective, StmtLikeParser};
use crate::{arena::parser::Parser, error::SyntaxError, token::Token, Context, PResult, Tokens};

impl<'a, I: Tokens> Parser<'a, I> {
    fn parse_import(&mut self) -> PResult<ModuleItem<'a>> {
        ...
        if peeked_is!(self, '.') {
            ...
            return Ok(ModuleItem::Stmt(Stmt::Expr(self.ast(ExprStmt {
        ...
        if let Ok(&Token::Str { .. }) = cur!(self, false) {
            let src = match bump!(self) {
                Token::Str { value, raw, .. } => self.ast(Str {
            ...
            expect!(self, ';');
            return Ok(ModuleItem::ModuleDecl(ModuleDecl::Import(self.ast(
                ImportDecl {
                    span: span!(self, start),
                    src,
                    specifiers: Vec::new_in(self.alloc),
                    type_only: false,
                    with,
        ...
        let mut specifiers = Vec::new_in(self.alloc);
        ...
        'import_maybe_ident: {
            if is!(self, BindingIdent) {
                ...
                specifiers.push(ImportSpecifier::Default(self.ast(ImportDefaultSpecifier {
                    span: local.span,
                    local,
                })));
            }
...

        {
            ...
            if eat!(self, '*') {
                ...
                specifiers.push(ImportSpecifier::Namespace(self.ast(
        ...
        Ok(ModuleItem::ModuleDecl(ModuleDecl::Import(self.ast(
            ImportDecl {
                span: span!(self, start),
                specifiers,
                src,
    ...
    fn parse_import_specifier(&mut self, type_only: bool) -> PResult<ImportSpecifier<'a>> {
        ...
        match self.parse_module_export_name()? {
            ModuleExportName::Ident(mut orig_name) => {
                ...
                if self.syntax().typescript() && orig_name.sym == "type" && is!(self, IdentName) {
                    ...
                    if possibly_orig_name.sym == "as" {
                        ...
                        if !is!(self, IdentName) {
                            ...
                            return Ok(ImportSpecifier::Named(self.ast(ImportNamedSpecifier {
                                span: span!(self, start),
                        ...
                        orig_name = self.ast(possibly_orig_name);
                        is_type_only = true;
                    }
                }
                ...
                let local = orig_name;
                Ok(ImportSpecifier::Named(self.ast(ImportNamedSpecifier {
                    span: span!(self, start),
            ...
            ModuleExportName::Str(orig_str) => {
                if eat!(self, "as") {
                    ...
                    syntax_error!(
                        self,
                        orig_str.span,
                        SyntaxError::ImportBindingIsString(orig_str.into_inner().value)
                    )
                }
            }
        }
    ...
    fn parse_export(&mut self, mut decorators: Vec<'a, Decorator<'a>>) -> PResult<ModuleDecl<'a>> {
        if !self.ctx().module {
            // Switch to module mode
            let ctx = Context {
                module: true,
        ...
        if declare {
            // TODO: Remove
            if let Some(decl) =
                self.try_parse_ts_declare(after_export_start, decorators.clone_in(self.alloc))?
            {
                return Ok(ModuleDecl::ExportDecl(self.ast(ExportDecl {
                    span: span!(self, start),
                    decl,
                })));
            }
        ...
        if self.input.syntax().typescript() && is!(self, IdentName) {
            ...
            // TODO: remove clone
            if let Some(decl) = self.try_parse_ts_export_decl(decorators.clone_in(self.alloc), sym)
            {
                return Ok(ModuleDecl::ExportDecl(self.ast(ExportDecl {
                    span: span!(self, start),
                    decl,
        ...
        if self.input.syntax().typescript() {
            ...
            if eat!(self, '=') {
                ...
                let expr = self.parse_expr()?;
                expect!(self, ';');
                return Ok(ModuleDecl::TsExportAssignment(self.ast(
                    TsExportAssignment {
                        span: span!(self, start),
                        expr,
                    },
            ...
            if eat!(self, "as") {
                ...
                expect!(self, ';');
                return Ok(ModuleDecl::TsNamespaceExport(self.ast(
        ...
        if !type_only && eat!(self, "default") {
            ...
            if self.input.syntax().typescript() {
                ...
                if is!(self, "interface") {
                    ...
                    return Ok(ModuleDecl::ExportDefaultDecl(self.ast(ExportDefaultDecl {
                        span: span!(self, start),
                        decl,
                    })));
                }
            ...
            if is!(self, "class") {
                ...
                return Ok(ModuleDecl::ExportDefaultDecl(self.ast(decl)));
                    ...
                    || (is!(self, ',') && (peeked_is!(self, '{') || peeked_is!(self, '*'))))
            {
                ...
                expect!(self, ';');
                return Ok(ModuleDecl::ExportDefaultExpr(self.ast(ExportDefaultExpr {
                    span: span!(self, start),
                    expr,
                })));
            }
            ...
            && peeked_is!(self, "enum")
        {
            ...
            return self
                ...
                .map(Decl::from)
                .map(|decl| {
                    ModuleDecl::ExportDecl(self.ast(ExportDecl {
                        span: span!(self, start),
                        decl,
                    }))
                });
                        ...
                        .unwrap_or(false))
        {
            ...
            if default.is_none() && is!(self, '*') && !peeked_is!(self, "as") {
                ...
                return Ok(ModuleDecl::ExportAll(self.ast(ExportAll {
                    span: span!(self, start),
                    src,
                    type_only,
                    with,
            ...
            if let Some(default) = default {
                ...
                specifiers.push(ExportSpecifier::Default(
                    self.ast(ExportDefaultSpecifier { exported: default }),
                ))
            }
            ...
            if has_ns {
                ...
                specifiers.push(ExportSpecifier::Namespace(self.ast(
            ...
            if has_default || has_ns {
                if is!(self, "from") {
                    let (src, with) = self.parse_from_clause_and_semi()?;
                    return Ok(ModuleDecl::ExportNamed(self.ast(NamedExport {
                        span: span!(self, start),
                        specifiers,
                        src: Some(src),
                        type_only,
        ...
        Ok(ModuleDecl::ExportDecl(self.ast(ExportDecl {
            span: span!(self, start),
            decl,
        })))
    }
    ...
    fn parse_named_export_specifier(
        &mut self,
        type_only: bool,
    ) -> PResult<Box<'a, ExportNamedSpecifier<'a>>> {
        let start = cur_pos!(self);
                ...
                if self.syntax().typescript() && orig_ident.sym == "type" && is!(self, IdentName) {
                    ...
                    if possibly_orig.sym == "as" {
                        ...
                        if !is!(self, IdentName) {
                            ...
                            return Ok(self.ast(ExportNamedSpecifier {
                                span: span!(self, start),
                                orig: ModuleExportName::Ident(self.ast(possibly_orig)),
                                exported: None,
                                is_type_only: true,
                            }));
                        }
                        ...
                        if maybe_as.sym == "as" {
                            if is!(self, IdentName) {
                                ...
                                return Ok(self.ast(ExportNamedSpecifier {
                                    ...
                                    orig: ModuleExportName::Ident(self.ast(possibly_orig)),
                                    exported: Some(ModuleExportName::Ident(self.ast(exported))),
                                    is_type_only: true,
                                }));
                            } else {
                                // `export { type as as }`
                                return Ok(self.ast(ExportNamedSpecifier {
                                    ...
                                    exported: Some(ModuleExportName::Ident(self.ast(maybe_as))),
                                    is_type_only: false,
                                }));
                            }
                        } else {
                        ...
                        is_type_only = true;
                        ModuleExportName::Ident(self.ast(possibly_orig))
                    }
                } else {
                    ModuleExportName::Ident(orig_ident)
                }
        ...
        Ok(self.ast(ExportNamedSpecifier {
            span: span!(self, start),
            orig,
            exported,
            is_type_only,
    ...
    fn parse_from_clause_and_semi(
        &mut self,
    ) -> PResult<(Box<'a, Str>, Option<Box<'a, ObjectLit<'a>>>)> {
        expect!(self, "from");
...
impl<'a> IsDirective<'a> for ModuleItem<'a> {
...
impl<'a, I: Tokens> StmtLikeParser<'a, ModuleItem<'a>> for Parser<'a, I> {
