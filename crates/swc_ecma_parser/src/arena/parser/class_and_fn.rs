...
impl<'a, I: Tokens> Parser<'a, I> {
    pub(crate) fn parse_async_fn_expr(&mut self) -> PResult<Expr<'a>> {
        ...
        expect!(self, "async");
        self.parse_fn(None, Some(start), Vec::new_in(self.alloc))
    }

    /// Parse function expression
    pub(crate) fn parse_fn_expr(&mut self) -> PResult<Expr<'a>> {
        self.parse_fn(None, None, Vec::new_in(self.alloc))
    }
    ...
    ) -> PResult<(Option<Ident>, Box<'a, Class<'a>>)> {
        ...
        T: OutputType<'a>,
    {
        ...
        if is_abstract {
            ...
            for member in class.body.iter() {
                match member {
                    ClassMember::ClassProp(class_prop) if class_prop.is_abstract => {
                        self.emit_err(class_prop.span, SyntaxError::TS1244)
                    }
                    ClassMember::Method(class_method) if class_method.is_abstract => {
                        self.emit_err(class_method.span, SyntaxError::TS1244)
                    }
                    _ => (),
                }
            }
        ...
        match T::finish_class(span!(self, start), ident, class, self.alloc) {
            Ok(v) => Ok(v),
    ...
    fn parse_super_class(
        &mut self,
    ) -> PResult<(Expr<'a>, Option<Box<'a, TsTypeParamInstantiation<'a>>>)> {
        let super_class = self.parse_lhs_expr()?;
        match super_class {
            Expr::TsInstantiation(ts_instantiation) => {
                let ts_instantiation = ts_instantiation.into_inner();
                Ok((ts_instantiation.expr, Some(ts_instantiation.type_args)))
            }
            _ => {
    ...
    pub(crate) fn parse_decorators(
        ...
        let mut decorators = Vec::new_in(self.alloc);
        let start = cur_pos!(self);
    ...
    fn parse_decorator(&mut self) -> PResult<Decorator<'a>> {
        ...
        let expr = if eat!(self, '(') {
            ...
            let expr = self
                ...
                .map(|ident| Expr::Ident(self.ast(ident)))?;
    ...
    fn parse_maybe_decorator_args(&mut self, expr: Expr<'a>) -> PResult<Expr<'a>> {
        ...
        Ok(Expr::Call(self.ast(CallExpr {
    ...
    fn parse_class_body(&mut self) -> PResult<Vec<'a, ClassMember<'a>>> {
        ...
        while !eof!(self) && !is!(self, '}') {
            if eat_exact!(self, ';') {
                ...
                elems.push(ClassMember::Empty(self.ast(EmptyStmt {
    ...
    fn parse_class_member(&mut self) -> PResult<ClassMember<'a>> {
        ...
        let declare_token = if declare {
            ...
            if self.is_class_method() {
                let key = Key::Public(PropName::Ident(
                    self.ast(IdentName::new("declare".into(), span!(self, start))),
                ));
        ...
        if let Some(accessor_token) = accessor_token {
            ...
            if self.is_class_method() {
                let key = Key::Public(PropName::Ident(
                    self.ast(IdentName::new("accessor".into(), accessor_token)),
                ));
                let is_optional = self.input.syntax().typescript() && eat!(self, '?');
                return self.make_method(
                    |p| p.parse_unique_formal_params(),
        ...
        if let Some(static_token) = static_token {
            ...
            if self.is_class_method() {
                let key = Key::Public(PropName::Ident(
                    self.ast(IdentName::new("static".into(), static_token)),
                ));
                let is_optional = self.input.syntax().typescript() && eat!(self, '?');
                return self.make_method(
                    |p| p.parse_unique_formal_params(),
    ...
    fn parse_static_block(&mut self, start: BytePos) -> PResult<ClassMember<'a>> {
        let body = self
            .with_ctx(Context {
        ...
        Ok(ClassMember::StaticBlock(
            self.ast(StaticBlock { span, body }),
        ))
    }
    ...
    fn parse_class_member_with_is_static(
            ...
            && accessibility.is_none()
        {
            ...
            if let Some(idx) = idx {
                return Ok(ClassMember::TsIndexSignature(self.ast(idx)));
            }
        }
        ...
        let key = if readonly.is_some() && is_one_of!(self, '!', ':') {
            Key::Public(PropName::Ident(
                self.ast(IdentName::new("readonly".into(), readonly.unwrap())),
            ))
        } else {
        ...
        if self.is_class_method() {
            ...
            if is_constructor {
                        ...
                        let span = match p {
                            ...
                            ParamOrTsParamProp::TsParamProp(ts_param_prop) => {
                                match &ts_param_prop.param {
                                    TsParamPropParam::Assign(assign_pat) => Some(assign_pat.span),
                                    _ => None,
                                }
                            }
                        };
                ...
                return Ok(ClassMember::Constructor(self.ast(Constructor {
                    span: span!(self, start),
                    accessibility,
                    key: match key {
                        Key::Public(key) => key,
    ...
    fn make_property(
        ...
        self.with_ctx(ctx).parse_with(|p| {
            ...
            if accessor_token.is_some() {
                return Ok(ClassMember::AutoAccessor(p.ast(AutoAccessor {
                    span: span!(p, start),
                    key,
                    value,
                    type_ann,
            ...
            Ok(match key {
                Key::Private(key) => {
                    ...
                    ClassMember::PrivateProp(p.ast(PrivateProp {
                        span: span!(p, start),
                        key,
                        value,
                        is_static,
                ...
                Key::Public(key) => {
                    ...
                    ClassMember::ClassProp(p.ast(ClassProp {
                        span,
                        key,
                        value,
                        is_static,
    ...
    fn parse_fn_inner(
        ...
        is_fn_expr: bool,
        is_ident_required: bool,
    ) -> PResult<(Option<Ident>, Box<'a, Function<'a>>)> {
        ...
        T: OutputType<'a>,
    {
        ...
        match T::finish_fn(
            span!(self, start_of_output_type.unwrap_or(start)),
            ident,
            f,
            self.alloc,
        ) {
        ...
        F: FnOnce(&mut Self) -> PResult<Vec<'a, Param<'a>>>,
    {
        ...
        match key {
            Key::Private(key) => {
                ...
                Ok(ClassMember::PrivateMethod(self.ast(PrivateMethod {
                    span,
            ...
            Key::Public(key) => {
                ...
                Ok(ClassMember::Method(self.ast(ClassMethod {
                    span,
...
impl<'a> OutputType<'a> for Expr<'a> {
    ...
    fn finish_fn(
        ...
        Ok(Expr::Fn(Box::new_in(FnExpr { ident, function }, alloc)))
    }
    ...
    fn finish_class(
        ...
        Ok(Expr::Class(Box::new_in(ClassExpr { ident, class }, alloc)))
    }
}
...
impl<'a> OutputType<'a> for ExportDefaultDecl<'a> {
    ...
    fn finish_fn(
        ...
        Ok(ExportDefaultDecl {
            span,
            decl: DefaultDecl::Fn(Box::new_in(FnExpr { ident, function }, alloc)),
        })
    }
    ...
    fn finish_class(
        ...
        Ok(ExportDefaultDecl {
            span,
            decl: DefaultDecl::Class(Box::new_in(ClassExpr { ident, class }, alloc)),
        })
    }
}
...
pub(crate) trait FnBodyParser<Body> {
...
impl<'a, I: Tokens> FnBodyParser<BlockStmtOrExpr<'a>> for Parser<'a, I> {
    fn parse_fn_body_inner(
        ...
        if is!(self, '{') {
            self.parse_block(false).map(|block_stmt| {
                ...
                BlockStmtOrExpr::BlockStmt(self.ast(block_stmt))
            })
        } else {
...
impl<'a, I: Tokens> FnBodyParser<Option<BlockStmt<'a>>> for Parser<'a, I> {
    fn parse_fn_body_inner(
        &mut self,
        is_simple_parameter_list: bool,
...
pub(crate) trait IsSimpleParameterList {
...
impl<'a> IsSimpleParameterList for Vec<'a, ParamOrTsParamProp<'a>> {
    fn is_simple_parameter_list(&self) -> bool {
        self.iter().all(|param| {
            param.is_ts_param_prop() || param.as_param().is_some_and(|param| param.pat.is_ident())
        })
    }
}

fn is_constructor(key: &Key) -> bool {
    key.as_public().is_some_and(|key| match key {
        PropName::Ident(ident_name) => ident_name.sym == "constructor",
        PropName::Str(s) => s.value == "constructor",
        _ => false,
    })
}

pub(crate) fn is_not_this(p: &Param) -> bool {
    !p.pat.as_ident().is_some_and(|ident| ident.id.sym == "this")
}
