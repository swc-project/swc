...
use super::{PResult, Parser, Token, Tokens};
use crate::{error::SyntaxError, Context};
...
impl<'a, I: Tokens> Parser<'a, I> {
    ...
    pub(crate) fn parse_jsx_ident(&mut self) -> PResult<Ident> {
    ...
    pub(crate) fn parse_jsx_namespaced_name(&mut self) -> PResult<JSXAttrName<'a>> {
        debug_assert!(self.input.syntax().jsx());
        ...
        if !eat!(self, ':') {
            return Ok(JSXAttrName::Ident(self.ast(ns)));
        }

        let name = self.parse_jsx_ident().map(IdentName::from)?;
        Ok(JSXAttrName::JSXNamespacedName(self.ast(
            JSXNamespacedName {
    ...
    /// identifier.
    pub(crate) fn parse_jsx_element_name(&mut self) -> PResult<JSXElementName<'a>> {
        debug_assert!(self.input.syntax().jsx());
        trace_cur!(self, parse_jsx_element_name);
        let start = cur_pos!(self);

        let mut node = match self.parse_jsx_namespaced_name()? {
            JSXAttrName::Ident(i) => JSXElementName::Ident(self.ast(i.into_inner().into())),
            JSXAttrName::JSXNamespacedName(i) => JSXElementName::JSXNamespacedName(i),
        };
        while eat!(self, '.') {
            let prop = self.parse_jsx_ident().map(IdentName::from)?;
            let new_node = JSXElementName::JSXMemberExpr(self.ast(JSXMemberExpr {
    ...
    pub(crate) fn parse_jsx_attr_value(&mut self) -> PResult<JSXAttrValue<'a>> {
        debug_assert!(self.input.syntax().jsx());
        ...
        match *cur!(self, true) {
            tok!('{') => {
                ...
                match node.expr {
                    ...
                    JSXExpr::Expr(..) => Ok(JSXAttrValue::JSXExprContainer(self.ast(node))),
                }
            }
            Token::Str { .. } => {
                let lit = self.parse_lit()?;
                Ok(JSXAttrValue::Lit(self.ast(lit)))
            }
            Token::JSXTagStart => {
                let expr = self.parse_jsx_element()?;
                match expr {
                    Either::Left(n) => Ok(JSXAttrValue::JSXFragment(self.ast(n))),
                    Either::Right(n) => Ok(JSXAttrValue::JSXElement(self.ast(n))),
                }
            }
    ...
    pub(crate) fn parse_jsx_empty_expr(&mut self) -> PResult<JSXEmptyExpr> {
        debug_assert!(self.input.syntax().jsx());
        let start = cur_pos!(self);
    ...
    pub(crate) fn parse_jsx_spread_child(&mut self) -> PResult<JSXSpreadChild<'a>> {
        debug_assert!(self.input.syntax().jsx());
        let start = cur_pos!(self);
        expect!(self, '{');
        expect!(self, "...");
    ...
    pub(crate) fn parse_jsx_expr_container(&mut self, _: BytePos) -> PResult<JSXExprContainer<'a>> {
        debug_assert!(self.input.syntax().jsx());

        let start = cur_pos!(self);
        bump!(self);
        let expr = if is!(self, '}') {
            self.parse_jsx_empty_expr()
                .map(|expr| self.ast(expr))
                .map(JSXExpr::JSXEmptyExpr)?
        } else {
            if is!(self, "...") {
                bump!(self);
            }
    ...
    pub(crate) fn parse_jsx_attr(&mut self) -> PResult<JSXAttrOrSpread<'a>> {
        debug_assert!(self.input.syntax().jsx());
        let start = cur_pos!(self);
        ...
        if eat!(self, '{') {
            ...
            expect!(self, '}');
            return Ok(JSXAttrOrSpread::SpreadElement(
        ...
        Ok(JSXAttrOrSpread::JSXAttr(self.ast(JSXAttr {
            span: span!(self, start),
            name,
            value,
        })))
    ...
    pub(crate) fn parse_jsx_opening_element_at(
        &mut self,
        start: BytePos,
    ) -> PResult<Either<JSXOpeningFragment, JSXOpeningElement<'a>>> {
        debug_assert!(self.input.syntax().jsx());
    ...
    pub(crate) fn parse_jsx_opening_element_after_name(
        &mut self,
        start: BytePos,
        name: JSXElementName<'a>,
    ) -> PResult<JSXOpeningElement<'a>> {
        debug_assert!(self.input.syntax().jsx());
        ...
        let mut attrs = Vec::new_in(self.alloc);
    ...
    fn parse_jsx_closing_element_at(
        &mut self,
        start: BytePos,
    ) -> PResult<Either<JSXClosingFragment, JSXClosingElement<'a>>> {
        debug_assert!(self.input.syntax().jsx());
    ...
    pub(crate) fn parse_jsx_element_at(
        &mut self,
        start_pos: BytePos,
    ) -> PResult<Either<JSXFragment<'a>, JSXElement<'a>>> {
        debug_assert!(self.input.syntax().jsx());
        ...
        self.with_ctx(ctx).parse_with(|p| {
            ...
            let mut children = Vec::new_in(p.alloc);
                    ...
                    match *cur!(p, true) {
                        Token::JSXTagStart => {
                            ...
                            children.push(p.parse_jsx_element_at(start).map(|e| match e {
                                Either::Left(e) => JSXElementChild::from(p.ast(e)),
                                Either::Right(e) => JSXElementChild::from(p.ast(e)),
                            })?);
                        }
    ...
    pub(crate) fn parse_jsx_element(&mut self) -> PResult<Either<JSXFragment<'a>, JSXElement<'a>>> {
    ...
    pub(crate) fn parse_jsx_text(&mut self) -> PResult<JSXText> {
        debug_assert!(self.input.syntax().jsx());
...
fn get_qualified_jsx_name(name: &JSXElementName) -> JsWord {
    ...
    match name {
        ...
        JSXElementName::JSXNamespacedName(nn) => format!("{}:{}", nn.ns.sym, nn.name.sym).into(),
        JSXElementName::JSXMemberExpr(member) => format!(
            "{}.{}",
            get_qualified_obj_name(&member.obj),
            member.prop.sym
        )
