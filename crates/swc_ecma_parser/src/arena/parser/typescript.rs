...
use super::{class_and_fn::IsSimpleParameterList, Parser};
use crate::{
    error::SyntaxError,
    lexer::TokenContexts,
    token::{Keyword, Token, Word},
    Context, PResult, Syntax, Tokens, TsSyntax,
};
...
impl<'a, I: Tokens> Parser<'a, I> {
    ...
    /// `tsParseEntityName`
    fn parse_ts_entity_name(&mut self, allow_reserved_words: bool) -> PResult<TsEntityName<'a>> {
        debug_assert!(self.input.syntax().typescript());
        ...
        let mut entity = TsEntityName::Ident(self.ast(init.into()));
        while eat!(self, '.') {
            let dot_start = cur_pos!(self);
            ...
            entity = TsEntityName::TsQualifiedName(self.ast(TsQualifiedName { span, left, right }));
        }
    ...
    fn parse_ts_type_ref(&mut self) -> PResult<TsTypeRef<'a>> {
    ...
    fn parse_ts_this_type_predicate(
        ...
        lhs: TsThisType,
    ) -> PResult<TsTypePredicate<'a>> {
        debug_assert!(self.input.syntax().typescript());

        let param_name = TsThisTypeOrIdent::TsThisType(self.ast(lhs));
    ...
    /// `tsParseImportType`
    fn parse_ts_import_type(&mut self) -> PResult<TsImportType<'a>> {
        let start = cur_pos!(self);
    ...
    /// `tsParseTypeQuery`
    fn parse_ts_type_query(&mut self) -> PResult<TsTypeQuery<'a>> {
        debug_assert!(self.input.syntax().typescript());

        let start = cur_pos!(self);
        expect!(self, "typeof");
        let expr_name = if is!(self, "import") {
            let ts_import_type = self.parse_ts_import_type()?;
            TsTypeQueryExpr::Import(self.ast(ts_import_type))
        } else {
            let ts_entity_name = self.parse_ts_entity_name(
                // allow_reserved_word
                true,
            )?;

            TsTypeQueryExpr::TsEntityName(self.ast(ts_entity_name))
        };
    ...
    pub(crate) fn parse_ts_type_params(
        ...
        permit_in_out: bool,
        permit_const: bool,
    ) -> PResult<Box<'a, TsTypeParamDecl<'a>>> {
    ...
    pub(crate) fn parse_ts_type_or_type_predicate_ann(
        ...
        self.in_type().parse_with(|p| {
            ...
            let node = TsType::TsTypePredicate(p.ast(TsTypePredicate {
                span: span!(p, type_pred_start),
                asserts: has_type_pred_asserts,
                param_name: TsThisTypeOrIdent::Ident(p.ast(type_pred_var.into())),
                type_ann,
            }));
    ...
    pub(crate) fn try_parse_ts_type_args(
        &mut self,
    ) -> Option<Box<'a, TsTypeParamInstantiation<'a>>> {
    ...
    /// `tsNextThenParseType`
    pub(crate) fn next_then_parse_ts_type(&mut self) -> PResult<TsType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    /// `tsParseEnumMember`
    fn parse_ts_enum_member(&mut self) -> PResult<TsEnumMember<'a>> {
        debug_assert!(self.input.syntax().typescript());
        ...
        let id = match *cur!(self, true) {
            ...
            Token::Num { value, ref raw } => {
                ...
                TsEnumMemberId::Str(self.ast(Str {
                    span,
            ...
            Token::LBracket => {
                ...
                TsEnumMemberId::Ident(
                    self.ast(Ident::new_no_ctxt(js_word!(""), span!(self, start))),
                )
            }
            _ => {
                let ident = self.parse_ident_name()?;
                TsEnumMemberId::Ident(self.ast(ident.into()))
            }
        };
    ...
    /// `tsParseModuleBlock`
    fn parse_ts_module_block(&mut self) -> PResult<TsModuleBlock<'a>> {
    ...
    fn parse_ts_module_or_ns_decl(&mut self, start: BytePos) -> PResult<Box<'a, TsModuleDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());

        let id = self.parse_ident_name()?;
        let body: TsNamespaceBody = if eat!(self, '.') {
            let inner_start = cur_pos!(self);
            let inner = self.parse_ts_module_or_ns_decl(inner_start)?.into_inner();
            let inner = TsNamespaceDecl {
                span: inner.span,
                id: match inner.id {
                    TsModuleName::Ident(i) => i.into_inner(),
                    _ => unreachable!(),
                },
                body: self.ast(inner.body.unwrap()),
                declare: inner.declare,
                global: inner.global,
            };
            TsNamespaceBody::TsNamespaceDecl(self.ast(inner))
        } else {
            let ts_module_block = self.parse_ts_module_block()?;
            TsNamespaceBody::TsModuleBlock(self.ast(ts_module_block))
        };

        Ok(self.ast(TsModuleDecl {
            span: span!(self, start),
            declare: false,
            id: TsModuleName::Ident(self.ast(id.into())),
            body: Some(body),
            global: false,
        }))
    }
    ...
    fn parse_ts_ambient_external_module_decl(
        &mut self,
        start: BytePos,
    ) -> PResult<Box<'a, TsModuleDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());

        let (global, id) = if is!(self, "global") {
            let id = self.parse_ident_name()?;
            (true, TsModuleName::Ident(self.ast(id.into())))
        ...
        let body = if is!(self, '{') {
            let ts_module_block = self.parse_ts_module_block()?;
            Some(TsNamespaceBody::TsModuleBlock(self.ast(ts_module_block)))
        } else {
            expect!(self, ';');
            None
        };
    ...
    pub fn parse_type(&mut self) -> PResult<TsType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    ///
    /// `tsParseType`
    pub(crate) fn parse_ts_type(&mut self) -> PResult<TsType<'a>> {
        ...
        .parse_with(|p| {
            ...
            Ok(TsType::TsConditionalType(p.ast(TsConditionalType {
                span: span!(p, start),
                check_type,
                extends_type,
                true_type,
    ...
    fn parse_ts_non_conditional_type(&mut self) -> PResult<TsType<'a>> {
    ...
    pub(crate) fn parse_ts_type_assertion(
        &mut self,
        start: BytePos,
    ) -> PResult<TsTypeAssertion<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    pub(crate) fn parse_ts_heritage_clause(&mut self) -> PResult<Vec<'a, TsExprWithTypeArgs<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_heritage_clause_element(&mut self) -> PResult<TsExprWithTypeArgs<'a>> {
        debug_assert!(self.input.syntax().typescript());
        ...
        let ident = self.parse_ident_name()?;
        let ident = Expr::Ident(self.ast(ident.into()));
    ...
    pub(crate) fn parse_ts_interface_decl(
        &mut self,
        start: BytePos,
    ) -> PResult<Box<'a, TsInterfaceDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    pub(crate) fn parse_ts_type_alias_decl(
        &mut self,
        start: BytePos,
    ) -> PResult<Box<'a, TsTypeAliasDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    pub(crate) fn parse_ts_import_equals_decl(
        ...
        is_export: bool,
        is_type_only: bool,
    ) -> PResult<Box<'a, TsImportEqualsDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());
        ...
        expect!(self, ';');
        Ok(self.ast(TsImportEqualsDecl {
            span: span!(self, start),
            id,
            is_export,
            is_type_only,
    ...
    fn parse_ts_module_ref(&mut self) -> PResult<TsModuleRef<'a>> {
        debug_assert!(self.input.syntax().typescript());

        if self.is_ts_external_module_ref()? {
            let ts_external_module_ref = self.parse_ts_external_module_ref()?;
            Ok(TsModuleRef::TsExternalModuleRef(
                self.ast(ts_external_module_ref),
            ))
        } else {
            let ts_entity_name =
                self.parse_ts_entity_name(/* allow_reserved_words */ false)?;
            Ok(TsModuleRef::TsEntityName(self.ast(ts_entity_name)))
        }
    }
    ...
    pub(crate) fn ts_look_ahead<T, F>(&mut self, op: F) -> PResult<T>
    where
    ...
    ) -> PResult<Either<TsCallSignatureDecl<'a>, TsConstructSignatureDecl<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    pub(crate) fn try_parse_ts_index_signature(
        ...
        readonly: bool,
        is_static: bool,
    ) -> PResult<Option<TsIndexSignature<'a>>> {
        if !cfg!(feature = "typescript") {
            return Ok(Default::default());
        }
        ...
        params.push(TsFnParam::Ident(self.ast(id)));
    ...
    fn parse_ts_property_name(&mut self) -> PResult<(bool, Expr<'a>)> {
                ...
                let key = match *cur!(p, true) {
                    ...
                    _ => p.parse_maybe_private_name().map(|e| match e {
                        Either::Left(e) => {
                            ...
                            Expr::PrivateName(p.ast(e))
                        }
                        Either::Right(e) => Expr::Ident(p.ast(e.into())),
                    }),
                };
    ...
    fn parse_ts_property_or_method_signature(
        ...
        start: BytePos,
        readonly: bool,
    ) -> PResult<Either<TsPropertySignature<'a>, TsMethodSignature<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    /// `tsParseTypeMember`
    fn parse_ts_type_member(&mut self) -> PResult<TsTypeElement<'a>> {
        debug_assert!(self.input.syntax().typescript());
        if is_one_of!(self, '(', '<') {
            return self
                .parse_ts_signature_member(SignatureParsingMode::TSCallSignatureDeclaration)
                .map(|e| match e {
                    Either::Left(e) => TsTypeElement::TsCallSignatureDecl(self.ast(e)),
                    Either::Right(e) => TsTypeElement::TsConstructSignatureDecl(self.ast(e)),
                });
        }
        ...
        if let Some(idx) = idx {
            return Ok(TsTypeElement::TsIndexSignature(self.ast(idx)));
        }
        ...
        if let Some(v) = self.try_parse_ts(|p| {
            ...
            if is_get {
                ...
                Ok(Some(TsTypeElement::TsGetterSignature(p.ast(
                    TsGetterSignature {
                        span: span!(p, start),
                        key,
                        computed,
                ...
                Ok(Some(TsTypeElement::TsSetterSignature(p.ast(
                    TsSetterSignature {
                        span: span!(p, start),
                        key,
                        computed,
        ...
        self.parse_ts_property_or_method_signature(start, readonly)
            .map(|e| match e {
                Either::Left(e) => TsTypeElement::TsPropertySignature(self.ast(e)),
                Either::Right(e) => TsTypeElement::TsMethodSignature(self.ast(e)),
            })
    }
    ...
    /// `tsParseTypeLiteral`
    fn parse_ts_type_lit(&mut self) -> PResult<TsTypeLit<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_object_type_members(&mut self) -> PResult<Vec<'a, TsTypeElement<'a>>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_mapped_type_param(&mut self) -> PResult<TsTypeParam<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    /// `tsParseMappedType`
    fn parse_ts_mapped_type(&mut self) -> PResult<TsMappedType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    /// `tsParseTupleType`
    fn parse_ts_tuple_type(&mut self) -> PResult<TsTupleType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn try_parse_ts_tuple_element_name(&mut self) -> Option<Pat<'a>> {
        if !cfg!(feature = "typescript") {
        ...
        self.try_parse_ts(|p| {
            ...
            Ok(Some(if let Some(dot3_token) = rest {
                Pat::Rest(p.ast(RestPat {
                    span: span!(p, start),
                    dot3_token,
                    arg: Pat::Ident(p.ast(ident.into())),
                    type_ann: None,
                }))
            } else {
                Pat::Ident(p.ast(ident.into()))
            }))
        })
    }

    /// `tsParseTupleElementType`
    fn parse_ts_tuple_element_type(&mut self) -> PResult<TsTupleElement<'a>> {
        debug_assert!(self.input.syntax().typescript());
        ...
        if eat!(self, "...") {
            ...
            return Ok(TsTupleElement {
                span: span!(self, start),
                label,
                ty: TsType::TsRestType(self.ast(TsRestType {
                    span: span!(self, start),
                    type_ann,
                })),
            });
        ...
        if eat!(self, '?') {
            ...
            return Ok(TsTupleElement {
                span: span!(self, start),
                label,
                ty: TsType::TsOptionalType(self.ast(TsOptionalType {
                    span: span!(self, start),
                    type_ann,
                })),
            });
    ...
    fn parse_ts_parenthesized_type(&mut self) -> PResult<TsParenthesizedType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_fn_or_constructor_type(
        &mut self,
        is_fn_type: bool,
    ) -> PResult<TsFnOrConstructorType<'a>> {
        ...
        Ok(if is_fn_type {
            TsFnOrConstructorType::TsFnType(self.ast(TsFnType {
                span: span!(self, start),
                type_params,
                params,
                type_ann,
            ...
            TsFnOrConstructorType::TsConstructorType(self.ast(TsConstructorType {
                span: span!(self, start),
                type_params,
                params,
                type_ann,
    ...
    fn parse_ts_lit_type_node(&mut self) -> PResult<TsLitType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_tpl_lit_type(&mut self) -> PResult<TsTplLitType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_tpl_type_elements(
        &mut self,
    ) -> PResult<(Vec<'a, TsType<'a>>, Vec<'a, TplElement>)> {
        if !cfg!(feature = "typescript") {
            return Ok((Vec::new_in(self.alloc), Vec::new_in(self.alloc)));
        }
    ...
    fn parse_ts_binding_list_for_signature(&mut self) -> PResult<Vec<'a, TsFnParam<'a>>> {
        if !cfg!(feature = "typescript") {
    ...
    fn try_parse_ts_type_or_type_predicate_ann(
        &mut self,
    ) -> PResult<Option<Box<'a, TsTypeAnn<'a>>>> {
        if !cfg!(feature = "typescript") {
            return Ok(None);
        }
    ...
    #[cfg_attr(feature = "tracing-spans", tracing::instrument(skip_all))]
    pub(crate) fn try_parse_ts_type_ann(&mut self) -> PResult<Option<Box<'a, TsTypeAnn<'a>>>> {
        if !cfg!(feature = "typescript") {
            return Ok(None);
        }
    ...
    /// `tsTryParseType`
    fn try_parse_ts_type(&mut self) -> PResult<Option<TsType<'a>>> {
        if !cfg!(feature = "typescript") {
            return Ok(None);
        }
    ...
    pub(crate) fn try_parse_ts_type_params(
        ...
        permit_in_out: bool,
        permit_const: bool,
    ) -> PResult<Option<Box<'a, TsTypeParamDecl<'a>>>> {
        if !cfg!(feature = "typescript") {
            return Ok(None);
        }
    ...
    /// `tsParseNonArrayType`
    fn parse_ts_non_array_type(&mut self) -> PResult<TsType<'a>> {
        if !cfg!(feature = "typescript") {
            unreachable!()
        }
        ...
        match *cur!(self, true) {
            ...
            | tok!("break") => {
                ...
                match kind {
                    Some(kind) if !peeked_is_dot => {
                        bump!(self);
                        return Ok(TsType::TsKeywordType(self.ast(TsKeywordType {
                            span: span!(self, start),
                            kind,
                        })));
                    }
            ...
            tok!('-') => {
                ...
                let lit = match lit {
                    Lit::Num(num) => {
                        ...
                        match num.raw {
                            ...
                            _ => {
                                write!(new_raw, "{}", num.value).unwrap();
                            }
                        };

                        TsLit::Number(self.ast(Number {
                            span: num.span,
                            value: -num.value,
                            raw: Some(new_raw.into()),
                        }))
                    ...
                    Lit::BigInt(big_int) => {
                        let big_int = big_int.into_inner();
                        let mut new_raw = String::from("-");

                        let value = big_int.value.into_inner();
                        match big_int.raw {
                            Some(raw) => {
                                new_raw.push_str(&raw);
                            }
                ...
                return Ok(TsType::TsLitType(self.ast(TsLitType {
                    span: span!(self, start),
                    lit,
                })));
            }
            ...
            tok!("this") => {
                ...
                if !self.input.had_line_break_before_cur() && is!(self, "is") {
                    ...
                    return Ok(TsType::TsThisType(self.ast(this_keyword)));
                }
            }
            tok!("typeof") => {
                return self
    ...
    fn parse_ts_array_type_or_higher(&mut self, readonly: bool) -> PResult<TsType<'a>> {
        ...
        while !self.input.had_line_break_before_cur() && eat!(self, '[') {
            if eat!(self, ']') {
                ty = TsType::TsArrayType(self.ast(TsArrayType {
                ...
                expect!(self, ']');
                ty = TsType::TsIndexedAccessType(self.ast(TsIndexedAccessType {
                    span: span!(self, ty.span_lo()),
                    readonly,
                    obj_type: ty,
                    index_type,
    ...
    /// `tsParseTypeOperator`
    fn parse_ts_type_operator(&mut self, op: TsTypeOperatorOp) -> PResult<TsTypeOperator<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    /// `tsParseInferType`
    fn parse_ts_infer_type(&mut self) -> PResult<TsInferType<'a>> {
        debug_assert!(self.input.syntax().typescript());
    ...
    fn parse_ts_type_operator_or_higher(&mut self) -> PResult<TsType<'a>> {
    ...
    pub(crate) fn parse_ts_expr_stmt(
        ...
        match &*expr.sym {
            ...
            "global" => {
                ...
                if is!(self, '{') {
                    let global = true;
                    let id = TsModuleName::Ident(self.ast(expr));
                    let body = self
                        .parse_ts_module_block()
                        .map(|t| self.ast(t))
                        .map(TsNamespaceBody::from)
                        .map(Some)?;
                    Ok(Some(Decl::TsModule(self.ast(TsModuleDecl {
                        span: span!(self, start),
                        global,
                        declare: false,
                        id,
            ...
            if is!(p, "function") {
                return p
                    ...
                    .map(|decl| match decl {
                        Decl::Fn(f) => {
                            let f = f.into_inner();
                            Decl::Fn(p.ast(FnDecl {
                                ident: f.ident,
                                declare: true,
                                function: p.ast(Function {
                                    span: Span {
                                        lo: declare_start,
                                        hi: f.function.span.hi,
                                    },
                                    ..f.function.into_inner()
                                }),
                            }))
                        }
                        _ => decl,
        ...
        let res = if is_one_of!(self, '<', JSXTagStart) {
            self.try_parse_ts(|p| {
                ...
                let params: Vec<Pat> = Vec::from_iter_in(
                    p.parse_formal_params()?.into_iter().map(|p| p.pat),
                    self.alloc,
                );
                expect!(p, ')');
    ...
    pub fn parse_ts_type_args(&mut self) -> PResult<Box<'a, TsTypeParamInstantiation<'a>>> {
        ...
        expect!(self, '>');
        Ok(self.ast(TsTypeParamInstantiation {
            span: span!(self, start),
            params,
        }))
    }
    ...
    fn parse_ts_intersection_type_or_higher(&mut self) -> PResult<TsType<'a>> {
    ...
    fn parse_ts_union_type_or_higher(&mut self) -> PResult<TsType<'a>> {
    ...
    where
        F: FnMut(&mut Self) -> PResult<TsType<'a>>,
    {
        ...
        if self.input.is(operator) {
            ...
            return Ok(TsType::TsUnionOrIntersectionType(self.ast(match kind {
                UnionOrIntersection::Union => {
                    TsUnionOrIntersectionType::TsUnionType(self.ast(TsUnionType {
                        span: span!(self, start),
                        types,
                    }))
                }
                UnionOrIntersection::Intersection => {
                    TsUnionOrIntersectionType::TsIntersectionType(self.ast(TsIntersectionType {
                        span: span!(self, start),
                        types,
                    }))
                }
