...
// Reference types - < can start type parameters
(i as Number)<T>(x);
(i as String)<T>(x);
(i as Boolean)<T>(x);
(i as Array)<T>(x);
...
// satisfies with reference types
(i satisfies Number)<T>(x);
(i satisfies MyType)<T>(x);
...
// Array types - non-callable (already complete)
(i as number[]) < 5;
(i as Array<number>) < 5;
