...
use common::ecma_reuse::{
    category_for_path, class_member_label, collect_files_for_category, decl_label, expr_label,
...
#[test]
fn span_snapshots() {
    let mut inputs = collect_files_for_category("span");
    inputs.extend(
        collect_files_for_category("comments")
            .into_iter()
    ...
    assert!(!inputs.is_empty(), "no span fixtures found");
...
fn run_span_case(input: PathBuf) {
    let category = category_for_path(&input);
    let case = Case {
        path: input.clone(),
        category,
    };
    ...
    let output = testing::run_test(false, |cm, handler| -> Result<(), ()> {
        ...
        let parsed = parsed
            .parsed
            .unwrap_or_else(|| panic!("span fixture {} produced no AST", input.display()));
    ...
    output
        .compare_to_file(snapshot_path_for(&input, ".span.swc-stderr"))
        .unwrap_or_else(|_| panic!("span snapshot mismatch: {}", input.display()));
}
