...
use common::ecma_reuse::{
    collect_files_for_category, parse_loaded_file, snapshot_path_for, span_of_class_member,
    span_of_decl, span_of_expr, span_of_module_decl, span_of_pat, span_of_program, span_of_stmt,
    span_of_ts_type, Case,
};

#[test]
fn comments_snapshots() {
    let mut inputs = collect_files_for_category("comments")
        .into_iter()
        .filter(|path| path.file_name().and_then(|name| name.to_str()) == Some("input.js"))
        .collect::<Vec<_>>();
    inputs.sort();

    assert!(!inputs.is_empty(), "no comments fixtures found");

    for input in inputs {
        run_comments_case(input);
    }
}

fn run_comments_case(input: PathBuf) {
    let case = Case {
        path: input.clone(),
