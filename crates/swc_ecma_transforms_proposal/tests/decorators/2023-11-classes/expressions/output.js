var _initClass, _A, _initClass1, _C, _initClass2, _class, _initClass3, _class1, _initClass4, _G, _initClass5, _class2, _initClass6, _class3, _I, _initClass7, _K, _L;
const dec = ()=>{};
const A = (class A {
    static{
        ({ c: [_A, _initClass] } = _apply_decs_2311(this, [
            dec
        ], []));
    }
    static{
        _initClass();
    }
}, _A);
const B = (class C {
    static{
        ({ c: [_C, _initClass1] } = _apply_decs_2311(this, [
            dec
        ], []));
    }
    static{
        _initClass1();
    }
}, _C);
const D = (class {
    static{
        ({ c: [_class, _initClass2] } = _apply_decs_2311(this, [
            dec
        ], []));
    }
    static{
        _initClass2();
    }
}, _class);
const E = ((class {
    static{
        ({ c: [_class1, _initClass3] } = _apply_decs_2311(this, [
            dec
        ], []));
    }
    static{
        _initClass3();
    }
}, _class1), 123);
const F = [
    (class G {
        static{
            ({ c: [_G, _initClass4] } = _apply_decs_2311(this, [
                dec
            ], []));
        }
        static{
            _initClass4();
        }
    }, _G),
    (class {
        static{
            ({ c: [_class2, _initClass5] } = _apply_decs_2311(this, [
                dec
            ], []));
        }
        static{
            _initClass5();
        }
    }, _class2)
];
const H = (class extends (_I = I) {
    static{
        ({ c: [_class3, _initClass6] } = _apply_decs_2311(this, [
            dec
        ], [], 0, void 0, _I));
    }
    static{
        _initClass6();
    }
}, _class3);
const J = (class K extends (_L = L) {
    static{
        ({ c: [_K, _initClass7] } = _apply_decs_2311(this, [
            dec
        ], [], 0, void 0, _L));
    }
    static{
        _initClass7();
    }
}, _K);
function classFactory() {
    var _initClass, _class;
    return class {
        static{
            ({ c: [_class, _initClass] } = _apply_decs_2311(this, [
                dec
            ], []));
        }
        static{
            _initClass();
        }
    }, _class;
}
