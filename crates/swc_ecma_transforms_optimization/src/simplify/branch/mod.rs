...
use swc_ecma_utils::{
    extract_var_ids, is_literal, prepend_stmt, ExprCtx, ExprExt, ExprFactory, IsEmpty, StmtExt,
...
impl VisitMut for Remover {
    ...
    fn visit_mut_expr(&mut self, e: &mut Expr) {
        ...
        optimize_expr(e, self.expr_ctx, &mut self.changed);
    }
    ...
    fn visit_mut_for_stmt(&mut self, s: &mut ForStmt) {
        ...
        optimize_for_stmt(s, self.expr_ctx, &mut self.changed);
    }
    ...
    fn visit_mut_object_pat(&mut self, p: &mut ObjectPat) {
        ...
        optimize_object_pat(p, self.expr_ctx, &mut self.changed);
    }
    ...
    fn visit_mut_seq_expr(&mut self, e: &mut SeqExpr) {
        ...
        e.exprs = exprs;
        optimize_seq_expr(e, self.expr_ctx, &mut self.changed);
    }

    fn visit_mut_stmt(&mut self, stmt: &mut Stmt) {
        stmt.visit_mut_children_with(self);

        optimize_stmt(stmt, self.expr_ctx, &mut self.changed);
    }
    ...
    fn visit_mut_switch_stmt(&mut self, s: &mut SwitchStmt) {
        ...
        optimize_switch_stmt(s, self.expr_ctx, &mut self.changed);
    }
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
pub fn optimize_object_pat(p: &mut ObjectPat, _expr_ctx: ExprCtx, changed: &mut bool) {
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
pub fn optimize_expr(e: &mut Expr, expr_ctx: ExprCtx, changed: &mut bool) {
    match e {
        Expr::Seq(s) => {
            if s.exprs.is_empty() {
                *e = Expr::dummy();
        ...
        Expr::Cond(cond)
            if !cond.test.may_have_side_effects(expr_ctx)
                && (cond.cons.is_undefined(expr_ctx)
                    || matches!(*cond.cons, Expr::Unary(UnaryExpr {
                            op: op!("void"),
                            ref arg,
                            ..
                        }) if !arg.may_have_side_effects(expr_ctx)))
                && (cond.alt.is_undefined(expr_ctx)
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
fn optimize_seq_expr(e: &mut SeqExpr, expr_ctx: ExprCtx, _changed: &mut bool) {
    if e.exprs.is_empty() {
        return;
    }
    ...
    let mut exprs = if should_preserved_this {
        e.exprs
            ...
            .filter_map(|(idx, e)| {
                ...
                ignore_result(e, true, expr_ctx)
            })
            .collect()
    } else {
        e.exprs
            .take()
            .move_flat_map(|e| ignore_result(e, false, expr_ctx))
    };
...
impl Remover {
        ...
        T: StmtLike + VisitMutWith<Self>,
    {
        ...
        optimize_stmt_likes(stmts, is_block_stmt, self.expr_ctx, &mut self.changed);
    }
}
...
fn ignore_result(e: Box<Expr>, drop_str_lit: bool, ctx: ExprCtx) -> Option<Box<Expr>> {
    match *e {
        ...
        | Expr::Lit(Lit::Regex(..)) => {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a literal");
            }

            None
        }

        Expr::Lit(Lit::Str(ref v)) if drop_str_lit || v.value.is_empty() => {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a string literal");
            }
        ...
        {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping an assignment");
            }
        ...
        Expr::Bin(BinExpr {
            ...
            match (left, right) {
                ...
                (Some(l), None) => {
                    if cfg!(feature = "debug") {
                        debug!("ignore_result: Dropping the left side of a binary expression");
                    }
                ...
                (None, Some(r)) => {
                    if cfg!(feature = "debug") {
                        debug!("ignore_result: Dropping the right side of a binary expression");
                    }
                ...
                (None, None) => {
                    if cfg!(feature = "debug") {
                        debug!("ignore_result: Dropping both sides of a binary expression");
                    }
        ...
        Expr::Object(ObjectLit { span, props, .. }) => {
            let props = props.move_flat_map(|v| match v {
                ...
                PropOrSpread::Prop(ref p) => {
                    if is_literal(&**p) {
                        if cfg!(feature = "debug") {
                            debug!("ignore_result: Dropping a property of an object literal");
                        }
        ...
        Expr::New(NewExpr {
            ...
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a new expression");
            }
        ...
        Expr::Call(CallExpr {
            ...
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a call expression");
            }
        ...
        Expr::Tpl(Tpl { span, exprs, .. }) => {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a template expression");
            }
        ...
        Expr::TaggedTpl(TaggedTpl { span, tag, tpl, .. }) if tag.is_pure_callee(ctx) => {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a tagged template expression");
            }
        ...
        Expr::Fn(..) => {
            if cfg!(feature = "debug") {
                debug!("ignore_result: Dropping a function expression");
            }
        ...
        Expr::Seq(SeqExpr {
            ...
            if exprs.is_empty() {
                if cfg!(feature = "debug") {
                    debug!("ignore_result: Dropping a sequence expression");
                }
        ...
        Expr::Cond(CondExpr {
            ...
            let alt = if let Some(alt) = ignore_result(alt, true, ctx) {
                ...
                if cfg!(feature = "debug") {
                    debug!("ignore_result: Dropping the alternative of a conditional expression");
                }
            ...
            let cons = if let Some(cons) = ignore_result(cons, true, ctx) {
                ...
                if cfg!(feature = "debug") {
                    debug!("ignore_result: Dropping the consequent of a conditional expression");
                }
...
fn optimize_loop_body(stmt: Stmt) -> Stmt {
    if stmt.is_empty() {
        return stmt;
    }
...
fn check_for_stopper(s: &[Stmt], only_conditional: bool) -> bool {
    ...
    impl Visit for Visitor {
        ...
        fn visit_try_stmt(&mut self, s: &TryStmt) {
            let old = self.in_cond;
            self.in_cond = true;
            s.block.visit_with(self);

            self.in_cond = true;
            s.handler.visit_with(self);
            self.in_cond = old;

            self.in_cond = false;
            s.finalizer.visit_with(self);
            self.in_cond = old;
        }
        ...
        fn visit_return_stmt(&mut self, s: &ReturnStmt) {
            if self.in_cond {
                self.found = true;
                return;
            }
        ...
        fn visit_throw_stmt(&mut self, s: &ThrowStmt) {
            if self.in_cond {
                self.found = true;
                return;
            }
...
fn reoptimize_stmt(stmt: &mut Stmt, expr_ctx: ExprCtx, changed: &mut bool) {
    match stmt {
        Stmt::Block(BlockStmt { stmts, .. }) => {
            for stmt in stmts.iter_mut() {
                optimize_stmt(stmt, expr_ctx, changed);
            }

            optimize_stmt_likes(stmts, true, expr_ctx, changed);
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
pub fn optimize_stmt(stmt: &mut Stmt, expr_ctx: ExprCtx, changed: &mut bool) {
    match stmt {
        Stmt::For(ForStmt { body, .. })
        | Stmt::ForIn(ForInStmt { body, .. })
        | Stmt::ForOf(ForOfStmt { body, .. })
    ...
    stmt.map_with_mut(|stmt| {
        match stmt {
            Stmt::If(IfStmt {
                ...
                if let (p, Known(v)) = test.cast_to_bool(expr_ctx) {
                    if cfg!(feature = "debug") {
                    ...
                    if !p.is_pure() {
                        if let Some(expr) = ignore_result(test, true, expr_ctx) {
                    ...
                    *changed = true;
                    ...
                    .into();
                    reoptimize_stmt(&mut block, expr_ctx, changed);
                    return block;
                }
                ...
                if alt.is_none() {
                    if let Stmt::Empty(..) = *cons {
                        *changed = true;
                        ...
                        return if let Some(expr) = ignore_result(test, true, expr_ctx) {
            ...
            Stmt::Expr(ExprStmt { span, expr, .. }) => {
                ...
                match ignore_result(expr, false, expr_ctx) {
            ...
            Stmt::Block(BlockStmt { span, stmts, ctxt }) => {
                    ...
                    && stmt_depth(&stmts[0]) <= 1
                {
                    ...
                    reoptimize_stmt(&mut v, expr_ctx, changed);
                    v
                } else {
            ...
            Stmt::Switch(mut s) => {
                ...
                let is_matching_literal = match *s.discriminant {
                    ...
                    ref e if e.is_nan() || e.is_global_ref_to(expr_ctx, "undefined") => true,
                    _ => false,
                };
                ...
                if s.cases.is_empty() {
                    ...
                    return match ignore_result(s.discriminant, true, expr_ctx) {
                    ...
                    s.cases.iter().position(|case| {
                        ...
                        if let Some(ref test) = case.test {
                            let v = match (&**test, &*s.discriminant) {
                                ...
                                _ => {
                                    if !test.is_nan()
                                        && !test.is_global_ref_to(expr_ctx, "undefined")
                                    {
                ...
                if let Some(i) = selected {
                    if !has_conditional_stopper(&s.cases[i].cons) {
                        ...
                        exprs.extend(ignore_result(s.discriminant, true, expr_ctx));

                        let mut stmts = s.cases[i].cons.take();
                        let mut decls = s.cases[0..i]
                            .iter()
                        ...
                        decls.extend(
                            cases
                                .flat_map(|case| {
                                    exprs.extend(
                                        case.test.and_then(|e| ignore_result(e, true, expr_ctx)),
                                    );
                                    case.cons
                                })
                        ...
                        if let Some(i) = idx {
                            for case in &s.cases[..i] {
                                for cons in &case.cons {
                                    vars.extend(cons.extract_var_ids().into_iter().map(|name| {
                                        VarDeclarator {
                                            span: DUMMY_SP,
                                            name: name.into(),
                                            init: None,
                    ...
                    s.cases = s.cases.move_flat_map(|case| {
                        ...
                        let res = match case.test {
                                ...
                                ) =>
                            {
                                ...
                                if cfg!(feature = "debug") {
                                    debug!(
                                        "Dropping case because it is unreachable (literal test)"
                                    );
                                }
                                None
                            }
...

                {
                    ...
                    let is_all_case_side_effect_free = s.cases.iter().all(|case| {
                        case.test
                            .as_ref()
                            .map(|e| e.is_ident() || !e.may_have_side_effects(expr_ctx))
                            .unwrap_or(true)
                    });
                        ...
                        && !has_conditional_stopper(&s.cases.last().unwrap().cons)
                    {
                        ...
                        exprs.extend(
                            s.cases
                                ...
                                .filter_map(|e| ignore_result(e, true, expr_ctx)),
                        );
            ...
            Stmt::While(s) => {
                if let (purity, Known(v)) = s.test.cast_to_bool(expr_ctx) {
                    if v {
                        if purity.is_pure() {
                            WhileStmt {
                                test: Lit::Bool(Bool {
            ...
            Stmt::DoWhile(s) => {
                ...
                if let Known(v) = s.test.as_pure_bool(expr_ctx) {
                    if v {
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
pub fn optimize_for_stmt(s: &mut ForStmt, expr_ctx: ExprCtx, _changed: &mut bool) {
    s.init = s.init.take().and_then(|e| match e {
        VarDeclOrExpr::Expr(e) => ignore_result(e, true, expr_ctx).map(VarDeclOrExpr::from),
        _ => Some(e),
    });
    ...
    s.update = s
        .update
        .take()
        .and_then(|e| ignore_result(e, true, expr_ctx));

    s.test = s.test.take().and_then(|e| {
        if let Known(value) = e.as_pure_bool(expr_ctx) {
            return if value {
                None
            } else {
                let span = e.span();
...
/// **NOTE**: This is **NOT** a public API. DO NOT USE IT.
pub fn optimize_switch_stmt(s: &mut SwitchStmt, expr_ctx: ExprCtx, changed: &mut bool) {
    if s.cases
        .iter()
        .any(|case| matches!(case.test.as_deref(), Some(Expr::Update(..))))
    {
    ...
    if s.cases.iter().all(|case| {
        if let Some(test) = case.test.as_deref() {
            if test.may_have_side_effects(expr_ctx) {
                return false;
            }
        }
        ...
        if cfg!(feature = "debug") {
            debug!("Dropping a switch statement because all cases are unreachable");
        }
    ...
    T: StmtLike,
{
    ...
    let mut iter = take(stmts).into_iter();
            ...
            Ok(stmt) => {
                let stmt = match stmt {
                    ...
                    Stmt::If(IfStmt {
                        ...
                        // check if
                        match test.cast_to_bool(expr_ctx) {
                            (purity, Known(val)) => {
                                *changed = true;
                                if !purity.is_pure() {
                                    let expr = ignore_result(test, true, expr_ctx);
        ...
        stmts.move_flat_map(|s| {
            ...
            match s {
                ...
                Stmt::Block(block) => {
                    ...
                    Some(Stmt::Block(BlockStmt { stmts, ..block }))
                }
                ...
                Stmt::Labeled(l) => {
                    ...
                    Some(Stmt::Labeled(LabeledStmt { body, ..l }))
                }
