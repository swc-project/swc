...
//! how they are used (assigned to variables, passed as arguments to
//! hooks/functions, used as JSX props, etc.). These names appear in React
...
use react_compiler_hir::{
    environment::Environment, object_shape::HookKind, FunctionId, HirFunction, IdentifierId,
    IdentifierName, Instruction, InstructionValue, JsxAttribute, JsxTag, PlaceOrSpread,
};

/// Assign generated names to anonymous function expressions.
///
/// Ported from TS `nameAnonymousFunctions` in
...
pub fn name_anonymous_functions(func: &mut HirFunction, env: &mut Environment) {
    ...
    // Update name_hint on FunctionExpression instruction values in the outer
    // function
    apply_name_hints_to_instructions(&mut func.instructions, &update_map);

    // Update name_hint on FunctionExpression instruction values in all arena
    // functions
...
struct Node {
    ...
    /// The generated name for this anonymous function (set based on usage
    /// context)
    generated_name: Option<String>,
    /// The existing `name` on the FunctionExpression (non-anonymous functions
    /// have this)
