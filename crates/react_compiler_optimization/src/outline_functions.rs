...
//! Port of OutlineFunctions from TypeScript
...
//! replaced with a `LoadGlobal` referencing the outlined function's generated
//! name.
//!
...
use react_compiler_hir::{
    environment::Environment, FunctionId, HirFunction, IdentifierId, InstructionValue,
    NonLocalBinding,
};
use react_compiler_ssa::enter_ssa::placeholder_function;

/// Outline anonymous function expressions that have no captured context
/// variables.
///
...
pub fn outline_functions(
    ...
    // Collect per-instruction actions to maintain depth-first name allocation
    // order. Each entry: (instr index, function_id to recurse into,
    // should_outline)
    enum Action {
    ...
    // Process actions sequentially: for each instruction, recurse first
    // (depth-first), then generate name and outline. This matches TS ordering
    // where inner functions get names allocated before outer ones.
