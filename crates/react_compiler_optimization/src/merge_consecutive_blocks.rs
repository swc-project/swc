...
use react_compiler_hir::{
    visitors, AliasingEffect, BlockId, BlockKind, Effect, HirFunction, Instruction, InstructionId,
    InstructionValue, Place, Terminal, GENERATED_SOURCE,
};
        ...
        let phis: Vec<_> = block
            ...
            .map(|phi| {
                assert_eq!(
                    phi.operands.len(),
                    1,
                    "Found a block with a single predecessor but where a phi has multiple ({}) \
                     operands",
                    phi.operands.len()
                );
