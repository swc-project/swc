...
//! We use abstract interpretation to record known constant values for
//! identifiers, with lack of a value indicating that the identifier does not
//! have a known constant value.
//!
//! Instructions which can be compile-time evaluated *and* whose operands are
//! known constants are replaced with the resulting constant value.
//!
//! This pass also exploits SSA form, tracking constant values of local
//! variables. For example, in `let x = 4; let y = x + 1` we know that `x = 4`
//! in the binary expression and can replace it with `Constant 5`.
//!
...
use react_compiler_hir::{
    environment::Environment, BinaryOperator, BlockKind, FloatValue, FunctionId, GotoVariant,
    HirFunction, IdentifierId, InstructionValue, NonLocalBinding, Phi, Place, PrimitiveValue,
    PropertyLiteral, SourceLocation, Terminal, UnaryOperator, UpdateOperator,
};
...
fn constant_propagation_impl(
    ...
    loop {
        ...
        // Now that predecessors are updated, prune phi operands that can never be
        // reached
        ...
        // TODO: port assertConsistentIdentifiers(fn) and
        // assertTerminalSuccessorsExist(fn) from TS HIR validation.
        // These are debug assertions that verify structural invariants
        // after the CFG cleanup helpers run.
    }
}
    ...
    match &instr.value {
        ...
        InstructionValue::LoadLocal { place, .. } => {
            ...
            if let Some(ref constant) = place_value {
                // Replace the LoadLocal with the constant value (including the constant's
                // original loc)
                func.instructions[instr_id.0 as usize].value =
...
/// Supports Unicode identifier characters per ECMAScript spec (ID_Start /
/// ID_Continue). Rejects JS reserved words (matching Babel's
/// `isValidIdentifier` default behavior).
...
/// Check if a character is valid as a continuation of a JS identifier
/// (ID_Continue + $ + \u200C + \u200D).
...
/// In JS: `""` → 0, `" "` → 0, `" 42 "` → 42, `"0x1A"` → 26, `"Infinity"` →
/// Infinity.
...
fn js_number_to_string(n: f64) -> String {
    ...
    // This may diverge from JS for edge cases around exponential notation
    // thresholds
    format!("{}", n)
}
