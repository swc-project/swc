...
//! This pass is conditional on `env.config.enable_jsx_outlining` (defaults to
//! false).
...
use react_compiler_hir::{
    environment::Environment, BasicBlock, BlockId, BlockKind, EvaluationOrder, FunctionId,
    HirFunction, IdentifierId, IdentifierName, Instruction, InstructionId, InstructionKind,
    InstructionValue, JsxAttribute, JsxTag, LValuePattern, NonLocalBinding, ObjectPattern,
    ObjectProperty, ObjectPropertyKey, ObjectPropertyOrSpread, ObjectPropertyType, ParamPattern,
    Pattern, Place, ReactFunctionType, ReturnVariant, Terminal, HIR,
};

/// Outline JSX expressions in inner functions into separate outlined
/// components.
///
...
fn process_and_outline_jsx(
    ...
    if let Some(result) = result {
        ...
        // Map from the LAST JSX instruction's eval order to the replacement
        // instructions In the TS code, `state.jsx.at(0)` is the first element
        // pushed during reverse iteration, which is the last JSX in forward
        // block order (highest eval order). After sorting by eval_order
        // ascending, that's jsx_group.last().
...
fn collect_props(
    ...
    let mut generate_name = |old_name: &str, _env: &mut Environment| -> String {
        ...
        // We don't have programContext in Rust, but this is needed for unique name
        // tracking
        new_name
    };
