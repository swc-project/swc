...
//! and fully inline the code. The original return is replaced with an
//! assignment to the IIFE's call expression lvalue.
//!
...
use react_compiler_hir::{
    environment::Environment, visitors, BasicBlock, BlockId, BlockKind, EvaluationOrder,
    FunctionId, GotoVariant, HirFunction, IdentifierId, IdentifierName, Instruction, InstructionId,
    InstructionKind, InstructionValue, LValue, Place, Terminal, GENERATED_SOURCE,
};
...
/// Inline immediately invoked function expressions into the enclosing
...
/// Returns true for "block" and "catch" block kinds which correspond to
/// statements in the source.
fn is_statement_block_kind(kind: BlockKind) -> bool {
    matches!(kind, BlockKind::Block | BlockKind::Catch)
}

/// Returns true if the function has a single exit terminal (throw/return) which
/// is a return.
