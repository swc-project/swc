...
//!
//! When validation flags are set, inserts `StartMemoize`/`FinishMemoize`
//! markers.
//!
...
use react_compiler_hir::{
    environment::Environment, ArrayElement, DependencyPathEntry, Effect, EvaluationOrder,
    HirFunction, IdentifierId, IdentifierName, Instruction, InstructionId, InstructionValue,
    ManualMemoDependency, ManualMemoDependencyRoot, Place, PlaceOrSpread, PropertyLiteral,
    SourceLocation,
};
...
pub fn drop_manual_memoization(
    ...
    //
    // queued_inserts maps InstructionId -> new Instruction to insert after that
    // instruction
    let mut queued_inserts: HashMap<InstructionId, Instruction> = HashMap::new();

    // Collect all block instruction lists up front to avoid borrowing func
    // immutably while needing to mutate it
...
fn process_manual_memo_call(
    ...
    if is_validation_enabled {
        // Bail out when we encounter manual memoization without inline function
        // expressions
...
fn collect_temporaries(
    ...
    match &instr.value {
        ...
        InstructionValue::LoadGlobal { binding, .. } => {
            ...
            // - The behavior is equivalent for direct `useMemo`/`useCallback` imports and
            //   `React.useMemo`/`React.useCallback` member accesses (handled below)
            // TODO: Use getGlobalDeclaration + getHookKindForType once the type system is
            // ported.
            if name == "useMemo" {
            ...
            env.record_diagnostic(
                CompilerDiagnostic::new(
                    ...
                    Some(if kind == ManualMemoKind::UseCallback {
                        ...
                        "The first argument to useMemo() must be a function that calculates a \
                         result to cache"
                            .to_string()
                    }),
                )
    ...
    for dep in &deps_info.deps {
        ...
        if let Some(d) = maybe_dep {
            ...
            env.record_diagnostic(
                CompilerDiagnostic::new(
                    ErrorCategory::UseMemo,
                    "Expected the dependency list to be an array of simple expressions (e.g. `x`, \
                     `x.y.z`, `x?.y?.z`)",
                    Some(
                        "Expected the dependency list to be an array of simple expressions (e.g. \
                         `x`, `x.y.z`, `x?.y?.z`)"
                            .to_string(),
                    ),
                )
