...
/// Prune `MaybeThrow` terminals for blocks that cannot throw, then clean up the
/// CFG.
                        ...
                        let mapped_terminal =
                            terminal_mapping.get(predecessor).copied().ok_or_else(|| {
                                CompilerDiagnostic::new(
                                    ...
                                    "Expected non-existing phi operand's predecessor to have been \
                                     mapped to a new terminal",
                                    Some(format!(
