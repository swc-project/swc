...
//!
//! Eliminates instructions whose values are unused, reducing generated code
//! size. Performs mark-and-sweep analysis to identify and remove dead code
//! while preserving side effects and program semantics.
//!
...
use react_compiler_hir::{
    ...
    visitors, ArrayPatternElement, BlockId, BlockKind, HirFunction, IdentifierId, InstructionKind,
    InstructionValue, ObjectPropertyOrSpread, Pattern,
};

/// Implements dead-code elimination, eliminating instructions whose values are
/// unused.
///
...
pub fn dead_code_elimination(func: &mut HirFunction, env: &Environment) {
    ...
    // Collect instructions to rewrite (two-phase: collect then apply to avoid
    // borrow conflicts)
...
/// Checks both the specific SSA id and (for named identifiers) any usage of
/// that name.
...
fn find_referenced_identifiers(func: &HirFunction, env: &Environment) -> State {
    ...
    // Collect block ids in reverse order (postorder - successors before
    // predecessors)
...
/// Rewrite a retained instruction (destructuring cleanup, StoreLocal ->
/// DeclareLocal).
...
fn pruneable_value(value: &InstructionValue, state: &State, env: &Environment) -> bool {
    match value {
        ...
        InstructionValue::StoreLocal { lvalue, .. } => {
            if lvalue.kind == InstructionKind::Reassign {
                // Reassignments can be pruned if the specific instance being assigned is never
                // read
                !is_id_used(state, lvalue.place.identifier)
            } else {
                // Declarations are pruneable only if the named variable is never read later
