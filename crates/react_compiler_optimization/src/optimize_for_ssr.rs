...
use react_compiler_hir::{
    ...
    is_set_state_type, is_start_transition_type,
...
pub fn optimize_for_ssr(func: &mut HirFunction, env: &Environment) {
    ...
    // For useReducer(reducer, initialArg, init) store a CallExpression of
    // init(initialArg).
    //
    ...
    //
    // - Replace FunctionExpression with Primitive(undefined) if it calls
    //   setState/startTransition
    // - Remove known event handler props and ref props from builtin JSX tags
    // - Replace Destructure of inlined state with StoreLocal
    // - Replace useEffectEvent(fn) with LoadLocal(fn)
    // - Replace useEffect/useLayoutEffect/useInsertionEffect with
    //   Primitive(undefined)
            ...
            match &instr.value {
                ...
                InstructionValue::Destructure { value, lvalue, loc } => {
                    ...
                    if inlined_state.contains_key(&value_id) {
                        // Invariant: destructuring pattern must be ArrayPattern with at least one
                        // Identifier item
                        if let react_compiler_hir::Pattern::Array(arr) = &lvalue.pattern {
                            if !arr.items.is_empty() {
...
/// Returns true if the function body contains a call to setState or
/// startTransition. This identifies functions that are event handlers and can
/// be replaced with undefined during SSR.
///
...
/// Returns true if the prop name matches the known event handler pattern
/// `on[A-Z]`.
