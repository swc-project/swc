...
/// same)
fn is_param_prop_expr(expr: &Expr) -> bool {
...
/// generated by TypeScript transform (pattern: `this.identifier = identifier`
/// where both identifiers are the same, and the statement has DUMMY_SP span
/// indicating it was generated, not from user code)
fn is_param_prop_init(stmt: &Stmt) -> bool {
    if let Stmt::Expr(ExprStmt { span, expr }) = stmt {
        // Generated param prop inits have DUMMY_SP, user code has real spans
        if *span != DUMMY_SP {
            return false;
        }
        return is_param_prop_expr(expr);
    }
    false
}
