...
fn eliminate_redundant_phi_impl(
    ...
    loop {
        ...
        for block_id in &block_ids {
            ...
            // Find any redundant phis: rewrite operands, identify redundant phis, remove
            // them. Matches TS behavior: each phi's operands are rewritten
            // before checking redundancy, so that rewrites from earlier phis in
            // the same block are visible to later phis.
