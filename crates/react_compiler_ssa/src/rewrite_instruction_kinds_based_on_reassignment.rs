...
//!
//! Note that declarations which were const in the original program cannot
//! become `let`, but the inverse is not true: a `let` which was reassigned in
//! the source may be converted to a `const` if the reassignment is not used and
...
use react_compiler_hir::{
    environment::Environment, visitors::each_pattern_operand, BlockKind, DeclarationId,
    HirFunction, InstructionKind, InstructionValue, ParamPattern, Place,
};

/// Create an invariant CompilerError (matches TS CompilerError.invariant).
/// When a loc is provided, creates a CompilerDiagnostic with an error detail
/// item (matching TS CompilerError.invariant which uses .withDetails()).
...
/// Format a Place like TS `printPlace()`: `<effect>
/// <name>$<id>[<range>]{reactive}`
...
/// map stores references to LValue/LValuePattern and mutates `kind` through
/// them. In Rust, we track instruction indices and apply changes in a second
/// pass.
enum DeclarationLoc {
    /// An LValue from DeclareLocal or StoreLocal — identified by (block_index,
    /// instr_index_in_block)
    Instruction {
        block_index: usize,
        instr_local_index: usize,
    },
    /// A parameter or context variable (seeded as Let, may be upgraded to Let
    /// on reassignment — already Let)
    ParamOrContext,
}
...
pub fn rewrite_instruction_kinds_based_on_reassignment(
    ...
    // Track which (block_index, instr_local_index) should have their lvalue.kind
    // set to Reassign
    ...
    // Track which (block_index, instr_local_index) Destructure instructions get a
    // specific kind
                        ...
                        if ident.name.is_none() {
                            ...
                            if let Some(existing) = declarations.get(&decl_id) {
                                ...
                                if block_kind == BlockKind::Value {
                                    return Err(invariant_error_with_loc(
                                        "TODO: Handle reassignment in a value block where the \
                                         original declaration was removed by dead code \
                                         elimination (DCE)",
                                        None,
                                        place.loc,
                                    ));
