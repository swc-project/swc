...
use std::{borrow::Cow, collections::BTreeMap, fs, fs::File, io::Read, ops::Range, path::Path};
...
use crate::{
    ...
    types::{SourceMap, SourceMapIndex},
};
...
impl<'a> RamBundle<'a> {
    ...
    /// as an entry point (startup code) for the app. The modules are stored in
    /// js-modules/ directory, next to the entry point. The js-modules/
    /// directory must ONLY contain files with integer names and the ".js"
    /// file suffix, along with the UNBUNDLE magic file.
...
///
/// With the help of the RAM bundle's indexed sourcemap, the bundle is split
/// into modules, where each module is represented by its minified source and
/// the corresponding sourcemap that we recover from the initial indexed
/// sourcemap.
...
/// Returns "true" if the given path points to the startup file of a file RAM
/// bundle
///
/// The method checks the directory structure and the magic number in UNBUNDLE
/// file.
