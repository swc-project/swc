...
//! Babel-compatible AST with memoization (useMemoCache) wired in.
//!
//! This is the final pass in the compilation pipeline.
//!
//! Corresponds to `src/ReactiveScopes/CodegenReactiveFunction.ts` in the TS
//! compiler.

use std::collections::{HashMap, HashSet};

use react_compiler_ast::{
    common::{BaseNode, Position as AstPosition, SourceLocation as AstSourceLocation},
    expressions::{self as ast_expr, ArrowFunctionBody, Expression, Identifier as AstIdentifier},
    jsx::{
    ...
    literals::{
        BooleanLiteral, NullLiteral, NumericLiteral, RegExpLiteral as AstRegExpLiteral,
    ...
    patterns::{
        ArrayPattern as AstArrayPattern, ObjectPatternProp, ObjectPatternProperty, PatternLike,
        RestElement,
    },
    statements::{
        BlockStatement, BreakStatement, CatchClause, ContinueStatement, DebuggerStatement,
        Directive, DirectiveLiteral, DoWhileStatement, EmptyStatement, ExpressionStatement,
...
use crate::{
    build_reactive_function::build_reactive_function,
    prune_hoisted_contexts::prune_hoisted_contexts,
    prune_unused_labels::prune_unused_labels,
    prune_unused_lvalues::prune_unused_lvalues,
    rename_variables::rename_variables,
    visitors::{visit_reactive_function, ReactiveFunctionVisitor},
};
...
pub fn codegen_function(
    ...
    // Per-hook-call wrapping is done inline during codegen
    ...
    if cache_count != 0 {
        ...
        if let Some((cache_index, ref hash)) = fast_refresh_state {
            ...
            // if ($[cacheIndex] !== "hash") { for (let $i = 0; $i < N; $i += 1) { $[$i] =
            // Symbol.for("react.memo_cache_sentinel"); } $[cacheIndex] = "hash"; }
...
/// Extract lval and declaration kind from a for-in/for-of iterable item
/// instruction.
    ...
    if let ReactiveValue::Instruction(ref value) = instr.value {
        match value {
            ...
            InstructionValue::UnsupportedNode {
                ...
                // We have the original AST node serialized as JSON; deserialize and emit it
                // directly
                ...
                match stmt {
                    ...
                    Statement::VariableDeclaration(ref var_decl) => {
                        ...
                        cx.record_error(CompilerErrorDetail {
                            category: ErrorCategory::Todo,
                            reason: format!(
                                "(CodegenReactiveFunction::codegenInstructionValue) Cannot \
                                 declare variables in a value block"
                            ),
                            description: None,
                            loc: None,
                            suggestions: None,
                    ...
                    _ => {
                        cx.record_error(CompilerErrorDetail {
                            category: ErrorCategory::Todo,
                            reason: format!(
                                "(CodegenReactiveFunction::codegenInstructionValue) Handle \
                                 conversion of statement to expression"
                            ),
                            description: None,
                            loc: None,
                            suggestions: None,
    ...
    match iv {
            ...
            if !matches!(
                ...
                };
                {
                    ...
                    err.push_diagnostic(
                        CompilerDiagnostic::new(
                            ErrorCategory::Invariant,
                            "[Codegen] Internal error: MethodCall::property must be an unpromoted \
        ...
        InstructionValue::TypeCastExpression {
            ...
            let wrapped = match (type_annotation_kind.as_deref(), type_annotation) {
                ...
                (Some("as"), Some(ta)) => Expression::TSAsExpression(ast_expr::TSAsExpression {
        ...
        InstructionValue::UnsupportedNode {
            ...
            // Try to deserialize the original AST node from JSON (mirrors statement-level
            // handler)
            match original_node {
                Some(node) => {
...
fn codegen_place(cx: &mut Context, place: &Place) -> Result<ExpressionOrJsxText, CompilerError> {
    ...
    // Override identifier loc with place.loc, matching TS: identifier.loc =
    // place.loc
...
fn convert_identifier(
    ...
    let name = match &ident.name {
        ...
        Some(react_compiler_hir::IdentifierName::Promoted(n)) => n.clone(),
        None => {
            // Use CompilerDiagnostic (with details array) to match TS
            // CompilerError.invariant() which creates a CompilerDiagnostic with
            // details: [{kind: "error", loc, message}].
            let reason = "Expected temporaries to be promoted to named identifiers in an earlier \
                          pass"
                .to_string();
...
/// Apply a source location to an ExpressionOrJsxText value, matching the TS
/// behavior where `value.loc = instrValue.loc` is set at the end of
...
/// Conditionally wrap a call expression in a hook guard IIFE if
/// enableEmitHookGuards is enabled and the callee is a hook.
...
/// Wraps the call in: `(function() { try { $guard(before); return callExpr; }
/// finally { $guard(after); } })()`
