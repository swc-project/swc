...
use react_compiler_hir::{
    environment::Environment, BlockId, ReactiveFunction, ReactiveTerminal,
...
impl<'a> ReactiveFunctionVisitor for Visitor<'a> {
    ...
    fn visit_terminal(&self, stmt: &ReactiveTerminalStatement, seen_labels: &mut HashSet<BlockId>) {
        ...
        // matching TS behavior where visitTerminal override does not call
        // traverseTerminal. Recursion into child blocks happens via
        // traverseBlock→visitTerminal for nested blocks. The TS visitor
        // only checks break/continue at the block level, not terminal child
        // blocks.
    }
}
