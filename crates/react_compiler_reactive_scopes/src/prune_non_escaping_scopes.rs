...
use react_compiler_hir::{
    environment::Environment, visitors::each_instruction_value_operand, ArrayPatternElement,
    DeclarationId, Effect, EvaluationOrder, IdentifierId, InstructionKind, InstructionValue,
    JsxAttribute, JsxTag, ObjectPropertyOrSpread, Pattern, Place, PlaceOrSpread, ReactiveFunction,
    ReactiveInstruction, ReactiveScopeBlock, ReactiveStatement, ReactiveTerminal,
    ReactiveTerminalStatement, ReactiveValue, ScopeId,
};

use crate::visitors::{
    transform_reactive_function, visit_reactive_function, ReactiveFunctionTransform,
    ReactiveFunctionVisitor, Transformed,
};
...
pub fn prune_non_escaping_scopes(
    ...
    // First build up a map of which instructions are involved in creating which
    // values, and which values are returned.
...
enum MemoizationLevel {
    ...
    /// don't need to be memoized
    Unmemoized,
    /// The value will never be memoized: used for values that can be cheaply
    /// compared w Object.is
    Never,
}
...
impl<'a> ReactiveFunctionVisitor for CollectDependenciesVisitor<'a> {
    ...
    fn visit_terminal(&self, stmt: &ReactiveTerminalStatement, state: &mut Self::State) {
        ...
        if let ReactiveTerminal::Return { value, .. } = &stmt.terminal {
            ...
            // If the return is within a scope, associate those scopes with the returned
            // value
    ...
    fn visit_scope(&self, scope: &ReactiveScopeBlock, state: &mut Self::State) {
        ...
        // If a scope reassigns any variables, set the chain of active scopes as a
        // dependency of those variables.
...
fn compute_memoized_identifiers(state: &CollectState) -> HashSet<DeclarationId> {
    ...
    // We need mutable access to the nodes, so we clone the state into mutable
    // structures
