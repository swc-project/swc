...
use react_compiler_hir::{
    environment::Environment, EvaluationOrder, Place, ReactiveFunction, ReactiveScopeBlock, ScopeId,
};
...
impl<'a> ReactiveFunctionVisitor for CheckInstructionsAgainstScopesVisitor<'a> {
    ...
    fn visit_place(&self, id: EvaluationOrder, place: &Place, state: &mut CheckState) {
        // getPlaceScope: check if the place's identifier has a scope that is active at
        // this id
        ...
        if let Some(scope_id) = identifier.scope {
                ...
                && !state.active_scopes.contains(&scope_id)
            {
                state.error = Some(CompilerDiagnostic::new(
                    ...
                    "Encountered an instruction that should be part of a scope, but where that \
                     scope has already completed",
                    Some(format!(
                        "Instruction [{:?}] is part of scope @{:?}, but that scope has already \
                         completed",
                        id, scope_id
                    )),
                ));
