...
use react_compiler_hir::{
    environment::Environment, BasicBlock, BlockId, EvaluationOrder, GotoVariant, HirFunction,
    InstructionValue, Place, PrunedReactiveScopeBlock, ReactiveBlock, ReactiveFunction,
    ReactiveInstruction, ReactiveLabel, ReactiveScopeBlock, ReactiveStatement, ReactiveSwitchCase,
...
impl<'a, 'b> Driver<'a, 'b> {
    ...
    fn visit_block(
        ...
        match &terminal {
                ...
                for case in cases.iter().rev() {
                    ...
                    if self.cx.is_scheduled(case_block_id) {
                        ...
                        if case_block_id != *fallthrough {
                            return Err(CompilerDiagnostic::new(
                                ErrorCategory::Invariant,
                                "Unexpected 'switch' where a case is already scheduled and block \
            ...
            Terminal::DoWhile {
                ...
                schedule_ids.push(
                    self.cx
                        .schedule_loop(*fallthrough, *test, Some(*loop_block))?,
                );
            ...
            Terminal::ForOf {
                ...
                schedule_ids.push(
                    self.cx
                        .schedule_loop(*fallthrough, *init, Some(*loop_block))?,
                );
    ...
    fn extract_value_block_result(
        ...
        // convert it to a LoadLocal of the value being stored, matching the TS
        // behavior.
    ...
    /// Converts the result of visit_value_block into a SequenceExpression that
    /// includes the instruction with its lvalue. This is needed for
    /// for/for-of/for-in init/test blocks where the instruction's lvalue
    /// assignment must be preserved.
    ///
    /// This also flattens nested SequenceExpressions that can occur from
    /// MaybeThrow handling in try-catch blocks.
    ///
