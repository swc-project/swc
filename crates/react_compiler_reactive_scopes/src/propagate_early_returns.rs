...
//! PropagateEarlyReturns — ensures reactive blocks honor early return
//! semantics.
//!
//! When a scope contains an early return, creates a sentinel-based check so
//! that cached scopes can properly replay the early return behavior.
//!
//! Corresponds to `src/ReactiveScopes/PropagateEarlyReturns.ts`.

use react_compiler_hir::{
    environment::Environment, BlockId, Effect, EvaluationOrder, IdentifierId, IdentifierName,
    InstructionKind, InstructionValue, LValue, NonLocalBinding, Place, PlaceOrSpread,
    PrimitiveValue, PropertyLiteral, ReactiveFunction, ReactiveInstruction, ReactiveLabel,
...
pub fn propagate_early_returns(func: &mut ReactiveFunction, env: &mut Environment) {
    ...
    // The TS version doesn't produce errors from this pass, so we ignore the
    // Result.
