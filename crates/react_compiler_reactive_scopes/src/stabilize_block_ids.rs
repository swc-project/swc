...
use react_compiler_hir::{
    environment::Environment, BlockId, ReactiveFunction, ReactiveScopeBlock, ReactiveTerminal,
...
pub fn stabilize_block_ids(func: &mut ReactiveFunction, env: &mut Environment) {
    // Pass 1: Collect referenced labels (preserving insertion order to match TS Set
    // behavior)
    ...
    // Build mappings: referenced block IDs -> sequential IDs (insertion-order
    // deterministic)
...
/// TS: `class RewriteBlockIds extends ReactiveFunctionVisitor<Map<BlockId,
/// BlockId>>`
