...
use react_compiler_hir::{
    environment::Environment, DeclarationId, EvaluationOrder, FunctionId, IdentifierName,
    InstructionValue, ParamPattern, Place, PrunedReactiveScopeBlock, ReactiveBlock,
    ReactiveFunction, ReactiveScopeBlock, ReactiveValue,
};
...
impl ReactiveFunctionVisitor for Visitor<'_> {
    ...
    /// TS: `visitBlock(block, state) { state.enter(() => {
    ...
    /// TS: `visitPrunedScope(scopeBlock, state) {
    /// this.traverseBlock(scopeBlock.instructions, state) }` No enter/leave
    /// — names assigned inside pruned scopes remain visible in
    ...
    /// TS: `visitScope(scope, state) { for (const [_, decl] of
    /// scope.scope.declarations) state.visit(decl.identifier);
    ...
    /// TS: `visitValue(id, value, state) { this.traverseValue(id, value,
    /// state); if (value.kind === 'FunctionExpression' || value.kind ===
    /// 'ObjectMethod') this.visitHirFunction(value.loweredFunc.func, state) }`
