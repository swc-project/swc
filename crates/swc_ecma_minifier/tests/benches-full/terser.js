!function(global1, factory) {
    ...
    function get_full_char(str, pos1) {
        if (is_surrogate_pair_head(str.charCodeAt(pos1))) {
            if (is_surrogate_pair_tail(str.charCodeAt(pos1 + 1))) return str.charAt(pos1) + str.charAt(pos1 + 1);
        } else if (is_surrogate_pair_tail(str.charCodeAt(pos1)) && is_surrogate_pair_head(str.charCodeAt(pos1 - 1))) return str.charAt(pos1 - 1) + str.charAt(pos1);
        return str.charAt(pos1);
    }
    ...
    class JS_Parse_Error extends Error {
        constructor(message, filename, line, col, pos1){
            super(), this.name = "SyntaxError", this.message = message, this.filename = filename, this.line = line, this.col = col, this.pos = pos1;
        }
    }
    function js_error(message, filename, line, col, pos1) {
        throw new JS_Parse_Error(message, filename, line, col, pos1);
    }
    ...
    function parse($TEXT, options) {
        ...
        var S = {
            input: "string" == typeof $TEXT ? function($TEXT, filename, html5_comments, shebang) {
                ...
                function find(what, signal_eof) {
                    var pos1 = S.text.indexOf(what, S.pos);
                    if (signal_eof && -1 == pos1) throw EX_EOF;
                    return pos1;
                }
                ...
                function token(type, value, is_comment) {
                    ...
                    const line = S.tokline, col = S.tokcol, pos1 = S.tokpos, nlb = S.newline_before;
                    ...
                    const tok = new AST_Token(type, value, line, col, pos1, nlb, comments_before, comments_after, filename);
        ...
        function croak(msg, line, col, pos1) {
            var ctx = S.input.context();
            js_error(msg, ctx.filename, null != line ? line : ctx.tokline, null != col ? col : ctx.tokcol, null != pos1 ? pos1 : ctx.tokpos);
        }
    ...
    class AST_Token {
        constructor(type, value, line, col, pos1, nlb, comments_before, comments_after, file){
            this.flags = nlb ? 1 : 0, this.type = type, this.value = value, this.line = line, this.col = col, this.pos = pos1, this.comments_before = comments_before, this.comments_after = comments_after, this.file = file, Object.seal(this);
        }
        get nlb() {
    ...
    function OutputStream(options) {
        ...
        var to_utf8 = options.ascii_only ? function(str, identifier = !1, regexp = !1) {
            return !(options.ecma >= 2015) || options.safari10 || regexp || (str = str.replace(/[\ud800-\udbff][\udc00-\udfff]/g, function(ch) {
                return "\\u{" + (is_surrogate_pair_head(ch.charCodeAt(0)) ? 0x10000 + (ch.charCodeAt(pos) - 0xd800 << 10) + ch.charCodeAt(pos + 1) - 0xdc00 : ch.charCodeAt(pos)).toString(16) + "}";
    ...
    AST_Scope.DEFMETHOD("drop_unused", function(compressor) {
        ...
        if (compressor.option("unused") && simple_args && is_func && !fn.uses_arguments) {
            for(var pos1 = 0, last = 0, i = 0, len = self1.args.length; i < len; i++){
                if (fn.argnames[i] instanceof AST_Expansion) {
                    if (has_flag(fn.argnames[i].expression, 0b00000001)) for(; i < len;){
                        var node = self1.args[i++].drop_side_effect_free(compressor);
                        node && (self1.args[pos1++] = node);
                    }
                    else for(; i < len;)self1.args[pos1++] = self1.args[i++];
                    last = pos1;
                    break;
                }
                ...
                if (trim || has_flag(fn.argnames[i], 0b00000001)) {
                    var node = self1.args[i].drop_side_effect_free(compressor);
                    if (node) self1.args[pos1++] = node;
                    else if (!trim) {
                        self1.args[pos1++] = make_node(AST_Number, self1.args[i], {
                            value: 0
                        });
                        continue;
                    }
                } else self1.args[pos1++] = self1.args[i];
                last = pos1;
            }
        ...
        if (can_inline) {
            ...
            if (simple_args && !fn.uses_arguments && !(compressor.parent() instanceof AST_Class) && !(fn.name && fn instanceof AST_Function) && (returned_value = function(stat) {
                ...
                if (function(decls, expressions) {
                    ...
                    for(var pos1 = expressions.length, i = 0, lines = fn.body.length; i < lines; i++){
                        var stat = fn.body[i];
                        if (stat instanceof AST_Var) for(var j = 0, defs = stat.definitions.length; j < defs; j++){
                            ...
                            if (append_var(decls, expressions, name, var_def.value), in_loop && fn.argnames.every((argname)=>argname.name != name.name)) {
                                ...
                                def.references.push(sym), expressions.splice(pos1++, 0, make_node(AST_Assign, var_def, {
                                    operator: "=",
                                    logical: !1,
                                    left: sym,
