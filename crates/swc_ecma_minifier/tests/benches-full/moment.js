...
!function(global, factory) {
    ...
    function(input) {
        ...
        return null == input ? day : (input1 = input, locale = this.localeData(), input = 'string' != typeof input1 ? input1 : !isNaN(input1) ? parseInt(input1, 10) : 'number' == typeof (input1 = locale.weekdaysParse(input1)) ? input1 : null, this.add(input - day, 'd'));
    ...
    proto$1.calendar = function(key, mom, now) {
        ...
        return format || !formatUpper ? format : (this._longDateFormat[key] = formatUpper.match(formattingTokens).map(function(tok) {
            return 'MMMM' !== tok && 'MM' !== tok && 'DD' !== tok && 'dddd' !== tok ? tok : tok.slice(1);
