						...
						t = function (exports) {
							...
							class RistrettoPoint {
								...
								static calcElligatorRistrettoMap(e) {
									const {
										d: t
									} = CURVE, r = mod(SQRT_M1 * e * e), n = mod((r + _1n) * ONE_MINUS_D_SQ);
									let o = BigInt(-1);
									const i = mod((o - t * r) * mod(r + t));
									let {
										isValid: s,
										value: a
									} = uvRatio(n, i), c = mod(a * e);
									edIsNegative(c) || (c = mod(-c)), s || (a = c), s || (o = r);
									const u = mod(o * (r - _1n) * D_MINUS_ONE_SQ - i),
										d = a * a,
										l = mod((a + a) * i),
										f = mod(u * SQRT_AD_MINUS_ONE),
										h = mod(_1n - d),
										p = mod(_1n + d);
									return new ExtendedPoint(mod(l * p), mod(h * f), mod(f * p), mod(l * h))
								}
								static hashToCurve(e) {
									const t = bytes255ToNumberLE((e = ensureBytes(e, 64)).slice(0, 32)),
										r = this.calcElligatorRistrettoMap(t),
										n = bytes255ToNumberLE(e.slice(32, 64)),
										o = this.calcElligatorRistrettoMap(n);
								...
								static fromHex(e) {
									...
									const {
										a: t,
										d: r
									} = CURVE, n = "RistrettoPoint.fromHex: the hex is not valid encoding of RistrettoPoint", o = bytes255ToNumberLE(e);
									if (!equalBytes(numberTo32BytesLE(o), e) || edIsNegative(o)) throw new Error(n);
									const i = mod(o * o),
										s = mod(_1n + t * i),
										a = mod(_1n - t * i),
										c = mod(s * s),
									...
									const m = mod(s * p),
										g = mod(y * m);
									if (!l || edIsNegative(g) || m === _0n) throw new Error(n);
								...
								toRawBytes() {
									...
									if (edIsNegative(n * d)) {
										let r = mod(t * SQRT_M1),
											n = mod(e * SQRT_M1);
										e = r, t = n, l = mod(c * INVSQRT_A_MINUS_D)
									} else l = u;
								...
								equals(e) {
									...
									const t = this.ep,
										r = e.ep,
										n = mod(t.x * r.y) === mod(t.y * r.x),
										o = mod(t.y * r.y) === mod(t.x * r.x);
									return n || o
								}
								add(e) {
							...
							class Point {
								...
								static fromHex(e, t = !0) {
									const {
										d: r,
										P: n
									} = CURVE, o = (e = ensureBytes(e, 32)).slice();
									o[31] = -129 & e[31];
									const i = bytesToNumberLE(o);
									if (t && i >= n) throw new Error("Expected 0 < hex < P");
									if (!t && i >= POW_2_256) throw new Error("Expected 0 < hex < 2**256");
									const s = mod(i * i),
										a = mod(s - _1n),
										c = mod(r * s + _1n);
									let {
							...
							class Signature {
								...
								static fromHex(e) {
									const t = ensureBytes(e, 64),
										r = Point.fromHex(t.slice(0, 32), !1),
										n = bytesToNumberLE(t.slice(32, 64));
