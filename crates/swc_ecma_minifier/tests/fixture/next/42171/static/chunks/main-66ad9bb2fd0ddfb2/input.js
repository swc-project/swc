...
const addLocale = function(path) {
    ...
    if (true) {
        return (0, _normalizeTrailingSlash).normalizePathTrailingSlash((__webpack_require__(4441).addLocale)(path, ...args));
    }
    return path;
};
...
const detectDomainLocale = function() {
    ...
    if (true) {
        return (__webpack_require__(2388).detectDomainLocale)(...args);
    }
};
...
function _initialize() {
    _initialize = _async_to_generator(function*() {
        ...
        if (true) {
            const { normalizeLocalePath  } = __webpack_require__(4769);
            const { detectDomainLocale  } = __webpack_require__(2388);
            const { parseRelativeUrl  } = __webpack_require__(2864);
            const { formatUrl  } = __webpack_require__(7795);
