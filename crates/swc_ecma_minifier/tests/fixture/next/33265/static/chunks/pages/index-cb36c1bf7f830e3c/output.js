        ...
        562: function(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
            ...
            var bytesToNumber = function(bytes, _temp) {
                ...
                if (void 0 !== _ref$signed && _ref$signed) {
                    ...
                    (number = BigInt(number)) > max && (number = (number = (number -= max) - max) - BigInt(2));
                }
                ...
                !function() {
                    for(var styleBox, cue, i = 0; i < cues.length; i++)cue = cues[i], styleBox = new CueStyleBox(window1, cue, styleOptions), paddedOverlay.appendChild(styleBox.div), function(window1, styleBox, containerBox, boxPositions) {
                        ...
                        if (cue.snapToLines) {
                            ...
                            Math.abs(position) > maxPosition && (position = (position = position < 0 ? -1 : 1) * (Math.ceil(maxPosition / step) * step)), linePos < 0 && (position += "" === cue.vertical ? containerBox.height : containerBox.width, axis = axis.reverse()), boxPosition.move(initialAxis, position);
                        } else {
