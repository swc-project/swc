    ...
    ts.transformModule = function(context) {
        ...
        context.onSubstituteNode = function(hint, node) {
            return (node = previousOnSubstituteNode(hint, node)).id && noSubstitution[node.id] ? node : 1 === hint ? function(node) {
                ...
                return node;
            }(node) : ts.isShorthandPropertyAssignment(node) ? function(node) {
                ...
                case 271:
                    return function(node) {
                        if (node.moduleSpecifier) {
                            var node1, innerExpr, generatedName = factory.getGeneratedNameForNode(node);
