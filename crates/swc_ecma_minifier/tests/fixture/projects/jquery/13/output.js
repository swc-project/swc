    ...
    if (jQuery.acceptData(elem)) {
        ...
        if (cache[id]) {
            if (name && (thisCache = pvt ? cache[id] : cache[id].data)) {
                for(jQuery.isArray(name) ? name = name.concat(jQuery.map(name, jQuery.camelCase)) : (name in thisCache) ? name = [
                    name
                ] : (name = jQuery.camelCase(name), name = (name in thisCache) ? [
                    name
                ] : name.split(" ")), i = 0, l = name.length; i < l; i++)delete thisCache[name[i]];
