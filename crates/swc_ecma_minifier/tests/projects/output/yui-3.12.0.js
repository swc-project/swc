...
!function() {
    var proto, prop, VERSION = "3.12.0", BASE = "http://yui.yahooapis.com/", DOC_LABEL = "yui3-js-enabled", CSS_STAMP_EL = "yui3-css-stamp", NOOP = function() {}, SLICE = Array.prototype.slice, APPLY_TO_AUTH = {
        "io.xdrReady": 1,
        "io.xdrResponse": 1,
        "SWF.eventHandler": 1
        ...
        return mods && mods.loader && (lCore = [].concat(lCore, YUI.Env.loaderExtras)), YUI.Env.core = Y.Array.dedupe([].concat(YUI.Env.core, lCore)), loader;
    }, clobber = function(r, s) {
        for(var i in s)s.hasOwnProperty(i) && (r[i] = s[i]);
    }, ALREADY_DONE = {
        success: !0
    };
    if (docEl && -1 == docClass.indexOf(DOC_LABEL)) {
        docClass && (docClass += " ");
        docClass += DOC_LABEL;
    ...
    proto = {
        applyConfig: function(o) {
            ...
            for(name in o)if (o.hasOwnProperty(name)) {
                attr = o[name];
                if (mods && "modules" == name) clobber(mods, attr);
                else if (aliases && "aliases" == name) clobber(aliases, attr);
                else if (groups && "groups" == name) clobber(groups, attr);
                else if ("win" == name) {
                    config[name] = attr && attr.contentWindow || attr;
                    config.doc = config[name] ? config[name].document : null;
                } else "_yuid" == name || (config[name] = attr);
            }
        ...
        _init: function() {
            ...
            if (!Env) {
                ...
                if (G_ENV && Y !== YUI) {
                    ...
                    for(prop in G_ENV)prop in Env || (Env[prop] = G_ENV[prop]);
                    delete YUI._YUI;
                }
                Y.id = Y.stamp(Y);
                instances[Y.id] = Y;
            ...
            if (doc && !doc.getElementById(CSS_STAMP_EL)) {
                (el = doc.createElement("div")).innerHTML = '<div id="' + CSS_STAMP_EL + '" style="position: absolute !important; visibility: hidden !important"></div>';
                YUI.Env.cssStampEl = el.firstChild;
                doc.body ? doc.body.appendChild(YUI.Env.cssStampEl) : docEl.insertBefore(YUI.Env.cssStampEl, docEl.firstChild);
            } else doc && doc.getElementById(CSS_STAMP_EL) && !YUI.Env.cssStampEl && (YUI.Env.cssStampEl = doc.getElementById(CSS_STAMP_EL));
        ...
        applyTo: function(id, method, args) {
            ...
            if (instance) {
                nest = method.split(".");
                m = instance;
                for(i = 0; i < nest.length; i += 1)(m = m[nest[i]]) || this.log("applyTo not found: " + method, "warn", "yui");
        ...
        add: function(name, fn, version, details) {
            ...
            for(i in instances)if (instances.hasOwnProperty(i) && !applied[(inst = instances[i]).id]) {
                applied[inst.id] = !0;
        ...
        _attach: function(r, moot) {
            ...
            for(i = 0; i < len; i++){
                mod = mods[name = r[i]];
                c.push(name);
                if (loader && loader.conditions[name]) for(j in loader.conditions[name])loader.conditions[name].hasOwnProperty(j) && (def = loader.conditions[name][j]) && (def.ua && Y.UA[def.ua] || def.test && def.test(Y)) && c.push(def.name);
            }
            len = (r = c).length;
            for(i = 0; i < len; i++)if (!done[r[i]]) {
                mod = mods[name = r[i]];
                ...
                if (mod) {
                    ...
                    for(j = 0; j < Y.Env._missed.length; j++)if (Y.Env._missed[j] === name) {
                    ...
                    if (loader && cache && cache[name] && cache[name].temp) {
                        ...
                        req = [];
                        for(j in loader.moduleInfo[name].expanded_map)loader.moduleInfo[name].expanded_map.hasOwnProperty(j) && req.push(j);
                        Y._attach(req);
                    }
                    ...
                    if (!moot && name && -1 === name.indexOf("skin-") && -1 === name.indexOf("css")) {
        ...
        use: function() {
            ...
            if (Y.Lang.isFunction(callback)) {
                args.pop();
                Y.config.delayUntil && (callback = Y._delayCallback(callback, Y.config.delayUntil));
            } else callback = null;
            ...
            } else Y._use(args, function(Y, response) {
        ...
        _use: function(args, callback) {
            ...
            var len, loader, handleBoot, i, Y = this, G_ENV = YUI.Env, mods = G_ENV.mods, Env = Y.Env, used = Env._used, aliases = G_ENV.aliases, queue = G_ENV._loaderQueue, firstArg = args[0], YArray = Y.Array, config = Y.config, boot = config.bootstrap, missing = [], r = [], fetchCSS = config.fetchCSS, process1 = function(names, skip) {
                ...
                if (names.length) {
                    if (aliases) {
                        len = names.length;
                        for(i = 0; i < len; i++)aliases[names[i]] && !mods[names[i]] ? a = [].concat(a, aliases[names[i]]) : a.push(names[i]);
                        names = a;
                    }
                    len = names.length;
                    for(i = 0; i < len; i++){
                        ...
                        if (!used[name]) {
                            ...
                            if (m) {
                                ...
                                use = m.details.use;
                            } else G_ENV._loaded[VERSION][name] ? used[name] = !0 : missing.push(name);
                            req && req.length && process1(req);
                            use && use.length && process1(use, 1);
                        }
                    }
                }
            ...
            if ("*" === firstArg) {
                args = [];
                for(i in mods)mods.hasOwnProperty(i) && args.push(i);
                return Y._attach(args) && handleLoader(), Y;
            }
            (mods.loader || mods["loader-base"]) && !Y.Loader && Y._attach([
            ...
            process1(args);
            (len = missing.length) && (len = (missing = YArray.dedupe(missing)).length);
            ...
            } else Y._attach(args) && handleLoader();
            return Y;
        },
        namespace: function() {
            for(var o, j, d, arg, a = arguments, i = 0; i < a.length; i++){
                o = this;
                if ((arg = a[i]).indexOf(".") > -1) {
                    d = arg.split(".");
                    for(j = "YAHOO" == d[0] ? 1 : 0; j < d.length; j++){
                        o[d[j]] = o[d[j]] || {};
                        o = o[d[j]];
                    }
                } else {
        ...
        error: function(msg, e, src) {
            var ret;
            this.config.errorFn && (ret = this.config.errorFn.apply(this, arguments));
            if (ret) this.message(msg, "error", "" + src);
        ...
        stamp: function(o, readOnly) {
            var uid;
            if (!o) return o;
            if (!(uid = o.uniqueID && o.nodeType && 9 !== o.nodeType ? o.uniqueID : "string" == typeof o ? o : o._yuid)) {
                uid = this.guid();
                if (!readOnly) try {
                    o._yuid = uid;
                } catch (e) {
    ...
    YUI.prototype = proto;
    for(prop in proto)proto.hasOwnProperty(prop) && (YUI[prop] = proto[prop]);
...
YUI.add("yui-base", function(Y, NAME) {
    ...
    function YArray(thing, startIndex, force) {
        ...
        if (force || YArray.test(thing)) try {
            return Native.slice.call(thing, startIndex);
        } catch (ex) {
            result = [];
            for(len = thing.length; startIndex < len; ++startIndex)result.push(thing[startIndex]);
            return result;
        }
        return [
            thing
    ...
    YArray.dedupe = Lang._isNative(Object.create) ? function(array) {
        ...
        for(i = 0, len = array.length; i < len; ++i)if (!hash[item = array[i]]) {
            hash[item] = 1;
            results.push(item);
        }
        return results;
    ...
    YArray.indexOf = Lang._isNative(Native.indexOf) ? function(array, value, from) {
        ...
        (from = ((from = +from || 0) > 0 || -1) * Math.floor(Math.abs(from))) < 0 && (from += len) < 0 && (from = 0);
    ...
    Y.merge = function() {
        for(var key, obj, i = 0, len = arguments.length, result = {}; i < len; ++i){
            obj = arguments[i];
            for(key in obj)hasOwn.call(obj, key) && (result[key] = obj[key]);
        }
        return result;
    };
    Y.mix = function(receiver, supplier, overwrite, whitelist, mode, merge) {
        ...
        if (mode) {
            2 === mode && Y.mix(receiver.prototype, supplier.prototype, overwrite, whitelist, 0, merge);
        ...
        if (whitelist) for(i = 0, len = whitelist.length; i < len; ++i){
            key = whitelist[i];
            ...
            for(key in from)if (hasOwn.call(from, key)) {
                exists = !alwaysOverwrite && key in to;
                merge && exists && isObject(to[key], !0) && isObject(from[key], !0) ? Y.mix(to[key], from[key], overwrite, null, 0, merge) : (overwrite || !exists) && (to[key] = from[key]);
            }
    ...
    YUI.Env.parseUA = function(subUA) {
        ...
        if (ua) {
            /windows|win32/i.test(ua) ? o.os = "windows" : /macintosh|mac_powerpc/i.test(ua) ? o.os = "macintosh" : /android/i.test(ua) ? o.os = "android" : /symbos/i.test(ua) ? o.os = "symbos" : /linux/i.test(ua) ? o.os = "linux" : /rhino/i.test(ua) && (o.os = "rhino");
            ...
            if ((m = ua.match(/AppleWebKit\/([^\s]*)/)) && m[1]) {
                ...
                o.safari = o.webkit;
                /PhantomJS/.test(ua) && (m = ua.match(/PhantomJS\/([^\s]*)/)) && m[1] && (o.phantomjs = numberify(m[1]));
                if (/ Mobile\//.test(ua) || /iPad|iPod|iPhone/.test(ua)) {
                    o.mobile = "Apple";
                    (m = ua.match(/OS ([^\s]*)/)) && m[1] && (m = numberify(m[1].replace("_", ".")));
                    o.ios = m;
                    o.os = "ios";
                    o.ipad = o.ipod = o.iphone = 0;
                    (m = ua.match(/iPad|iPod|iPhone/)) && m[0] && (o[m[0].toLowerCase()] = o.ios);
                } else {
                    (m = ua.match(/NokiaN[^\/]*|webOS\/\d\.\d/)) && (o.mobile = m[0]);
                    if (/webOS/.test(ua)) {
                        o.mobile = "WebOS";
                        (m = ua.match(/webOS\/([^\s]*);/)) && m[1] && (o.webos = numberify(m[1]));
                    }
                    if (/ Android/.test(ua)) {
                        /Mobile/.test(ua) && (o.mobile = "Android");
                        (m = ua.match(/Android ([^\s]*);/)) && m[1] && (o.android = numberify(m[1]));
                    }
                    if (/Silk/.test(ua)) {
                        (m = ua.match(/Silk\/([^\s]*)\)/)) && m[1] && (o.silk = numberify(m[1]));
                        if (!o.android) {
                            o.android = 2.34;
                            o.os = "Android";
                        }
                ...
                if ((m = ua.match(/OPR\/(\d+\.\d+)/)) && m[1]) o.opera = numberify(m[1]);
                else if ((m = ua.match(/(Chrome|CrMo|CriOS)\/([^\s]*)/)) && m[1] && m[2]) {
                    o.chrome = numberify(m[2]);
                    o.safari = 0;
                    "CrMo" === m[1] && (o.mobile = "chrome");
                } else (m = ua.match(/AdobeAIR\/([^\s]*)/)) && (o.air = m[0]);
            }
            if (!o.webkit) {
                if (/Opera/.test(ua)) {
                    (m = ua.match(/Opera[\s\/]([^\s]*)/)) && m[1] && (o.opera = numberify(m[1]));
                    (m = ua.match(/Version\/([^\s]*)/)) && m[1] && (o.opera = numberify(m[1]));
                    if (/Opera Mobi/.test(ua)) {
                        o.mobile = "opera";
                        (m = ua.replace("Opera Mobi", "").match(/Opera ([^\s]*)/)) && m[1] && (o.opera = numberify(m[1]));
                    }
                    (m = ua.match(/Opera Mini[^;]*/)) && (o.mobile = m[0]);
                } else if ((m = ua.match(/MSIE ([^;]*)|Trident.*; rv:([0-9.]+)/)) && (m[1] || m[2])) o.ie = numberify(m[1] || m[2]);
                else if (m = ua.match(/Gecko\/([^\s]*)/)) {
                    o.gecko = 1;
                    if ((m = ua.match(/rv:([^\s\)]*)/)) && m[1]) {
        ...
        if (!subUA) {
            if ("object" == typeof process && process.versions && process.versions.node) {
...
YUI.add("get", function(Y, NAME) {
    ...
    Y.Get = Get = {
        ...
        _load: function(type, urls, options, callback) {
            ...
            return options || (options = {}), options.type = type, options._onFinish = Get._onTransactionFinish, this._env || this._getEnv(), transaction = this._getTransaction(urls, options), this._queue.push({
                callback: callback,
        ...
        _next: function() {
            var item;
            if (!this._pending && (item = this._queue.shift())) {
                this._pending = item;
                item.transaction.execute(item.callback);
            }
        },
        _purge: function(nodes) {
            for(var index, node, purgeNodes = this._purgeNodes, isTransaction = nodes !== purgeNodes; node = nodes.pop();)if (node._yuiget_finished) {
    ...
    Transaction.prototype = {
        ...
        execute: function(callback) {
            ...
            if ("executing" !== state) {
                ...
                self._queue = queue = [];
                self.options.timeout && (self._timeout = setTimeout(function() {
                    self.abort("Timeout");
                }, self.options.timeout));
                self._reqsWaiting = requests.length;
                for(i = 0, len = requests.length; i < len; ++i)(req = requests[i]).async || "css" === req.type ? self._insert(req) : queue.push(req);
                self._next();
            }
        },
        purge: function() {
        ...
        _createNode: function(name, attrs, doc) {
            ...
            for(attr in attrs)attrs.hasOwnProperty(attr) && node.setAttribute(CUSTOM_ATTRS[attr] || attr, attrs[attr]);
            return node;
        },
        _finish: function() {
            ...
            if ("done" !== this._state) {
                this._state = "done";
                for(i = 0, len = this._callbacks.length; i < len; ++i)this._callbacks[i].call(thisObj, errors, this);
                data = this._getEventData();
                if (errors) {
                    options.onTimeout && "Timeout" === errors[errors.length - 1].error && options.onTimeout.call(thisObj, data);
                    options.onFailure && options.onFailure.call(thisObj, data);
                } else options.onSuccess && options.onSuccess.call(thisObj, data);
        ...
        _insert: function(req) {
            ...
            if (!node) {
                nodeType = isScript ? "script" : !env.cssLoad && ua.gecko ? "style" : "link";
            ...
            if (isScript) {
                ...
                if (req.async) node.async = !0;
                else {
                    env.async && (node.async = !1);
                    env.preservesScriptOrder || (this._pending = req);
                }
            } else !env.cssLoad && ua.gecko ? node.innerHTML = (req.attributes.charset ? '@charset "' + req.attributes.charset + '";' : "") + '@import "' + req.url + '";' : node.setAttribute("href", req.url);
            if (isScript && ua.ie && (ua.ie < 9 || document.documentMode && document.documentMode < 9)) node.onreadystatechange = function() {
                ...
                env.cssFail || isScript || (cssTimeout = setTimeout(onError, req.timeout || 3000));
            } else this._poll(req);
        ...
        _poll: function(newReq) {
            ...
            for(i = 0; i < pendingCSS.length; ++i){
                ...
                if (isWebKit) {
                    ...
                    for(; --j >= 0;)if (sheets[j].href === nodeHref) {
        ...
        _progress: function(err, req) {
            ...
            options.onProgress && options.onProgress.call(options.context || this, this._getEventData(req));
            if (req.autopurge) {
...
YUI.add("intl-base", function(Y, NAME) {
    ...
    Y.mix(Y.namespace("Intl"), {
        lookupBestLang: function(preferredLanguages, availableLanguages) {
            ...
            Y.Lang.isString(preferredLanguages) && (preferredLanguages = preferredLanguages.split(SPLIT_REGEX));
            for(i = 0; i < preferredLanguages.length; i += 1)if ((language = preferredLanguages[i]) && "*" !== language) for(; language.length > 0;){
                if (result = scan(language)) return result;
                if ((index = language.lastIndexOf("-")) >= 0) {
                    language = language.substring(0, index);
                    index >= 2 && "-" === language.charAt(index - 2) && (language = language.substring(0, index - 2));
                } else break;
            }
            return "";
        }
...
YUI.add("yui-log", function(Y, NAME) {
    ...
    INSTANCE.log = function(msg, cat, src, silent) {
        ...
        if (c.debug) {
            if (void 0 !== (src = src || "")) {
                excl = c.logExclude;
                !(incl = c.logInclude) || src in incl ? incl && src in incl ? bail = !incl[src] : excl && src in excl && (bail = excl[src]) : bail = 1;
            ...
            if (!bail) {
                if (c.useBrowserConsole) {
                    ...
                    if (Y.Lang.isFunction(c.logFn)) c.logFn.call(Y, msg, cat, src);
                        ...
                        console[f](m);
                    } else typeof opera !== UNDEFINED && opera.postError(m);
                }
                if (publisher && !silent) {
                    publisher !== Y || publisher.getEvent(LOGEVENT) || publisher.publish(LOGEVENT, {
                        broadcast: 2
                    });
    ...
    Y.Loader.prototype = {
        _populateCache: function() {
            ...
            if (cache && !self.ignoreRegistered) {
                ...
                for(i in cache)cache.hasOwnProperty(i) && (self.conditions[i] = Y.merge(cache[i]));
        ...
        _resetModules: function() {
            ...
            for(i in self.moduleInfo)if (self.moduleInfo.hasOwnProperty(i)) {
                ...
                if (details = YUI.Env.mods[name] ? YUI.Env.mods[name].details : null) {
                    self.moduleInfo[name]._reset = !0;
                    self.moduleInfo[name].requires = details.requires || [];
                    self.moduleInfo[name].optional = details.optional || [];
                    self.moduleInfo[name].supersedes = details.supercedes || [];
                }
                if (mod.defaults) for(o in mod.defaults)mod.defaults.hasOwnProperty(o) && mod[o] && (mod[o] = mod.defaults[o]);
                delete mod.langCache;
                delete mod.skinCache;
                mod.skinnable && self._addSkin(self.skin.defaultSkin, mod.name);
            }
        },
        REGEX_CSS: /\.css(?:[?;].*)?$/i,
        FILTER_DEFS: {
        ...
        _inspectPage: function() {
            ...
            for(i in ON_PAGE)if (ON_PAGE.hasOwnProperty(i) && (v = ON_PAGE[i]).details) {
                m = self.moduleInfo[v.name];
                req = v.details.requires;
                mr = m && m.requires;
                m ? !m._inspected && req && mr.length !== req.length && delete m.expanded : m = self.addModule(v.details, i);
                m._inspected = !0;
            }
        },
        _requires: function(mod1, mod2) {
            ...
            if (!m || !other) return !1;
            rm = m.expanded_map;
            if ((after_map = m.after_map) && mod2 in after_map) return !0;
        ...
        _config: function(o) {
            ...
            if (o) {
                for(i in o)if (o.hasOwnProperty(i)) {
                    ...
                    if ("require" === i) self.require(val);
                        ...
                        for(j in val)if (val.hasOwnProperty(j)) {
                            groupName = j;
                            group = val[j];
                            self.addGroup(group, groupName);
                            if (group.aliases) for(a in group.aliases)group.aliases.hasOwnProperty(a) && self.addAlias(group.aliases[a], a);
                        }
            ...
            if (self.filterName && self.coverage && "COVERAGE" === self.filterName && L.isArray(self.coverage) && self.coverage.length) {
                for(i = 0; i < self.coverage.length; i++){
                    mod = self.coverage[i];
                    self.moduleInfo[mod] && self.moduleInfo[mod].use ? mods = [].concat(mods, self.moduleInfo[mod].use) : mods.push(mod);
                }
        ...
        _addSkin: function(skin, mod, parent) {
            ...
            if (mod && !info[name = this.formatSkin(skin, mod)]) {
        ...
        addGroup: function(o, name) {
            ...
            self.groups[name] = o;
            if (o.patterns) {
                for(i in o.patterns)if (o.patterns.hasOwnProperty(i)) {
                    o.patterns[i].group = name;
                    self.patterns[i] = o.patterns[i];
                }
            }
            if (mods) {
                for(i in mods)if (mods.hasOwnProperty(i)) {
                    "string" == typeof (v = mods[i]) && (v = {
                        name: i,
                        fullpath: v
                    });
                    v.group = name;
        ...
        addModule: function(o, name) {
            ...
            this.moduleInfo[name] && this.moduleInfo[name].temp && (o = Y.merge(this.moduleInfo[name], o));
            o.name = name;
            if (!o || !o.name) return null;
            if (!o.type) {
                o.type = "js";
                (p = o.path || o.fullpath) && this.REGEX_CSS.test(p) && (o.type = "css");
            }
            ...
            if (this.requires) for(i = 0; i < this.requires.length; i++)o.requires.push(this.requires[i]);
            ...
            o.requires.length && (o.requires = this.filterRequires(o.requires) || []);
            ...
            if (subs) {
                sup = o.supersedes || [];
                l = 0;
                for(i in subs)if (subs.hasOwnProperty(i)) {
                    ...
                    if (smod.skinnable) {
                        o.skinnable = !0;
                        if ((overrides = this.skin.overrides) && overrides[i]) for(j = 0; j < overrides[i].length; j++){
                    ...
                    if (s.lang && s.lang.length) {
                        ...
                        for(j = 0; j < langs.length; j++){
                            ...
                            o.lang = o.lang || [];
                            lang in (flatLang = flatLang || yArray.hash(o.lang)) || o.lang.push(lang);
            ...
            if (plugins = o.plugins) {
                for(i in plugins)if (plugins.hasOwnProperty(i)) {
            ...
            o.supersedes && (o.supersedes = this.filterRequires(o.supersedes));
            if (o.after) {
                o.after = this.filterRequires(o.after);
                o.after_map = yArray.hash(o.after);
            }
            if (o.configFn && !1 === o.configFn(o)) {
        ...
        _explodeRollups: function() {
            ...
            if (!self.allowRollup) {
                for(i in r)if (r.hasOwnProperty(i) && (m = self.getModule(i)) && m.use) {
                    len = m.use.length;
                    for(a = 0; a < len; a++)if ((m2 = self.getModule(m.use[a])) && m2.use) {
                        len2 = m2.use.length;
                        for(v = 0; v < len2; v++)r[m2.use[v]] = !0;
                    } else r[m.use[a]] = !0;
                }
                self.required = r;
            }
        ...
        getRequires: function(mod) {
            ...
            for(i = 0; i < r.length; i++)if (!hash[r[i]]) {
                d.push(r[i]);
                hash[r[i]] = !0;
                if (m = this.getModule(r[i])) {
            ...
            if (o && this.loadOptional) {
                for(i = 0; i < o.length; i++)if (!hash[o[i]]) {
                    d.push(o[i]);
                    hash[o[i]] = !0;
                    if (m = info[o[i]]) {
                        add = this.getRequires(m);
            ...
            if (cond = this.conditions[name]) {
                mod._parsed = !1;
                if (testresults && ftests) oeach(testresults, function(result, id) {
                    var condmod = ftests[id].name;
                    if (!hash[condmod] && ftests[id].trigger === name && result && ftests[id]) {
                        hash[condmod] = !0;
                        d.push(condmod);
                    }
                });
                else for(i in cond)if (cond.hasOwnProperty(i) && !hash[i] && (def = cond[i]) && (!def.ua && !def.test || def.ua && Y.UA[def.ua] || def.test && def.test(Y, r))) {
                    hash[i] = !0;
                    d.push(i);
                    if (m = this.getModule(i)) {
                        add = this.getRequires(m);
                        for(j = 0; j < add.length; j++)d.push(add[j]);
                    }
                }
            ...
            if (mod.skinnable) {
                ...
                for(i in YUI.Env.aliases)YUI.Env.aliases.hasOwnProperty(i) && Y.Array.indexOf(YUI.Env.aliases[i], name) > -1 && (skinpar = i);
            ...
            if (intl) {
                if (mod.lang && !mod.langPack && Y.Intl) {
                    ...
                    (packName = this.getLangPackName(lang, name)) && d.unshift(packName);
                }
                d.unshift(INTL);
            }
            return mod.expanded_map = yArray.hash(d), mod.expanded = YObject.keys(mod.expanded_map), mod.expanded;
        },
        ...
        getProvides: function(name) {
            ...
            if (m && !m.provides) {
                o = {};
                (s = m.supersedes) && yArray.each(s, function(v) {
        ...
        _setup: function() {
            ...
            for(j in l)l.hasOwnProperty(j) && Y.mix(l, this.getProvides(j));
        ...
        _explode: function() {
            ...
            r = self.required;
            for(name in r)if (r.hasOwnProperty(name) && !done[name]) {
                done[name] = !0;
                if (m = self.getModule(name)) {
                    if (expound = m.expound) {
        ...
        getModule: function(mname) {
            ...
            if (!m || m && m.ext) {
                for(pname in patterns)if (patterns.hasOwnProperty(pname)) {
        ...
        _reduce: function(r) {
            ...
            for(i in r)if (r.hasOwnProperty(i)) {
                m = this.getModule(i);
                ((this.loaded[i] || ON_PAGE[i]) && !this.forceMap[i] && !this.ignoreRegistered || type && m && m.type !== type) && delete r[i];
                ignore && ignore[i] && delete r[i];
                if (s = m && m.supersedes) for(j = 0; j < s.length; j++)s[j] in r && delete r[s[j]];
            }
            return r;
        },
        ...
        _onSuccess: function() {
            ...
            self.skipped = {};
            for(i in self.inserted)self.inserted.hasOwnProperty(i) && ((mod = self.getModule(i)) && rreg && "js" === mod.type && !(i in YUI.Env.mods) ? failed.push(i) : Y.mix(self.loaded, self.getProvides(i)));
            fn = self.onSuccess;
            msg = failed.length ? "notregistered" : "success";
            success = !failed.length;
            fn && fn.call(self.context, {
                msg: msg,
                data: self.data,
                success: success,
                failed: failed,
        ...
        _insert: function(source, o, type, skipcalc) {
            ...
            complete = function(d) {
                ...
                if (d && d.data && d.data.length && "success" === d.type) for(i = 0; i < d.data.length; i++){
                ...
                if (actions === comp) {
                    ...
                    if (self._refetch.length) {
                        for(i = 0; i < self._refetch.length; i++){
                            ...
                            for(o = 0; o < deps.length; o++)self.inserted[deps[o]] || (mods[deps[o]] = deps[o]);
                        }
                        if ((mods = Y.Object.keys(mods)).length) {
                            self.require(mods);
                            if ((resMods = self.resolve(!0)).cssMods.length) {
        ...
        _filter: function(u, name, group) {
            ...
            if (u) {
                hasFilter && (f = L.isString(modFilter) ? this.FILTER_DEFS[modFilter.toUpperCase()] || null : modFilter);
                f && (u = u.replace(RegExp(f.searchExp, "g"), f.replaceStr));
            }
            return u;
        },
        ...
        resolve: function(calc, s) {
            ...
            (self.skin.overrides || "sam" !== self.skin.defaultSkin || self.ignoreRegistered) && self._resetModules();
            calc && self.calculate();
            s = s || self.sorted;
            addSingle = function(m) {
                if (m) {
                    !1 === (group = m.group && self.groups[m.group] || NOT_FOUND).async && (m.async = group.async);
                    url = m.fullpath ? self._filter(m.fullpath, s[i]) : self._url(m.path, s[i], group.base || m.base);
                    if (m.attributes || !1 === m.async) {
                        url = {
                            url: url,
                            async: m.async
                        };
            ...
            for(i = 0; i < len; i++){
                ...
                if (groupName && group) {
                    ...
                    "root" in group && L.isValue(group.root) && (m.root = group.root);
            ...
            for(j in comboSources)if (comboSources.hasOwnProperty(j)) {
                ...
                url = j;
                if (len = (mods = comboSources[j]).length) {
                    for(i = 0; i < len; i++)if (!inserted[mods[i]]) {
                        if ((m = mods[i]) && (m.combine || !m.ext)) {
                        ...
                        } else mods[i] && addSingle(mods[i]);
                    }
                }
            }
            for(j in resCombos)if (resCombos.hasOwnProperty(j)) {
                comboSep = resCombos[base = j].comboSep || self.comboSep;
                maxURLLength = resCombos[base].maxURLLength || self.maxURLLength;
                for(type in resCombos[base])if ("js" === type || "css" === type) {
                    ...
                    if (len) {
                        if (baseLen > maxURLLength) {
                            ...
                            for(s = 0; s < len; s++){
                                u.push(urls[s]);
                                if ((tmpBase = base + u.join(comboSep)).length > maxURLLength) {
                                    m = u.pop();
