!function(window1, document1, undefined) {
    ...
    function minErr(module) {
        return function() {
            ...
            message = (message = "[" + (module ? module + ":" : "") + code + "] " + template.replace(/\{\d+\}/g, function(match) {
            ...
            for(i = 2; i < arguments.length; i++)message = message + (2 == i ? "?" : "&") + "p" + (i - 2) + "=" + encodeURIComponent((obj = arguments[i], "function" == typeof obj ? obj.toString().replace(/ \{[\s\S]*$/, "") : void 0 === obj ? "undefined" : "string" != typeof obj ? JSON.stringify(obj) : obj));
    ...
    function nextUid() {
        for(var digit, index = uid.length; index;){
            if (57 == (digit = uid[--index].charCodeAt(0))) {
                uid[index] = "A";
                return uid.join("");
            }
    ...
    function copy(source, destination) {
        ...
        if (destination) {
            ...
            destination = source;
            source && (isArray(source) ? destination = copy(source, []) : isDate(source) ? destination = new Date(source.getTime()) : isRegExp(source) ? destination = RegExp(source.source) : isObject(source) && (destination = copy(source, {})));
        }
        return destination;
    }
    ...
    function parseKeyValue(keyValue) {
        ...
        return forEach((keyValue || "").split("&"), function(keyValue) {
            if (keyValue) {
                key = tryDecodeURIComponent((key_value = keyValue.split("="))[0]);
                if (isDefined(key)) {
    ...
    function bootstrap(element, modules) {
        ...
        window1.name = window1.name.replace(NG_DEFER_BOOTSTRAP, "");
        angular1.resumeBootstrap = function(extraModules) {
    ...
    function jqLitePatchJQueryRemove(name, dispatchThis, filterElems, getterIfNoArguments) {
        ...
        function removePatch(param) {
            ...
            if (!getterIfNoArguments || null != param) for(; list.length;){
                set = list.shift();
                for(setIndex = 0, setLength = set.length; setIndex < setLength; setIndex++){
                    element = jqLite(set[setIndex]);
                    fireEvent ? element.triggerHandler("$destroy") : fireEvent = !fireEvent;
    ...
    function JQLite(element) {
        ...
        if (isString(element)) {
            ...
            jqLite(document1.createDocumentFragment()).append(this);
    ...
    var JQLitePrototype = JQLite.prototype = {
        ready: function(fn) {
            ...
            if ("complete" === document1.readyState) setTimeout(trigger);
            else {
    ...
    forEach({
        ...
        css: function(element, name, value) {
            name = camelCase(name);
            if (isDefined(value)) element.style[name] = value;
            else {
                var val;
    ...
    forEach({
        ...
        on: function onFn(element, type, fn, unsupported) {
            ...
            handle || jqLiteExpandoStore(element, "handle", (events = events1, (eventHandler = function(event, type) {
                ...
                event.target || (event.target = event.srcElement || document1);
            ...
            }).elem = element, handle = eventHandler));
            forEach(type.split(" "), function(type) {
    ...
    function annotate(fn) {
        ...
        if ("function" == typeof fn) {
            if (!($inject = fn.$inject)) {
                $inject = [];
                fn.length && forEach(fn.toString().replace(STRIP_COMMENTS, "").match(FN_ARGS)[1].split(FN_ARG_SPLIT), function(arg) {
    ...
    function createInjector(modulesToLoad) {
        ...
        forEach(function loadModules(modulesToLoad) {
    ...
    function Browser(window1, document1, $log, $sniffer) {
        ...
        self.url = function(url, replace) {
            location !== window1.location && (location = window1.location);
            if (!url) return newLocation || location.href.replace(/%27/g, "'");
        ...
        self.onUrlChange = function(callback) {
            if (!urlChangeInit) {
                ...
                $sniffer.hashchange ? jqLite(window1).on("hashchange", fireUrlChange) : self.addPollFn(fireUrlChange);
                urlChangeInit = !0;
            }
        ...
        self.cookies = function(name, value) {
            ...
            if (name) value === undefined ? rawDocument.cookie = escape(name) + "=;path=" + cookiePath + ";expires=Thu, 01 Jan 1970 00:00:00 GMT" : isString(value) && (cookieLength = (rawDocument.cookie = escape(name) + "=" + escape(value) + ";path=" + cookiePath).length + 1) > 4096 && $log.warn("Cookie '" + name + "' possibly not set or overflowed because it was too large (" + cookieLength + " > 4096 bytes)!");
                ...
                if (rawDocument.cookie !== lastCookieString) {
                    ...
                    lastCookies = {};
                    for(i = 0; i < cookieArray.length; i++)(index = (cookie = cookieArray[i]).indexOf("=")) > 0 && undefined === lastCookies[name = unescape(cookie.substring(0, index))] && (lastCookies[name] = unescape(cookie.substring(index + 1)));
                }
                return lastCookies;
            }
        };
            ...
            function($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
                ...
                Attributes.prototype = {
                    ...
                    $set: function(key, value, writeAttr, attrName) {
                        ...
                        ("A" === (nodeName = nodeName_(this.$$element)) && "href" === key || "IMG" === nodeName && "src" === key) && (this[key] = value = $$sanitizeUri(value, "src" === key));
                ...
                function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
                    for(var nodeLinkFn, childLinkFn, directives, attrs, linkFnFound, linkFns = [], i = 0; i < nodeList.length; i++){
                        ...
                        childLinkFn = (nodeLinkFn = (directives = collectDirectives(nodeList[i], [], attrs, 0 === i ? maxPriority : undefined, ignoreDirective)).length ? applyDirectivesToNode(directives, nodeList[i], attrs, transcludeFn, $rootElement, null, [], [], previousCompileContext) : null) && nodeLinkFn.terminal || !nodeList[i].childNodes || !nodeList[i].childNodes.length ? null : compileNodes(nodeList[i].childNodes, nodeLinkFn ? nodeLinkFn.transclude : transcludeFn);
                    ...
                    return linkFnFound ? function(scope, nodeList, $rootElement, boundTranscludeFn) {
                        ...
                        for(i = 0, n = 0, ii = linkFns.length; i < ii; n++){
                            ...
                            } else childLinkFn && childLinkFn(scope, node.childNodes, undefined, boundTranscludeFn);
                        }
                    } : null;
                }
                ...
                function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
                    ...
                    switch(nodeType){
                        case 1:
                            ...
                            if (isString(className = node.className) && "" !== className) for(; match = CLASS_DIRECTIVE_REGEXP.exec(className);){
                        ...
                        case 8:
                            try {
                                if (match = COMMENT_DIRECTIVE_REGEXP.exec(node.nodeValue)) {
                ...
                function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                    for(var newScopeDirective, directive, directiveName, $template, linkFn, directiveValue, terminalPriority = -Number.MAX_VALUE, controllerDirectives = (previousCompileContext = previousCompileContext || {}).controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasElementTranscludeDirective = !1, $compileNode = templateAttrs.$$element = jqLite(compileNode), replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, i = 0, ii = directives.length; i < ii; i++){
                        ...
                        if (directiveValue = directive.scope) {
                            ...
                            if (!directive.templateUrl) {
                                ...
                                isObject(directiveValue) && (newIsolateScopeDirective = directive);
                            }
                        }
                        ...
                        if (directiveValue = directive.transclude) {
                            ...
                            if ("element" == directiveValue) {
                                ...
                                compileNode = ($compileNode = templateAttrs.$$element = jqLite(document1.createComment(" " + directiveName + ": " + templateAttrs[directiveName] + " ")))[0];
                        ...
                        if (directive.template) {
                            ...
                            if (directive.replace) {
                                ...
                                compileNode = ($template = jqLite("<div>" + trim(directiveValue) + "</div>").contents())[0];
                                if (1 != $template.length || 1 !== compileNode.nodeType) throw $compileMinErr("tplrt", "Template for directive '{0}' must have exactly one root element. {1}", directiveName, "");
                    ...
                    function getControllers(require, $element, elementControllers) {
                        ...
                        if (isString(require)) {
                            ...
                            value = null;
                            elementControllers && "data" === retrievalMethod && (value = elementControllers[require]);
                            if (!(value = value || $element[retrievalMethod]("$" + require + "Controller")) && !optional) throw $compileMinErr("ctreq", "Controller '{0}', required by directive '{1}', can't be found!", require, directiveName);
                    ...
                    function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                        ...
                        $element = (attrs = compileNode === linkNode ? templateAttrs : function(src, dst) {
                            dst = dst || {};
                            for(var key in src)src.hasOwnProperty(key) && "$$" !== key.substr(0, 2) && (dst[key] = src[key]);
                            return dst;
                        }(templateAttrs, new Attributes(jqLite(linkNode), templateAttrs.$attr))).$$element;
                        ...
                        transcludeFn = boundTranscludeFn && function(scope, cloneAttachFn) {
                            var transcludeControllers;
                            if (arguments.length < 2) {
                                cloneAttachFn = scope;
                                scope = undefined;
                            }
                            return hasElementTranscludeDirective && (transcludeControllers = elementControllers), boundTranscludeFn(scope, cloneAttachFn, transcludeControllers);
                        };
                        ...
                        for(i = 0, ii = preLinkFns.length; i < ii; i++)try {
                        ...
                        for(i = postLinkFns.length - 1; i >= 0; i--)try {
                ...
                function mergeTemplateAttributes(dst, src) {
                    ...
                    forEach(dst, function(value, key) {
                        if ("$" != key.charAt(0)) {
                            src[key] && (value += ("style" === key ? ";" : " ") + src[key]);
                ...
                function compileTemplateUrl(directives, $compileNode, tAttrs, $rootElement, childTranscludeFn, preLinkFns, postLinkFns, previousCompileContext) {
                    ...
                    return $compileNode.empty(), $http.get($sce.getTrustedResourceUrl(templateUrl), {
                        ...
                        if (origAsyncDirective.replace) {
                            compileNode = ($template = jqLite("<div>" + trim(content) + "</div>").contents())[0];
                            if (1 != $template.length || 1 !== compileNode.nodeType) throw $compileMinErr("tplrt", "Template for directive '{0}' must have exactly one root element. {1}", origAsyncDirective.name, templateUrl);
                            tempTemplateAttrs = {
                                $attr: {}
                            };
                            ...
                            compile: function() {
                                return {
                                    pre: function(scope, element, attr) {
                                        ...
                                        if (interpolateFn = $interpolate(attr[name], !0, function(node, attrNormalizedName) {
            ...
            function($injector, $window) {
                return function(expression, locals) {
                    ...
                    if (isString(expression)) {
                        constructor = (match = expression.match(CNTRL_REG))[1], identifier = match[3];
                        expression = controllers.hasOwnProperty(constructor) ? controllers[constructor] : getter(locals.$scope, constructor, !0) || getter($window, constructor, !0);
            ...
            transformResponse: [
                function(data) {
                    if (isString(data)) {
                        ...
                        JSON_START.test(data) && JSON_END.test(data) && (data = fromJson(data));
                    }
                    return data;
                }
            ],
                    ...
                    var chain = [
                        function(config) {
                            ...
                            return isUndefined(config.data) && forEach(headers, function(value, header) {
                                ...
                                (config.cache || defaults.cache) && !1 !== config.cache && "GET" == config.method && (cache = isObject(config.cache) ? config.cache : isObject(defaults.cache) ? defaults.cache : defaultCache);
                                if (cache) {
                                ...
                                isUndefined(cachedResp) && $httpBackend(config.method, url, reqData, function(status, response, headersString) {
            ...
            function($browser, $window, $document) {
                ...
                return $browserDefer = $browser.defer, callbacks = $window.angular.callbacks, rawDocument = $document[0], function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
                    ...
                    if ("jsonp" == lowercase(method)) {
                        ...
                        var url1, done, script, doneWrapper, jsonpDone = (url1 = url.replace("JSON_CALLBACK", "angular.callbacks." + callbackId), done = function() {
                    ...
                    function completeRequest(callback, status, response, headersString) {
                        ...
                        jsonpDone = xhr = null;
                        callback(status = 1223 == (status = "file" == protocol && 0 === status ? response ? 200 : 404 : status) ? 204 : status, response, headersString);
        ...
        this.$get = [
            ...
            function($parse, $exceptionHandler, $sce) {
                ...
                function $interpolate(text, mustHaveExpression, trustedContext) {
                    for(var startIndex, endIndex, fn, exp, index = 0, parts = [], length = text.length, hasInterpolation = !1, concat = []; index < length;)if (-1 != (startIndex = text.indexOf(startSymbol, index)) && -1 != (endIndex = text.indexOf(endSymbol, startIndex + startSymbolLength))) {
                ...
                function interval(fn, delay, count, invokeApply) {
                    ...
                    return count = isDefined(count) ? count : 0, promise.then(null, null, fn), promise.$$intervalId = setInterval(function() {
                        ...
                        if (count > 0 && iteration >= count) {
                            ...
                            clearInterval1(promise.$$intervalId);
    ...
    function LocationHashbangUrl(appBase, hashPrefix) {
        ...
        this.$$parse = function(url) {
            ...
            this.$$path = (path = this.$$path, windowsFilePathExp = /^\/?.*?:(\/.*)/, (0 === (url1 = withoutHashUrl).indexOf(appBase) && (url1 = url1.replace(appBase, "")), windowsFilePathExp.exec(url1)) ? path : (firstPathSegmentMatch = windowsFilePathExp.exec(path)) ? firstPathSegmentMatch[1] : path);
            this.$$compose();
        };
        this.$$compose = function() {
            var search = toKeyValue(this.$$search), hash = this.$$hash ? "#" + encodeUriSegment(this.$$hash) : "";
    ...
    function $LocationProvider() {
        ...
        this.$get = [
            ...
            function($rootScope, $browser, $sniffer, $rootElement) {
                ...
                ($location = new LocationMode(appBase, "#" + hashPrefix)).$$parse($location.$$rewrite(initialUrl));
                ...
                $rootScope.$watch(function() {
                    ...
                    return $location.$$replace = !1, changeCounter;
                });
                return $location;
    ...
    Lexer.prototype = {
        ...
        readIdent: function() {
            for(var lastDot, peekIndex, methodName, ch, parser = this, ident = "", start = this.index; this.index < this.text.length && ("." === (ch = this.text.charAt(this.index)) || this.isIdent(ch) || this.isNumber(ch));){
            ...
            if (lastDot) {
                ...
                for(; peekIndex < this.text.length;){
    ...
    Parser.prototype = {
        ...
        primary: function() {
            if (this.expect("(")) {
                ...
                (primary = token.fn) || this.throwError("not a primary expression", token);
                if (token.json) {
                    primary.constant = !0;
                    primary.literal = !0;
                }
        ...
        statements: function() {
            for(var statements = [];;){
                this.tokens.length > 0 && !this.peek("}", ")", ";", "]") && statements.push(this.filterChain());
        ...
        objectIndex: function(obj) {
            ...
            return this.consume("]"), extend(function(self, locals) {
                ...
                if (o) {
                    if ((v = ensureSafeObject(o[i], parser.text)) && v.then && parser.options.unwrapPromises) {
                        p = v;
                        if (!("$$v" in v)) {
                            p.$$v = undefined;
                            p.then(function(val) {
    ...
    function setter(obj, path, setValue, fullExp, options) {
        ...
        for(var key, element = path.split("."), i = 0; element.length > 1; i++){
            ...
            if ((obj = propertyObj).then && options.unwrapPromises) {
    ...
    function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
        return ensureSafeMemberName(key0, fullExp), ensureSafeMemberName(key1, fullExp), ensureSafeMemberName(key2, fullExp), ensureSafeMemberName(key3, fullExp), ensureSafeMemberName(key4, fullExp), options.unwrapPromises ? function(scope, locals) {
            ...
            if (null === pathVal || pathVal === undefined) return pathVal;
            if ((pathVal = pathVal[key0]) && pathVal.then) {
                promiseWarning(fullExp);
                if (!("$$v" in pathVal)) {
            ...
            if (!key1 || null === pathVal || pathVal === undefined) return pathVal;
            if ((pathVal = pathVal[key1]) && pathVal.then) {
                promiseWarning(fullExp);
                if (!("$$v" in pathVal)) {
                    (promise = pathVal).$$v = undefined;
                    promise.then(function(val) {
            ...
            if (!key2 || null === pathVal || pathVal === undefined) return pathVal;
            if ((pathVal = pathVal[key2]) && pathVal.then) {
                promiseWarning(fullExp);
                if (!("$$v" in pathVal)) {
                    (promise = pathVal).$$v = undefined;
                    promise.then(function(val) {
            ...
            if (!key3 || null === pathVal || pathVal === undefined) return pathVal;
            if ((pathVal = pathVal[key3]) && pathVal.then) {
                promiseWarning(fullExp);
                if (!("$$v" in pathVal)) {
                    (promise = pathVal).$$v = undefined;
                    promise.then(function(val) {
            ...
            if (!key4 || null === pathVal || pathVal === undefined) return pathVal;
            if ((pathVal = pathVal[key4]) && pathVal.then) {
                promiseWarning(fullExp);
                if (!("$$v" in pathVal)) {
                    (promise = pathVal).$$v = undefined;
                    promise.then(function(val) {
                ...
                return $parseOptions.csp = $sniffer.csp, promiseWarning = function(fullExp) {
                    ...
                    switch(typeof exp){
                        case "string":
                            ...
                            parsedExpression = new Parser(lexer, $filter, $parseOptions).parse(exp, !1);
                        ...
                        return this.$watch(function() {
                            if (isObject(newValue = objGetter(self))) {
                                if (isArrayLike(newValue)) {
                                    ...
                                    if (oldLength !== (newLength = newValue.length)) {
                                        changeDetected++;
                                        oldValue.length = oldLength = newLength;
                                    }
                                    for(var newLength, key, i = 0; i < newLength; i++)if (oldValue[i] !== newValue[i]) {
                                        changeDetected++;
                                    ...
                                    if (oldLength > newLength) {
                                        changeDetected++;
                                        for(key in oldValue)if (oldValue.hasOwnProperty(key) && !newValue.hasOwnProperty(key)) {
                                            oldLength--;
                                            delete oldValue[key];
                                        }
                                    }
    ...
    function padNumber(num, digits, trim) {
        ...
        num = "" + num;
        for(; num.length < digits;)num = "0" + num;
    ...
    function dateFilter($locale) {
        ...
        return function(date, format) {
            ...
            for(; format;)if (match = DATE_FORMATS_SPLIT.exec(format)) format = (parts = concat(parts, match, 1)).pop();
            else {
                parts.push(format);
                format = null;
            }
    ...
    function limitToFilter() {
        return function(input, limit) {
            ...
            limit = int(limit);
            if (isString(input)) return limit ? limit >= 0 ? input.slice(0, limit) : input.slice(limit, input.length) : "";
            var i, n, out = [];
    ...
    var htmlAnchorDirective = valueFn({
        ...
        compile: function(element, attr) {
            if (msie <= 8) {
                ...
                element.append(document1.createComment("IE fix"));
            }
            if (!attr.href && !attr.name) return function(scope, element) {
    ...
    function FormController(element, attrs) {
        ...
        form.$setValidity = function(validationToken, isValid, control) {
            ...
            if (isValid) {
                ...
                invalidCount || toggleValidCss(isValid);
                if (queue) {
                    ...
                    compile: function() {
                        return {
                            pre: function(scope, formElement, attr, controller) {
                                ...
                                parentFormCtrl && formElement.on("$destroy", function() {
    ...
    function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        if (pattern) {
            if (match = pattern.match(/^\/(.*)\/([gim]*)$/)) {
                pattern = RegExp(match[1], match[2]);
    ...
    var inputDirective = [
        ...
        function($scope, $exceptionHandler, $attr, $element, $parse) {
            ...
            this.$setValidity = function(validationErrorKey, isValid) {
                if (!isValid !== $error[validationErrorKey]) {
                    if (isValid) {
            ...
            $scope.$watch(function() {
                ...
                if (ctrl.$modelValue !== value) {
                    ...
                    for(; idx--;)value = formatters[idx](value);
            ...
            link: function(scope, elm, attr, ctrl) {
                if (ctrl) {
                    ...
                    var validator = function(value) {
                        ...
                        return ctrl.$setValidity("required", !0), value;
                    };
            ...
            return {
                ...
                link: function($scope, $element, $attr, ctrl, $transclude) {
                    ...
                    $scope.$watch($attr.ngIf, function(value) {
                        if (toBoolean(value)) childScope || $transclude(childScope = $scope.$new(), function(clone) {
                            clone[clone.length++] = document1.createComment(" end ngIf: " + $attr.ngIf + " ");
                            block = {
                                clone: clone
                            };
        ...
        function($parse, $animate) {
            ...
            return {
                ...
                link: function($scope, $element, $attr, ctrl, $transclude) {
                    ...
                    if (!(match = lhs.match(/^(?:([\$\w]+)|\(([\$\w]+)\s*,\s*([\$\w]+)\))$/))) throw ngRepeatMinErr("iidexp", "'_item_' in '_item_ in _collection_' should be an identifier or '(_key_, _value_)' expression, but got '{0}'.", lhs);
                    ...
                    $scope.$watchCollection(rhs, function(collection) {
                        ...
                        for(index = 0; index < length; index++){
                            value = collection[key = collection === collectionKeys ? index : collectionKeys[index]];
                            assertNotHasOwnProperty(trackById = trackByIdFn(key, value, index), "`track by` id");
                        ...
                        for(key in lastBlockMap)if (lastBlockMap.hasOwnProperty(key)) {
                            elementsToRemove = getBlockElements((block = lastBlockMap[key]).clone);
                        ...
                        for(index = 0, length = collectionKeys.length; index < length; index++){
                            ...
                            nextBlockOrder[index - 1] && (previousNode = getBlockEnd(nextBlockOrder[index - 1]));
                            if (block.scope) {
                            ...
                            block.scope || $transclude(childScope, function(clone) {
                                clone[clone.length++] = document1.createComment(" end ngRepeat: " + expression + " ");
                        ...
                        optionsExp ? function(scope, selectElement, ctrl) {
                                ...
                                scope.$apply(function() {
                                    ...
                                    if (multiple) {
                                        ...
                                        for(groupIndex = 0, groupLength = optionGroupsCache.length; groupIndex < groupLength; groupIndex++){
                                            ...
                                            for(index = 1, length = optionGroup.length; index < length; index++)if ((optionElement = optionGroup[index].element)[0].selected) {
                                                key = optionElement.val();
                                                keyName && (locals[keyName] = key);
                                                if (trackFn) for(trackIndex = 0; trackIndex < collection.length; trackIndex++){
                                    ...
                                    else if (trackFn) for(trackIndex = 0; trackIndex < collection.length; trackIndex++){
                                        locals[valueName] = collection[trackIndex];
                            ...
                            function render() {
                                ...
                                for(index = 0; index < (length = keys.length); index++){
                                    ...
                                    if (!(optionGroup = optionGroups[optionGroupName = groupByFn(scope, locals) || ""])) {
                                        optionGroup = optionGroups[optionGroupName] = [];
                                        optionGroupNames.push(optionGroupName);
                                    }
                                    if (multiple) selected = isDefined(selectedSet.remove(trackFn ? trackFn(scope, locals) : valueFn(scope, locals)));
                                    else {
                                        if (trackFn) {
                                            var modelCast = {};
                                ...
                                multiple || (nullOption || null === modelValue ? optionGroups[""].unshift({
                                    id: "",
                                    label: "",
                                ...
                                for(groupIndex = 0, groupLength = optionGroupNames.length; groupIndex < groupLength; groupIndex++){
                                    optionGroup = optionGroups[optionGroupName = optionGroupNames[groupIndex]];
                                    ...
                                    } else (existingParent = (existingOptions = optionGroupsCache[groupIndex])[0]).label != optionGroupName && existingParent.element.attr("label", existingParent.label = optionGroupName);
                                    lastElement = null;
                                    for(index = 0, length = optionGroup.length; index < length; index++){
                                        ...
                                        if (existingOption = existingOptions[index + 1]) {
                                            ...
                                            "" === option.id && nullOption ? element = nullOption : (element = optionTemplate.clone()).val(option.id).attr("selected", option.selected).text(option.label);
                                    ...
                                    index++;
                                    for(; existingOptions.length > index;)existingOptions.pop().element.remove();
                                }
                            ...
                            ngModelCtrl.$render = function() {
                                ...
                                } else isUndefined(viewValue) && emptyOption ? selectElement.val("") : selectCtrl.renderUnknownOption(viewValue);
                            };
                            selectElement.on("change", function() {
                                scope.$apply(function() {
                                    unknownOption.parent() && unknownOption.remove();
    ...
    jqLite(document1).ready(function() {
        !function(element, bootstrap) {
            ...
            forEach(names, function(name) {
                names[name] = !0;
                append(document1.getElementById(name));
                name = name.replace(":", "\\:");
            ...
            forEach(elements, function(element) {
                if (!appElement) {
                    ...
                    } else forEach(element.attributes, function(attr) {
