(function() {
    ...
    _.reduceRight = _.foldr = function(obj, iterator, memo, context) {
        ...
        each(obj, function(value, index, list) {
            ...
            if (initial) memo = iterator.call(context, memo, obj[index], index, list);
            else {
                memo = obj[index];
                initial = !0;
            }
    ...
    _.range = function(start, stop, step) {
        ...
        step = arguments[2] || 1;
        for(var length = Math.max(Math.ceil((stop - start) / step), 0), idx = 0, range = Array(length); idx < length;){
            range[idx++] = start;
            start += step;
        }
        return range;
    ...
    _.throttle = function(func, wait, options) {
        ...
        return function() {
            ...
            if (remaining <= 0) {
                ...
                previous = now;
                result = func.apply(context, args);
            } else timeout || !1 === options.trailing || (timeout = setTimeout(later, remaining));
            return result;
        };
    };
