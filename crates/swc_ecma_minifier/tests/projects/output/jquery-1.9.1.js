 ...
 */ function(window1, undefined) {
   ...
   */ jQuery.event = {
    ...
    setDocument, document1, docElem, documentIsXML, rbuggyQSA, rbuggyMatches, matches, contains, sortOrder, // Instance-specific data
    ...
    rtrim1 = RegExp("^" + whitespace + "+|((?:^|[^\\\\])(?:\\\\.)*)" + whitespace + "+$", "g"), rcomma = RegExp("^" + whitespace + "*," + whitespace + "*"), rcombinators = RegExp("^" + whitespace + "*([\\x20\\t\\r\\n\\f>+~])" + whitespace + "*"), rpseudo = new RegExp(pseudos), ridentifier = RegExp("^" + identifier + "$"), matchExpr = {
    ...
    rquickExpr1 = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/, rinputs = /^(?:input|select|textarea|button)$/i, rheader = /^h\d$/i, rescape = /'|\\/g, rattributeQuotes = /\=[\x20\t\r\n\f]*([^'"\]]*)[\x20\t\r\n\f]*\]/g, // CSS escapes http://www.w3.org/TR/CSS21/syndata.html#escaped-characters
    ...
    function Sizzle(selector, context, results, seed) {
        if ((context ? context.ownerDocument || context : preferredDoc) !== document1 && setDocument(context), context = context || document1, results = results || [], !selector || "string" != typeof selector) return results;
        if (1 !== (nodeType = context.nodeType) && 9 !== nodeType) return [];
        if (!documentIsXML && !seed) {
            // Shortcuts
            if (match = rquickExpr1.exec(selector)) {
                // Speed-up: Sizzle("#ID")
                if (m = match[1]) {
                    if (9 === nodeType) {
                        // Check parentNode to catch when Blackberry 4.6 returns
        ...
        var selector1 = selector.replace(rtrim1, "$1"), context1 = context, results1 = results, seed1 = seed, match, elem, m, nodeType, // QSA vars
        i, groups, old, nid, newContext, newSelector, i1, tokens, token, type, find, match1 = tokenize(selector1);
        if (!seed1 && 1 === match1.length) {
            if (// Take a shortcut and set the context if the root selector is an ID
            (tokens = match1[0] = match1[0].slice(0)).length > 2 && "ID" === (token = tokens[0]).type && 9 === context1.nodeType && !documentIsXML && Expr.relative[tokens[1].type]) {
                if (!(context1 = Expr.find.ID(token.matches[0].replace(runescape, funescape), context1)[0])) return results1;
                selector1 = selector1.slice(tokens.shift().value.length);
            }
            for(// Fetch a seed set for right-to-left matching
            i1 = matchExpr.needsContext.test(selector1) ? 0 : tokens.length; // Abort if we hit a combinator
            i1-- && (token = tokens[i1], !Expr.relative[type = token.type]);)if ((find = Expr.find[type]) && (seed1 = find(token.matches[0].replace(runescape, funescape), rsibling.test(tokens[0].type) && context1.parentNode || context1))) {
                if (// If seed is empty or no tokens remain, we can return early
                tokens.splice(i1, 1), !(selector1 = seed1.length && toSelector(tokens))) return push.apply(results1, slice.call(seed1, 0)), results1;
                break;
            }
        }
        return(// Compile and execute a filtering function
        // Provide `match` to avoid retokenization if we modified the selector above
        compile(selector1, match1)(seed1, context1, documentIsXML, results1, rsibling.test(selector1)), results1);
    }
     ...
     */ setDocument = Sizzle.setDocument = function(node) {
        ...
        return doc !== document1 && 9 === doc.nodeType && doc.documentElement && (// Set our document
        document1 = doc, docElem = doc.documentElement, // Support tests
        ...
        hasDuplicate = !1, [
            0,
            0
        ].sort(sortOrder), support.detectDuplicates = hasDuplicate), document1;
    }, Sizzle.matches = function(expr, elements) {
        return Sizzle(expr, null, null, elements);
    }, Sizzle.matchesSelector = function(elem, expr) {
        // rbuggyQSA always contains :focus, so no need for an existence check
        if ((elem.ownerDocument || elem) !== document1 && setDocument(elem), // Make sure that attribute selectors are quoted
        ...
        var val;
        return ((elem.ownerDocument || elem) !== document1 && setDocument(elem), documentIsXML || (name1 = name1.toLowerCase()), val = Expr.attrHandle[name1]) ? val(elem) : documentIsXML || support.attributes ? elem.getAttribute(name1) : ((val = elem.getAttributeNode(name1)) || elem.getAttribute(name1)) && !0 === elem[name1] ? name1 : val && val.specified ? val.value : null;
    ...
    compile = Sizzle.compile = function(selector, group /* Internal Use Only */ ) {
        ...
        if (!cached) {
            for(group || (group = tokenize(selector)), i = group.length; i--;)(cached = function matcherFromTokens(tokens) {
                ...
                matchContext = addCombinator(function(elem) {
                    ...
                    if ((matcher = Expr.filter[tokens[i].type].apply(null, tokens[i].matches))[expando]) {
                        ...
                        }(i > 1 && elementMatcher(matchers), i > 1 && toSelector(tokens.slice(0, i - 1)).replace(rtrim1, "$1"), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens(tokens = tokens.slice(j)), j < len && toSelector(tokens));
                    }
            ...
            cached = compilerCache(selector, (matcherCachedRuns = 0, bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0, superMatcher = function(seed, context, xml, results, expandContext) {
                ...
                for(outermost && (outermostContext = context !== document1 && context, cachedruns = matcherCachedRuns); null != (elem = elems[i]); i++){
                    if (byElement && elem) {
