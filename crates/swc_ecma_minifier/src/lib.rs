...
struct VarMap<V> {
    map: nohash_hasher::IntMap<SyntaxContext, FxHashMap<Atom, V>>,
}
...
impl<V: Clone> VarMap<V> {
    fn insert(&mut self, ctxt: SyntaxContext, sym: Atom, value: V) {
        let map = self.map.entry(ctxt).or_insert_with(FxHashMap::default);
...
#[derive(Default)]
struct VarSet {
    map: nohash_hasher::IntMap<SyntaxContext, FxHashSet<Atom>>,
}
...
impl VarSet {
    ...
    fn insert(&mut self, ctxt: SyntaxContext, sym: &Atom) {
        let set = self.map.entry(ctxt).or_insert_with(FxHashSet::default);
        if !set.contains(sym) {
