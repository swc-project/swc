...
use super::{pure_misc::DropOpts, Optimizer};
...
impl Optimizer<'_> {
    ...
    pub(super) fn optimize_const_cond(&mut self, e: &mut Expr) {
        ...
        let (p, Value::Known(v)) = cond.test.cast_to_bool(self.ctx.expr_ctx) else {
            return;
        };
        ...
        if p.is_pure() {
            ...
            self.ignore_return_value_with_opts(
                &mut cond.test,
    ...
    pub(super) fn compress_conds_as_logical(&mut self, e: &mut Expr) {
        ...
        if let Value::Known(Type::Bool) = cond.cons.get_type(self.ctx.expr_ctx) {
            let lb = cond.cons.as_pure_bool(self.ctx.expr_ctx);
        ...
        let rt = cond.alt.get_type(self.ctx.expr_ctx);
        if let Value::Known(Type::Bool) = rt {
            let rb = cond.alt.as_pure_bool(self.ctx.expr_ctx);
    ...
    pub(super) fn compress_conds_as_arithmetic(&mut self, e: &mut Expr) {
        ...
        match (&mut *cond.cons, &mut *cond.alt) {
            (
                ...
                && (!cond.test.is_bin()
                    || cond.test.get_type(self.ctx.expr_ctx) == Value::Known(Type::Bool)) =>
            {
            ...
            (
                ...
                && (!cond.test.is_bin() || can_absorb_negate(&cond.test, self.ctx.expr_ctx)) =>
            {
    ...
    pub(super) fn drop_logical_operands(&mut self, e: &mut Expr) {
        ...
        if bin.left.may_have_side_effects(self.ctx.expr_ctx) {
            return;
        }
        ...
        let _lb = bin.left.as_pure_bool(self.ctx.expr_ctx);
