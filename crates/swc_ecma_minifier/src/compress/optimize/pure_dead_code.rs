...
impl Optimizer<'_> {
    ...
    fn drop_duplicate_terminate<T: StmtLike>(&mut self, stmts: &mut [T]) {
        ...
        if let Some(before_last) = stmts.last_mut() {
            if drop(before_last, last, false, self.ctx.expr_ctx) {
                self.changed = true;
        ...
        T: StmtLike,
    {
        ...
        if !maybe_par!(
            stmts.iter().any(|stmt| match stmt.as_stmt() {
                Some(Stmt::If(s)) => s.test.cast_to_bool(self.ctx.expr_ctx).1.is_known(),
                _ => false,
            }),
            *crate::LIGHT_TASK_PARALLELS
        ) {
        ...
        stmts
            ...
            .for_each(|stmt| match stmt.try_into_stmt() {
                Ok(stmt) => match stmt {
                    Stmt::If(mut s) => {
                        if let Value::Known(v) = s.test.cast_to_bool(self.ctx.expr_ctx).1 {
