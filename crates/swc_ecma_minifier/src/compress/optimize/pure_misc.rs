...
use super::{
    pure_strings::{convert_str_value_to_tpl_cooked, convert_str_value_to_tpl_raw},
    BitCtx, Optimizer,
};
...
impl Optimizer<'_> {
    ...
    pub(super) fn eval_spread_object(&mut self, e: &mut ObjectLit) {
        ...
        if e.props.iter().any(|p| should_skip(p, self.ctx.expr_ctx))
    ...
    pub(super) fn compress_array_join(&mut self, e: &mut Expr) {
        ...
        let separator = if call.args.is_empty() {
            ...
            if is_pure_undefined(self.ctx.expr_ctx, &call.args[0].expr) {
                Wtf8Atom::from(",")
            } else {
        ...
        let cannot_join_as_str_lit = arr
            ...
            .any(|v| match &*v.expr {
                e if is_pure_undefined(self.ctx.expr_ctx, e) => false,
        ...
        if cannot_join_as_str_lit {
            ...
            for (last, elem) in arr.elems.take().into_iter().identify_last() {
                if let Some(ExprOrSpread { spread: None, expr }) = elem {
                    match &*expr {
                        e if is_pure_undefined(self.ctx.expr_ctx, e) => {
    ...
    pub(super) fn optimize_builtin_object(&mut self, e: &mut Expr) {
        ...
        match e {
            ...
            | Expr::Call(CallExpr {
                ...
                .is_one_of_global_ref_to(self.ctx.expr_ctx, &["Array", "Object", "RegExp"]) =>
            {
        ...
        match e {
            ...
            ) || (callee.is_global_ref_to(self.ctx.expr_ctx, "RegExp")
    ...
    pub(super) fn ignore_return_value_of_return_stmt_with_opts(
        &mut self,
        s: &mut Stmt,
        opts: DropOpts,
    ) {
        match s {
            Stmt::Return(s) => {
                if let Some(arg) = &mut s.arg {
                    self.ignore_return_value_with_opts(arg, opts);
                    if arg.is_invalid() {
                        report_change!(
            ...
            Stmt::Block(s) => {
                for stmt in &mut s.stmts {
                    self.ignore_return_value_of_return_stmt_with_opts(stmt, opts);
                }
            }

            Stmt::If(s) => {
                self.ignore_return_value_of_return_stmt_with_opts(&mut s.cons, opts);
                if let Some(alt) = &mut s.alt {
                    self.ignore_return_value_of_return_stmt_with_opts(alt, opts);
                }
            }
    ...
    pub(super) fn ignore_return_value_with_opts(&mut self, e: &mut Expr, opts: DropOpts) {
        if e.is_invalid() {
            return;
        }
        ...
        match e {
            Expr::Seq(seq) => {
                if let Some(last) = seq.exprs.last_mut() {
                    self.ignore_return_value_with_opts(last, opts);
                }
            ...
            Expr::New(NewExpr {
                ...
                args,
                ..
            }) if callee.is_pure_callee(self.ctx.expr_ctx) || ctxt.has_mark(self.marks.pure) => {
                report_change!("ignore_return_value: Dropping a pure call");
                self.changed = true;
        ...
        if self.options.conditionals || self.options.expr {
            ...
            {
                self.ignore_return_value_with_opts(cons, opts);
                self.ignore_return_value_with_opts(alt, opts);
            ...
            || self.options.expr
        {
            match e {
                ...
                Expr::Bin(
                    ...
                    self.ignore_return_value_with_opts(&mut be.right, opts);
                ...
                Expr::Tpl(tpl) => {
                    ...
                    for expr in tpl.exprs.iter_mut() {
                        self.ignore_return_value_with_opts(&mut *expr, opts);
                    }
            ...
            Expr::Assign(assign @ AssignExpr { op: op!("="), .. }) => {
                ...
                if let Some(l) = assign.left.as_ident() {
                    if let Expr::Ident(r) = &*assign.right {
                        if l.ctxt == r.ctxt
                            && l.ctxt != self.ctx.expr_ctx.unresolved_ctxt
                            && l.sym == r.sym
                        {
                            self.changed = true;
                            *e = *assign.right.take();
                        ...
                        PropOrSpread::Prop(p) => match &mut **p {
                            Prop::KeyValue(kv) => {
                                if !kv.key.is_computed()
                                    && !kv.value.may_have_side_effects(self.ctx.expr_ctx)
                                {
                ...
                match &mut **callee {
                    ...
                    Expr::Arrow(callee) => match &mut *callee.body {
                        ...
                        BlockStmtOrExpr::Expr(body) => {
                            self.ignore_return_value_with_opts(body, opts);
        ...
        if self.options.side_effects && self.options.pristine_globals {
            match e {
                ...
                Expr::Member(MemberExpr {
                    ...
                    Expr::Array(..) => {
                        self.ignore_return_value_with_opts(obj, opts);
                        *e = self
                            .make_ignored_expr(
                                *span,
    ...
    pub(super) fn drop_neeedless_pat(&mut self, p: &mut Pat) {
        if let Pat::Assign(assign) = p {
            if is_pure_undefined(self.ctx.expr_ctx, &assign.right) {
                *p = *assign.left.take();
                self.changed = true;
                report_change!(
    ...
    pub(super) fn compress_useless_cond_expr(&mut self, expr: &mut Expr) {
        ...
        if (cond.cons.get_type(self.ctx.expr_ctx) != Value::Known(Type::Bool))
            || (cond.alt.get_type(self.ctx.expr_ctx) != Value::Known(Type::Bool))
        {
            return;
        }
