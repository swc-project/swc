    ...
    pub(super) fn optimize_member_expr(
        ...
        if !self.options.pristine_globals
            || self
                .ctx
                .bit_ctx
                .intersects(BitCtx::IsCallee.union(BitCtx::IsLhsOfAssign))
        {
            return None;
        }
        ...
        let op = match prop {
            ...
            MemberProp::Computed(c) => match &*c.expr {
                ...
                _ => {
                    let Known(s) = c.expr.as_pure_string(self.ctx.expr_ctx) else {
                        return None;
                    };
                ...
                match op {
                    KnownOp::Index(idx) => {
                        ...
                        elems.drain(..).flatten().for_each(|elem| {
                            self.ctx.expr_ctx.extract_side_effects_to(&mut exprs, *elem.expr);
                        });
                    ...
                    KnownOp::IndexStr(key) if key != "length" /* handled in simplify */ => {
                        ...
                        if is_known_symbol {
                            ...
                            let optimized_len = elems
                                .iter()
                                .flatten()
                                .filter(|elem| elem.expr.may_have_side_effects(self.ctx.expr_ctx))
                                .count();
            ...
            Expr::Object(ObjectLit { props, span }) => {
                ...
                if is_known_symbol {
                    ...
                    let optimized_len = props
                        .iter()
                        .filter(|prop| {
                            matches!(prop, PropOrSpread::Prop(prop) if matches!(&**prop, Prop::KeyValue(prop) if prop.value.may_have_side_effects(self.ctx.expr_ctx)))
                        })
                        .count();
                ...
                Some(*self.ctx.expr_ctx.preserve_effects(
                    *span,
                    if is_known_symbol {
