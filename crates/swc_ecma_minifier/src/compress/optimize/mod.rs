...
impl Optimizer<'_> {
    ...
    fn ignore_return_value(&mut self, e: &mut Expr) -> Option<Expr> {
        ...
        if self.drop_console(e) || self.is_dropped_console_result(e) {
            report_change!("ignore_return_value: Dropping an unused console call");
            self.changed = true;
            return None;
        }
...
impl VisitMut for Optimizer<'_> {
    ...
    fn visit_mut_expr_stmt(&mut self, n: &mut ExprStmt) {
        ...
        // The expression-statement result is unused.  Drop the Function-method
        // call generated by `drop_console` here without treating any nested
        // expression-statement descendant as unused.
        if self.is_dropped_console_result(&n.expr) {
            self.changed = true;
            n.expr = Expr::undefined(DUMMY_SP);
        }
