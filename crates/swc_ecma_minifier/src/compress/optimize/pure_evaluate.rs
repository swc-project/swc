...
impl Optimizer<'_> {
    ...
    pub(super) fn eval_array_spread(&mut self, e: &mut Expr) {
        ...
        if self.ctx.bit_ctx.intersects(
            BitCtx::IsDeleteArg
                .union(BitCtx::IsUpdateArg)
                .union(BitCtx::IsLhsOfAssign),
        ) {
            return;
        }
    ...
    pub(super) fn eval_logical_expr(&mut self, e: &mut Expr) {
        ...
        let (purity, lv) = b.left.cast_to_bool(self.ctx.expr_ctx);
                ...
                match call.args.len() {
                    ...
                    1 => {
                        if let Value::Known(start) =
                            call.args[0].expr.as_pure_number(self.ctx.expr_ctx)
                        {
                    ...
                    _ => {
                        let start = call.args[0].expr.as_pure_number(self.ctx.expr_ctx);
                        let end = call.args[1].expr.as_pure_number(self.ctx.expr_ctx);
    ...
    pub(super) fn eval_number_method_call(&mut self, e: &mut Expr) {
        ...
        if &*method.sym == "toFixed" {
            ...
            if let Some(precision) = args
                ...
                .map_or(Some(0f64), |arg| {
                    eval_as_number(self.ctx.expr_ctx, &arg.expr)
                })
            {
        ...
        if &*method.sym == "toPrecision" {
            ...
            if let Some(precision) = args
                .first()
                .and_then(|arg| eval_as_number(self.ctx.expr_ctx, &arg.expr))
            {
        ...
        if &*method.sym == "toString" {
            if let Some(base) = args.first().map_or(Some(10f64), |arg| {
                eval_as_number(self.ctx.expr_ctx, &arg.expr)
            }) {
                if base.trunc() == 10. {
                    let value = num.value.to_js_string().into();
    ...
    pub(super) fn eval_opt_chain(&mut self, e: &mut Expr) {
        ...
        match &mut *opt.base {
            OptChainBase::Member(MemberExpr { span, obj, .. }) => {
                //
                if is_pure_undefined_or_null(self.ctx.expr_ctx, obj) {
                    self.changed = true;
                    report_change!(
            ...
            OptChainBase::Call(OptCall { span, callee, .. }) => {
                if is_pure_undefined_or_null(self.ctx.expr_ctx, callee) {
                    self.changed = true;
                    report_change!(
