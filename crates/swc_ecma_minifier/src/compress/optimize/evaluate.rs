    ...
    fn eval_global_vars(&mut self, e: &mut Expr) {
        ...
        match e {
            Expr::Ident(Ident {
                ...
                debug!("evaluate: `undefined` -> `void 0`");
                self.changed = true;
                *e = *undefined(*span);
            }
    ...
    fn eval_numbers(&mut self, e: &mut Expr) {
        ...
        if let Expr::Call(..) = e {
            if let Some(value) = eval_as_number(e) {
                self.changed = true;
                debug!("evaluate: Evaluated an expression as `{}`", value);
    ...
    pub(super) fn optimize_bin_and_or(&mut self, e: &mut BinExpr) {
        ...
        if let Expr::Bin(left) = &mut *e.left {
            ...
            if let Known(v) = v {
                ...
                if v && e.op == op!("&&") {
                    self.changed = true;
                    debug!("Removing `b` from `a && b && c` because b is always truthy");
                ...
                if !v && e.op == op!("||") {
                    self.changed = true;
                    debug!("Removing `b` from `a || b || c` because b is always falsy");
