...
impl Optimizer<'_> {
    ...
    pub(super) fn make_bool_short(
        ...
        match e {
            Expr::Cond(cond) => {
                ...
                if negate_cost(self.ctx.expr_ctx, &cond.test, true, false) >= 0 {
                    return;
                }
                ...
                if in_bool_ctx {
                    match *op {
                        op!("||") => {
                            ...
                            if let Value::Known(false) = right.as_pure_bool(self.ctx.expr_ctx) {
                                report_change!(
                        ...
                        op!("&&") => {
                            ...
                            if let (_, Value::Known(false)) = left.cast_to_bool(self.ctx.expr_ctx) {
                                report_change!(
        ...
        let cost = negate_cost(self.ctx.expr_ctx, e, in_bool_ctx, ignore_return_value);
    ...
    pub(super) fn optimize_negate_eq(&mut self, e: &mut Expr) {
        ...
        let arg_can_negate = can_absorb_negate(arg, self.ctx.expr_ctx);
    ...
    pub(super) fn compress_cmp_with_long_op(&mut self, e: &mut BinExpr) {
        ...
        let should_optimize = is_typeof_unaray(&e.left, &e.right)
            ...
            || (self.options.comparisons && {
                if let Value::Known(l) = e.left.get_type(self.ctx.expr_ctx) {
                    if let Value::Known(r) = e.right.get_type(self.ctx.expr_ctx) {
                        l == r
                    } else {
                        false
                    }
        ...
        match e {
            ...
            | Expr::Bin(BinExpr {
                ...
                if let (Value::Known(Type::Bool), Value::Known(Type::Bool)) = (lt, rt) {
                    let rb = right.as_pure_bool(self.ctx.expr_ctx);
                    let rb = match rb {
                        Value::Known(v) => v,
    ...
    pub(super) fn compress_useless_deletes(&mut self, e: &mut Expr) {
        ...
        let convert_to_true = match &*delete.arg {
            ...
            e if is_pure_undefined(self.ctx.expr_ctx, e) => true,

            Expr::Ident(i) => i.ctxt != self.ctx.expr_ctx.unresolved_ctxt,
            ...
            Expr::Bin(BinExpr {
                ...
                let rn = right.as_pure_number(self.ctx.expr_ctx);
            ...
            Expr::Unary(e @ UnaryExpr { op: op!("!"), .. }) => {
                if let Expr::Seq(SeqExpr { exprs, .. }) = &mut *e.arg {
...

                    {
                        ...
                        // Negate last element.
                        negate(self.ctx.expr_ctx, last, false, false);
                    }
    ...
    pub(super) fn optimize_expr_in_bool_ctx(
        ...
        match n {
            ...
            _ => {
                let v = n.as_pure_bool(self.ctx.expr_ctx);
    ...
    fn can_swap_bin_operands(&mut self, l: &Expr, r: &Expr, is_for_rel: bool) -> bool {
        match (l, r) {
            ...
            (Expr::Member(..) | Expr::Call(..) | Expr::Assign(..), r)
                if is_pure_undefined(self.ctx.expr_ctx, r) =>
            {
                true
            }
        ...
        let can_swap = can_swap
            || (matches!(op, op!("*") | op!("&") | op!("|") | op!("^"))
                ...
                && left
                    ...
                    .filter(|b| b.op.precedence() == op.precedence())
                    .is_none()
                && !left.may_have_side_effects(self.ctx.expr_ctx)
