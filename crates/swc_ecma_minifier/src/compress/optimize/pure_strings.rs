...
impl Optimizer<'_> {
    ...
    pub(super) fn eval_str_addition(&mut self, e: &mut Expr) {
        ...
        match l_l.get_type(self.ctx.expr_ctx) {
            Known(Type::Str) => {}
            _ => return,
        }
        match r_l.get_type(self.ctx.expr_ctx) {
            Known(Type::Str) => {}
            _ => return,
        }

        let lls = l_l.as_pure_string(self.ctx.expr_ctx);
        let rls = r_l.as_pure_string(self.ctx.expr_ctx);
    ...
    pub(super) fn concat_str(&mut self, e: &mut Expr) {
        ...
        ) = e
        {
            ...
            {
                let type_of_second = left.right.get_type(self.ctx.expr_ctx);
                let type_of_third = bin.right.get_type(self.ctx.expr_ctx);

                if let Value::Known(Type::Str) = type_of_second {
                    if let Value::Known(Type::Str) = type_of_third {
                        if let Value::Known(second_str) = left.right.as_pure_wtf8(self.ctx.expr_ctx)
                        {
                            if let Value::Known(third_str) =
                                bin.right.as_pure_wtf8(self.ctx.expr_ctx)
                            {
                                #[cfg(feature = "debug")]
