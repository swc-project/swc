...
impl Pure<'_> {
    ...
    pub(super) fn lift_seqs_of_bin(&mut self, e: &mut Expr) {
        ...
        if let Expr::Seq(left) = &mut *bin.left {
            ...
            self.changed = true;
            debug!("sequences: Lifting sequence in a binary expression");
    ...
    pub(super) fn lift_seqs_of_cond_assign(&mut self, e: &mut Expr) {
        ...
        if let Expr::Seq(test) = &mut *cond.test {
            ...
            if test.exprs.len() >= 2 {
                ...
                self.changed = true;
                debug!("sequences: Lifting sequences in a assignment with cond expr");
        ...
        for idx in 0..e.exprs.len() {
            ...
            ) = (&mut *a, &mut *b)
            {
                ...
                }) = &mut **b_callee
                {
                    ...
                    b.take();
                    self.changed = true;
                    debug!("sequences: Reducing `(a = foo, a.call())` to `((a = foo).call())`");
