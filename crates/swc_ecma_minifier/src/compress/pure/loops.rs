...
impl Pure<'_> {
    ...
    pub(super) fn loop_to_for_stmt(&mut self, s: &mut Stmt) {
        ...
        match s {
            Stmt::While(stmt) => {
                self.changed = true;
                debug!("loops: Converting a while loop to a for loop");
                *s = Stmt::For(ForStmt {
                    span: stmt.span,
                    init: None,
            ...
            Stmt::DoWhile(stmt) => {
                ...
                if let Value::Known(true) = val {
                    self.changed = true;
                    debug!("loops: Converting an always-true do-while loop to a for loop");
        ...
        if let Stmt::Block(body) = &mut *s.body {
            ...
            }) = first
            {
                if let Stmt::Break(BreakStmt { label: None, .. }) = &**cons {
                    ...
                    debug!("loops: Optimizing a for loop with an if-then-break");
            ...
            }) = first
            {
                if let Stmt::Break(BreakStmt { label: None, .. }) = &**alt {
                    ...
                    debug!("loops: Optimizing a for loop with an if-else-break");
    ...
    pub(super) fn merge_for_if_break(&mut self, s: &mut ForStmt) {
        ...
        }) = &mut *s.body
        {
            if let Stmt::Break(BreakStmt { label: None, .. }) = &**cons {
                ...
                // this pass.
                self.changed = true;
                debug!("loops: Compressing for-if-break into a for statement");
