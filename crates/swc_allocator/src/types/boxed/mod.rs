...
use crate::{allocators::Global, util::Allocator};
...
#[cfg_attr(
    ...
    deprecated(
        note = "This type is slow due to less optimizations (e.g. lack of optimizations based on \
                specialization compared to the standard library) if the cargo feature `nightly` \
                is not enabled. You can enable `swc_allocator/nightly` to make this type faster, \
                or `swc_allocator/skip-warning` to suppress this warning."
    )
)]
pub struct Box<T: ?Sized, A: Allocator = Global>(Inner<T, A>);
...
impl<T> Box<T> {
    ...
    /// `Box::pin(x)` does the same as
    /// <code>[Box::into_pin]\([Box::new]\(x))</code>. Consider using
...
impl<T, A: Allocator> Box<T, A> {
    ...
    /// Constructs a new `Pin<Box<T, A>>`. If `T` does not implement [`Unpin`],
    /// then `x` will be pinned in memory and unable to be moved.
    ///
    /// Constructing and pinning of the `Box` can also be done in two steps:
    /// `Box::pin_in(x, alloc)` does the same as
    /// <code>[Box::into_pin]\([Box::new_in]\(x, alloc))</code>. Consider using
    /// [`into_pin`](Box::into_pin) if you already have a `Box<T, A>`, or if you
    /// want to construct a (pinned) `Box` in a different way than with
    /// [`Box::new_in`].
    #[cfg(not(no_global_oom_handling))]
    #[must_use]
    ...
    pub fn into_inner(boxed: Self) -> T {
        ...
        // Though the default `Drop` implementation drops the both the pointer and the
        // allocator, here we only want to drop the allocator.
...
impl<T> Box<[T]> {
    ...
    /// Constructs a new boxed slice with uninitialized contents, with the
    ...
    /// Constructs a new boxed slice with uninitialized contents. Returns an
    ...
    /// Constructs a new boxed slice with uninitialized contents, with the
    /// memory being filled with `0` bytes. Returns an error if the
    /// allocation fails
    ///
    /// See [`MaybeUninit::zeroed`][zeroed] for examples of correct and
    /// incorrect usage of this method.
...
impl<T, A: Allocator> Box<[T], A> {
    /// Constructs a new boxed slice with uninitialized contents in the provided
    /// allocator.
    ///
    /// # Examples
    ///
    ...
    /// Constructs a new boxed slice with uninitialized contents in the provided
    /// allocator, with the memory being filled with `0` bytes.
    ///
    /// See [`MaybeUninit::zeroed`][zeroed] for examples of correct and
    /// incorrect usage of this method.
    ///
    ...
    /// Constructs a new boxed slice with uninitialized contents in the provided
    /// allocator. Returns an error if the allocation fails.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Constructs a new boxed slice with uninitialized contents in the provided
    /// allocator, with the memory being filled with `0` bytes. Returns an
    /// error if the allocation fails.
    ///
    /// See [`MaybeUninit::zeroed`][zeroed] for examples of correct and
    /// incorrect usage of this method.
...
impl<T: ?Sized, A: Allocator> Box<T, A> {
    ...
    ///
    /// Note: this is an associated function, which means that you have
    /// to call it as `Box::into_raw_with_allocator(b)` instead of
    /// `b.into_raw_with_allocator()`. This is so that there is no conflict
    ...
    pub fn into_non_null(b: Self) -> (NonNull<T>, A) {
        // Box is recognized as a "unique pointer" by Stacked Borrows, but internally it
        // is a raw pointer for the type system. Turning it directly into a raw
        // pointer would not be recognized as "releasing" the unique pointer to
        // permit aliased raw accesses, so all raw pointer methods have to go
        // through `Box::leak`. Turning *that* to a raw pointer
        // behaves correctly.
    ...
    A: 'static,
{
    ...
    ///
    /// Constructing and pinning a `Box` with
    /// <code><Pin<Box\<T>>>::from([Box::new]\(x))</code> can also be
    /// written more concisely using <code>[Box::pin]\(x)</code>.
    /// This `From` implementation is useful if you already have a `Box<T>`, or
    /// you are constructing a (pinned) `Box` in a different way than with
    /// [`Box::new`].
    #[inline(always)]
 ...
 *        Unpin because of trait objects, for which the structural auto trait
 *        functionality does not apply (e.g., Box<dyn Foo> would otherwise
 *        not be Unpin).
 *
