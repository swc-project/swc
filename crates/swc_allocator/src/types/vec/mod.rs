...
pub struct Vec<T, A = Global>(Inner<T, A>)
where
    A: Allocator;
...
impl<T> Vec<T> {
    ...
    ///
    /// fn main() {
    ///     let layout = Layout::array::<u32>(16).expect("overflow cannot
    /// happen");
    ///
    ///     let vec = unsafe {
...
impl<T, A: Allocator> Vec<T, A> {
    ...
    ///
    /// These requirements are always upheld by any `ptr` that has been
    /// allocated via `Vec<T, A>`. Other allocation sources are allowed if
    /// the invariants are upheld.
    ///
    /// Violating these may cause problems like corrupting the allocator's
    /// internal data structures. For example it is **not** safe
    /// to build a `Vec<u8>` from a pointer to a C `char` array with length
    /// `size_t`. It's also not safe to build one from a `Vec<u16>` and its
    /// length, because the allocator cares about the alignment, and these
    /// two types have different alignments. The buffer was allocated with
    /// alignment 2 (for `u16`), but after turning it into a `Vec<u8>` it'll
    /// be deallocated with alignment 1.
    ///
    /// The ownership of `ptr` is effectively transferred to the
    /// `Vec<T>` which may then deallocate, reallocate or change the
    /// contents of memory pointed to by the pointer at will. Ensure
    ...
    /// inserted in the given `Vec<T>`. The collection may reserve more
    ...
    /// insufficient capacity. The caller should use [`reserve`] or
    ...
    pub fn clear(&mut self) {
        ...
        // - Setting `self.len` before calling `drop_in_place` means that, if an
        //   element's `Drop` impl panics, the vector's `Drop` impl will do nothing
        //   (leaking the rest of the elements) instead of dropping some twice.
        unsafe {
            self.len = 0;
    ...
    /// data (e.g. by reading from a file) before marking the data as
    ...
    /// Safety: changing returned .2 (&mut usize) is considered the same as
    /// calling `.set_len(_)`.
    ///
    /// This method provides unique access to all vec parts at once in
    /// `extend_from_within`.
    unsafe fn split_at_spare_mut_with_len(
        ...
        // SAFETY:
        // - `ptr` is guaranteed to be valid for `self.len` elements
        // - `spare_ptr` is pointing one element past the buffer, so it doesn't overlap
        //   with `initialized`
        unsafe {
...
/// [lexicographically](core::cmp::Ord#lexicographical-comparison).
...
impl<T, A: Allocator> Drop for Vec<T, A> {
    ...
    fn drop(&mut self) {
        unsafe {
            // use drop for [T]
            // use a raw slice to refer to the elements of the vector as weakest necessary
            // type; could avoid questions of validity in certain cases
