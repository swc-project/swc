...
use rkyv::{
    ...
    Archive, ArchiveUnsized, Place, Serialize, SerializeUnsized,
};

use super::Box;

impl<T: ArchiveUnsized + ?Sized> Archive for Box<'_, T> {
...
impl<T: SerializeUnsized<S> + ?Sized, S: Fallible + ?Sized> Serialize<S> for Box<'_, T> {
    fn serialize(
        &self,
        serializer: &mut S,
...
//         let out = ptr_meta::from_raw_parts_mut(data_address.cast(),
// metadata);
...
// for ArchivedBox<T> {     #[inline]
//     fn eq(&self, other: &Box<U>) -> bool {
//         self.get().eq(other.as_ref())
//     }
// }

// impl<T: ArchivePointee + PartialOrd<U> + ?Sized, U: ?Sized>
// PartialOrd<Box<U>> for ArchivedBox<T> {     #[inline]
