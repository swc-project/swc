...
// impl<T: Archive, D: Fallible + ?Sized> Deserialize<Vec<T>, D> for
// ArchivedVec<T::Archived> where
//     [T::Archived]: DeserializeUnsized<[T], D>,
//     D::Error: Source,
// {
//     fn deserialize(&self, deserializer: &mut D) -> Result<Vec<T>, <D as
// Fallible>::Error> {         let metadata =
// self.as_slice().deserialize_metadata();         let layout = <[T] as
// LayoutRaw>::layout_raw(metadata).into_error()?;         let data_address = if
// layout.size() > 0 {             unsafe { alloc::alloc(layout) }
//         } else {
