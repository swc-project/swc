...
impl<'ast> Visitor<'ast> for ContextIdentifierVisitor<'_> {
    ...
    fn enter_identifier(&mut self, node: &'ast Identifier, _scope_stack: &[ScopeId]) {
        ...
        // Only process identifiers that resolve to a binding (referenced or
        // declaration)
...
/// Recursively walk an LVal pattern to find all reassignment target
/// identifiers.
...
/// Check if a binding declared at `binding_scope` is captured by a function at
/// `function_scope`. Returns true if the binding is declared above the function
/// (in the parent scope or higher).
...
/// - It is reassigned AND referenced from inside a nested function (`reassigned
///   && referencedByInnerFn`)
///
