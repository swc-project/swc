...
use react_compiler_hir::{
    ...
    visitors::{each_terminal_successor, terminal_fallthrough},
    *,
};
...
pub struct HirBuilder<'a> {
    ...
    /// Maps the outer scope's BindingId to the source location where it was
    /// referenced.
    context: IndexMap<BindingId, Option<SourceLocation>>,
    /// Resolved bindings: maps a BindingId to the HIR IdentifierId created for
    /// it.
    bindings: IndexMap<BindingId, IdentifierId>,
    /// Names already used by bindings, for collision avoidance.
    /// Maps name string -> how many times it has been used (for appending _0,
    /// _1, ...).
    ...
    pub fbt_depth: u32,
    /// The scope of the function being compiled (for context identifier
    /// checks).
    function_scope: ScopeId,
    /// The scope of the outermost component/hook function (for
    /// gather_captured_context).
    component_scope: ScopeId,
    /// Set of BindingIds for variables declared in scopes between
    /// component_scope and any inner function scope, that are referenced
    /// from an inner function scope. These need StoreContext/LoadContext
    /// instead of StoreLocal/LoadLocal.
    context_identifiers: std::collections::HashSet<BindingId>,
    /// Index mapping identifier byte offsets to source locations and JSX
    /// status.
...
impl<'a> HirBuilder<'a> {
    ...
    /// - `bindings`: optional pre-existing bindings (e.g., from a parent
    ///   function)
    ...
    /// Create a new unique TypeVar type, allocated from the environment's type
    /// arena so that TypeIds are consistent with identifier type slots.
    ...
    /// Returns the 'a reference to avoid conflicts with mutable borrows on
    /// self.
    ...
    /// Merge bindings (binding_id -> IdentifierId) from a child builder back
    /// into this builder. This matches TS behavior where parent and child
    /// share the same #bindings map by reference, so bindings resolved by
    /// the child are automatically visible to the parent.
    ...
    /// Terminate the current block with the given terminal and start a new
    /// block.
    ///
    /// If `next_block_kind` is `Some`, a new current block is created with that
    /// kind. Returns the BlockId of the completed block.
    ...
    /// Use `terminate_with_continuation()` to make it current, or `complete()`
    /// to save it directly.
    ...
    /// Like `enter_reserved`, but the closure returns a `Result<Terminal,
    /// CompilerDiagnostic>`.
    ...
    /// Like `enter`, but the closure returns a `Result<Terminal,
    /// CompilerDiagnostic>`.
    pub fn try_enter(
        &mut self,
        kind: BlockKind,
    ...
    /// Create a temporary identifier with a fresh id, returning its
    /// IdentifierId.
    ...
    /// 2. Check for unreachable blocks containing FunctionExpression
    ///    instructions
            ...
            if !rpo_blocks.contains_key(id) {
                ...
                if has_function_expr {
                    ...
                    self.env.record_error(CompilerErrorDetail {
                        ...
                        reason: "Support functions with unreachable code that may contain hoisted \
                                 declarations"
                            .to_string(),
                        description: None,
                        loc,
    ...
    ///
    /// On first encounter, creates a new Identifier with the given name and a
    ...
    /// Map a BindingId to an HIR IdentifierId, with an optional source
    /// location.
    pub fn resolve_binding_with_loc(
        ...
        // In TS, resolveBinding records fbt errors when node.name === 'fbt'. After a
        // name collision causes a rename (e.g., "fbt" -> "fbt_0"), TS's
        // scope.rename changes the AST node's name, preventing subsequent fbt
        // error recording. We simulate this by checking whether the
        ...
        if name == "fbt" {
            ...
            if should_record_fbt_error {
                ...
                self.env.record_error(CompilerErrorDetail {
                    ...
                    description: Some(
                        "Local variables named `fbt` may conflict with the fbt plugin and are not \
                         yet supported"
                            .to_string(),
                    ),
                    loc: error_loc,
        ...
        if is_reserved_word(name) {
            ...
            self.env.record_diagnostic(
                CompilerDiagnostic::new(
                    ...
                    Some(format!(
                        "`{}` is a reserved word in JavaScript and cannot be used as an \
                         identifier name",
                        name
                    )),
                )
    ...
    /// This overrides any previously-set loc (which may have come from a
    /// reference site).
    ...
    /// - ImportDefault, ImportSpecifier, ImportNamespace (program-scope import
    ///   binding)
