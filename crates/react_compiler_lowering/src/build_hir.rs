...
use crate::{
    find_context_identifiers::find_context_identifiers,
    hir_builder::HirBuilder,
    identifier_loc_index::{build_identifier_loc_index, IdentifierLocIndex},
    FunctionNode,
};
...
/// Get the Babel-style type name of an Expression node (e.g. "Identifier",
/// "NumericLiteral").
...
/// ... } } or { "type": "TypeAnnotation", "typeAnnotation": { "type":
/// "GenericTypeAnnotation", ... } } We extract the inner typeAnnotation's
/// `type` field name.
...
fn lower_value_to_temporary(
    ...
    // Optimization: if loading an unnamed temporary, skip creating a new
    // instruction
...
///
/// For local/context identifiers, returns a Place referencing the binding's
/// identifier. For globals/imports, emits a LoadGlobal instruction and returns
/// the temporary Place.
fn lower_identifier(
    builder: &mut HirBuilder,
    name: &str,
            ...
            if let VariableBinding::Global { ref name } = binding {
                if name == "eval" {
                    builder.record_error(CompilerErrorDetail {
                        ...
                        description: Some(
                            "Eval is an anti-pattern in JavaScript, and the code executed cannot \
                             be evaluated by React Compiler"
                                .to_string(),
                        ),
                        loc: loc.clone(),
                        suggestions: None,
...
fn lower_member_expression_with_object(
    ...
    // OptionalMemberExpression has the same shape as MemberExpression for property
    // access
    use react_compiler_ast::expressions::Expression;
...
fn lower_expression(
    ...
    match expr {
            ...
            match &unary.operator {
                react_compiler_ast::operators::UnaryOperator::Delete => {
                    ...
                    match &*unary.argument {
                        ...
                        _ => {
                            // delete on non-member expression (e.g., delete x) - not commonly
                            // supported
                            builder.record_error(CompilerErrorDetail {
                                reason: "Unsupported delete target".to_string(),
                                category: ErrorCategory::Todo,
            ...
            match update.argument.as_ref() {
                ...
                Expression::Identifier(ident) => {
                    ...
                    if builder.is_context_identifier(&ident.name, start) {
                        builder.record_error(CompilerErrorDetail {
                            category: ErrorCategory::Todo,
                            reason: "(BuildHIR::lowerExpression) Handle UpdateExpression to \
                    ...
                    match &binding {
                        VariableBinding::Global { .. } => {
                            builder.record_error(CompilerErrorDetail {
                                category: ErrorCategory::Todo,
                                reason: "UpdateExpression where argument is a global is not yet \
                                         supported"
                                    .to_string(),
                                description: None,
                                loc: loc.clone(),
                    ...
                    let identifier = match binding {
                        ...
                        _ => {
                            builder.record_error(CompilerErrorDetail {
                                category: ErrorCategory::Todo,
                                reason: "(BuildHIR::lowerExpression) Support UpdateExpression \
                                         where argument is a global"
                                    .to_string(),
                                description: None,
                                loc: loc.clone(),
                                suggestions: None,
            ...
            if matches!(expr.operator, AssignmentOperator::Assign) {
                ...
                let binary_op = match expr.operator {
                    ...
                    | AssignmentOperator::NullishAssign => {
                        ...
                        builder.record_error(CompilerErrorDetail {
                            reason: "Logical assignment operators (||=, &&=, ??=) are not yet \
                                     supported"
                                .to_string(),
                            category: ErrorCategory::Todo,
                            loc: loc.clone(),
        ...
        Expression::ArrowFunctionExpression(_) => {
            // The expression type is already known to be ArrowFunctionExpression at this
            // point, so lower_function's non-function invariant cannot fail.
            // Safe to unwrap.
        ...
        Expression::TaggedTemplateExpression(tagged) => {
            ...
            if !tagged.quasi.expressions.is_empty() {
                builder.record_error(CompilerErrorDetail {
                    category: ErrorCategory::Todo,
                    reason: "(BuildHIR::lowerExpression) Handle tagged template with \
                             interpolations"
                        .to_string(),
                    description: None,
                    loc: loc.clone(),
            ...
            if quasi.value.raw != quasi.value.cooked.clone().unwrap_or_default() {
                builder.record_error(CompilerErrorDetail {
                    category: ErrorCategory::Todo,
                    reason: "(BuildHIR::lowerExpression) Handle tagged template where cooked \
                             value is different from raw value"
                        .to_string(),
                    description: None,
                    loc: loc.clone(),
                    suggestions: None,
        ...
        Expression::MetaProperty(meta) => {
            ...
            if meta.meta.name == "import" && meta.property.name == "meta" {
                ...
                builder.record_error(CompilerErrorDetail {
                    category: ErrorCategory::Todo,
                    reason: "(BuildHIR::lowerExpression) Handle MetaProperty expressions other \
                             than import.meta"
                        .to_string(),
                    description: None,
                    loc: loc.clone(),
        ...
        Expression::JSXElement(jsx_element) => {
            ...
            for attr_item in &jsx_element.opening_element.attributes {
                use react_compiler_ast::jsx::{
                    JSXAttributeItem, JSXAttributeName, JSXAttributeValue,
                };
                match attr_item {
                            ...
                            JSXAttributeName::JSXIdentifier(id) => {
                                ...
                                if name.contains(':') {
                                    builder.record_error(CompilerErrorDetail {
                                        category: ErrorCategory::Todo,
                                        reason: format!(
                                            "(BuildHIR::lowerExpression) Unexpected colon in \
                                             attribute name `{}`",
                                            name
                                        ),
                                        description: None,
        ...
        Expression::TypeCastExpression(tc) => {
            ...
            // Flow TypeCastExpression: typeAnnotation is a TypeAnnotation node wrapping the
            // actual type
            let inner_type = tc
                .type_annotation
                .get("typeAnnotation")
...
///
/// Implements the TS BlockStatement hoisting pass: identifies forward
/// references to block-scoped bindings and emits DeclareContext instructions to
/// hoist them.
...
fn lower_block_statement_inner(
    ...
    use react_compiler_ast::{scope::BindingKind as AstBindingKind, statements::Statement};

    // Look up the block's scope to identify hoistable bindings.
    // Use the scope override if provided (for function body blocks that share the
    // function's scope).
    ...
    // Exclude bindings whose declaration_type is "FunctionExpression" since named
    // function expression names are local to the expression and should never be
    // hoisted.
    ...
    // Track which bindings have been "declared" (their declaration statement has
    // been seen)
    ...
    for body_stmt in &block.body {
        ...
        for (binding_id, name, kind, decl_type, decl_start) in &hoistable {
            ...
            // Find the first reference (not declaration) to this binding in the statement's
            // range. Exclude JSX identifier references since TS hoisting
            // traversal only visits Identifier nodes, not JSXIdentifier nodes.
        ...
        for info in &will_hoist {
            ...
            // Hoisted identifiers also become context identifiers (matching TS
        ...
        match body_stmt {
            ...
            _ => {
                // For other statement types (e.g. ForStatement with
                // VariableDeclaration in init), we rely on the
                // reference_to_binding check for forward references.
                // Any bindings declared by child scopes won't be in this
                // block's scope anyway.
            }
        }
...
fn lower_statement(
    ...
    match stmt {
        ...
        Statement::VariableDeclaration(var_decl) => {
            use react_compiler_ast::{patterns::PatternLike, statements::VariableDeclarationKind};
            if matches!(var_decl.kind, VariableDeclarationKind::Var) {
                ...
                // Treat `var` as `let` so references to the variable don't
                // break
            }
            ...
            for declarator in &var_decl.declarations {
                ...
                if let Some(init) = &declarator.init {
                    ...
                    builder.record_error(CompilerErrorDetail {
                        reason: "Expected variable declaration to be an identifier if no \
                ...
                let init_loc = match &for_stmt.init {
                    ...
                    Some(init) => match init.as_ref() {
                        ...
                        react_compiler_ast::statements::ForInit::Expression(expr) => {
                            ...
                            builder.record_error(CompilerErrorDetail {
                                category: ErrorCategory::Todo,
                                reason: "(BuildHIR::lowerStatement) Handle non-variable \
                                         initialization in ForStatement"
                                    .to_string(),
                                description: None,
                                loc: loc.clone(),
                                suggestions: None,
        ...
        Statement::TryStatement(try_stmt) => {
            ...
            let handler_clause = match &try_stmt.handler {
                ...
                None => {
                    builder.record_error(CompilerErrorDetail {
                        category: ErrorCategory::Todo,
                        reason: "(BuildHIR::lowerStatement) Handle TryStatement without a catch \
                                 clause"
                            .to_string(),
                        description: None,
                        loc: loc.clone(),
            ...
            if try_stmt.finalizer.is_some() {
                builder.record_error(CompilerErrorDetail {
                    category: ErrorCategory::Todo,
                    reason: "(BuildHIR::lowerStatement) Handle TryStatement with a finalizer \
                             ('finally') clause"
                        .to_string(),
                    description: None,
                    loc: loc.clone(),
                ...
                if let Some(param) = &handler_clause.param {
                    ...
                    if is_destructuring {
                        ...
                        for id_loc in id_locs {
                            builder.record_error(CompilerErrorDetail {
                                reason: "(BuildHIR::lowerAssignment) Could not find binding for \
                                         declaration."
                                    .to_string(),
                                category: ErrorCategory::Invariant,
                                loc: id_loc,
            ...
            // Use the catch param's loc for the assignment, matching TS:
            // handlerBinding.path.node.loc
        ...
        Statement::WithStatement(with_stmt) => {
            ...
            builder.record_error(CompilerErrorDetail {
                ...
                description: Some(
                    "'with' syntax is considered deprecated and removed from JavaScript \
                     standards, consider alternatives"
                        .to_string(),
                ),
                loc: loc.clone(),
                suggestions: None,
        ...
        | Statement::ExportAllDeclaration(_) => {
            ...
            builder.record_error(CompilerErrorDetail {
                category: ErrorCategory::Syntax,
                reason: "JavaScript `import` and `export` statements may only appear at the top \
                         level of a module"
                    .to_string(),
                description: None,
                loc: loc.clone(),
...
///
/// Receives a `FunctionNode` (discovered by the entrypoint) and lowers it to
/// HIR. The `id` parameter provides the function name (which may come from the
/// variable declarator rather than the function node itself, e.g. `const Foo =
/// () => {}`).
pub fn lower(
    func: &FunctionNode<'_>,
    _id: Option<&str>,
    ...
    // Extract params, body, generator, is_async, loc, scope_id, and the AST
    // function's own id Note: `id` param may include inferred names (e.g., from
    // `const Foo = () => {}`), but the HIR function's `id` field should only
    // include the function's own AST id (FunctionDeclaration.id or
    // FunctionExpression.id, NOT arrow functions).
    ...
    // Pre-compute context identifiers: variables captured across function
    // boundaries
    let context_identifiers = find_context_identifiers(func, scope_info);

    // Build identifier location index from the AST (replaces serialized
    // referenceLocs/jsxReferencePositions)
            ...
            let temp = if !member.computed
                ...
                match &*member.property {
                    ...
                    _ => {
                        builder.record_error(CompilerErrorDetail {
                            reason: format!(
                                "(BuildHIR::lowerAssignment) Handle {} properties in \
                                 MemberExpression",
                                expression_type_name(&member.property)
                            ),
                            category: ErrorCategory::Todo,
                ...
                if matches!(
                    ...
                    builder.record_error(CompilerErrorDetail {
                        reason: "(BuildHIR::lowerAssignment) Expected private name to appear as a \
                ...
                match prop {
                    react_compiler_ast::patterns::ObjectPatternProperty::RestElement(rest) => {
                        match &*rest.argument {
                            PatternLike::Identifier(id) => {
                                ...
                                if can_use_direct {
                                    match lower_identifier_for_assignment(
                                        ...
                                        Some(IdentifierForAssignment::Global { .. }) => {
                                            builder.record_error(CompilerErrorDetail {
                                                reason: "Expected reassignment of globals to \
                                                         enable forceTemporaries"
                                                    .to_string(),
                                                category: ErrorCategory::Todo,
                            ...
                            _ => {
                                builder.record_error(CompilerErrorDetail {
                                    reason: format!(
                                        "(BuildHIR::lowerAssignment) Handle {} rest element in \
                                         ObjectPattern",
                                        match &*rest.argument {
                    ...
                    react_compiler_ast::patterns::ObjectPatternProperty::ObjectProperty(
                        ...
                        if obj_prop.computed {
                            builder.record_error(CompilerErrorDetail {
                                reason: "(BuildHIR::lowerAssignment) Handle computed properties \
                                         in ObjectPattern"
                                    .to_string(),
                                category: ErrorCategory::Todo,
...
/// The `value_place` is stored into a temporary; we also return it as an
/// InstructionValue via LoadLocal for the top-level call.
...
fn lower_function_declaration(
    ...
    // behavior where the parent and child share the same #bindings map by
    // reference.
    builder.merge_bindings(child_bindings);
    ...
    // Note: we must resolve from the function's INNER scope, not using
    // reference_to_binding directly. This matches TS behavior where Babel's
    // `path.scope.getBinding()` resolves from the function declaration's inner
    // scope. If there's an inner variable that shadows the function name (e.g.,
    // `function hasErrors() { let hasErrors = ... }`), Babel's scope resolution
    // finds the inner binding, not the outer function binding.
    ...
    match body {
        ...
        FunctionBody::Block(block) => {
            directives = block
                ...
                .collect();
            // Use lower_block_statement_with_scope to get hoisting support for the function
            // body. Pass the function scope since in Babel, a function body
            // BlockStatement shares the function's scope (node_to_scope maps
            // the function node, not the block).
...
fn lower_jsx_member_expression(
    ...
    // Use the full member expression's loc for instruction locs (matching TS:
    // exprPath.node.loc)
    ...
    let object = match &*expr.object {
        JSXMemberExprObject::JSXIdentifier(id) => {
            let id_loc = convert_opt_loc(&id.base.loc);
            let start = id.base.start.unwrap_or(0);
            // Use identifier's own loc for the place, but member expression's loc for the
            // instruction
...
fn lower_jsx_element(
    ...
    use react_compiler_ast::jsx::{JSXChild, JSXExpressionContainerExpr};
    match child {
...
fn trim_jsx_text(original: &str) -> Option<String> {
    ...
    // That check is handled by the caller (lower_jsx_element) before calling this
    // function.
...
fn lower_reorderable_expression(
    ...
    if !is_reorderable_expression(builder, expr, true) {
        builder.record_error(CompilerErrorDetail {
            category: ErrorCategory::Todo,
            reason: format!(
                "(BuildHIR::node.lowerReorderableExpression) Expression type `{}` cannot be \
                 safely reordered",
...
fn is_reorderable_expression(
    ...
    match expr {
        ...
        Expression::MemberExpression(member) => {
            // Allow member expressions where the innermost object is a global or
            // module-local
...
/// Returns the "type" field value, e.g. "TSTypeReference",
...
///
/// Walks through all identifier references (via `reference_to_binding`) and
/// checks which ones resolve to bindings declared in scopes between the
/// function's parent scope and the component scope. These are "free variables"
/// that become the function's `context`.
