...
use react_compiler_ast::{
    declarations::*, expressions::*, jsx::*, patterns::*, scope::ScopeInfo, statements::*,
};
...
fn scope_info_round_trip() {
    ...
    println!(
        "\n{passed}/{total} fixtures passed scope info round-trip ({skipped} skipped - no \
         scope.json)"
    );
...
/// Fallback walker for serde_json::Value fields (class bodies, type
/// annotations, decorators, etc.)
...
fn visit_expr(expr: &mut Expression, si: &ScopeInfo) {
    match expr {
        ...
        Expression::ArrayExpression(e) => {
            for el in e.elements.iter_mut().flatten() {
                visit_expr(el, si);
            }
        }
...
fn visit_pat(pat: &mut PatternLike, si: &ScopeInfo) {
    match pat {
        ...
        PatternLike::ArrayPattern(ap) => {
            for el in ap.elements.iter_mut().flatten() {
                visit_pat(el, si);
            }
...
fn scope_resolution_rename() {
    ...
    if !failures.is_empty() {
        let show_count = failures.len().min(5);
        let mut msg = format!(
            "\n{} of {total} fixtures failed scope resolution rename (showing first \
             {show_count}):\n\n",
            failures.len()
        );
        for (name, err) in failures.iter().take(show_count) {
