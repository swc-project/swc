...
/// Identifies a binding (variable declaration) in the binding table. Copy-able,
/// used as an index.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
pub struct BindingData {
    ...
    /// Used to distinguish declaration sites from references in
...
pub struct ImportBindingData {
    /// The module specifier string (e.g., "react" in `import {useState} from
    /// 'react'`).
    pub source: String,
    pub kind: ImportBindingKind,
    /// For named imports: the imported name (e.g., "bar" in `import {bar as
    /// baz} from 'foo'`). None for default and namespace imports.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct ScopeInfo {
    /// All scopes, indexed by ScopeId. scopes[id.0] gives the ScopeData for
    /// that scope.
    pub scopes: Vec<ScopeData>,
    /// All bindings, indexed by BindingId. bindings[id.0] gives the
    /// BindingData.
    ...
    /// Maps an Identifier AST node's start offset to the binding it resolves
    /// to. Only present for identifiers that resolve to a binding (not
    /// globals). Uses IndexMap to preserve insertion order (source order
    /// from serialization).
...
impl ScopeInfo {
    ...
    /// Look up the binding for an identifier reference by its AST node start
    /// offset. Returns None for globals/unresolved references.
    ...
    /// Look up a binding by name in the scope that contains the identifier at
    /// `start`. Used as a fallback when position-based lookup
    /// (`resolve_reference`) returns a binding whose name doesn't match --
    /// e.g., when Babel's Flow component transform creates multiple params
