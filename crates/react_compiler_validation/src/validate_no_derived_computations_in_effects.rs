...
use react_compiler_hir::{
    ...
    ReturnVariant, SourceLocation, Type,
};
...
fn get_identifier_name_with_loc(
    ...
    // Fall back: find another identifier with the same declaration_id that has a
    // name.
...
/// Thin wrapper around canonical `each_instruction_operand` that maps Places to
/// (id, loc) pairs.
...
fn validate_effect(
    ...
    // Consider setStates in the effect's dependency array as being part of
    // effectSetStateUsages
...
/// Records errors directly on the Environment (matching TS `env.recordError()`
/// behavior).
...
///
/// NOTE: This intentionally does NOT use the canonical
/// `each_instruction_value_operand` because: (1) `validate_effect_non_exp` only
/// matches specific variants (ComputedLoad, PropertyLoad, BinaryExpression,
/// TemplateLiteral, CallExpression, MethodCall), so
/// FunctionExpression/ObjectMethod context handling is unnecessary; and (2) the
/// caller does not have access to `env` which the canonical function requires
