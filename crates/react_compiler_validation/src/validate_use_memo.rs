...
/// Returns VoidUseMemo errors separately (for logging via logErrors, not as
/// compile errors).
        ...
        for (loc, ident_name) in unused_use_memos.values() {
            void_memo_errors.push_diagnostic(
                CompilerDiagnostic::new(
                    ...
                    Some(
                        "This useMemo() value is unused. useMemo() is for computing and caching \
                         values, not for arbitrary side effects"
                            .to_string(),
                    ),
                )
...
fn handle_possible_use_memo_call(
    ...
    if validate_no_void_use_memo && !has_non_void_return(body_func) {
        ...
        if let Some(callee_loc) = callee.loc {
            // The callee is always useMemo/React.useMemo since we checked is_use_memo
            // above. The identifierName in Babel's AST SourceLocation is
            // "useMemo".
            ...
            if let InstructionValue::StoreContext { lvalue, .. } = &instr.value {
                if context.contains(&lvalue.place.identifier) {
                    errors.push_diagnostic(
                        CompilerDiagnostic::new(
                            ErrorCategory::UseMemo,
                            "useMemo() callbacks may not reassign variables declared outside of \
                             the callback",
                            Some(
                                "useMemo() callbacks must be pure functions and cannot reassign \
                                 variables defined outside of the callback function"
                                    .to_string(),
                            ),
                        )
                        .with_detail(CompilerDiagnosticDetail::Error {
...
/// Thin wrapper around canonical
/// `each_instruction_value_operand_with_functions` that maps to ids.
