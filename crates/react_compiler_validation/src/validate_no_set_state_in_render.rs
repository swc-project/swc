...
//! Validates that the function does not unconditionally call setState during
//! render.
//!
...
use react_compiler_hir::{
    dominator::compute_unconditional_blocks, environment::Environment, BlockId, HirFunction,
    Identifier, IdentifierId, InstructionValue, Type,
};
                            ...
                            if enable_use_keyed_state {
                                errors.push(
                                    CompilerDiagnostic::new(
                                        ...
                                        Some(
                                            "Calling setState during render may trigger an \
                                             infinite loop.\n* To reset state when other \
                                             state/props change, use `const [state, setState] = \
                                             useKeyedState(initialState, key)` to reset `state` \
                                             when `key` changes.\n* To derive data from other \
                                             state/props, compute the derived data during render \
                                             without using state"
                                                .to_string(),
                                        ),
                                    )
