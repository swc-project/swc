...
/// Corresponds to TS `RefAccessRefType` — the subset of `RefAccessType` that
/// can appear inside `Structure.value` and be joined via
...
impl RefAccessType {
    /// Try to convert a `RefAccessType` to a `RefAccessRefType` (the
    /// Ref/RefValue/Structure subset).
                ...
                match &instr.value {
                    ...
                    InstructionValue::UnaryExpression {
                        ...
                        if *operator == UnaryOperator::Not {
                            ...
                            {
                                /*
                                 * Record an error suggesting the `if (ref.current == null)`
                                 * pattern, but also record the
                                 * lvalue as a guard so that we don't emit a
                                ...
                                errors.push(
                                    ...
                                    .with_detail(
                                        CompilerDiagnosticDetail::Hint {
                                            message: "To initialize a ref only once, check that \
                                                      the ref is null with the pattern `if \
                                                      (ref.current == null) { ref.current = ... }`"
                                                .to_string(),
                                        },
                                    ),
                                );
