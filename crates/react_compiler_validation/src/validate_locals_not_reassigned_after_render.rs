...
pub fn validate_locals_not_reassigned_after_render(func: &HirFunction, env: &mut Environment) {
    ...
    // Record accumulated errors (from async function checks in inner functions)
    // first
    ...
    if let Some(reassignment_place) = reassignment {
        ...
        env.record_diagnostic(
            CompilerDiagnostic::new(
                ...
                Some(format!(
                    "Reassigning {} after render has completed can cause inconsistent behavior on \
                     subsequent renders. Consider using state instead",
                    variable_name
                )),
            )
...
///
/// Side effects: accumulates async-function reassignment diagnostics into
/// `diagnostics`.
                ...
                | InstructionValue::ObjectMethod { lowered_func, .. } => {
                    ...
                    if let Some(ref reassignment_place) = reassignment {
                        if inner_is_async {
                            ...
                            // Return null (don't propagate further) — matches
                            // TS behavior
                        } else {
