...
use react_compiler_hir::{
    environment::Environment, DeclarationId, DependencyPathEntry, Identifier, IdentifierId,
    IdentifierName, InstructionKind, InstructionValue, ManualMemoDependency,
    ManualMemoDependencyRoot, Place, ReactiveBlock, ReactiveFunction, ReactiveInstruction,
    ReactiveScopeBlock, ReactiveStatement, ReactiveValue, ScopeId,
};

/// State tracked during manual memo validation within a
/// StartMemoize..FinishMemoize range.
struct ManualMemoBlockState {
    /// Reassigned temporaries (declaration_id -> set of identifier ids that
    /// were reassigned to it).
...
///
/// Walks the reactive function looking for StartMemoize/FinishMemoize
/// instructions and checks that:
...
fn visit_instruction(instr: &ReactiveInstruction, state: &mut VisitorState) {
    ...
    match &instr.value {
                ...
                if let Some(scope_id) = ident.scope {
                    if !state.scopes.contains(&scope_id) && !state.pruned_scopes.contains(&scope_id)
                    {
                        let diag = CompilerDiagnostic::new(
                            ...
                            Some(
                                "React Compiler has skipped optimizing this component because the \
                                 existing manual memoization could not be preserved. This \
                                 dependency may be mutated later, which could cause the value to \
                                 change unexpectedly"
                                    .to_string(),
                            ),
                        )
        ...
        ReactiveValue::Instruction(InstructionValue::FinishMemoize {
            ...
            // TS: CompilerError.invariant(state.manualMemoState.manualMemoId ===
            // value.manualMemoId, ...)
            assert!(
    ...
    let diag = CompilerDiagnostic::new(
        ...
        Some(
            "React Compiler has skipped optimizing this component because the existing manual \
             memoization could not be preserved. This value was memoized in source but not in \
             compilation output"
                .to_string(),
        ),
    )
...
fn record_temporaries(instr: &ReactiveInstruction, state: &mut VisitorState) {
    ...
    // Record deps from the instruction value first (before setting lvalue
    // temporary)
...
fn record_deps_in_value(value: &ReactiveValue, state: &mut VisitorState) {
    match value {
        ...
        ReactiveValue::Instruction(iv) => {
            ...
            // TS: if (value.kind === 'StoreLocal' || value.kind === 'StoreContext' ||
...
/// Validate that an inferred dependency matches a source dependency or was
/// produced within the manual memo block.
fn validate_inferred_dep(
    ...
    let extra = if is_named(ident) {
        // Use the original dep_id/dep_path (matching TS
        // prettyPrintScopeDependency(dep))
    ...
    let description = format!(
        "React Compiler has skipped optimizing this component because the existing manual \
         memoization could not be preserved. The inferred dependencies did not match the manually \
         specified dependencies, which could cause the value to change more or less frequently \
         than expected. {}",
        extra
    );
