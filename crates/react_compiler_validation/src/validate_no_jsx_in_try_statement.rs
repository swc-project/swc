...
//! leading them to use patterns such as `let el; try { el = <Component /> }
//! catch { ... }` to attempt to catch rendering errors. Such code will fail to
//! catch errors in rendering, but developers may not realize this right away.
//!
//! This validation pass errors for JSX created within a try block. JSX is
//! allowed within a catch statement, unless that catch is itself nested inside
//! an outer try.
//!
