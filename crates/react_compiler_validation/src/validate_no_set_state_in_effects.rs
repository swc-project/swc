...
//! Validates against calling setState in the body of an effect (useEffect and
//! friends), while allowing calling setState in callbacks scheduled by the
//! effect.
//!
//! Calling setState during execution of a useEffect triggers a re-render, which
//! is often bad for performance and frequently has more efficient and
//! straightforward alternatives. See https://react.dev/learn/you-might-not-need-an-effect for examples.
//!
...
use react_compiler_hir::{
    dominator::{compute_post_dominator_tree, post_dominator_frontier},
    ...
    PlaceOrSpread, PropertyLiteral, SourceLocation, Terminal, Type,
};
...
/// Uses the canonical `each_instruction_value_operand_with_functions` from
/// visitors.
...
/// Checks inner function body for direct setState calls. Returns the callee
/// Place info if a setState call is found in the function body.
/// Tracks ref-derived values to allow setState when the value being set comes
/// from a ref.
fn get_set_state_call(
    ...
    // First pass: collect ref-derived values (needed before building control
    // dominator checker) We do a pre-pass to seed ref_derived_values so the
    // control dominator checker has them.
