...
///
/// Note: takes `&mut HirFunction` (deviating from the read-only validation
/// convention) because it sets `has_invalid_deps` on StartMemoize instructions
/// when validation errors are found, so that ValidatePreservedManualMemoization
/// can skip those blocks.
...
// =============================================================================
// Path helpers (matching TS areEqualPaths, isSubPath,
                    ...
                    if let Some(cb) = callbacks.as_mut() {
                        ...
                        if let Some(sm) = sm {
                            ...
                            if cb.validate_memo {
                                ...
                                // Use ALL dependencies collected since StartMemoize cleared the
                                // set. This matches TS: `const
                                // inferred = Array.from(dependencies)`
                            ...
                            // Restore saved state (matching TS: dependencies.clear(),
                            // locals.clear()) We restore instead of
                            // just clearing because we need the outer deps back
...
fn validate_dependencies(
    ...
    // retainWhere logic: keep dep[ix] only if no earlier entry is equal or a
    // subpath prefix Mirrors TS: retainWhere(inferred, (dep, ix) => {
    ...
    for dep in &filtered_missing {
        ...
        } = dep
        {
            ...
            if is_stable_type(ty) {
                hint = ". Refs, setState functions, and other \"stable\" values generally do not \
                        need to be added as dependencies, but this variable may change over time \
                    ...
                    if let InferredDependency::Local { identifier, .. } = matching {
                        ...
                        if is_effect_event_function_type(matching_ty) {
                            ...
                            diagnostic.details.push(CompilerDiagnosticDetail::Error {
                                loc: dep.loc.or(manual_memo_loc),
                                message: Some(format!(
                                    "Functions returned from `useEffectEvent` must not be \
                                     included in the dependency array. Remove `{dep_str}` from \
                                     the dependencies."
                                )),
                                identifier_name: None,
                            });
                            ...
                            diagnostic.details.push(CompilerDiagnosticDetail::Error {
                                loc: dep.loc.or(manual_memo_loc),
                                message: Some(format!(
                                    "Overly precise dependency `{dep_str}`, use `{inferred_str}` \
                                     instead"
                                )),
                                identifier_name: None,
                            });
    ...
    let (reason, description) = match category {
                ...
                if !extra.is_empty() {
                    Some(
                        "Extra dependencies can cause a value to update more often than it \
                         should, resulting in performance problems such as excessive renders or \
        ...
        ErrorCategory::EffectExhaustiveDependencies => {
            ...
            let desc_parts: Vec<&str> = [
                if !missing.is_empty() {
                    Some(
                        "Missing dependencies can cause an effect to fire less often than it \
                         should",
                    )
                } else {
                    None
                ...
                if !extra.is_empty() {
                    Some(
                        "Extra dependencies can cause an effect to fire more often than it \
                         should, resulting in performance problems such as excessive renders and \
                         side effects",
                    )
                } else {
                    None
...
/// Collect lvalue identifier ids from instruction value (for the default
/// branch). Thin wrapper around canonical `each_instruction_value_lvalue` that
/// maps to ids.
