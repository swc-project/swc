...
/// Validates that components used in JSX are not dynamically created during
/// render.
///
                    ...
                    if let JsxTag::Place(tag_place) = tag {
                        if let Some(location) = known_dynamic_components.get(&tag_place.identifier)
                        {
                            ...
                            let diagnostic = CompilerDiagnostic::new(
                                ...
                                Some(
                                    "Components created during render will reset their state each \
                                     time they are created. Declare components outside of render"
                                        .to_string(),
                                ),
                            )
