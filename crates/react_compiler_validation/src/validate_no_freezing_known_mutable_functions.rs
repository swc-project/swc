...
/// Validates that functions with known mutations (ie due to types) cannot be
/// passed where a frozen value is expected.
///
/// Because a function that mutates a captured variable is equivalent to a
/// mutable value, and the receiver has no way to avoid calling the function,
/// this pass detects functions with *known* mutations (Mutate or
/// MutateTransitive, not conditional) that are passed where a frozen value is
/// expected and reports an error.
                        ...
                        'effects: for effect in aliasing_effects {
                            match effect {
                                ...
                                | AliasingEffect::MutateTransitiveConditionally { value, .. } => {
                                    // Only propagate existing known mutations for conditional
                                    // effects
                                    if let Some(known_mutation) =
                                        context_mutation_effects.get(&value.identifier)
                                    {
...
/// If an operand with Effect::Freeze is a known-mutable function, emit a
/// diagnostic.
        ...
        if let Some(mutation_info) = context_mutation_effects.get(&operand.identifier) {
            ...
            diagnostics.push(
                CompilerDiagnostic::new(
                    ...
                    Some(format!(
                        "This argument is a function which may reassign or mutate {} after \
