...
use swc_ecma_visit::{
    arena::{visit_mut_pass, VisitMut, VisitMutWith},
    noop_visit_mut_type_arena,
};

pub use super::rename::rename;
use super::rename::{renamer, Renamer};
...
/// value of [Config].
pub fn hygiene(alloc: &Allocator) -> impl Pass<'_> + VisitMut<'_> {
    hygiene_with_config(alloc, Default::default())
}
...
pub fn hygiene_with_config(alloc: &Allocator, config: Config) -> impl Pass<'_> + VisitMut<'_> {
    (
        renamer(alloc, config, HygieneRenamer),
...
impl VisitMut<'_> for HygieneRemover {
    noop_visit_mut_type_arena!();
