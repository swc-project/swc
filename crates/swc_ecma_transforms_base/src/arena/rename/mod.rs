...
use swc_ecma_visit::{
    arena::{visit_mut_pass, Fold, VisitMut, VisitMutWith, VisitWith},
    noop_visit_mut_type, noop_visit_mut_type_arena,
};
...
pub fn rename<'a>(alloc: &'a Allocator, map: &'a RenameMap) -> impl Pass<'a> + VisitMut<'a> {
    rename_with_config(alloc, map, Default::default())
}

pub fn rename_with_config<'this, 'a: 'this>(
    alloc: &'a Allocator,
    map: &'this RenameMap,
    config: Config,
) -> impl Pass<'a> + VisitMut<'a> + 'this {
...
pub fn renamer<R>(alloc: &Allocator, config: Config, renamer: R) -> impl Pass<'_> + VisitMut<'_>
where
    R: Renamer,
{
...
impl<'a, R> RenamePass<'a, R>
where
    R: Renamer,
{
    fn get_unresolved<N>(&self, n: &N, has_eval: bool) -> FxHashSet<Atom>
    where
        N: VisitWith<'a, IdCollector> + VisitWith<'a, CustomBindingCollector<Id>>,
    {
        let usages = {
    ...
    ($name:ident, $T:ty) => {
        ...
        fn $name(&mut self, n: &mut $T) {
            if !self.config.ignore_eval && contains_eval(n, true) {
                ...
                if !map.is_empty() {
                    n.visit_mut_with(&mut rename_with_config(
                        self.alloc,
                        &map,
                        self.config.clone(),
                    ));
...
impl<'a, R> VisitMut<'a> for RenamePass<'a, R>
where
    R: Renamer,
{
    noop_visit_mut_type_arena!();

    unit!(visit_mut_arrow_expr, ArrowExpr<'a>);

    unit!(visit_mut_setter_prop, SetterProp<'a>);

    unit!(visit_mut_getter_prop, GetterProp<'a>);

    unit!(visit_mut_constructor, Constructor<'a>);

    unit!(visit_mut_fn_expr, FnExpr<'a>);

    unit!(visit_mut_method_prop, MethodProp<'a>);

    unit!(visit_mut_class_method, ClassMethod<'a>);

    unit!(visit_mut_private_method, PrivateMethod<'a>);

    fn visit_mut_fn_decl(&mut self, n: &mut FnDecl<'a>) {
        if !self.config.ignore_eval && contains_eval(n, true) {
            n.visit_mut_children_with(self);
        } else {
    ...
    fn visit_mut_class_decl(&mut self, n: &mut ClassDecl<'a>) {
        if !self.config.ignore_eval && contains_eval(n, true) {
            n.visit_mut_children_with(self);
        } else {
            let id = n.ident.to_id();
    ...
    fn visit_mut_default_decl(&mut self, n: &mut DefaultDecl<'a>) {
        match n {
            DefaultDecl::Class(n) => {
                n.visit_mut_children_with(self);
            }
    ...
    fn visit_mut_module(&mut self, m: &mut Module<'a>) {
        self.load_cache();
        ...
        if !map.is_empty() {
            m.visit_mut_with(&mut rename_with_config(
                self.alloc,
                &map,
                self.config.clone(),
            ));
    ...
    fn visit_mut_script(&mut self, m: &mut Script<'a>) {
        self.load_cache();
