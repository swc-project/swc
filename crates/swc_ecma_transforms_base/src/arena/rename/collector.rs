...
use swc_ecma_utils::{arena::ident::IdentLike, stack_size::maybe_grow_default};
use swc_ecma_visit::{
    arena::{Visit, VisitWith},
    noop_visit_type_arena, visit_obj_and_computed_arena,
};
...
impl<I> Visit<'_> for CustomBindingCollector<I>
where
    I: IdentLike + Eq + Hash + Send + Sync,
{
    noop_visit_type_arena!();
...
pub(super) fn collect_decls<'a, I, N>(
    n: &N,
    top_level_mark_for_eval: Option<Mark>,
) -> (FxHashSet<I>, FxHashSet<I>)
where
    I: IdentLike + Eq + Hash + Send + Sync,
    N: VisitWith<'a, CustomBindingCollector<I>>,
{
