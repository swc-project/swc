...
use react_compiler_hir::{
    ...
    visitors,
    visitors::{ScopeBlockInfo, ScopeBlockTraversal},
...
/// We track processed instructions/terminals by their lvalue IdentifierId +
/// block id. In TS this uses reference identity (Set<Instruction | Terminal>).
...
fn traverse_optional_block(
    ...
    let (test_terminal, base_object) = match &maybe_test_block.terminal {
        ...
        Terminal::Optional {
            ...
            if !is_optional {
                // Non-optional load: record that PropertyLoads from inner optional are
                // hoistable
    ...
    ctx.processed_instrs_in_optional
        .insert(ProcessedInstr::Terminal(match &test_terminal {
            Terminal::Branch { .. } => {
                ...
                // The terminal belongs to either maybe_test_block or the fallthrough block of
                // inner optional We need to identify which block this terminal
                // belongs to. For the base case, it's test_block_id.
                // For nested optional, it's the fallthrough block.
                // We'll use the block_id approach based on what we know.
                // Actually, we tracked the terminal by its block, so we need to find which
                // block contains this terminal. Let's use a pragmatic approach:
                // The test terminal we matched was from maybe_test_block or from the inner
                // fallthrough block. We'll search for it.

                // For the base case (Branch terminal at maybe_test_block), block_id =
                // test_block_id For the nested case, the test terminal is at
                // the fallthrough block of inner optional In either case, we
                // stored the terminal as test_terminal which comes from a known block.
                ...
                // we can just look up which block it belongs to by finding blocks whose
                // terminal pointer matches. But that's expensive. Instead,
                // let's use the block approach and find the block from the
...
/// set of property string paths de-duplicates. If unconditional reads from
/// `<base>` are hoistable (i.e., `<base>` is in the set), we replace
/// `<base>?.PROPERTY` with `<base>.PROPERTY`.
///
    ...
    loop {
        ...
        for original_idx in to_process {
            ...
            for entry in &full_path.path {
                // If the base is known to be non-null (in the set), replace optional with
                // non-optional
...
/// Returns the set of LoweredFunction FunctionIds that are assumed to be
/// invoked. The `temporaries` map is shared across recursive calls (matching TS
/// behavior where the same Map is passed to recursive invocations for inner
/// functions).
...
/// Uses 'active'/'done' state tracking to correctly handle cycles (backedges in
/// loops).
///
...
/// neighbors that are 'done' (not 'active'). Active neighbors are part of a
/// cycle and should be filtered out, allowing non-null info to propagate
/// through non-cyclic paths.
...
fn recursively_propagate_non_null(
    ...
    // Compute intersection of 'done' neighbors only (filter out 'active' = cycle
    // nodes)
    ...
    // Compare with previous value — can't just check size due to
...
struct ReactiveScopeDependencyTreeHIR {
    ...
    dep_roots: IndexMap<IdentifierId, (DependencyNode, bool)>,     /* node + reactive (preserves
                                                                    * insertion order like JS
                                                                    * Map) */
}
