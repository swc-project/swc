...
//! Aligns scopes of object method values to that of their enclosing object
...
use react_compiler_hir::{
    environment::Environment, EvaluationOrder, HirFunction, IdentifierId, InstructionValue,
    ObjectPropertyOrSpread, ScopeId,
};
                ...
                InstructionValue::ObjectExpression { properties, .. } => {
                    for prop_or_spread in properties {
                        ...
                        if object_method_decls.contains(&operand_place.identifier) {
                            ...
                            // TS: CompilerError.invariant(operandScope != null && lvalueScope !=
                            // null, ...)
                            let operand_sid = operand_scope.expect(
                                "Internal error: Expected all ObjectExpressions and ObjectMethods \
                                 to have non-null scope.",
                            );
...
pub fn align_object_method_scopes(func: &mut HirFunction, env: &mut Environment) {
    // Handle inner functions first (TS recurses before processing the outer
    // function)
    for (_block_id, block) in &func.body.blocks {
        for &instr_id in &block.instructions {
            let instr = &func.instructions[instr_id.0 as usize];
    ...
    // Use a HashMap to accumulate min/max across all scopes mapping to the same
    // root, matching TS behavior where root.range is updated in-place during
    // iteration.
