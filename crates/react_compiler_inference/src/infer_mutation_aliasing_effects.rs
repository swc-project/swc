...
use react_compiler_hir::{
    ...
    type_config::{ValueKind, ValueReason},
    visitors, AliasingEffect, AliasingSignature, BlockId, DeclarationId, Effect, FunctionId,
    HirFunction, IdentifierId, InstructionKind, InstructionValue, MutationReason, ParamPattern,
    Place, PlaceOrSpread, PlaceOrSpreadOrHole, ReactFunctionType, SourceLocation, Type,
};
...
/// Infers mutation/aliasing effects for all instructions and terminals in
/// `func`.
///
/// Corresponds to TS `inferMutationAliasingEffects(fn,
    ...
    while !queued_states.is_empty() {
        ...
        if iteration_count > 100 {
            return Err(CompilerDiagnostic::new(
                ...
                "[InferMutationAliasingEffects] Potential infinite loop: A value, temporary \
                 place, or effect was not cached properly",
                None,
            ));
        }
...
/// Unique allocation-site identifier, replacing TS's object-identity on
/// InstructionValue.
...
impl InferenceState {
    ...
    /// Check the kind of a place, recording the usage location for error
    /// reporting.
    fn kind_with_loc(
        &self,
    ...
    fn freeze_value(&mut self, value_id: ValueId, reason: ValueReason) {
        ...
        // Note: In TS, this also transitively freezes FunctionExpression
        // captures if enableTransitivelyFreezeFunctionExpressions is
        // set. We skip that here since we don't have access to the
        // function arena from within state.
    }
...
struct Context {
    ...
    /// Cache of ValueIds keyed by effect hash, ensuring stable allocation-site
    /// identity across fixpoint iterations. Mirrors TS
    ...
    /// Cache of temporary places created for aliasing signature config
    /// temporaries. Keyed by (lvalue_identifier_id, temp_name) to ensure
...
impl Context {
    ...
    /// Get or create a stable ValueId for a given effect, ensuring fixpoint
    /// convergence.
        ...
        for instr_id in &block.instructions {
            ...
            match &instr.value {
                ...
                InstructionValue::JsxFragment { .. } | InstructionValue::JsxExpression { .. } => {
                    // Passing objects created with spread to jsx can't mutate
                    // them
                }
                InstructionValue::PropertyLoad { .. } => {
                    // Properties must be frozen since the original value was
                    // frozen
                }
...
fn infer_block(
    ...
    let action = {
        ...
        match &block.terminal {
            react_compiler_hir::Terminal::Try {
                handler,
            ...
            react_compiler_hir::Terminal::MaybeThrow {
...
fn apply_signature(
                            ...
                            } = effect
                            {
                                diagnostic.details.push(
                                    react_compiler_diagnostics::CompilerDiagnosticDetail::Hint {
                                        message: "Hint: If this value is a Ref (value returned by \
                                                  `useRef()`), rename the variable to end in \
                                                  \"Ref\"."
                                            .to_string(),
                                    },
                                );
    ...
    // edge cases where effects don't cover the lvalue (e.g. missing signature
    // entries).
    ...
    match effect {
        ...
        AliasingEffect::Create {
            ...
            assert!(
                ...
                "[InferMutationAliasingEffects] Cannot re-initialize variable within an \
                 instruction"
            );
        ...
        | AliasingEffect::Capture { ref from, ref into } => {
            ...
            // For Alias, destination must already be initialized (Capture/MaybeAlias are
            // exempt)
            assert!(
                is_capture || is_maybe_alias || initialized.contains(&into.identifier),
                "[InferMutationAliasingEffects] Expected destination to already be initialized \
        ...
        AliasingEffect::Apply {
            ...
            if let Some(sig) = signature {
                ...
                if let Some(ref incompatible_msg) = sig.known_incompatible {
                    if env.enable_validations() {
                        let mut diagnostic = CompilerDiagnostic::new(
                            ...
                            Some(
                                "This API returns functions which cannot be memoized without \
                                 leading to stale UI. To prevent this, by default React Compiler \
                                 will skip memoizing this component/hook. However, you may see \
                                 issues if values from this API are passed to other \
                ...
                // Legacy signature
                let mut todo_errors: Vec<react_compiler_diagnostics::CompilerErrorDetail> =
                    Vec::new();
                ...
                )
            {
                    ...
                    && context.hoisted_context_declarations.contains_key(&decl_id)
                {
                    ...
                    let mut diagnostic = CompilerDiagnostic::new(
                        ...
                        Some(format!(
                            "{} is accessed before it is declared, which prevents the earlier \
...
fn compute_signature_for_instruction(
    ...
    match value {
        ...
        InstructionValue::JsxExpression {
            ...
            for prop in props {
                if let react_compiler_hir::JsxAttribute::Attribute {
                    place: prop_place, ..
                } = prop
                {
...
fn get_argument_effect(
    ...
    Effect,
    Option<react_compiler_diagnostics::CompilerErrorDetail>,
) {
    if !is_spread {
        (sig_effect, None)
...
/// Returns true if all of the arguments are both non-mutable (immutable or
/// frozen) _and_ are not functions which might mutate their arguments.
///
...
fn compute_effects_for_aliasing_signature_config(
    ...
    // Create temporaries (cached by lvalue + temp_name to be stable across fixpoint
    // iterations)
...
/// Build an AliasingSignature from a function expression's
/// params/returns/aliasing effects. Corresponds to TS
...
fn get_write_error_reason(abstract_value: &AbstractValue) -> String {
    if abstract_value.reason.contains(&ValueReason::Global) {
        "Modifying a variable defined outside a component or hook is not allowed. Consider using \
         an effect"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::JsxCaptured) {
        "Modifying a value used previously in JSX is not allowed. Consider moving the modification \
         before the JSX"
            .to_string()
    ...
    {
        "Modifying component props or hook arguments is not allowed. Consider using a local \
         variable instead"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::State) {
        "Modifying a value returned from 'useState()', which should not be modified directly. Use \
         the setter function to update instead"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::ReducerState) {
        "Modifying a value returned from 'useReducer()', which should not be modified directly. \
         Use the dispatch function to update instead"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::Effect) {
        "Modifying a value used previously in an effect function or as an effect dependency is not \
         allowed. Consider moving the modification before calling useEffect()"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::HookCaptured) {
        "Modifying a value previously passed as an argument to a hook is not allowed. Consider \
         moving the modification before calling the hook"
            .to_string()
    } else if abstract_value.reason.contains(&ValueReason::HookReturn) {
        "Modifying a value returned from a hook is not allowed. Consider moving the modification \
         into the hook where the value is constructed"
            .to_string()
    } else {
...
/// Returns the successor blocks used for traversal in mutation/aliasing
/// inference.
///
...
/// aliasing effects (Create + Capture) vs Place elements (Create or
/// CreateFrom).
enum PatternItem<'a> {
    Place(&'a Place),
    Spread(&'a Place),
