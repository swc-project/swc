...
//! 1. Forward data-flow: identify all macro tags (including property loads like
//!    `fbt.param`)
//! 2. Reverse data-flow: merge arguments of macro invocations into the same
//!    scope

use std::collections::{HashMap, HashSet};

use react_compiler_hir::{
    environment::Environment, visitors, HirFunction, IdentifierId, InstructionValue, JsxTag,
    PrimitiveValue, PropertyLiteral, ScopeId,
};

/// Whether a macro requires its arguments to be transitively inlined (e.g.,
/// fbt) or just avoids having the top-level values be converted to variables
/// (e.g., fbt.param).
#[derive(Debug, Clone)]
enum InlineLevel {
    Transitive,
...
/// Main entry point. Returns the set of identifier IDs that are fbt/macro
/// operands.
...
/// Expand the scope range on the environment, reading from identifier's
/// mutable_range. Equivalent to TS `expandFbtScopeRange`.
