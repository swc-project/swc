...
//! each other. Two scopes must either be entirely disjoint or one must be
//! nested within the other. This pass detects overlapping scopes and merges
//! them.
//!
...
use react_compiler_hir::{
    ...
    visitors,
    visitors::{each_instruction_lvalue_ids, each_terminal_operand_ids},
...
fn collect_scope_info(func: &HirFunction, env: &Environment) -> ScopeInfo {
    ...
    // The TS uses Set<ReactiveScope> which preserves insertion order and
    // deduplicates. We must NOT sort by ScopeId here — the insertion order
    // determines which scope becomes the root in the disjoint set union.
...
/// Merges reactive scopes that have overlapping ranges.
///
/// Corresponds to TS `mergeOverlappingReactiveScopesHIR(fn: HIRFunction):
/// void`.
pub fn merge_overlapping_reactive_scopes_hir(func: &mut HirFunction, env: &mut Environment) {
    ...
    // This matches TS behavior where groupScope.range is updated in-place during
    // iteration.
    ...
    // In TS, identifier.mutableRange shares the same object reference as
    // scope.range. When scope.range is updated, ALL identifiers referencing
    // that range object automatically see the new values — even identifiers
    // whose scope was later set to null. In Rust, we must explicitly find and
    // update identifiers whose mutable_range matches a root scope's original
    // range.
    let mut original_root_ranges: HashMap<ScopeId, (EvaluationOrder, EvaluationOrder)> =
        HashMap::new();
    ...
    // Sync mutable_range for ALL identifiers whose mutable_range matches the
    // ORIGINAL range of a root scope that was updated. In TS,
    // identifier.mutableRange shares the same object reference as scope.range,
    // so when scope.range is updated, all identifiers referencing that range
    // object automatically see the new values — even identifiers whose scope
    // was later set to null. In Rust, we must explicitly find and update these.
    ...
    // Rewrite all references: for each place that had a scope, point to the merged
    // root. Note: we intentionally do NOT update mutable_range for repointed
    // identifiers, matching TS behavior where identifier.mutableRange still
    // references the old scope's range object after scope repointing.
...
/// Used to check for Primitive type on FunctionExpression/ObjectMethod
/// operands.
