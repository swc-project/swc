...
use react_compiler_hir::{
    environment::Environment, visitors, DeclarationId, EvaluationOrder, HirFunction, IdentifierId,
    ...
    for (_group_id, state) in &scopes {
            ...
            || scope.range.end.0 > max_instruction.0 + 1
        {
            return Err(CompilerDiagnostic::new(
                ...
                &format!(
                    "Invalid mutable range for scope: Scope @{} has range [{}:{}] but the valid \
                     range is [1:{}]",
                    scope.id.0,
                    scope.range.start.0,
                    scope.range.end.0,
...
///
/// Corresponds to TS `findDisjointMutableValues(fn: HIRFunction):
