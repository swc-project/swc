...
use react_compiler_hir::{
    dominator::post_dominator_frontier, environment::Environment, object_shape::HookKind, visitors,
...
pub fn infer_reactive_places(
            ...
            for (phi_idx, phi) in block.phis.iter().enumerate() {
                ...
                for (op_idx, (_pred, operand)) in phi.operands.iter().enumerate() {
                    ...
                    if op_reactive {
                        ...
                        break; // TS breaks here — remaining operands NOT
                               // visited
                    }
                }
                if is_phi_reactive {
    ...
    // Propagate reactivity to inner functions (read-only phase, just queries
    // reactive_map)
