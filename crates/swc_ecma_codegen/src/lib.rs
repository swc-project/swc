    ...
    S: SourceMapperExt,
{
    ...
    fn emit_num_lit_internal(
        ...
        if self.cfg.minify {
            if num.value.is_infinite() && num.raw.is_some() {
                self.wr.write_str_lit(DUMMY_SP, num.raw.as_ref()?)?;
            } else {
    ...
    fn emit_bin_expr_trailing(&mut self, node: &BinExpr) -> Result {
        ...
        if is_kwd_op {
            keyword!(emitter, self, node.op.as_str());
        } else {
    ...
    fn emit_first_of_list5(
        ...
        let is_empty = children.is_none() || start > children? || count == 0;
                    ...
                    match self.cm.span_to_snippet(parent_node) {
                        Ok(snippet) => {
                            if snippet.len() < 3 {
                                ...
                                let last_char = snippet.chars().last()?;
    ...
    fn emit_list5<N: Node>(
        ...
        let is_empty = children.is_none() || start > children?.len() || count == 0;
...
impl MacroNode for Class {
    fn emit(&mut self, emitter: &mut Macro) -> Result {
        if self.super_class.is_some() {
            ...
            {
                let starts_with_alpha_num = self.super_class.as_ref()?.starts_with_alpha_num();
...
impl MacroNode for Bool {
    fn emit(&mut self, emitter: &mut Macro) -> Result {
        ...
        keyword!(emitter, if self.value { "true" } else { "false" });
    }
}
...
impl Macro {
    fn emit_bin_expr_trailing(&mut self, node: &BinExpr) -> Result<(), anyhow::Error> {
        ...
        if is_kwd_op {
            keyword!(emitter, node.op.as_str());
        } else {
