...
pub trait Node: Spanned {
    ...
    /// Modifies span of the node to match the code generated by
    /// [`Node::emit_with`].
    ...
    S: SourceMapperExt,
{
    ...
    fn emit_lit(&mut self, node: &Lit) -> Result {
        ...
        match *node {
            ...
            Lit::Null(Null { .. }) => keyword!(self, "null"),
    ...
    fn emit_meta_prop_expr(&mut self, node: &MetaPropExpr) -> Result {
        ...
        match node.kind {
            MetaPropKind::ImportMeta => keyword!(self, "import.meta"),

            MetaPropKind::NewTarget => keyword!(self, "new.target"),
        }
    }
    ...
    fn emit_bool(&mut self, n: &Bool) -> Result {
        ...
        if n.value {
            keyword!(self, n.span, "true")
        } else {
            keyword!(self, n.span, "false")
        }
    }
    ...
    fn emit_accessibility(&mut self, n: Option<Accessibility>) -> Result {
        if let Some(a) = n {
            match a {
                Accessibility::Public => keyword!(self, self, "public"),
                Accessibility::Protected => keyword!(self, self, "protected"),
                Accessibility::Private => keyword!(self, self, "private"),
            }
            space!(self);
        }
    ...
    S: SourceMapperExt,
{
    ...
    fn emit_block_stmt_inner(&mut self, node: &BlockStmt, skip_first_src_map: bool) -> Result {
        ...
        srcmap!(self, self, node, false, true);
        punct!(self, self, "}");
    ...
    fn emit_return_stmt(&mut self, n: &ReturnStmt) -> Result {
        ...
        if let Some(ref arg) = self.arg {
            let starts_with_alpha_num = match &**arg {
                Expr::Fn(..) | Expr::Class(..) => true,
                _ => arg.starts_with_alpha_num(),
            };
    ...
    fn emit_if_stmt(&mut self, n: &IfStmt) -> Result {
        ...
        let should_emit_as_block = !matches!(&*n.cons, Stmt::Block(..));

        if should_emit_as_block {
            formatting_space!(emitter);
            punct!(emitter, "{");
        ...
        if let Some(ref alt) = n.alt {
            ...
            if matches!(&**alt, Stmt::If(..)) {
                space!(emitter);
                emit!(alt);
            } else if matches!(&**alt, Stmt::Block(..)) {
                formatting_space!(emitter);
                emit!(alt);
            } else {
                formatting_space!(emitter);
    ...
    fn emit_switch_stmt(&mut self, n: &SwitchStmt) -> Result {
        ...
        punct!(emitter, "{");
        emitter.emit_list(self.span, Some(&n.cases), ListFormat::CaseBlockClauses)?;
        punct!(emitter, "}");
    }
    ...
    fn emit_switch_case(&mut self, n: &SwitchCase) -> Result {
        ...
        if n.cons.len() == 1
            && matches!(n.cons[0], Stmt::Block(..))
            && !emitter
                .comments
                .as_ref()
                .map_or(false, |c| c.has_leading(n.cons[0].span().lo))
        {
            space!(emitter);
            emit!(n.cons[0]);
            return;
    ...
    fn emit_throw_stmt(&mut self, n: &ThrowStmt) -> Result {
        ...
        let starts_with_alpha_num = n.arg.starts_with_alpha_num();
        if starts_with_alpha_num {
            space!(emitter);
        } else {
            formatting_space!(emitter);
    ...
    fn emit_try_stmt(&mut self, n: &TryStmt) -> Result {
        ...
        if let Some(ref handler) = n.handler {
            formatting_space!(emitter);
            emit!(handler);
        }
    ...
    fn emit_do_while_stmt(&mut self, node: &DoWhileStmt) -> Result {
        ...
        if matches!(*node.body, Stmt::Block(..)) {
            formatting_space!(emitter);
            emit!(node.body);
            formatting_space!(emitter);
        } else {
    ...
    fn emit_for_stmt(&mut self, n: &ForStmt) -> Result {
        ...
        if let Some(ref update) = n.update {
            emit!(update);
        }
