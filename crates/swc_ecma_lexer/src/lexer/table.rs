...
#[derive(Debug)]
struct TrieNode<K: Copy + Eq + Hash, V> {
    children: FxHashMap<K, TrieNode<K, V>>,
    value: Option<V>,
}

impl<K: Copy + Eq + Hash, V> Default for TrieNode<K, V> {
    fn default() -> Self {
        Self {
...
impl<K: Copy + Eq + Hash, V> TrieNode<K, V> {
    fn new() -> Self {
        Self {
            children: FxHashMap::default(),
            value: None,
...
#[derive(Debug)]
pub struct TrieMap<K: Copy + Eq + Hash, V> {
    root: TrieNode<K, V>,
}

impl<K: Copy + Eq + Hash, V> Default for TrieMap<K, V> {
    fn default() -> Self {
        Self {
...
impl<K: Copy + Eq + Hash + Default, V> TrieMap<K, V> {
    pub fn new() -> Self {
        Self {
            root: TrieNode::new(),
        }
...
macro_rules! known_ident_convert {
    ( $s:expr, $($name:literal => $variant:expr),* ) => {{
        ...
        if $s.len() >= MAX_KNOWN_IDENT_LEN || !LEN_TABLE[$s.len()] {
            ...
            static TIRE: Lazy<TrieMap<u8, Token>> = Lazy::new(|| {
                let mut t = TrieMap::new();
                $(
                    let prev = t.insert(&$name.as_bytes()[1..], $variant);
                    debug_assert!(prev.is_none());
                )*
                t
            });
            TIRE.get(&$s.as_bytes()[1..]).cloned()
        }
    }};
}
