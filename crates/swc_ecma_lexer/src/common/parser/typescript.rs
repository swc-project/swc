...
pub(super) fn try_parse_ts_type_ann<'a, P: Parser<'a>>(p: &mut P) -> PResult<Option<TsTypeAnn>> {
    if !cfg!(feature = "typescript") {
        return Ok(None);
    }
    ...
    if p.input_mut().is(&P::Token::COLON) {
        let pos = p.cur_pos();
        parse_ts_type_ann(p, /* eat_colon */ true, pos).map(Some)
    } else {
        Ok(None)
    }
}
