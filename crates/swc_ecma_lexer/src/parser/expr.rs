...
use crate::{
    common::parser::{
        ...
        expr::{
            ...
            parse_bin_expr, parse_bin_op_recursively, parse_dynamic_import_or_import_meta,
            parse_lit, parse_member_expr_or_new_expr, parse_new_expr, parse_subscripts, parse_tpl,
        },
            ...
            let mut arg = {
                    ...
                    && (is!(self, IdentRef) || (is!(self, "...") && peeked_is!(self, IdentRef)))
                {
                    ...
                    let expr = if spread.is_some() {
                        parse_bin_expr(self.include_in_expr(true).deref_mut())?
                    } else {
                        let mut expr = parse_bin_expr(self.include_in_expr(true).deref_mut())?;
