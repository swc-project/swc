    ...
    pub fn msg(&self) -> Cow<'static, str> {
        match self {
            ...
            SyntaxError::InvalidIdentInStrict(identifier_name) => format!(
                "`{identifier_name}` cannot be used as an identifier in strict mode"
            )
            .into(),
            ...
            SyntaxError::PrivateNameModifier(modifier) => format!(
                "'{modifier}' modifier cannot be used with a private identifier"
            )
            .into(),
            ...
            SyntaxError::TS1274(word) => format!(
                "'{word}' modifier can only appear on a type parameter of a class, interface or type \
                 alias"
            )
            .into(),
            SyntaxError::TS1277(word) => format!(
                "'{word}' modifier can only appear on a type parameter of a function, method or class"
            )
            .into(),
            ...
            SyntaxError::TsNonNullAssertionNotAllowed(word) => format!(
                "Typescript non-null assertion operator is not allowed with '{word}'"
            )
            .into(),
