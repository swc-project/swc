...
use react_compiler_hir::{
    ...
    object_shape::{
        ShapeRegistry, BUILT_IN_ARRAY_ID, BUILT_IN_FUNCTION_ID, BUILT_IN_JSX_ID,
        BUILT_IN_MIXED_READONLY_ID, BUILT_IN_OBJECT_ID, BUILT_IN_PROPS_ID, BUILT_IN_REF_VALUE_ID,
        BUILT_IN_SET_STATE_ID, BUILT_IN_USE_REF_ID,
    },
...
/// Allocate a new TypeVar in the types arena (standalone, no &mut Environment
/// needed).
...
/// Recursively pre-resolve LoadGlobal types for an inner function and its
/// children.
...
/// Check if a property access looks like a ref pattern (e.g. `ref.current`,
/// `fooRef.current`). Matches TS `isRefLikeName` in InferTypes.ts.
...
///
/// Note: Function equality only compares return types (matching TS
/// `funcTypeEquals` which ignores `shapeId` and `isConstructor`). Phi equality
/// always returns false because the TS `phiTypeEquals` has a bug where `return
/// false` is outside the `if` block, so it unconditionally returns false.
...
/// Recursively generate equations for an inner function (accessed via
/// FunctionId).
...
fn apply_function(
    ...
    for (_block_id, block) in &func.body.blocks {
        ...
        for &instr_id in &block.instructions {
            ...
            // LValues from instruction values (StoreLocal, StoreContext, DeclareLocal,
            // DeclareContext, Destructure)
...
fn apply_instruction_operands(
    ...
    match value {
        ...
        InstructionValue::TaggedTemplateExpression { tag, .. } => {
            ...
            // The template quasi's subexpressions are not separate operands in
            // this HIR
        }
        ...
        InstructionValue::FunctionExpression { .. } | InstructionValue::ObjectMethod { .. } => {
            // Inner functions are handled separately via recursion in
            // apply_function
        }
