...
/// Deprecated alias for [`ArrowFunctionBody`].
///
/// This alias preserves only the former type name. The former
/// `BlockStmtOrExpr::BlockStmt` variant has been replaced by
/// [`ArrowFunctionBody::FunctionBody`].
#[deprecated(
    note = "Use `ArrowFunctionBody`; its block variant is `ArrowFunctionBody::FunctionBody`"
)]
pub type BlockStmtOrExpr = ArrowFunctionBody;
