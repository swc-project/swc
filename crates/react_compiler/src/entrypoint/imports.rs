...
use react_compiler_ast::{
    ...
    expressions::{CallExpression, Expression, Identifier},
    literals::StringLiteral,
    patterns::{ObjectPattern, ObjectPatternProp, ObjectPatternProperty, PatternLike},
    scope::ScopeInfo,
    statements::{Statement, VariableDeclaration, VariableDeclarationKind, VariableDeclarator},
    Program, SourceType,
};
...
use super::{
    compile_result::{DebugLogEntry, LoggerEvent, OrderedLogItem},
    plugin_options::{CompilerTarget, PluginOptions},
...
impl ProgramContext {
    ...
    /// Check if a function at the given start position has already been
    /// compiled. This is a workaround for Babel not consistently respecting
    /// skip().
    ...
    /// Call this after construction to seed conflict detection with program
    /// scope bindings.
    ...
    /// Generate a unique identifier name that doesn't conflict with existing
    /// bindings.
    ///
    /// For hook names (use*), preserves the original name to avoid breaking
    /// hook-name-based type inference. For other names, prefixes with
    /// underscore similar to Babel's generateUid.
    ...
    /// Add an import specifier, reusing an existing one if it was already
    /// added.
    ///
