...
//!
//! This module is a port of Program.ts from the TypeScript compiler. It
//! orchestrates the compilation of a program by:
...
use react_compiler_ast::{
    ...
    statements::*,
    visitor::{walk_program_mut, AstWalker, MutVisitor, VisitResult, Visitor},
    File, Program,
};
use react_compiler_diagnostics::{
    CompilerError, CompilerErrorDetail, CompilerErrorOrDiagnostic, ErrorCategory, SourceLocation,
};
use react_compiler_hir::{environment_config::EnvironmentConfig, ReactFunctionType};
...
use super::{
    ...
    imports::{
        add_imports_to_program, get_react_compiler_runtime_module, validate_restricted_imports,
        ProgramContext,
    },
    pipeline,
    plugin_options::{CompilerOutputMode, GatingConfig, PluginOptions},
    suppression::{
...
struct CompileSource<'a> {
    ...
    /// Original AST source location (with index and filename) for logger
    /// events.
...
/// Simple check for valid JavaScript identifier (alphanumeric + underscore + $,
/// starting with letter/$/_ ) Also rejects reserved words like `true`, `false`,
/// `null`, etc.
...
/// Check if an expression is a React API call (e.g., `forwardRef` or
/// `React.forwardRef`).
#[allow(dead_code)]
...
/// (VariableDeclarator, etc.) which is passed explicitly since we don't have
/// Babel paths.
...
/// Returns false for primitive type annotations that indicate this is NOT a
/// component.
...
fn get_react_function_type(
    ...
    //  detected via __componentDeclaration / __hookDeclaration from the Hermes
    // parser)
...
/// Convert CompilerDiagnostic details into serializable CompilerErrorItemInfo
/// items.
...
/// Convert an optional AST SourceLocation to a LoggerSourceLocation with
/// filename.
fn to_logger_loc(
...
/// Convert a diagnostics SourceLocation to a LoggerSourceLocation with
/// filename.
...
fn log_error(
    ...
    // Use the filename from the AST node's loc (set by parser's sourceFilename
    // option), not from plugin options (which may have a different prefix like
    // '/').
...
/// Process a single function: check directives, attempt compilation, handle
/// results.
///
/// Returns `Ok(Some(codegen_fn))` when the function was compiled and should be
/// applied, `Ok(None)` when the function was skipped or lint-only,
...
/// Check if the program already has a `c` import from the React Compiler
/// runtime module. If so, the file was already compiled and should be skipped.
...
struct FunctionInfo<'a> {
    ...
    /// True if the node has `__componentDeclaration` set by the Hermes parser
    /// (Flow component syntax)
    is_component_declaration: bool,
    /// True if the node has `__hookDeclaration` set by the Hermes parser (Flow
    /// hook syntax)
    is_hook_declaration: bool,
}
...
/// Get the variable declarator name (for inferring function names from `const
/// Foo = () => {}`)
...
struct FunctionDiscoveryVisitor<'a, 'ast> {
    ...
    /// Depth counter for loop expression positions (while.test, for-in.right,
    /// etc.). When > 0, functions are treated as non-program-scope in 'all'
    /// mode.
...
struct CompiledFnForReplacement {
    ...
    codegen_fn: CodegenFunction,
    /// Whether this is an original function (vs outlined). Gating only applies
    /// to original.
    #[allow(dead_code)]
...
/// Check if a compiled function is referenced before its declaration at the top
/// level. This is needed for the gating rewrite: hoisted function declarations
/// that are referenced before their declaration site need a special gating
/// pattern.
fn get_functions_referenced_before_declaration(
    program: &Program,
    compiled_fns: &[CompiledFnForReplacement],
) -> HashSet<u32> {
    // Collect function names and their start positions for compiled
    ...
    // Walk through program body in order. For each statement, check if it
    // references any of the function names before the function's declaration.
    for stmt in &program.body {
...
/// Check if a statement references an identifier at the top level (not inside
/// nested functions).
...
/// Build a function expression that preserves the original function's
/// structure. For FunctionDeclarations, converts to FunctionExpression.
...
/// Apply compiled functions back to the AST by replacing original function
/// nodes with their compiled versions, inserting outlined functions, and adding
/// imports.
fn apply_compiled_functions(
    ...
    // If gating is enabled, determine which functions are referenced before
    // declaration
    ...
    // For FunctionDeclarations: insert right after the parent (matching TS
    // insertAfter behavior) For FunctionExpression/ArrowFunctionExpression:
    // append at end of program body   (matching TS pushContainer behavior)
...
///   `function Foo(props) { ... }` -> `const Foo = gating() ? function Foo(...)
/// { compiled } : function Foo(...) { original };`
///
/// For export default function with name:
///   `export default function Foo(props) { ... }` -> `const Foo = gating() ?
/// ... : ...; export default Foo;`
///
...
fn apply_gated_function_conditional(
    ...
    // If this was an export default function with a name, insert `export default
    // Name;` after
...
///
/// This is used when a function declaration is referenced before its
/// declaration site. Instead of wrapping in a conditional expression (which
/// would break hoisting), we:
...
/// 4. Insert a new `function Foo(arg0, ...) { if (gating_result) return
///    Foo_optimized(...); else return Foo_unoptimized(...); }` after
fn apply_gated_function_hoisted(
    ...
    // Insert nodes. The TS code uses insertBefore for the gating result and
    // optimized fn, and insertAfter for the dispatcher. The order in the output
    // should be:   ... (existing statements before fn_idx) ...
...
/// Visitor that replaces a compiled function in the AST by matching
/// `base.start`.
...
/// Visitor that renames all occurrences of an identifier in expression
/// position.
...
///
/// Receives a full program AST, scope information (unused for now), and
...
/// - processFn: per-function compilation with directive and suppression
///   handling
...
pub fn compile_program(mut file: File, scope: ScopeInfo, options: PluginOptions) -> CompileResult {
    ...
    // Create a temporary context for early-return paths (before full context is set
    // up)
    ...
    // Extract the source filename from the AST (set by parser's sourceFilename
    // option). This is the bare filename (e.g., "foo.ts") without path
    // prefixes, which the TS compiler uses in logger event source locations.
    ...
    // Initialize known referenced names from scope bindings for UID collision
    // detection
    ...
    // Clone env_config once for all function compilations (avoids per-function
    // clone while satisfying the borrow checker — compile_fn needs &mut context
    // + &env_config)
    ...
    // In TS, outlined functions from outlineJSX are appended to the compilation
    // queue and processed after all original functions, so their events appear
    // at the end. Regular outlined functions (from OutlineFunctions pass) don't
    // get separate events.
    for compiled in &compiled_fns {
        for outlined in &compiled.codegen_fn.outlined {
    ...
    // TS invariant: if there's a module scope opt-out, no functions should have
    // been compiled
    ...
    // In the TS compiler, dynamic gating from directives takes precedence over
    // plugin-level gating. Gating only applies to 'original' functions, not
    // 'outlined' ones.
    ...
    let replacements: Vec<CompiledFnForReplacement> = compiled_fns
        ...
        .map(|cf| {
            ...
            // Determine per-function gating: dynamic gating from directives OR plugin-level
            // gating. Dynamic gating (from `use memo if(identifier)`) takes
            // precedence.
    ...
    // Serialize the modified File AST directly to a JSON string and wrap as
    // RawValue. This avoids double-serialization (File→Value→String) by going
    // File→String directly. The RawValue is embedded verbatim when the
    // CompileResult is serialized.
...
/// Convert internal BindingRename structs to the serializable BindingRenameInfo
/// format.
fn convert_renames(
