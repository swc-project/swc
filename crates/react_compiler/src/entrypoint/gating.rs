...
use react_compiler_ast::{common::BaseNode, expressions::*, patterns::PatternLike, statements::*};
use react_compiler_diagnostics::{CompilerDiagnostic, ErrorCategory};

use super::{imports::ProgramContext, plugin_options::GatingConfig};
        ...
        if rewrite.referenced_before_declared {
            ...
            if let CompiledFunctionNode::FunctionDeclaration(compiled) = rewrite.compiled_fn {
                ...
                return Err(CompilerDiagnostic::new(
                    ...
                    "Expected compiled node type to match input type: got non-FunctionDeclaration \
            ...
            if !rewrite.is_export_default {
                if let Some(fn_name) = get_fn_decl_name(&original_stmt) {
                    // Convert function declaration to: const fnName = gating() ? compiled :
                    // original
...
/// Helper to create a simple Identifier with the given name and default
/// BaseNode.
...
/// Handles both bare FunctionDeclaration and ExportNamedDeclaration wrapping
/// one.
