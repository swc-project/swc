...
use react_compiler_hir::{
    environment::{Environment, OutputMode},
...
use super::{
    compile_result::{
        CodegenFunction, CompilerErrorDetailInfo, CompilerErrorItemInfo, DebugLogEntry,
        LoggerPosition, LoggerSourceLocation, OutlinedFunction,
    },
    imports::ProgramContext,
    plugin_options::CompilerOutputMode,
};
...
pub fn compile_fn(
    ...
    // Register the memo cache import as a side effect of codegen, matching TS
    // behavior where addMemoCacheImport() is called during
    // codegenReactiveFunction. This must happen BEFORE the env.has_errors()
    // check so the import persists even when the pipeline returns Err (e.g.,
    // when validation errors are accumulated but codegen succeeded).
...
/// fake positions to all Identifier nodes and building the binding/reference
/// maps.
fn build_outlined_scope_info(
    ...
    // Process body - walk all statements to assign positions and collect variable
    // declarations
...
fn outlined_assign_pattern_positions(
    ...
    use react_compiler_ast::{patterns::PatternLike, scope::*};
...
/// Run the compilation pipeline passes on an HIR function (everything after
/// lowering).
///
...
/// Log CompilerError diagnostics as CompileError events, matching TS
/// `env.logErrors()` behavior. These are logged for telemetry/lint output but
/// not accumulated as compile errors.
