...
/// Captures the start and end range of a pair of eslint-disable ...
/// eslint-enable comments. In the case of a CommentLine or a relevant Flow
/// suppression, both the disable and enable point to the same comment.
///
/// The enable comment can be missing in the case where only a disable block is
/// present, ie the rest of the file has potential React violations.
#[derive(Debug, Clone)]
...
/// Check if a comment value matches `eslint-disable-next-line <rule>` for any
/// rule in `rule_names`.
...
/// Check if a comment value matches `eslint-disable <rule>` for any rule in
/// `rule_names`.
...
/// Check if a comment value matches `eslint-enable <rule>` for any rule in
/// `rule_names`.
...
/// Parse eslint-disable/enable and Flow suppression comments from program
/// comments. Equivalent to findProgramSuppressions in Suppression.ts
    ...
    for suppression in suppressions {
        ...
        let description = format!(
            "React Compiler only works when your components follow all the rules of React, \
             disabling them may result in unexpected or incorrect behavior. Found suppression `{}`",
