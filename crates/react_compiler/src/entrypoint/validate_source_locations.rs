...
//! Validates that important source locations from the original code are
//! preserved in the generated AST. This ensures that Istanbul coverage
//! instrumentation can properly map back to the original source code.
//!
...
use react_compiler_ast::{
    common::SourceLocation as AstSourceLocation,
    expressions::{
        ArrowFunctionBody, ArrowFunctionExpression, Expression, FunctionExpression,
        ObjectExpressionProperty,
    },
    patterns::PatternLike,
    statements::{ForInOfLeft, ForInit, Statement, VariableDeclaration},
};
...
pub fn validate_source_locations(
    ...
    // (JS Map preserves insertion order, which is AST traversal order = source
    // order)
...
fn report_missing_location(env: &mut Environment, loc: &AstSourceLocation, node_type: &str) {
    ...
    env.record_diagnostic(
        CompilerDiagnostic::new(
            ...
            Some(format!(
                "Source location for {} is missing in the generated output. This can cause \
                 coverage instrumentation to fail to track this code properly, resulting in \
...
fn report_wrong_node_type(
    ...
    env.record_diagnostic(
        CompilerDiagnostic::new(
            ...
            Some(format!(
                "Source location for {} exists in the generated output but with wrong node \
                 type(s): {}. This can cause coverage instrumentation to fail to track this code \
                 properly, resulting in inaccurate coverage reports.",
                expected_type,
                actual.join(", ")
            )),
        )
...
/// Returns the Babel type name if this statement variant is an "important
/// instrumented type".
...
/// Returns the Babel type name if this expression variant is an "important
/// instrumented type".
