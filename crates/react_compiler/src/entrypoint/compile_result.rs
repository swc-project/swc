use react_compiler_ast::{
    expressions::Identifier as AstIdentifier, patterns::PatternLike, statements::BlockStatement,
};
...
/// Source location with index and filename fields for logger event
/// serialization. Matches the Babel SourceLocation format that the TS compiler
/// emits in logger events.
...
pub enum CompileResult {
    ...
    Success {
        ...
        /// identified by the binding's declaration start position in the
        /// source.
        #[serde(skip_serializing_if = "Vec::is_empty")]
        renames: Vec<BindingRenameInfo>,
        /// Timing data for profiling. Only populated when __profiling is
        /// enabled.
        #[serde(skip_serializing_if = "Vec::is_empty")]
...
/// An item in the ordered log, which can be either a logger event or a debug
/// entry.
#[derive(Debug, Clone, Serialize)]
...
pub enum LoggerEvent {
    ...
    /// Same as CompileError but serializes fnLoc before detail (matching TS
    /// program.ts output)
