...
//             pass.compute_var_names_interference_graph(&liveness, &cfg,
// &escaped_locals);

//         let coloring = GreedyGraphColoring::new(interference_graph,
// coloring_tie_breaker);         coloring.color();
//     }
// }
...
//         // First create a node for each non-escaped variable. We add these
// nodes         // in the order in which they appear in the code because we
// want         // the names that appear earlier in the code to be used
...
//         // interferenceBitSet[i] = indices of all variables that should have
// an edge         // with orderedVariables[i]
...
//             // NOTE(user): In theory, we CAN coalesce function names just
// like any             // variables. Our Liveness analysis captures this just
// like it as             // described in the specification. However, we saw
// some zipped and             // unzipped size increase after this. We are not
// totally sure why             // that is but, for now, we will respect the
// dead functions and not play around             // with it
//             if v.metadata.is_fn {
//                 continue;
//             }

//             // NOTE: we skip class declarations for a combination of two
// reasons:             // 1. they are block-scoped, so we would need to rewrite
// them as class             // expressions      e.g. `class C {}` -> `var C =
// class {}` to avoid             // incorrect semantics
//             //      (see
// testDontCoalesceClassDeclarationsWithDestructuringDeclaration).
// //    This is possible but increases pre-gzip code size and complexity.
//             // 2. since function declaration coalescing seems to cause a size
// regression (as             // discussed    above) we assume that coalescing
// class names may             // cause a similar size regression.
//             if v.metadata.is_class {
//                 continue;
//             }

//             // Skip lets and consts that have multiple variables declared in
// them, otherwise             // this produces incorrect semantics. See test
// case "testCapture".             // Skipping vars technically isn't needed for
// correct semantics, but works             // around a Safari bug for var redeclarations (https://github.com/google/closure-compiler/issues/3164)
...
//             let state: &LinearFlowState<LiveVarLattice> =
// cfg_node.get_annotation();

//             // Check the live states and add edge when possible. An edge
// between             // two variables means that they are alive at
...
//                 LiveRangeChecker::new(Node::clone(&**cfg_node.get_value()),
// ordered_vars, state);
...
//                 // Skip duplicate pairs. Also avoid merging a variable with
// itself.                 if v1_index > v2_index {
//                     continue;
//                 }
...
//                 {
//                     // Skip nodes that were not added. They are globals and
// escaped locals.                     continue;
//                 }

//                 if (v1.metadata.is_param && v2.metadata.is_param)
//                     || interference_bit_set[v1_index].contains(v2_index)
//                 {
//                     // Add an edge between variable pairs that are both
// parameters                     // because we don't want parameters to share a
// name.                     let v1_ix = node_ix[&v1.name];
...
// /// Indices of written and read variables are put in a list. These two lists
// are /// used to mark each written variable as "crossing" all read variables.
// ///
// ///
...
//     pub fn new(root: Node, ordered_vars: Vec<Var>, state:
// LinearFlowState<LiveVarLattice>) -> Self {         Self {
//             root,
//             state,
//             ordered_vars,
