...
//!
//! This implementation is mostly copied from the `core::slice::sort` module,
//! with minimal changes to support stable Rust and parallel `is_less` (e.g.
//! `Fn` rather than `FnMut`).
//!
//! ---
//!
//! This module contains a sorting algorithm based on Orson Peters'
//! pattern-defeating quicksort, published at: <https://github.com/orlp/pdqsort>
//!
//! Unstable sorting is compatible with core because it doesn't allocate memory,
//! unlike our stable sorting implementation.
//!
//! In addition it also contains the core logic of the stable sort used by
//! `slice::sort` based on TimSort.
...
/// Inserts `v[0]` into pre-sorted sequence `v[1..]` so that whole `v[..]`
/// becomes sorted.
///
    ...
    F: Fn(&T, &T) -> bool,
{
    ...
    for i in (0..offset).rev() {
        // SAFETY: we tested that `offset` must be at least 1, so this loop is only
        // entered if len >= 2.We ensured that the slice length is always at
        // least 2 long. We know that start_found will be at least one less than
        // end, and the range is exclusive. Which gives us i always
        // <= (end - 2).
        unsafe {
...
///
/// Returns `true` if the slice is sorted at the end. This function is *O*(*n*)
/// worst-case.
#[cold]
...
///
/// Partitioning is performed block-by-block in order to minimize the cost of
/// branching operations. This idea is presented in the [BlockQuicksort][pdf]
/// paper.
///
...
///
/// Returns the number of elements equal to the pivot. It is assumed that `v`
/// does not contain elements smaller than the pivot.
...
fn break_patterns<T>(v: &mut [T]) {
    ...
    if len >= 8 {
        ...
        for i in 0..3 {
            ...
            // decrease by `len` until it fits into the range `[0, len - 1]`.
    ...
    F: Fn(&T, &T) -> bool,
{
    ...
    if swaps < MAX_SWAPS {
        ...
        // descending or mostly descending, so reversing will probably help sort
        // it faster.
        v.reverse();
        (len - 1 - b, true)
    }
    ...
    F: Fn(&T, &T) -> bool + Sync,
{
    ...
    // If both partitions are up to this length, we continue sequentially. This
    // number is as small as possible but so that the overhead of Rayon's task
    // scheduling is still negligible.
    ...
    loop {
        ...
        // by shuffling some elements around. Hopefully we'll choose a better
        // pivot this time.
        if !was_balanced {
            break_patterns(v);
            limit -= 1;
        ...
        // elements greater than the pivot. This case is usually hit when the
        // slice contains many duplicate elements.
...
/// # Safety
///
/// The argument `buf` is used as a temporary buffer and must hold at least
/// `v.len() / 2`.
...
////////////////////////////////////////////////////////////////////////////
// Everything above this line is copied from `core::slice::sort` (with very
// minor tweaks). Everything below this line is custom parallelization for
// rayon.
////////////////////////////////////////////////////////////////////////////

/// Splits two sorted slices so that they can be merged in parallel.
///
/// Returns two indices `(a, b)` so that slices `left[..a]` and `right[..b]`
/// come before `left[a..]` and `right[b..]`.
    ...
    F: Fn(&T, &T) -> bool,
{
    ...
    if left_len >= right_len {
        ...
        // Find the first element in `right` that is greater than or equal to
        // `left[left_mid]`.
        let mut a = 0;
        let mut b = right_len;
        while a < b {
...
/// Merges slices `left` and `right` in parallel and stores the result into
/// `dest`.
///
/// # Safety
///
...
///
/// Even if `is_less` panics at any point during the merge process, this
/// function will fully copy all elements from `left` and `right` into `dest`
...
where
    ...
    F: Fn(&T, &T) -> bool + Sync,
{
    // Slices whose lengths sum up to this value are merged sequentially. This
    // number is slightly larger than `CHUNK_LENGTH`, and the reason is that
    // merging is faster than merge sorting, so merging needs a bit coarser
    // granularity in order to hide the overhead of Rayon's task scheduling.
    ...
    // 2. Copies the remaining elements as soon as one of the two sides is
    //    exhausted.
    //
    // Panic safety:
    //
    // If `is_less` panics at any point during the merge process, `s` will get
    // dropped and copy the remaining parts of `left` and `right` into `dest`.
    let mut s = State {
    ...
    if left_len == 0 || right_len == 0 || left_len + right_len < MAX_SEQUENTIAL {
        ...
        // Function `split_for_merge` might panic. If that happens, `s` will get
        // destructed and copy the whole `left` and `right` into `dest`.
        ...
        // Prevent the destructor of `s` from running. Rayon will ensure that both calls
        // to `par_merge` happen. If one of the two calls panics, they will
        // ensure that elements still get copied into `dest_left` and
        // `dest_right``.
        mem::forget(s);
    ...
    // Finally, `s` gets dropped if we used sequential merge, thus copying the
    // remaining elements all at once.

    // When dropped, copies arrays `left_start..left_end` and
    // `right_start..right_end` into `dest`, in that order.
    struct State<T> {
        left_start: *mut T,
        left_end: *mut T,
        right_start: *mut T,
...
///
/// Chunks of `v` are stored in `chunks` as intervals (inclusive left and
/// exclusive right bound). Argument `buf` is an auxiliary buffer that will be
/// used during the procedure. If `into_buf` is true, the result will be stored
/// into `buf`, otherwise it will be in `v`.
///
/// # Safety
///
/// The number of chunks must be positive and they must be adjacent: the right
/// bound of each chunk must equal the left bound of the following chunk.
///
    ...
    F: Fn(&T, &T) -> bool + Sync,
{
    ...
    // After recursive calls finish we'll have to merge chunks `(start, mid)` and
    // `(mid, end)` from `src` into `dest`. If the current invocation has to
    // store the result into `buf`, we'll merge chunks from `v` into `buf`, and
    // vice versa.
    //
    // Recursive calls flip `into_buf` at each level of recursion. More concretely,
    // `par_merge` merges chunks from `buf` into `v` at the first level, from
    // `v` into `buf` at the second level etc.
    let (src, dest) = if into_buf { (v, buf) } else { (buf, v) };

    // Panic safety:
    //
    // If `is_less` panics at any point during the recursive calls, the destructor
    // of `guard` will be executed, thus copying everything from `src` into
    // `dest`. This way we ensure that all chunks are in fact copied into
    // `dest`, even if the merge process doesn't finish.
    let guard = MergeHole {
        start: src.add(start),
        end: src.add(end),
        dest: dest.add(start),
...
where
    ...
    F: Fn(&T, &T) -> bool + Sync,
{
    // Slices of up to this length get sorted using insertion sort in order to avoid
    // the cost of buffer allocation.
    const MAX_INSERTION: usize = 20;

    // The length of initial chunks. This number is as small as possible but so that
    // the overhead of Rayon's task scheduling is still negligible.
    ...
    // dtors running on copies if `is_less` panics.
    let mut buf = Vec::<T>::with_capacity(len);
    let buf = buf.as_mut_ptr();

    // If the slice is not longer than one chunk would be, do sequential merge sort
    // and return.
    if len <= CHUNK_LENGTH {
    ...
    // However, descending chunks will not be sorted - they will be simply left
    // intact.
    let mut iter = {
...
mod tests {
    ...
    use super::{heapsort, split_for_merge};
