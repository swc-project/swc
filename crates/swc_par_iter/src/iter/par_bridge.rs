...
///
/// This creates a "bridge" from a sequential iterator to a parallel one, by
/// distributing its items across the Rayon thread pool. This has the advantage
/// of being able to parallelize just about anything, but the resulting
/// `ParallelIterator` can be less efficient than if you started with `par_iter`
...
///
/// Iterator items are pulled by `next()` one at a time, synchronized from each
/// thread that is ready for work, so this may become a bottleneck if the serial
/// iterator can't keep up with the parallel demand. The items are not buffered
/// by `IterBridge`, so it's fine to use this with large or even unbounded
/// iterators.
///
/// The resulting iterator is not guaranteed to keep the order of the original
/// iterator.
///
/// # Examples
///
...
///
/// This type is created when using the `par_bridge` method on `ParallelBridge`.
/// See the [`ParallelBridge`] documentation for details.
///
